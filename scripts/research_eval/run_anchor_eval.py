#!/usr/bin/env python3
"""Anchor evaluation: all three bots vs passive/aggressive/random/tight anchors."""
...
ANCHORS = ["passive", "aggressive", "random", "tight"]
N_MATCHES = 3


def run_match(candidate_key, anchor_strategy, port, seed, timeout=90):
    ...
    anchor_cmd = [sys.executable, str(ANCHOR),
        "--host", "127.0.0.1", "--port", str(port),
        "--name", f"Anchor_{anchor_strategy}",
        "--strategy", anchor_strategy, "--seed", str(seed + 1)]
    ...
    try:
        ...
        time.sleep(0.5)
        pb = subprocess.Popen(anchor_cmd, cwd=str(REPO), env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        for line in (a_stderr + "\n" + b_stderr).split("\n"):
            if "{" in line and ("TELEMETRY" in line or "cumulative_net" in line):
                try:
                    ...
                    elif "Anchor" in bn: anchor_earn = earn
                except: pass
        if cand_earn == 0 and anchor_earn != 0: cand_earn = -anchor_earn
        elif anchor_earn == 0 and cand_earn != 0: anchor_earn = -cand_earn
        winner = "cand" if cand_earn > 0 else ("anchor" if cand_earn < 0 else "draw")
...
def main():
    ...
    for cand in CANDIDATES:
        ...
        for anchor in ANCHORS:
            label = f"{cand}_vs_{anchor}"
            print(f"\n=== {label} ({N_MATCHES} matches) ===", flush=True)
            matches = []
            for m in range(N_MATCHES):
                port = base_port + hash(f"{cand}_{anchor}_{m}") % 2000
                seed = base_seed + hash(f"{cand}_{anchor}") % 500 + m * 10
                r = run_match(cand, anchor, port, seed)
                matches.append(r)
                w = r["winner"]
            ...
            losses = sum(1 for r in valid if r["winner"] == "anchor")
            draws = sum(1 for r in valid if r["winner"] == "draw")
            avg = sum(r["cand_earn"] for r in valid) / len(valid) if valid else 0
            all_results[cand][anchor] = {
                "wins": wins, "losses": losses, "draws": draws,
                "avg_earn": avg, "matches": matches,
            }
            print(f"  => {wins}W/{losses}L/{draws}D avg_earn={avg:.0f}", flush=True)

    print("\n=== ANCHOR EVALUATION SUMMARY ===", flush=True)
    print(f"{'Candidate':<6} {'Anchor':<12} {'W/L/D':>10} {'AvgEarn':>10}", flush=True)
    print("-" * 44, flush=True)
    for cand in CANDIDATES:
        for anchor in ANCHORS:
            r = all_results[cand][anchor]
            wld = f"{r['wins']}/{r['losses']}/{r['draws']}"
            print(f"{cand:<6} {anchor:<12} {wld:>10} {r['avg_earn']:>10.0f}", flush=True)

    output = Path("scripts/research_eval/results")
    output.mkdir(exist_ok=True)
    (output / "anchor_eval.json").write_text(json.dumps(all_results, indent=2, default=str))
    print(f"\nSaved to scripts/research_eval/results/anchor_eval.json", flush=True)
