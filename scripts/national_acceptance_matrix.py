...
The matrix executes Botzone JSON bots through sever/bot_adapter.py and the
national TCP GameEngine rules, but in-process. This keeps the same protocol
translation and legality validator while avoiding socket orchestration for
pairwise checks.
"""
...
def resolve_bot(token: str) -> BotSpec:
    raw = Path(token)
    candidates: list[Path] = []
    if raw.exists():
        candidates.append(raw)
    if token.startswith("v") and token[1:].isdigit():
        candidates.append(ROOT / "bots" / f"claude_v{token[1:]}")
    if token.isdigit():
        candidates.append(ROOT / "bots" / f"claude_v{token}")
        candidates.append(ROOT / "bots" / f"bot{token}")
    if token.startswith("claude_v") or token.startswith("bot"):
        candidates.append(ROOT / "bots" / token)
    ...
    raise SystemExit(f"bot not found or missing main.py: {token}")
...
async def run_pair(bot_a: BotSpec, bot_b: BotSpec, hands: int) -> dict[str, Any]:
    ...
    for idx, spec in enumerate((bot_a, bot_b)):
        ...
        if telemetry.get("bot_failures", 0):
            issues.append(f"{spec.label}: bot_failures={telemetry['bot_failures']}")
        if telemetry.get("invalid_actions", 0):
            issues.append(f"{spec.label}: invalid_actions={telemetry['invalid_actions']}")
...
async def run_matrix(bots: list[BotSpec], hands: int) -> dict[str, Any]:
    ...
    for result in results:
        ...
        for label in (a, b):
            ...
            summary[label]["bot_failures"] += pdata["adapter"].get("bot_failures", 0)
            summary[label]["invalid_actions"] += pdata["adapter"].get("invalid_actions", 0)
            summary[label]["passed_compliance"] = (
                summary[label]["passed_compliance"]
                and result["passed_compliance"]
                and pdata["illegal_actions"] == 0
                and pdata["timeouts"] == 0
                and pdata["adapter"].get("bot_failures", 0) == 0
                and pdata["adapter"].get("invalid_actions", 0) == 0
            )
...
def format_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "| bot | compliance | net chips | illegal | timeout | bot failures | invalid actions |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for label in labels:
        row = report["summary"][label]
        lines.append(
            "| {label} | {compliance} | {net} | {illegal} | {timeout} | {failures} | {invalid} |".format(
                label=label,
    ...
    failures = [
        ...
        if not result["passed_compliance"]
    ]
    if failures:
        lines.extend(["", "## Issues", ""])
        for result in failures:
            lines.append(
                f"- {result['bot_a']} vs {result['bot_b']}: "
                + "; ".join(result["issues"])
            )
    return "\n".join(lines) + "\n"
