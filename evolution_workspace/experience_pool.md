...
- **Source**: claude_v6 (r=1429.6, rd=43.2). 2nd-worst of all claude bots. 150pts behind v12 (1575). Flatlined last 15 periods.
- **Reference**: bot5 (anti-exploitation framework, Rank 1). bot5 has NO bb_vs_raise/sb_vs_reraise handlers, returns None.
- **5 critical gaps identified**:
  1. `bb_vs_raise` has hardcoded logic (lines 554-578): 3bet with strength≥0.72, bluff with 0.38-0.52, fold/call thresholds. bot5 returns None → simulation handles it. REMOVE these handlers.
  2. `sb_vs_reraise` has hardcoded logic (lines 580-593): 4bet with 0.85+, call with 0.60+. bot5 returns None. REMOVE.
...
### Key Differences: v6 vs bot5
- v6 has `cbet_rate`/`drift_detected` in opponent model — bot5 doesn't. These add noise, not signal.
- v6 `allow_low_frequency_blocker_bluff` uses deterministic hash. bot5 uses `random.random()` with `bluff_freq_bonus` scaling.
- v6 `choose_raise` max_ratio hard-coded to 1.45. bot5 has `max_ratio = 2.2 if allow_river_overbet else 1.45`.
