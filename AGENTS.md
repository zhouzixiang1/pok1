...
├── scripts/                    # Botzone upload/match/reset, official-certification, national arena, evolution ops scripts
...
`ref/DanLM` and `ref/neuron_poker` are gitlinks in the index, but this checkout currently has no `.gitmodules`; `git submodule status` may fail or report noise. Do not repair or stage gitlink changes unless the task is specifically about references/submodules.
...
Before starting work, follow the pull-remote-state rule in 本仓库改代码规范 below (`git pull --ff-only --tags`; or `git fetch --tags origin` plus a temporary worktree from the updated `origin/main` if not fast-forwardable).
...
python -m rl.scripts.train --arch transformer
