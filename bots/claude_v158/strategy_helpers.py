...
def river_value_raise_tier(round_idx, to_call, made_strength, value_profile, board_texture, opponent_model, pot, my_chips, min_raise, my_round_bet):
    ...
    if confidence < 0.10:
        return None
    # v158 NEW: Two-band value sizing. Thin band keeps 0.45-0.55x for
    # marginal holdings (second pair / weak top pair). Strong band lifts
    # to 0.65-0.85x per match-analysis mandate (river avg_raise 0.4x pot
    # was undercharging strong hands). OFFENSE on to_call==0 barrel path
    # — NOT a fold-side mechanism.
    if made_strength < 0.55:
        # Thin value: 0.45x at made_str=0.45 -> 0.55x at made_str=0.55
        t = (made_strength - 0.45) / 0.10
        ratio = 0.45 + 0.10 * t
    else:
        # Strong/made value: 0.65x at made_str=0.55 -> 0.85x at made_str=0.82
        t = (made_strength - 0.55) / 0.27
        ratio = 0.65 + 0.20 * t
    fold_river = opponent_model.get('fold_to_bet_river', 0.44)
    ...
    try:
        band = 'strong' if made_strength >= 0.55 else 'thin'
        tier = value_profile.get('tier', 'none') if value_profile else 'none'
        sys.stderr.write(
            'RIVER_VALUE_TIER ratio_milli=%+d made=%.2f tier=%s band=%s conf=%.2f\n'
            % (round(ratio * 1000), made_strength, tier, band, confidence)
        )
    except Exception:
        pass
    target = int(pot * ratio)
...
if __name__ == "__main__":
    ...
    # Strong band: made_str=0.60 should return ~0.69x pot (was 0.59x)
    ...
    assert 0.64 <= _strong_ratio <= 0.72, f'v158 strong060: ratio {_strong_ratio:.3f} outside 0.64-0.72'
    print(f'v158 strong060: ratio={_strong_ratio:.3f} PASS')
    # Strong band: made_str=0.70 should return ~0.77x pot
    ...
    assert 0.72 <= _strong2_ratio <= 0.82, f'v158 strong070: ratio {_strong2_ratio:.3f} outside 0.72-0.82'
    print(f'v158 strong070: ratio={_strong2_ratio:.3f} PASS')
    # Strong band: made_str=0.80 should return ~0.84x pot
    ...
    assert 0.82 <= _strong3_ratio <= 0.86, f'v158 strong080: ratio {_strong3_ratio:.3f} outside 0.82-0.86'
    print(f'v158 strong080: ratio={_strong3_ratio:.3f} PASS')
    ...
    print('v158 river_value_raise_tier two-band: 5/5 PASS')
