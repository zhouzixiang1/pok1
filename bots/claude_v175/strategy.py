...
def _is_offsuit_commitment_risk(my_cards, opponent_model=None):
    ...
    v173: A2o-A5o carve-out vs ultra-loose openers (VPIP>=0.65, conf>=0.15) --
    ~35% equity clears ~36% pot odds; A6o-A8o stay gated for all opponents.
    """
    ...
    if high == 14 and low == 9:         # A9o -- marginal keeper
        return False
    if opponent_model is not None and high == 14 and 2 <= low <= 5:
        conf = opponent_model.get('confidence', 0.0)
        vpip = opponent_model.get('vpip', 0.58)
        if conf >= 0.15 and vpip >= 0.65:
            return False
    return True
...
def _bb_vs_raise_bucket_action(hand_cat, opponent_model, pot_odds, preflop_strength, win_rate, trash_hand, my_cards):
    ...
    if my_cards is not None and _is_offsuit_commitment_risk(my_cards, opponent_model):
        try:
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        ...
        if action == 'raise' and _is_offsuit_commitment_risk(req['my_cards'], opponent_model):
            try:
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if _is_offsuit_commitment_risk(req['my_cards'], opponent_model):
            try:
...
def get_action(req, requests):
    ...
    if to_call > 0:
        # v174b: SPR gate NARROWED from full-scope (all streets, made<0.55) to
        # river-only + genuinely-weak (made<0.45). The full-scope version caused
        # clear_regression (4W-10L vs parent) by over-folding marginal value hands
        # on flop/turn. Surgical application: river trash/weak one-pair facing bets
        # only. Catastrophic all-ins are handled upstream by the L1161 block.
        if round_idx == 3 and made_strength < 0.45 and _spr_commitment_gate(
                round_idx, my_chips, made_strength, value_profile, to_call, pot,
                win_rate, anti_lock_pressure,
                line_profile.get('line_label', 'balanced'), draw_strength):
            print(f"SPR_FOLD_GT0 r={round_idx} made={made_strength:.2f} tc={to_call} "
                  f"pot={pot} win={win_rate:.3f} [LIVE_PATH]", file=sys.stderr)
            return -1
        ...
        if round_idx == 0:
            ...
            if round_idx == 3 and made_strength < 0.55 and bet_size_bucket(spot_info["last_raise_pot_ratio"]) in ("medium", "large"):
                call_margin += 0.10
            # v175 CROSSOVER+MUTATION: Continuous river call-margin tighten for
            # the marginal over-calling band (one-pair ~0.22 + two-pair ~0.40 per
            # HAND_CLASS_SCORE). v173's DOMINANT exploitable leak is postflop
            # over-calling (H2H WR 0.20 vs v169, 0.30 vs v165, 0.40 vs v152/v154
            # per experience_pool RECONCILED_LEAK). The v167 +0.10 above is a
            # FLAT boost gated only on medium/large bet SIZE; this adds an
            # opponent-polarization-SCALED tighten (large_bet_ratio + multi-street
            # barrel signals) that ALSO fires on SMALL river value-bets — the gap
            # neither v167 (bet-size gated) nor _river_stackoff_guard (binary,
            # large-bet + bad-odds only) nor _river_reraise_tighten (0.55-0.80
            # re-raises only) covers.
            # CROSSOVER: imports v174's strategic concept (more river fold
            # pressure on weak hands in to_call>0) but resolves the conflict in
            # favor of a CONTINUOUS margin (FOLD-SIDE RULE: binary postflop fold
            # gates are dead 13+ gens) over v174's binary _spr_commitment_gate
            # (made<0.45) which over-folded and net-hurt (v174 rating 1400 <
            # v173 1521). Band targets 0.20-0.45 = one-pair through two-pair
            # (actual HAND_CLASS_SCORE values, NOT the aspirational 0.45-0.55
            # from the pool — verified: pair=0.22, two-pair=0.40 via
            # made_hand_metric).
            # MUTATION (experience_pool option b): one continuous-EV heuristic.
            if (round_idx == 3 and not anti_lock_pressure
                    and 0.20 <= made_strength < 0.45
                    and opponent_model is not None
                    and opponent_model.get("confidence", 0.0) >= 0.15
                    and spot_info.get("facing_postflop_aggression", False)
                    and (value_profile is None
                         or value_profile.get("tier") not in ("strong", "nut"))):
                _overcall_polar = clamp(
                    (opponent_model.get("large_bet_ratio", 0.32) - 0.28) / 0.20, 0.0, 1.0)
                _overcall_multi = clamp(
                    (spot_info.get("opp_postflop_bet_count", 0) - 1) / 2.0, 0.0, 1.0)
                _overcall_size_f = clamp(
                    spot_info.get("last_raise_pot_ratio", 0.0) / 0.80, 0.25, 1.0)
                _overcall_delta = (0.08 * opponent_model["confidence"]
                                   * (0.40 + 0.35 * _overcall_polar
                                      + 0.25 * _overcall_multi)
                                   * _overcall_size_f)
                call_margin += _overcall_delta
                print(f"OVERCALL_TIGHTEN made={made_strength:.2f} "
                      f"delta_milli={round(_overcall_delta*1000):+d} "
                      f"polar={_overcall_polar:.2f} multi={_overcall_multi:.1f} "
                      f"size_f={_overcall_size_f:.2f} "
                      f"conf={opponent_model['confidence']:.2f}",
                      file=sys.stderr)
