...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position):
    ...
    if spot_info["facing_postflop_aggression"]:
        ...
        if spot_info.get("opp_postflop_bet_count", 0) >= 2:
            margin += 0.030 if size_bucket == "small" else 0.022
...
def realized_postflop_equity(
    ...
    # --- Pressure-based EQR for medium/strong hands ---
    # When facing postflop aggression, opponent's range narrows.
    # Discount equity based on: number of streets with aggression,
    # position, round, and bet sizing.
    if spot_info.get('facing_postflop_aggression', False) or spot_info.get('facing_raise', False):
        eqr = 1.0

        # Per-bet discount: each postflop bet faced reduces EQR
        opp_bets = spot_info.get('opp_postflop_bet_count', 0)
        if opp_bets >= 1:
            eqr -= 0.04  # First bet: mild discount
        if opp_bets >= 2:
            eqr -= 0.06  # Second bet (barrel): stronger discount
        if opp_bets >= 3:
            eqr -= 0.04  # Third bet: very polarized range

        # Street-specific discount
        if round_idx == 2:
            eqr -= 0.03  # Turn: range more defined
        elif round_idx == 3:
            eqr -= 0.06  # River: most defined range, biggest discount

        # Position discount
        if not has_position:
            eqr -= 0.03  # OOP: less information, more uncertainty

        # One-pair discount on river (weakest 'made' hand class)
        if pair_profile is not None and pair_profile['made_class'] == 1 and round_idx == 3:
            eqr -= 0.08

        # Size-based discount (larger bets = stronger range)
        size = bet_size_bucket(spot_info.get('last_raise_pot_ratio', 0))
        if size == 'large':
            eqr -= 0.05
        elif size == 'medium':
            eqr -= 0.02

        eqr = clamp(eqr, 0.55, 1.0)
        return win_rate * eqr
