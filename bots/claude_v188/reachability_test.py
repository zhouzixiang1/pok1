...
def main_river_potodds_equity_margin():
    """v188: prove _river_potodds_equity_margin FIRES (>0 delta) for weak
    one-pair / weak-two-pair made hands facing a >=0.5x pot river bet where
    calibrated equity (made_strength) is below the price (pot_odds), and does
    NOT fire for strong-tier hands, draw hands, or overbettor opponents.
    ...
    threshold at strategy.py L1693, converting leak-band calls into folds.
    Mirrors main_turn_board_danger_margin() scaffolding.
    """
    import opponent
    ...
    #    equity_gap = 0.30 - 0.25 = 0.05 > 0 -> FIRES.
    #    delta = (0.48 - 0.30) + 0.05*0.5 = 0.205.
    ...
    # FIRE 2: made=0.40 weak-two-pair facing 0.7x pot, pot_odds=0.45.
    #    equity_gap = 0.45 - 0.40 = 0.05 > 0 -> FIRES.
    #    delta = (0.48 - 0.45) + 0.05*0.5 = 0.055.
    spot_07x = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.70}
    delta_fire2 = opponent._river_potodds_equity_margin(
        3, 0.40, 0.05, vp_thin, spot_07x, 0.45, None)
    assert delta_fire2 > 0.0, (
        "REACHABILITY FAIL: made=0.40 facing 0.7x pot (pot_odds=0.45) should "
        "fire delta>0, got %.4f" % delta_fire2)

    # FIRE 3: made=0.30 facing allin-size 0.55x pot, pot_odds=0.40.
    #    equity_gap = 0.40 - 0.30 = 0.10 > 0 -> FIRES.
    #    delta = (0.48 - 0.40) + 0.10*0.5 = 0.13.
    spot_055x = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.55}
    delta_fire3 = opponent._river_potodds_equity_margin(
        3, 0.30, 0.05, vp_thin, spot_055x, 0.40, None)
    assert delta_fire3 > 0.0, (
        "REACHABILITY FAIL: made=0.30 facing allin 0.55x pot (pot_odds=0.40) "
        "should fire delta>0, got %.4f" % delta_fire3)
    ...
    # NON-FIRE 4 (bonus): bet<0.5x pot must NOT fire (below leak band).
    spot_small = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.40}
    delta_small = opponent._river_potodds_equity_margin(
        3, 0.30, 0.05, vp_thin, spot_small, 0.40, None)
    assert delta_small == 0.0, (
        "REACHABILITY FAIL: bet<0.5x pot must be exempt, got %.4f"
        % delta_small)

    print(
        "REACHABILITY PASS: v188 river pot-odds-equity margin FIRES for "
        "made=0.25(%.4f), 0.40(%.4f), 0.30(%.4f); does NOT fire for "
        "strong(%.4f), draw(%.4f), overbettor(%.4f), small_bet(%.4f)"
        % (delta_fire1, delta_fire2, delta_fire3,
