...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
    ...
    if not spot_info.get('facing_postflop_aggression', False):
        return 0.0
    # 3. Bet-size gate: >=0.5x pot (the uncovered leak band; _river_stackoff_guard
    #    already handles >=0.75x via binary fold, so overlap is harmless)
    ...
    if opponent_model is not None:
        ...
        if (sizing is not None
            ...
            return 0.0
    # 8. Pot-odds-vs-equity gate: fire only when calibrated equity proxy
    #    (made_strength) is below the price (pot_odds).
    equity_gap = pot_odds - made_strength
    if equity_gap <= 0.0:
        return 0.0
    ...
    try:
        sys.stderr.write(
            'RIVER_POTODDS_EQUITY delta_milli=%+d made=%.2f pot_odds=%.3f '
            'gap=%.3f bet_ratio=%.2f reason=fired\n'
            % (round(delta * 1000), made_strength, pot_odds, equity_gap, bet_ratio))
    except Exception:
        pass
    return delta
...
if __name__ == '__main__':
    ...
    # v188 _river_potodds_equity_margin self-test (M5/M6: non-zero in leak band)
    ...
    assert _river_potodds_equity_margin(3, 0.28, 0.05, _pe_vp_marginal, {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.30}, 0.33, None) == 0.0, 'bet<0.5x must be exempt'
    ...
    print('RIVER_POTODDS_EQUITY self-test PASS: fire_delta=%.4f' % _pe_delta_fire)
