...
def get_action(req, requests):
    ...
    if big_pot and round_idx == 3 and (value_profile is None or value_profile["tier"] not in ("strong", "nut")):
        if blocker_profile is None or not blocker_profile["eligible"]:
            return 0
    # v129 OFFENSIVE: turn value second-barrel when flop aggressor with
    # top-pair-good-kicker+ on non-deteriorated board. Lifts turn-barrel
    # rate from ~33.5% toward ~45-55% in eligible value spots. Orthogonal
    # to all defensive fold-gates (does NOT subtract call_margin).
