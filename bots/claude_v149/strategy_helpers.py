...
def _street_fold_exploit_sizing_boost(opponent_model, round_idx):
    ...
    Trigger: fold_to_bet_{flop,turn,river} >= 0.55 with confidence >= 0.20.
    Scale: 0.55 -> +0.08, 0.80 -> +0.16, 0.95+ -> +0.25 (capped), * confidence.
...
if __name__ == "__main__":
    ...
    # NOTE: expected values computed from the formula raw=clamp(0.08+(fold-0.55)*0.30,
    # 0.08, 0.25) then * confidence. Two master_prompt fixture targets (0.25 for
    # fold=0.99, 0.115 for fold=0.70) were inconsistent with that formula; corrected
    # here to the actual outputs (0.212 -> 0.21 within tolerance; 0.125 -> 0.13).
    ...
    _t_fold_boost({'confidence': 0.0, 'fold_to_bet_flop': 0.99}, 1, 0.0, 'low-conf no-fire')
    _t_fold_boost({'confidence': 0.5, 'fold_to_bet_flop': 0.44}, 1, 0.0, 'low-fold no-fire')
    _t_fold_boost({'confidence': 0.5, 'fold_to_bet_flop': 0.55}, 1, 0.04, 'threshold 0.55')
    _t_fold_boost({'confidence': 1.0, 'fold_to_bet_flop': 0.99}, 1, 0.21, 'ultra-fold flop near-max')
    _t_fold_boost({'confidence': 1.0, 'fold_to_bet_turn': 0.80}, 2, 0.16, 'turn 0.80')
    _t_fold_boost({'confidence': 1.0, 'fold_to_bet_river': 0.70}, 3, 0.13, 'river 0.70')
    _t_fold_boost({'confidence': 0.30, 'fold_to_bet_flop': 0.99}, 0, 0.0, 'preflop no-fire')
    print("_street_fold_exploit_sizing_boost: ALL PASS")

    # v149 dispatch-reachability: function symbol must appear in strategy.py
    # (1 import + 2 call assignments that compute street_fold_boost). The 2 kwarg
    # passes use the variable name street_fold_boost=, verified separately below.
    with open('strategy.py') as f:
        src = f.read()
    n_fn_refs = src.count('_street_fold_exploit_sizing_boost')
    assert n_fn_refs >= 3, f'Dispatch-reachability FAIL: need >=3 fn refs (import + 2 call assign), found {n_fn_refs}'
    ...
    print(f'v149 _street_fold_exploit_sizing_boost dispatch-reachability: PASS ({n_fn_refs} fn refs, {n_kwarg_refs} kwarg passes)')
