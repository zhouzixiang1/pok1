...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, board_texture=None):
    if round_idx <= 0:
        return False
    ...
    # Mutation: board texture-aware fold thresholds
    # On completed draw boards, opponent aggression is more credible — tighten folds
    texture_bonus = 0.0
...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if fragile_pair_raise_fold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # Crossover from v6: structured fold discipline vs postflop aggression
        if should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, board_texture):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            if not anti_lock_call_continue and not strong_made_continue:
