...
def get_action(req, requests):
    ...
    # v12: Opponent style classification for threshold deltas
    ...
    # v12: Apply opponent style deltas (safe, confidence-scaled)
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            # v12: Apply opponent style call margin delta
            call_margin += style_deltas["call_margin_delta"]
        if anti_lock_pressure:
            call_margin -= 0.07
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")):
        raise_amount = choose_raise(
            ...
            nutted_risk_score=nutted_risk["risk"],
            match_sizing_delta=match_profile["sizing_delta"] + style_deltas["value_size_delta"],
        )
        if raise_amount is not None:
            return raise_amount
    return 0