...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, board_texture=None):
    if round_idx <= 0:
        return 0.0
    ...
    # Opponent checking after previous aggression: possible trap, reduce margin
    if spot_info.get("opp_postflop_bet_count", 0) >= 1 and spot_info.get("opp_current_round_check_count", 0) >= 1:
        margin -= 0.015

    # Scare cards completing draws on turn for weak hands
    if round_idx == 2 and weak_showdown and board_texture is not None:
        if board_texture["flush_pressure"] >= 0.75 or board_texture["straight_pressure"] >= 0.65:
            margin += 0.020
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "bb_vs_raise":
        ...
        if 0.38 <= preflop_strength <= 0.52 and confidence >= 0.25 and fold_to_raise > 0.45:
            ...
            # Reduce bluff frequency vs tight preflop raisers
            if opponent_model.get("pfr", 0.24) < 0.30:
                bluff_freq *= 0.60
            # Reduce bluff if opponent cbets frequently (less fold equity postflop)
...
def get_action(req, requests):
    ...
    win_rate = estimate_weighted_win_rate(my_cards, public_cards, combos, weights, simulations, preflop_strength=preflop_strength)
    ...
    if critical_spot and extra > 0:
        refined = estimate_weighted_win_rate(my_cards, public_cards, combos, weights, extra, preflop_strength=preflop_strength)
        win_rate = (win_rate * simulations + refined * extra) / (simulations + extra)

    # Marginal spot extra iterations: add more precision when win_rate is 0.40-0.60
    if 0.40 <= win_rate <= 0.60 and not critical_spot and len(public_cards) < 5:
        marginal_extra = max(100, int(simulations * 0.30))
        refined_marginal = estimate_weighted_win_rate(my_cards, public_cards, combos, weights, marginal_extra, preflop_strength=preflop_strength)
        win_rate = (win_rate * simulations + refined_marginal * marginal_extra) / (simulations + marginal_extra)
    ...
    if to_call > 0:
        ...
        turn_checkraise_exploit = (
            round_idx == 2
            and spot_info["facing_postflop_aggression"]
            and bet_size_bucket(spot_info["last_raise_pot_ratio"]) in ("small", "medium")
            and value_profile is not None
        ...
        if not preflop_defensive_only and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit or turn_checkraise_exploit):
            raise_amount = choose_raise(
                state["min_raise_action"],
                my_chips,
                state["my_round_bet"],
                ...
                pressure_line=flop_checkraise_exploit or turn_checkraise_exploit,
    ...
    # SPR scaling: reduce sizing when shallow/committed to avoid overbetting
    spr_sizing_delta = (spr_info["sizing_scale"] - 1.0) * 0.08  # ranges from 0 to -0.016
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or barrel_raise:
        raise_amount = choose_raise(
            ...
            nutted_risk_score=nutted_risk["risk"],
            match_sizing_delta=match_profile["sizing_delta"] + barrel_sizing_delta + spr_sizing_delta,
        )
