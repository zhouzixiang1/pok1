...
def board_texture_profile(public_cards):
    ...
    info["draw_heavy"] = info["flush_pressure"] >= 0.75 or info["straight_pressure"] >= 0.65

    sorted_ranks = sorted(board_ranks, reverse=True)
    info["high_card_pair"] = len(sorted_ranks) >= 2 and sorted_ranks[0] == sorted_ranks[1]

    w = info["wetness"]
    if w < 0.20:
        info["texture_class"] = "dry"
    elif w < 0.45:
        info["texture_class"] = "semi-wet"
    else:
        info["texture_class"] = "wet"
...
def draw_profile(hole_cards, public_cards, board_texture=None):
    ...
    if info["flush_draw"]:
        ...
        for suit, count in suit_counts.items():
            if count == 4:
                hole_in_suit = [c for c in hole_cards if card_suit(c) == suit]
                if hole_in_suit:
                    flush_suit = suit
                    break
        if flush_suit is not None:
            suit_in_play = sum(1 for c in cards if card_suit(c) == flush_suit)
            flush_outs = 13 - suit_in_play
        else:
            flush_outs = 9
    else:
        flush_outs = 0
    ...
    if info["straight_draw"] == "open_ended":
        straight_outs = 8
    elif info["straight_draw"] == "double_gutshot":
        straight_outs = 8
    elif info["straight_draw"] == "gutshot":
        straight_outs = 4
    info["nut_draw"] = flush_nut_draw or straight_nut_draw
    info["outs_estimate"] = flush_outs + straight_outs
    if info["combo_draw"]:
        info["outs_estimate"] = flush_outs + straight_outs
    return info
...
def river_thin_value_profile(hole_cards, public_cards, pair_profile, board_texture, value_profile, nutted_risk):
    ...
    made_strength = made_hand_metric(hole_cards, public_cards)
