...
def classify_opponent_style(opponent_model):
    """Classify opponent into one of 5 types and return threshold deltas.
    All deltas default to 0.0 for unknown opponents (low confidence).
    """
    confidence = opponent_model.get("confidence", 0.0)
    vpip = opponent_model.get("vpip", 0.52)
    pfr = opponent_model.get("pfr", 0.24)
    postflop_aggr = opponent_model.get("postflop_aggr", 0.36)
    fold_to_raise = opponent_model.get("fold_to_raise", 0.44)

    # Scale all deltas by confidence — no adjustments for unknown opponents
    scale = clamp(confidence, 0.0, 1.0)

    deltas = {
        "style": "balanced",
        "strong_delta": 0.0,    # adjustment to 'strong' threshold
        "medium_delta": 0.0,    # adjustment to 'medium' threshold
        "bluff_bonus": 0.0,     # bonus to bluff frequency threshold
        "call_margin_delta": 0.0,  # adjustment to call margin
        "value_size_delta": 0.0,   # adjustment to value bet sizing
    }
    ...
    if vpip < 0.40 and fold_to_raise > 0.48:
        deltas["style"] = "nit"
        deltas["strong_delta"] = -0.02 * scale  # wider value range (can value bet lighter)
        deltas["bluff_bonus"] = 0.06 * scale     # bluff more (folds often)
        deltas["call_margin_delta"] = 0.01 * scale  # fold more facing bets
        deltas["value_size_delta"] = 0.04 * scale    # bigger value bets (they fold or call light)

    # Maniac: high VPIP, high PFR, high postflop aggr
    elif vpip > 0.62 and (pfr > 0.30 or postflop_aggr > 0.45):
        deltas["style"] = "maniac"
        deltas["strong_delta"] = 0.03 * scale   # tighter value (they bluff a lot)
        deltas["medium_delta"] = 0.02 * scale   # fewer marginal calls
        deltas["bluff_bonus"] = -0.04 * scale    # bluff less (doesn't fold)
        deltas["call_margin_delta"] = -0.02 * scale  # call more (they bluff)
        deltas["value_size_delta"] = -0.02 * scale    # smaller value (keep them in)

    # Calling station: high VPIP, low PFR, low postflop aggr
    elif vpip > 0.55 and pfr < 0.20 and postflop_aggr < 0.30:
        deltas["style"] = "calling_station"
        deltas["strong_delta"] = -0.01 * scale   # value bet wider
        deltas["bluff_bonus"] = -0.06 * scale     # never bluff
        deltas["call_margin_delta"] = 0.01 * scale   # fold more (they have it)
        deltas["value_size_delta"] = 0.06 * scale     # bigger value bets (they call)

    # Fold-heavy: high fold_to_raise
    elif fold_to_raise > 0.52 and confidence >= 0.30:
        deltas["style"] = "fold_heavy"
        deltas["strong_delta"] = -0.01 * scale
        deltas["bluff_bonus"] = 0.04 * scale     # bluff more (they fold)
        deltas["call_margin_delta"] = 0.005 * scale
        deltas["value_size_delta"] = -0.02 * scale  # smaller to keep them calling
