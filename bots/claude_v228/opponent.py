...
def _nut_disadvantage_commit_check(board_texture, made_strength, value_profile):
    """v228 mutation: only extend upper two-pair folds on nut-disadvantage boards.

    v224's blanket multi-barrel ceiling (0.42->0.48) improved coverage of the
    documented 0.20<=made<0.45 over-call band, but its own archive recommends
    gating the upper-two-pair sub-band on board texture before widening further.
    This keeps v203's original 0.42 ceiling on dry boards while allowing folds
    for non-nut strong hands when flush/straight/paired textures create clear
    nut disadvantage.
    """
    if board_texture is None:
        return False
    if made_strength < 0.42 or made_strength >= 0.48:
        return False
    if value_profile is not None and value_profile.get('tier') == 'nut':
        return False
    flush_danger = board_texture.get('flush_pressure', 0.0) >= 0.75
    straight_danger = board_texture.get('straight_pressure', 0.0) >= 0.65
    paired_danger = board_texture.get('paired', False)
    return flush_danger or straight_danger or paired_danger
...
def _multibarrel_line_fold(om, spot_info, round_idx, to_call, pot,
    ...
    Fires when ALL hold:
      ...
      - made_strength < 0.42, or 0.42-0.48 only on nut-disadvantage boards
        (v228 mutation; trips+ at 0.58 preserved)
    ...
    if spot_info.get('opp_current_round_bet_count', 0) < 1:
        return False
    # Guard 3: weak bluff-catcher band. Preserve v203's base ceiling at 0.42;
    # v228 mutates only the upper two-pair sub-band (0.42-0.48) from v224 by
    # requiring a nut-disadvantage board (flush/straight/paired pressure).
    # This imports Beta's value-heavy-barrel coverage without blanket over-folds
    # in matchups where Alpha is already stronger.
    if made_strength >= 0.48:
        return False
    if made_strength >= 0.42 and not _nut_disadvantage_commit_check(
            board_texture, made_strength, value_profile):
        return False
    ...
    # with 0.42-0.48 additionally requiring nut-disadvantage texture.
