...
def _action_profile_signal(context):
    """Confidence-scaled consumption of the reducer-owned action profile.

    ``opponent.action_profile`` is the bounded, connection-scoped distribution
    of opponent action kinds produced by the system tracker (the
    ``action_profile`` profile dimension).  It is consumed here exactly as the
    snapshot ships it; policy never reconstructs, rescans, or tracks it.  A
    missing, malformed, or low-confidence profile returns neutral zeros so the
    parent baseline stays byte-identical until credible evidence arrives; a
    confident aggressive or fold-heavy profile tilts the range and fold-equity
    estimates that the typed-intent entrypoints already consume through
    ``_opponent_posterior``.
    """

    opponent = context.get("opponent") or {}
    opponent = opponent if isinstance(opponent, dict) else {}
    profile = opponent.get("action_profile") or {}
    profile = profile if isinstance(profile, dict) else {}
    neutral = {"aggression": 0.0, "fold_skew": 0.0, "weight": 0.0}
    if not profile:
        return neutral
    aggressive = (
        _number(profile.get("raise_frequency"), 0.0)
        + _number(profile.get("bet_frequency"), 0.0)
        + _number(profile.get("allin_frequency"), 0.0)
        + _number(profile.get("raise"), 0.0)
        + _number(profile.get("allin"), 0.0)
    )
    passive = (
        _number(profile.get("check_frequency"), 0.0)
        + _number(profile.get("call_frequency"), 0.0)
        + _number(profile.get("check"), 0.0)
        + _number(profile.get("call"), 0.0)
        + _number(profile.get("pass"), 0.0)
    )
    folds = (
        _number(profile.get("fold_frequency"), 0.0)
        + _number(profile.get("fold"), 0.0)
    )
    total = aggressive + passive + folds
    if total <= 0.0:
        observed_aggression = ACTION_AGGRESSION_PRIOR
        observed_fold = FOLD_TO_RAISE_PRIOR
    else:
        observed_aggression = _bounded(
            aggressive / total, 0.0, 1.0, ACTION_AGGRESSION_PRIOR
        )
        observed_fold = _bounded(
            folds / total, 0.0, 1.0, FOLD_TO_RAISE_PRIOR
        )
    scalar = profile.get("aggression")
    if isinstance(scalar, (int, float)) and not isinstance(scalar, bool):
        observed_aggression = _bounded(
            float(scalar), 0.0, 1.0, observed_aggression
        )
    confidence = _bounded(
        profile.get("confidence"),
        0.0,
        1.0,
        _bounded(opponent.get("confidence"), 0.0, 1.0, 0.0),
    )
    weight = _bounded(
        profile.get("adaptation_weight"),
        0.0,
        MAX_ADAPTATION_WEIGHT,
        min(MAX_ADAPTATION_WEIGHT, confidence * MAX_ADAPTATION_WEIGHT),
    )
    if weight <= 0.0:
        return neutral
    return {
        "aggression": _bounded(
            weight * (observed_aggression - ACTION_AGGRESSION_PRIOR),
            -0.20,
            0.35,
            0.0,
        ),
        "fold_skew": _bounded(
            weight * (observed_fold - FOLD_TO_RAISE_PRIOR),
            -0.20,
            0.30,
            0.0,
        ),
...
def _opponent_posterior(context):
    ...
    # The reducer-owned action_profile is a second bounded aggression/fold
    # view; it is consumed with its own confidence-scaled weight and folds
    # into the same reachable range/sizing consumers as the rates posterior.
    # A sparse or absent profile returns zeros, leaving this byte-identical.
    profile_signal = _action_profile_signal(context)
    aggression_delta += profile_signal["aggression"]
...
def _current_pressure_adjustment(context):
    ...
    if street in {"flop", "turn", "river"}:
        # Postflop aggression tilt is consumed ONLY from the bounded,
        # reducer-owned ``action_profile`` snapshot (the ``action_profile``
        # profile dimension) and its already-confidence-scaled
        # ``adaptation_weight``.  Policy never reads non-schema self-tracked
        # per-street aggression fields: doing so would reconstruct opponent
        # match state outside ``decision_context``.  A sparse or absent
        # profile leaves this neutral, so the parent baseline is byte-identical
        # until credible, connection-scoped evidence arrives.
        profile = opponent.get("action_profile")
        profile = profile if isinstance(profile, dict) else {}
        weight = _bounded(
            opponent.get("adaptation_weight"), 0.0, MAX_ADAPTATION_WEIGHT
        )
        if profile and weight > 0.0:
            aggressive = (
                _number(profile.get("raise_frequency"), 0.0)
                + _number(profile.get("bet_frequency"), 0.0)
                + _number(profile.get("allin_frequency"), 0.0)
            ...
            if total > 0.0:
                ...
                current_action_tilt += (
                    0.22 * weight * (observed - ACTION_AGGRESSION_PRIOR)
                )
...
def _opponent_fold_defense(opponent):
    """Confidence-scaled facing-a-bet defense from the reducer-owned opponent.

    Consumes the bounded ``action_profile``/``rates`` view shipped by the
    system tracker (the ``action_profile`` profile dimension).  Aggressive
    evidence widens hero's calling range (defends more, returns positive);
    passive evidence tightens it (folds more, returns negative).  The
    tracker's ``adaptation_weight`` is already confidence-scaled, so a sparse
    or absent snapshot returns exactly ``0.0`` and the parent baseline is
    byte-identical.  No second tracker, no history scan: pure dict math over
    the snapshot fields the reducer already produced.
    """

    opponent = opponent if isinstance(opponent, dict) else {}
    weight = _bounded(opponent.get("adaptation_weight"), 0.0, MAX_ADAPTATION_WEIGHT)
    if weight <= 0.0:
        return 0.0
    rates = opponent.get("rates") if isinstance(opponent.get("rates"), dict) else {}
    aggression = _bounded(
        rates.get("aggression"), 0.0, 1.0, ACTION_AGGRESSION_PRIOR
    )
    # ``action_profile`` is a second bounded aggression view when present; it
    # folds in only with concrete evidence so the parent baseline is exact.
    profile = opponent.get("action_profile")
    if isinstance(profile, dict) and profile:
        keys = (
            "raise_frequency", "bet_frequency", "allin_frequency",
            "check_frequency", "call_frequency", "fold_frequency",
        )
        total = sum(_number(profile.get(key), 0.0) for key in keys)
        if total > 0.0:
            aggressive = sum(
                _number(profile.get(key), 0.0)
                for key in keys[:3]
            )
            aggression = 0.5 * aggression + 0.5 * (aggressive / total)
    delta = weight * (aggression - ACTION_AGGRESSION_PRIOR)
    return _bounded(delta, -0.05, 0.06, 0.0)
...
def get_baseline_decision(context):
    ...
    legal = context.get("legal", {}) or {}
    betting = context.get("betting", {}) or {}
    # Direct reducer-owned opponent read: the incremental opponent model is
    # consumed here as well as inside the posterior helpers.  ``fold_defense``
    # is zero for sparse/absent evidence (``adaptation_weight`` near zero), so
    # this branch is byte-identical to the parent baseline; only confident,
    # connection-scoped evidence tilts hero's facing-a-bet defense.
    fold_defense = _opponent_fold_defense(context.get("opponent") or {})
    kinds = set(legal.get("policy_kinds", ()))
    ...
    if fraction is not None and (to_call == 0 or equity >= pot_odds + 0.22):
        ...
        if raised is not None:
            return raised
    # A confidently aggressive opponent bluffs wider, so hero defends by
    # calling more (fold_defense > 0 raises the fold floor); a confidently
    # passive bettor is value-heavy, so hero folds more (fold_defense < 0).
    # Sparse evidence keeps fold_defense at zero, preserving the parent fold.
    if (
        to_call > 0
        and call_equity + 0.035 + fold_defense < pot_odds
        and "fold" in kinds
    ):
        return {"kind": "fold"}
    if "pass" in kinds:
...
def _realized_call_equity(context, equity):
    ...
    if street not in {"flop", "turn"}:
        # River overbet defense: a passive (low-aggression) opponent overbets a
        # value-polarized range, so a marginal made hand must clear a higher
        # pot-odds floor.  Grow a bounded penalty as opponent aggression falls
        # below a neutral ~0.5 prior, scale it by the reducer-supplied
        # credibility, and cap it.  Sparse evidence (confidence near zero)
        # leaves the parent equity byte-identical; every other path is
        # unchanged.
        if street == "river":
            to_call = max(0.0, _number(betting.get("to_call"), 0.0))
            pot = max(0.0, _number(betting.get("pot"), 0.0))
            if pot > 0.0 and to_call >= pot:
                opponent = context.get("opponent") or {}
                opponent = opponent if isinstance(opponent, dict) else {}
                rates = opponent.get("rates") or {}
                rates = rates if isinstance(rates, dict) else {}
                aggression = _bounded(
                    rates.get("aggression"), 0.0, 1.0, 0.5
                )
                confidence = _bounded(
                    opponent.get("confidence"), 0.0, 1.0, 0.0
                )
                penalty = min(
                    0.15, max(0.0, 0.5 - aggression) * 0.30 * confidence
                )
...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    for fraction in _candidate_raise_fractions(context, safe_equity):
        ...
        # Size-conditional fold equity: a larger commitment tracks the
        # fold_to_jam posterior rather than the small-sizing fold_to_raise.
        # When the posteriors collapse (low-confidence equal priors) the
        # commit_ratio term vanishes and the score reduces to the legacy
        # single blended raise rate; the effect emerges only from the
        # fold_to_raise/fold_to_jam gap, never from a confidence gate.
        commit_ratio = _bounded(risk / (pot + risk), 0.0, 1.0, 0.0)
        fold_rate = raise_fold_rate + commit_ratio * (
            jam_fold_rate - raise_fold_rate
        )
        score = (
            fold_rate * pot
...
def _self_test_action_profile_counterfactual():
    """Prove action_profile consumption is confidence-scaled and reachable.

    Absent or low-confidence evidence is neutral so the parent baseline is
    preserved; a confident aggressive/fold-heavy profile produces a non-zero
    tilt that flows into the reachable ``_opponent_posterior`` range/sizing
    consumers.  Pure dict math: no I/O, no precompute, no candidate execution.
    """

    neutral = _action_profile_signal({})
    assert neutral["aggression"] == 0.0, neutral
    assert neutral["fold_skew"] == 0.0, neutral
    missing = _action_profile_signal({"opponent": {"rates": {"aggression": 0.9}}})
    assert missing["aggression"] == 0.0 and missing["fold_skew"] == 0.0, missing
    sparse = _action_profile_signal({
        "opponent": {"action_profile": {"raise_frequency": 0.9}, "confidence": 0.0},
    })
    assert sparse["aggression"] == 0.0, sparse
    confident_aggressive = _action_profile_signal({
        "opponent": {
            "action_profile": {"raise_frequency": 0.8, "check_frequency": 0.2},
            "confidence": 1.0,
            "adaptation_weight": MAX_ADAPTATION_WEIGHT,
        },
    })
    assert confident_aggressive["aggression"] > 0.0, confident_aggressive
    confident_fold = _action_profile_signal({
        "opponent": {
            "action_profile": {"fold_frequency": 0.7, "call_frequency": 0.3},
            "confidence": 1.0,
            "adaptation_weight": MAX_ADAPTATION_WEIGHT,
        },
    })
    assert confident_fold["fold_skew"] > 0.0, confident_fold
    base = _opponent_posterior({})
    tilted = _opponent_posterior({
        "opponent": {
            "action_profile": {"fold_frequency": 0.7, "call_frequency": 0.3},
            "confidence": 1.0,
            "adaptation_weight": MAX_ADAPTATION_WEIGHT,
    ...
    assert tilted["raise_fraction"] > base["raise_fraction"], (base, tilted)

    # Direct incremental-opponent-model read consumed by get_baseline_decision.
    # Sparse/absent evidence is exactly neutral (parent baseline preserved);
    # confident aggression defends (positive), confident passivity folds more.
    assert _opponent_fold_defense({}) == 0.0
    assert _opponent_fold_defense({"rates": {"aggression": 0.95}}) == 0.0
    assert _opponent_fold_defense({
        "adaptation_weight": MAX_ADAPTATION_WEIGHT,
        "rates": {"aggression": 0.95},
    }) > 0.0
    assert _opponent_fold_defense({
        "adaptation_weight": MAX_ADAPTATION_WEIGHT,
        "rates": {"aggression": 0.05},
    }) < 0.0
    assert _opponent_fold_defense({
        "adaptation_weight": MAX_ADAPTATION_WEIGHT,
        "action_profile": {"raise_frequency": 0.8, "check_frequency": 0.2},
    ...
    # Postflop range tilt is consumed only from the bounded reducer-owned
    # action_profile (never non-schema self-tracked fields).  A confident
    # aggressive profile widens the sampled range (positive tilt); sparse or
    # absent evidence is exactly neutral, preserving the parent baseline.
    flop_neutral = _current_pressure_adjustment({"hand": {"street": "flop"}})
    assert flop_neutral["range_tilt"] == 0.0, flop_neutral
    flop_no_weight = _current_pressure_adjustment({
        "hand": {"street": "flop"},
        "opponent": {
    ...
    assert flop_no_weight["range_tilt"] == 0.0, flop_no_weight
    flop_aggressive = _current_pressure_adjustment({
        "hand": {"street": "flop"},
        "opponent": {
            "adaptation_weight": MAX_ADAPTATION_WEIGHT,
            "action_profile": {"raise_frequency": 0.8, "check_frequency": 0.2},
    ...
    assert flop_aggressive["range_tilt"] > 0.0, flop_aggressive

    # Typed-intent counterfactual: the SAME facing-a-bet margin flips between
    # fold and pass purely from confident opponent evidence.  The fold branch
    # in get_baseline_decision folds when call_equity + 0.035 + fold_defense
    # < pot_odds; sparse evidence preserves the parent fold, while confident
    # aggression raises the floor enough to defend (call instead of fold).
    call_equity, pot_odds = 0.40, 0.45
    parent_fold = call_equity + 0.035 + _opponent_fold_defense({}) < pot_odds
    aggressive_fold = (
        call_equity
        + 0.035
        + _opponent_fold_defense({
            "adaptation_weight": MAX_ADAPTATION_WEIGHT,
            "rates": {"aggression": 0.95},
        })
        < pot_odds
    )
    assert parent_fold, "sparse evidence must preserve the parent fold intent"
    assert not aggressive_fold, (
        "confident aggression must flip fold -> pass (defend wider)"
    )


if __name__ == "__main__":
    _self_test_action_profile_counterfactual()
