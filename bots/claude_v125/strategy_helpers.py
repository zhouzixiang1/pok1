...
def facing_barrel_continuation(spot_info, round_idx, made_strength, draw_strength, value_profile, opponent_model, pot, to_call):
    ...
    CROSSOVER v125 (imported from v113; absent in v118/v116-lineage): recovers
    the v101-era pot-odds-grounded defensive fold signal that v116 dropped as
    "dead code". v113's H2H edge vs modern aggressive opponents (v107 +8.7pp,
    v117 +13.5pp, v111 +4.5pp, v96 +6.2pp — all ≥50g) is the strongest
    diverging matchup signal vs v118, and the only structural feature v113 has
    that v118 lacks is this function + its call_margin wiring.
    ...
    MUTATION v125 (option a — threshold adjustment, ~13%): bump deep-barrel
    amplifier 1.15 -> 1.30 (the value validated by v122, which committed
    successfully with this exact coefficient and won mirror vs v116 4-2).
    Effect: on barrel_depth>=3 multi-street value lines, fold pressure is
    13% stronger. Still capped at 0.08 so it cannot force a fold.
    """
    if round_idx < 2:
        return 0.0
    ...
    # Deeper barrels tighten slightly more. MUTATION v125: 1.15 -> 1.30.
    if barrel_depth >= 3:
        signal *= 1.30
