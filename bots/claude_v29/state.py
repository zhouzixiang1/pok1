...
def classify_preflop_tier(my_cards):
    """Classify hole cards into tiers for preflop decision making.
    
    Returns: int (1-5), where 1 = best, 5 = garbage fold)
    
    Card encoding: card // 4 + 2 = rank (2-14), card % 4 = suit
    """
    r1 = card_number(my_cards[0])
    r2 = card_number(my_cards[1])
    high = max(r1, r2)
    ...
    if pair and high >= 2:  # 22-66
        return 2
    if high == 14 and low >= 10:  # ATo+, AJs+
        return 2
    if high == 14 and suited:  # AXs
        return 2
    if high == 13 and low == 11 and suited:  # KJs
        return 2
    if high == 12 and low == 11 and suited:  # QJs
        return 2
    if high == 11 and low == 10 and suited:  # JTs
        return 2
    if high == 10 and low == 9 and suited:  # T9s
        return 2
    if high == 13 and low == 12:  # KQo
        return 2
    
    # Tier 3: Playable BTN/call BB
    if suited and gap == 1 and high >= 5:  # 54s+
        return 3
    if suited and gap == 2 and high >= 8:  # 86s+
        return 3
    if suited and gap == 3 and high >= 9:  # 97s+
        return 3
    if suited and high == 13 and low >= 9:  # K9s
        return 3
    if suited and high == 12 and low >= 9:  # Q9s
        return 3
    if high == 13 and low >= 10:  # KTo, KJo
        return 3
    if high == 12 and low == 10:  # QTo
        return 3
    if high == 11 and low == 10:  # JTo
        return 3
    
    # Tier 4: Defend BB only
    if suited:  # Any two suited
        return 4
    if gap <= 1 and high >= 5:  # Connected 54o+
        return 4
    if high == 14:  # Any ace
        return 4
    if high == 13:  # Any king
        return 4
    if gap <= 2 and high >= 6:  # One-gapper 64o+
        return 4
    if gap <= 3 and high >= 8:  # Two-gapper 86o+
        return 4
