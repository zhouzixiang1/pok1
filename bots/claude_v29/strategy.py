...
from betting import (
    ...
    classify_river_hand, river_3branch_decision,
...
def get_action(req, requests):
    ...
    if round_idx == 0 and preflop_strength is not None:
        ...
        # Preflop BB defense vs raise: tier-based widening
        if to_call > 0 and len(public_cards) == 0:
            preflop_tier = classify_preflop_tier(my_cards)
            if preflop_tier <= 2:
                if preflop_3bet_candidate and win_rate >= 0.50:
                    raise_amount = choose_raise(
                        state.get("min_raise_action", state["round_raise"]),
                        my_chips, state["my_round_bet"],
                ...
                pot_odds_pf = to_call / (pot + to_call)
                if pot_odds_pf <= 0.28 or to_call <= BIG_BLIND * 2:
                    return 0
                return -1
            else:
                return -1
    ...
    if round_idx == 3 and len(public_cards) >= 5:
        ...
        if river_action is not None:
            if anti_lock_pressure and river_action == -1 and to_call < my_chips:
                anti_lock_attack = choose_anti_lock_pressure_action(
                    state, my_chips, to_call, pot, round_idx, win_rate,
                    opponent_model, remaining_hands,
                    value_profile=value_profile, draw_info=draw_info,
                    blocker_profile=blocker_profile, board_texture=board_texture,
                )
                if anti_lock_attack is not None:
                    return anti_lock_attack
                return 0
    ...
    strong = 0.71 if round_idx == 0 else 0.67 if round_idx == 1 else 0.63 if round_idx == 2 else 0.62
    medium = 0.56 if round_idx == 0 else 0.52 if round_idx == 1 else 0.50
