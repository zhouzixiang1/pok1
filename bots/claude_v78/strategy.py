...
def sizing_exploit_adjustment(opponent_model, round_idx):
    ...
    MUTATION: Widened activation thresholds from v23's (0.55, 0.20) to (0.47, 0.24)
    to exploit bet-size patterns against a broader range of opponents (~15% wider).
    """
    confidence = opponent_model.get('confidence', 0.0)
    if confidence < 0.15:
        return 0.0
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None):
    ...
    if round_idx == 1:
        ...
        threshold = 0.38
    # Repeated aggression: fold more
    if opp_bets >= 2:
        threshold += 0.05
    # Barrel-freq modulation from opponent model
    if opponent_model is not None and opponent_model.get("confidence", 0) >= 0.15:
        ...
        if barrel >= 0.60:
            ...
            threshold -= 0.03
    return made_strength < threshold
