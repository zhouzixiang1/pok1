...
def _preflop_shove_defense_fold(my_cards, preflop_strength, state, spot_info, opponent_model=None):
    """Preflop shove/4-bet defense gate (v201 NEW axis).
    ...
    v217 MUTATION (exp-pool heuristic: "vs WIDE open-shovers VPIP>0.60 → fold
    LESS"): the original gate was purely hand-strength based with NO opponent
    awareness — the only fold-gate lacking it. Against wide/aggressive opps
    (high VPIP), their shove/4bet range includes many non-premiums, so calling
    wider is +EV. This directly targets v204's over-fold weakness vs aggressive
    bluffers (v206 WR 0.35, v211 WR 0.43) where v193 (no fold gate) wins by
    calling down. Value-heavy shovers (low VPIP: v196/v212/v214) still face
    the original tight gate.
    """
    opp_allin = state.get('opponent_allin', False)
    opp_pf_raises = spot_info.get('opp_preflop_raises', 0)
    if not (opp_allin or opp_pf_raises >= 2):
        return False
    ...
    # v217: VPIP-aware continuation. Wide opponents (VPIP > 0.60) shove/4bet
    # with expanded ranges — widen our calling band accordingly.
    vpip = None
    if opponent_model is not None:
        vpip = opponent_model.get('vpip')
    if vpip is not None and vpip > 0.60:
        # Wide opponent: mid-pairs continue at pot_odds up to 0.45
        if hand_cat == 'mid_pair' and pot_odds < 0.45:
            return False
        # Very wide opponent (VPIP > 0.70): also continue with small pairs,
        # suited broadways, and suited connectors at cheap odds
        if vpip > 0.70 and pot_odds < 0.38:
            if hand_cat in ('small_pair', 'broadway_suited', 'suited_connector'):
                return False
    # Standard path (tight/unknown opponents): original thresholds
...
def _verify_preflop_shove_defense():
    ...
    # v217 VPIP-aware tests
    om_wide = {'vpip': 0.72}  # very wide opponent
    om_tight = {'vpip': 0.35}  # value-heavy tight opponent
    # Mid-pair 88 vs shove, pot_odds 0.40: folds vs tight, continues vs wide
    state_mid = {'opponent_allin': True, 'to_call': 8000, 'pot': 12000}  # pot_odds ~0.40
    assert _preflop_shove_defense_fold(eights, 0.62, state_mid, spot1), '88 folds vs tight opp @odds 0.40'
    assert not _preflop_shove_defense_fold(eights, 0.62, state_mid, spot1, om_wide), '88 continues vs wide opp @odds 0.40'
    # Small pair 55 vs shove, cheap odds: folds vs tight, continues vs very wide
    state_cheap2 = {'opponent_allin': True, 'to_call': 1000, 'pot': 4000}  # pot_odds 0.20
    fives_c = [c(5, 0), c(5, 1)]
    assert _preflop_shove_defense_fold(fives_c, 0.55, state_cheap2, spot1), '55 folds vs tight @cheap'
    assert not _preflop_shove_defense_fold(fives_c, 0.55, state_cheap2, spot1, om_wide), '55 continues vs very wide @cheap'
    assert _preflop_shove_defense_fold(fives_c, 0.55, state_cheap2, spot1, om_tight), '55 folds vs tight @cheap (no change)'
    # KTo still folds vs wide at expensive odds (only pairs/suited widen)
    assert _preflop_shove_defense_fold(kto, 0.58, state_shove, spot1, om_wide), 'KTo still folds vs wide @expensive'
    print('PREFLOP_SHOVE_DEFENSE_VERIFY PASS (incl. v217 VPIP-aware)')
