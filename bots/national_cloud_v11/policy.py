...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    if (
        ...
        # The called-range haircut grows with the opponent-posterior jam fold
        # rate: a selective jam-caller (high fold_to_jam) keeps a far stronger
        # calling range than a wide caller, so the same raw equity realizes
        # less when called.  Calibrated so the FOLD_TO_JAM_PRIOR prior
        # reproduces the historical 0.105 penalty byte-for-byte, then clamped
        # to a tight band so sparse observed extremes stay bounded.  This opens
        # a second opponent.terminal_response channel (calling-range strength)
        # into the jam called-EV alongside the existing jam_fold_rate weighting.
        jam_haircut = _bounded(
            0.105 * (jam_fold_rate / FOLD_TO_JAM_PRIOR), 0.05, 0.18
        )
        jam_equity = _bounded(safe_equity - jam_haircut, 0.0, 1.0)
...
if __name__ == "__main__":
    # Embedded fixture for the opponent.terminal_response -> jam called-EV
    # channel.  Two decision_context objects differ ONLY in
    # opponent.terminal_response.fold_to_jam at a non-river all-in-candidate
    # spot.  The selective caller (0.55) must keep a materially stronger
    # calling range (lower called all-in equity) and flip at least one final
    # socket-validated typed intent versus the 0.28 prior, which stays
    # byte-identical to the historical flat 0.105 haircut.
    _FTJ_EQUITY = 0.64
    _FTJ_CONFIDENCE = 1.0
    _FTJ_SAMPLES = 10000

    def _ftj_context(fold_to_jam):
        return {
            "legal": {
                "policy_kinds": ["pass", "fold", "allin"],
                "min_raise_to": 100,
                "max_raise_to": 2000,
            },
            "betting": {
            ...
            "hand": {
                ...
                "number": 10,
                "total_hands": 70,
                "remaining_including_current": 61,
            },
            "line": {"preflop_spot": ""},
            "cards": {
            ...
            "opponent": {
                "terminal_response": {
                    "fold_to_jam": fold_to_jam,
                    "adaptation_weight": MAX_ADAPTATION_WEIGHT,
                },
            },
        }

    def _ftj_called_allin_equity(context):
        # Mirror the jam branch exactly: same posterior, same safe-equity
        # build, same bounded haircut, so the printed value is the equity
        # actually fed to _called_showdown_ev for the jam candidate.
        jam_fold_rate = _opponent_posterior(context)["fold_to_jam"]
        uncertainty = min(
            0.22, 0.50 / math.sqrt(max(4.0, float(_FTJ_SAMPLES)))
        )
        safe_equity = _bounded(
            _FTJ_EQUITY
            - uncertainty * (1.0 - _FTJ_CONFIDENCE)
            + _strategy_adjustments(context)["equity_delta"],
            0.0,
            1.0,
        )
        ...
        return _bounded(safe_equity - jam_haircut, 0.0, 1.0)

    _ctx_prior = _ftj_context(FOLD_TO_JAM_PRIOR)
    _ctx_selective = _ftj_context(0.55)
    _equity_prior = _ftj_called_allin_equity(_ctx_prior)
    _equity_selective = _ftj_called_allin_equity(_ctx_selective)
    _intent_prior = _decision_from_equity(
        _ctx_prior, _FTJ_EQUITY, _FTJ_CONFIDENCE, _FTJ_SAMPLES
    )
    _intent_selective = _decision_from_equity(
        _ctx_selective, _FTJ_EQUITY, _FTJ_CONFIDENCE, _FTJ_SAMPLES
    )
    print("called_allin_equity fold_to_jam=0.28:", _equity_prior)
    print("called_allin_equity fold_to_jam=0.55:", _equity_selective)
    print("typed_intent fold_to_jam=0.28:", _intent_prior)
    print("typed_intent fold_to_jam=0.55:", _intent_selective)
    # Byte-identity invariant: at the prior the posterior fold_to_jam equals
    # the prior, so the conditioned haircut collapses to the historical 0.105.
    _prior_jam_fold_rate = _opponent_posterior(_ctx_prior)["fold_to_jam"]
    assert _prior_jam_fold_rate == FOLD_TO_JAM_PRIOR, (
        "prior fold_to_jam must leave the posterior at the prior"
    )
    assert (
        _bounded(0.105 * (_prior_jam_fold_rate / FOLD_TO_JAM_PRIOR), 0.05, 0.18)
        == 0.105
    ), "prior fold_to_jam must reproduce the historical 0.105 haircut"
    assert _equity_selective < _equity_prior - 0.03, (
        "selective caller must keep a materially stronger calling range"
    )
    assert _intent_selective != _intent_prior, (
        "terminal_response fold_to_jam must change a socket-validated intent"
    )
