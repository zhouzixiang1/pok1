...
def choose_raise(
    ...
    # Protective sizing floor — deny correct odds to draws on wet boards
    if round_idx in (1, 2) and value_profile is not None and value_profile.get('tier') in ('strong', 'nut'):
        from postflop import protective_sizing_floor
        p_floor = protective_sizing_floor(board_texture, value_profile, round_idx, pot_after_call)
        if p_floor > ratio:
            ratio = p_floor
...
def _was_flop_aggressor(req, my_id, state):
    """Detect if we bet/raised on the flop in this hand."""
    history = req.get('history', [])
    for record in history:
        if record['player_id'] == my_id and record['round'] == 1:
            if record['action_type'] in ('raise', 'allin'):
                return True
    return False


def _was_preflop_raiser(req, my_id):
    """Detect if we raised/bet preflop (round 0)."""
    history = req.get('history', [])
    for record in history:
        if record['player_id'] == my_id and record['round'] == 0:
            if record['action_type'] in ('raise', 'allin'):
                return True
    return False
...
def evaluate_turn_barrel(spot_info, opponent_model, board_texture, made_strength,
    ...
    barrel_type is one of: 'value', 'semi_bluff', 'bluff', or None.
    sizing_hint is a pot-fraction multiplier (e.g. 0.55 = 55% of pot).
    """
    if not was_flop_aggressor:
    ...
    # Value barrel: strong/nut hands on any texture
    if tier in ('strong', 'nut'):
        sizing = 0.55 + 0.20 * wetness  # larger on wet boards
        if tier == 'nut':
            sizing = max(sizing, 0.65)
        return {'barrel': True, 'barrel_type': 'value', 'sizing_hint': sizing}

    # Semi-bluff barrel: strong draws on wet boards with fold equity
    draw_quality = draw_info.get('quality', 0.0) if draw_info else 0.0
    if draw_quality >= 0.16 and confidence >= 0.15 and fold_to_raise > 0.46:
...
def evaluate_flop_cbet(spot_info, opponent_model, board_texture, made_strength,
                       draw_strength, draw_info, value_profile, opp_archetype='unknown'):
    """Evaluate flop c-bet as preflop raiser.

    Returns: {'cbet': bool, 'cbet_type': str or None, 'sizing_hint': float}
    cbet_type is one of: 'value', 'semi_bluff', 'bluff', or None.
    sizing_hint is a pot-fraction multiplier (e.g. 0.55 = 55% of pot).
    """
    if board_texture is None:
        return {'cbet': False, 'cbet_type': None, 'sizing_hint': 0.0}
    ...
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    draw_quality = draw_info.get('quality', 0.0) if draw_info else 0.0
    has_draw = draw_info.get('semi_bluff', False) if draw_info else False

    # Branch 1: Value c-bet — strong/nut hands on any texture
    if tier in ('strong', 'nut'):
        sizing = 0.55 + 0.15 * wetness
        return {'cbet': True, 'cbet_type': 'value', 'sizing_hint': sizing}

    # Branch 2: Semi-bluff c-bet — draws with fold equity
    if has_draw and draw_quality >= 0.14 and confidence >= 0.15 and fold_to_raise > 0.44:
        sizing = 0.45 + 0.10 * wetness
        return {'cbet': True, 'cbet_type': 'semi_bluff', 'sizing_hint': sizing}

    # Branch 3: Bluff c-bet — air on DRY boards vs fold-happy opponents only
    if (opp_archetype != 'calling_station'
            and confidence >= 0.20
            and fold_to_raise > 0.48
            and made_strength < 0.22
            and draw_strength < 0.10
            and wetness <= 0.20):
        sizing = 0.38 + 0.07 * wetness
        return {'cbet': True, 'cbet_type': 'bluff', 'sizing_hint': sizing}

    # Branch 4: Check-back — marginal/wet/unknown
    return {'cbet': False, 'cbet_type': None, 'sizing_hint': 0.0}


def evaluate_turn_checkraise(spot_info, opponent_model, board_texture, made_strength,
                             draw_strength, draw_info, value_profile, was_preflop_raiser,
                             opp_archetype='unknown'):
    """Turn check-raise as non-PFR. Returns dict like evaluate_flop_cbet."""
    if was_preflop_raiser:
        return {'checkraise': False, 'cr_type': None, 'sizing_hint': 0.0}
    if board_texture is None:
        return {'checkraise': False, 'cr_type': None, 'sizing_hint': 0.0}
    ...
    # Value check-raise: strong/nut hands
    if tier in ('strong', 'nut'):
        sizing = 0.55 + 0.20 * wetness
        if tier == 'nut':
            sizing = max(sizing, 0.65)
        return {'checkraise': True, 'cr_type': 'value', 'sizing_hint': sizing}

    # Semi-bluff: strong draws with fold equity
    if (draw_quality >= 0.18
            and confidence >= 0.20
            and fold_to_raise > 0.48):
        sizing = 0.50 + 0.15 * wetness
        return {'checkraise': True, 'cr_type': 'semi_bluff', 'sizing_hint': sizing}

    # Bluff: only vs confirmed fold-happy opponents (never vs calling_station)
    if (opp_archetype != 'calling_station'
            and confidence >= 0.30
            and fold_to_raise > 0.55
            and made_strength < 0.20
            and draw_strength < 0.10
            and wetness >= 0.25):
        sizing = 0.45 + 0.10 * wetness
        return {'checkraise': True, 'cr_type': 'bluff', 'sizing_hint': sizing}

    return {'checkraise': False, 'cr_type': None, 'sizing_hint': 0.0}


def evaluate_delayed_cbet(spot_info, opponent_model, board_texture, made_strength,
                          draw_strength, draw_info, value_profile, has_position):
    """Evaluate delayed c-bet on turn after PFR checked flop and opponent checked behind.

    Returns: {'delayed_cbet': bool, 'cbet_type': str or None, 'sizing_hint': float}
    cbet_type is one of: 'value', 'semi_bluff', 'bluff', or None.
    sizing_hint is a pot-fraction multiplier.
    """
    if board_texture is None:
        return {'delayed_cbet': False, 'cbet_type': None, 'sizing_hint': 0.0}
    ...
    # Value delayed c-bet: strong/nut hands bet for value
    if tier in ('strong', 'nut'):
        sizing = 0.50 + 0.15 * wetness
        if tier == 'nut':
            sizing = max(sizing, 0.60)
        return {'delayed_cbet': True, 'cbet_type': 'value', 'sizing_hint': sizing}

    # Semi-bluff: draws on wet boards with fold equity
    if draw_quality >= 0.14 and confidence >= 0.15 and fold_to_raise > 0.44:
        sizing = 0.42 + 0.12 * wetness
        return {'delayed_cbet': True, 'cbet_type': 'semi_bluff', 'sizing_hint': sizing}

    # Bluff: air on dry boards vs confirmed fold-happy opponents
    if (confidence >= 0.25
            and fold_to_raise > 0.50
            and made_strength < 0.22
            and draw_strength < 0.10
            and wetness <= 0.20):
        sizing = 0.38 + 0.08 * wetness
        return {'delayed_cbet': True, 'cbet_type': 'bluff', 'sizing_hint': sizing}
...
def get_action(req, requests):
    ...
    was_flop_aggressor = _was_flop_aggressor(req, my_id, state) if round_idx >= 2 else False
    was_pfr = _was_preflop_raiser(req, my_id) if round_idx > 0 else False
    ...
    # Archetype-aware postflop threshold adjustments
    if round_idx > 0 and opp_archetype != 'unknown':
        if opp_archetype == 'calling_station':
            medium -= 0.02  # Value bet thinner — they call with worse
            strong -= 0.01
        elif opp_archetype == 'nit':
            medium -= 0.03  # Bet more — they fold too much
        elif opp_archetype == 'lag':
            strong += 0.02  # Need stronger to value bet into aggression
            medium += 0.01
    ...
    if to_call > 0:
        ...
        # v42 direct equity fold gate: fold when equity clearly insufficient vs pot odds
        # Bypasses the complex call_margin system for clearly losing spots
        if (round_idx > 0
            and realized_rate < pot_odds - 0.08
            and (value_profile is None or value_profile.get("tier") not in ("strong", "nut"))
            and draw_strength < 0.14
            and made_strength < 0.40
            and not anti_lock_call_continue
            and not strong_made_continue):
            return -1
        # River barrel fold: opponent bet multiple streets, we have weak showdown
        if (round_idx == 3
            and spot_info.get("opp_postflop_bet_count", 0) >= 2
            and made_strength < 0.35
            and draw_strength < 0.10
            and (value_profile is None or value_profile.get("tier") not in ("strong", "nut"))
            and not anti_lock_call_continue
            and not strong_made_continue):
        ...
        if should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class=street_texture["class"], opp_archetype=opp_archetype):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            if not anti_lock_call_continue and not strong_made_continue:
        ...
        flop_checkraise_exploit = (
            ...
            and (
                ...
                or blocker_raise
                or (opp_archetype == 'lag' and made_strength >= 0.38 and draw_strength >= 0.08)
            )
        )

        turn_checkraise = False
        turn_cr_info = None
        if round_idx == 2 and not was_pfr:
            turn_cr_info = evaluate_turn_checkraise(
                spot_info, opponent_model, board_texture, made_strength,
                draw_strength, draw_info, value_profile, was_pfr,
                opp_archetype=opp_archetype,
            )
            turn_checkraise = turn_cr_info['checkraise']
        ...
        if not preflop_defensive_only and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit or turn_checkraise):
            raise_amount = choose_raise(
                state["min_raise_action"],
                my_chips,
                state["my_round_bet"],
                ...
                pressure_line=flop_checkraise_exploit or turn_checkraise,
    ...
    # ── River showdown value extraction ────────────────────────────────────────
    # Small calibrated bet with medium-strength hands vs wide opponents
    if round_idx == 3 and to_call == 0 and made_strength >= 0.35:
        from postflop import river_showdown_extraction
        extract_ratio = river_showdown_extraction(
            made_strength, value_profile, opponent_model, board_texture,
            round_idx, to_call, opp_archetype,
        )
        if extract_ratio > 0.0:
            extract_amount = max(
                state['min_raise_action'],
                int(pot * extract_ratio),
            )
    ...
    if (round_idx == 2 and to_call == 0 and was_flop_aggressor
        ...
        if barrel['barrel']:
            ...
            barrel_amount = max(
                state['min_raise_action'],
                int(pot * barrel_ratio),
            )
            if barrel_amount < my_chips and barrel_amount > 0:
                return barrel_amount

    # ── Delayed c-bet evaluation (PFR checked flop, turn aggression) ──────
    was_pfr_turn = was_pfr and round_idx == 2
    if (round_idx == 2 and to_call == 0 and was_pfr_turn and not was_flop_aggressor
            and not anti_lock_pressure
            and spot_info.get('last_opp_action_type') == 'check'):
        delayed = evaluate_delayed_cbet(
            spot_info, opponent_model, board_texture, made_strength,
            draw_strength, draw_info, value_profile,
            spot_info['has_position'],
        )
        if delayed['delayed_cbet']:
            delayed_amount = max(
                state['min_raise_action'],
                int(pot * delayed['sizing_hint']),
            )
            if delayed_amount < my_chips and delayed_amount > 0:
                return delayed_amount
    ...
    # ── Flop c-bet evaluation as PFR ──────────────────────────────────────
    if (round_idx == 1 and to_call == 0 and was_pfr
            and not anti_lock_pressure
            and not donk.get('eligible', False)
            and not probe.get('eligible', False)):
        cbet = evaluate_flop_cbet(
            spot_info, opponent_model, board_texture, made_strength,
            draw_strength, draw_info, value_profile,
            opp_archetype=opp_archetype,
        )
        if cbet['cbet']:
            cbet_amount = max(
                state['min_raise_action'],
                int(pot * cbet['sizing_hint']),
            )
            if cbet_amount < my_chips and cbet_amount > 0:
                return cbet_amount
    ...
    if opp_archetype == 'nit':
        river_bluff_threshold -= 0.05   # Bluffs more effective vs nits
        probe_fold_threshold -= 0.04
    elif opp_archetype == 'calling_station':
        river_bluff_threshold = 1.0      # Never bluff river vs calling stations
