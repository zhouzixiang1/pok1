...
def classify_preflop_gap_hand(my_cards, preflop_strength=None):
    """Identify dominated broadway hands like KTo, QJo that should limp rather than raise.

    These hands have high cards but large gaps, making them prone to domination
    when building big preflop pots. Limping avoids building big pots out of position.
    """
    profile = preflop_hand_profile(my_cards)
    if profile["pair"]:
        return None
