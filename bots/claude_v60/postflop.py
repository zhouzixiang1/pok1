...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opp_archetype='unknown'):
    ...
    if opp_archetype == 'nit':
        archetype_delta = 0.04   # NITs only bet strong — fold more (lower eff_made)
    elif opp_archetype == 'lag':
        archetype_delta = -0.03  # LAGs bluff — fold less (raise eff_made)
    elif opp_archetype == 'calling_station':
        archetype_delta = 0.02   # CS bets are honest — fold marginal (lower eff_made)
    ...
    if texture_class == "paired" and not has_draw and tier not in ("strong", "nut"):
        if round_idx >= 2 and eff_made < 0.30 and (size_bucket in ("medium", "large") or opp_bets >= 2):
            return True
    # Multi-street barrel fold: opponent bet 2+ postflop streets — strong line
    opp_postflop_bets = spot_info.get('opp_postflop_bet_count', 0)
    if opp_postflop_bets >= 2 and not has_draw and tier not in ('strong', 'nut'):
        if round_idx == 2 and eff_made < 0.30 and size_bucket in ('medium', 'large'):
            return True
...
def flop_cbet_strategy(made_strength, draw_strength, draw_info, value_profile,
                        board_texture, opponent_model, has_position,
                        texture_class='none', opp_archetype='unknown'):
    """Flop c-bet decision by texture class. Returns {action, ratio, reason}."""
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    has_draw = draw_strength >= 0.14
    confidence = opponent_model.get('confidence', 0.0)
    fold_to_raise = opponent_model.get('fold_to_raise', 0.44)
    ...
    if tier in ('strong', 'nut'):
        if texture_class == 'dry':
            return {'action': 'bet', 'ratio': 0.40, 'reason': 'dry_value_small'}
        elif texture_class in ('draw_heavy', 'monotone'):
            return {'action': 'bet', 'ratio': 0.65 + 0.08 * wetness, 'reason': 'wet_value_protect'}
        return {'action': 'bet', 'ratio': 0.50, 'reason': 'value_standard'}

    if has_draw and draw_info.get('semi_bluff', False):
        if texture_class in ('draw_heavy', 'monotone', 'semi_connected'):
            return {'action': 'bet', 'ratio': 0.50, 'reason': 'semi_bluff_wet'}
        if confidence >= 0.15 and fold_to_raise >= 0.48:
            return {'action': 'bet', 'ratio': 0.35, 'reason': 'semi_bluff_fe'}
        return {'action': 'check', 'ratio': 0.0, 'reason': 'draw_dry_check'}

    if tier == 'thin':
        if texture_class == 'dry':
            return {'action': 'bet', 'ratio': 0.35, 'reason': 'thin_dry_small'}
        return {'action': 'check', 'ratio': 0.0, 'reason': 'thin_wet_check'}

    if opp_archetype == 'calling_station':
        return {'action': 'check', 'ratio': 0.0, 'reason': 'air_vs_cs'}

    if made_strength < 0.25:
        if texture_class == 'dry':
            if confidence >= 0.15 and fold_to_raise >= 0.42:
                return {'action': 'bet', 'ratio': 0.33, 'reason': 'dry_bluff'}
            if has_position:
                return {'action': 'bet', 'ratio': 0.30, 'reason': 'dry_bluff_ip'}
        elif texture_class == 'paired':
            if fold_to_raise >= 0.42:
                return {'action': 'bet', 'ratio': 0.30, 'reason': 'paired_bluff'}
        elif texture_class in ('draw_heavy', 'monotone'):
            return {'action': 'check', 'ratio': 0.0, 'reason': 'air_wet_check'}
        elif texture_class == 'semi_connected' and has_position:
            if confidence >= 0.15 and fold_to_raise >= 0.45:
                return {'action': 'bet', 'ratio': 0.35, 'reason': 'semi_stab_ip'}
        return {'action': 'check', 'ratio': 0.0, 'reason': 'air_no_fe'}

    return {'action': 'check', 'ratio': 0.0, 'reason': 'default_check'}


def should_delayed_turn_cbet(made_strength, draw_strength, draw_info, value_profile,
                              board_texture, opponent_model, has_position,
                              texture_class='none', opp_archetype='unknown'):
    """Delayed turn c-bet: PFR checked flop, now bets turn.
    Returns: {'action': 'bet'|'check', 'ratio': float, 'reason': str}
    """
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    has_draw = draw_strength >= 0.14
    wetness = board_texture.get('wetness', 0.0) if board_texture else 0.0
    ...
    # Strong/nut: delayed value bet
    if tier in ('strong', 'nut'):
        ratio = 0.45 if texture_class == 'dry' else 0.50 + 0.05 * wetness
        return {'action': 'bet', 'ratio': clamp(ratio, 0.40, 0.60), 'reason': 'delayed_value'}

    # Semi-bluff with draw on wet/dynamic board
    if has_draw and draw_info.get('semi_bluff', False):
        if wetness >= 0.25 or board_texture.get('dynamic', False):
            return {'action': 'bet', 'ratio': 0.48, 'reason': 'delayed_semi_bluff'}
        if fold_to_raise >= 0.46 and has_position:
            return {'action': 'bet', 'ratio': 0.40, 'reason': 'delayed_draw_fe'}

    # Thin value on dry/static boards
    if tier == 'thin' and texture_class == 'dry' and made_strength >= 0.38:
        return {'action': 'bet', 'ratio': 0.40, 'reason': 'delayed_thin_dry'}

    # Selective bluff vs folding opponents on dry/paired boards
    if made_strength < 0.25 and texture_class in ('dry', 'paired'):
        if confidence >= 0.15 and fold_to_raise >= 0.48:
            if opp_archetype != 'calling_station':
                return {'action': 'bet', 'ratio': 0.42, 'reason': 'delayed_bluff'}

    # Thin tier on wet: check (let opponent barrel into us)
    if tier == 'thin' and texture_class in ('draw_heavy', 'monotone'):
        return {'action': 'check', 'ratio': 0.0, 'reason': 'delayed_thin_wet_check'}

    # vs calling station with weak hand: never bluff delayed
    if opp_archetype == 'calling_station' and tier not in ('strong', 'nut'):
        return {'action': 'check', 'ratio': 0.0, 'reason': 'delayed_vs_cs'}

    return {'action': 'check', 'ratio': 0.0, 'reason': 'delayed_default_check'}
