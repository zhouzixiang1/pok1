from constants import (
    ...
    RAISE_RATIO_PREFLOP_OPEN, RAISE_RATIO_PREFLOP_FACING,
    RAISE_RATIO_FLOP, RAISE_RATIO_TURN, RAISE_RATIO_RIVER,
)
...
from postflop import (
    ...
    flop_cbet_strategy, should_delayed_turn_cbet,
)
...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, texture_class="none"):
    ...
    return clamp(margin, 0.0, 0.07)  # Mutation: tightened from 0.08 (-12.5%)
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # Crossover from v13: tightened from 0.42 to 0.48 — v13's disciplined OOP
        # defense prevents calling too wide out of position vs aggressive 3-bettors
...
def get_action(req, requests):
    ...
    # ── Delayed turn c-bet (PFR checked flop, bet turn) ─────────────────
    if round_idx == 2 and to_call == 0 and was_pfr and not was_flop_aggressor:
        delayed = should_delayed_turn_cbet(
            made_strength, draw_strength, draw_info, value_profile,
            board_texture, opponent_model, spot_info['has_position'],
            texture_class=street_texture['class'],
            opp_archetype=opp_archetype,
        )
        if delayed['action'] == 'bet' and delayed['ratio'] > 0:
            delayed_amount = max(state['min_raise_action'], int(pot * delayed['ratio']))
