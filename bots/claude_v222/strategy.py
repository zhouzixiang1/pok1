...
def choose_raise(
    ...
    # v222 crossover (imported from v221): preflop SPR commitment gate.
    # Cap marginal one-pair-maker raises (preflop_strength 0.50-0.62) so the
    # projected SPR-if-called stays >= 4, preventing -20k stack-off exposure.
    # Soft cap: only applied when it yields a legal raise >= min_raise, so it
    # never suppresses legitimate raising (premium/trash hands bypass entirely).
    if round_idx == 0 and preflop_strength is not None:
        _raise_to_total = amount + my_round_bet
        _capped_total = _preflop_spr_commitment_gate(
            _raise_to_total, my_chips, my_round_bet, pot, preflop_strength
        )
        if _capped_total != _raise_to_total and _capped_total > my_round_bet:
            _capped_amount = _capped_total - my_round_bet
            if _capped_amount >= min_raise:
                amount = min(amount, _capped_amount)
...
def get_action(req, requests):
    ...
    if round_idx == 0 and preflop_strength is not None:
        # v222 crossover (imported from v221): preflop shove/4-bet defense.
        # Fold marginal hands (KTo, small pairs without odds, weak offsuit)
        # facing a preflop all-in or 4-bet+ shove. Mid-pair continuation is
        # VPIP-gated (see _preflop_shove_defense_fold). Bypassed under
        # anti-lock pressure (folding may hand opponent the match).
        if not anti_lock_pressure and _preflop_shove_defense_fold(
