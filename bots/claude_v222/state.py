...
def estimate_preflop_strength(my_cards):
    ...
    Corrected (v222 crossover import from v221/v187): the OLD v180 formula
    scored K2o (0.65) above 55 (0.64) and clamped 88-AA all to ~1.0. v180
    carried this mis-rank for 30+ generations. This formula matches real HU
    equities:
...
def _preflop_spr_commitment_gate(raise_to, my_chips, already_invested, pot,
    ...
    """Preflop SPR commitment gate (v222 crossover import from v221/v194).
    ...
    control), preventing -20k stack-off exposure. Premium hands (>0.62) and
    trash (<0.50) bypass the gate entirely.
...
def _preflop_shove_defense_fold(my_cards, preflop_strength, state, spot_info, opponent_model=None):
    """Preflop shove/4-bet defense gate (v222 crossover import from v221/v201).
    ...
    Mid pairs (77-99) continue only with cheap odds.
    Everything else folds. Returns True=fold, False=continue.

    v222 MUTATION (the single mutation): VPIP-GATED mid_pair continuation.
    v221 used an UNCONDITIONAL pot_odds<0.42 threshold, which the H2H data +
    prior critic flagged as -EV vs VALUE-HEAVY shovers (v206/v205/v212, all of
    whom v180 beats at 0.55-0.58 but v221 loses to at 0.35-0.40). Those
    opponents shove TIGHT, value-heavy ranges (QQ+/AK) where 77-99 are crushed.
    Against WIDE open-shovers (v186/v198, where v221 wins 0.65-0.70 but v180
    loses at 0.47-0.48) mid-pairs have real equity, so the looser 0.42 call is
    correct. The gate now keys off the opponent model's VPIP:
      - WIDE  (VPIP > 0.60): continue 77-99 at pot_odds < 0.42 (loose)
      - TIGHT (VPIP <= 0.60): continue 77-99 at pot_odds < 0.35 (strict)
    This preserves v180's value-heavy-shover defense while importing v221's
    edge versus wide preflop aggressors. Premiums/big-cards unaffected.
    """
    ...
    if hand_cat == 'mid_pair':
        # MUTATION: VPIP-gated continuation threshold.
        opp_vpip = opponent_model.get('vpip', 0.58) if opponent_model else 0.58
        mid_continue_odds = 0.42 if opp_vpip > 0.60 else 0.35
        if pot_odds < mid_continue_odds:
            return False
    return True
