...
from fold_gates import (
    _should_checkraise_trap, _spr_commitment_gate, _allin_board_texture_fold,
)
...
def choose_raise(
    ...
    # Value-tier sizing floor: ensure pot-building bets on made hands.
    # Prevents stacking downward modifiers from pushing value bets below
    # street-aware minimums (fixes global 0.4x pot underbetting bias).
    _tier = value_profile.get('tier', 'none')
    if (_tier in ('nut', 'strong') and not semi_bluff and not blocker_bluff
            and not probe_mode and not inducing_value and thin_cap is None):
        if round_idx == 1:
            _value_floor = 0.50
        elif round_idx == 2:
            _value_floor = 0.55
            ...
            _value_floor = low_ratio
        if _value_floor > low_ratio:
            low_ratio = _value_floor
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        ...
        raise_buckets = {'premium', 'strong_pair', 'mid_pair', 'big_cards',
                         'broadway_suited', 'suited_ace', 'suited_connector'}
        limp_buckets = {'small_pair', 'playable'}
        ...
        if hand_cat in raise_buckets and raise_amount is not None:
            return raise_amount
        if hand_cat in limp_buckets:
            return 0  # limp
        return -1  # fold trash

    if spot_info["preflop_spot"] == "bb_vs_limp":
        hand_cat = classify_preflop_hand(req["my_cards"])
        iso_buckets = {'premium', 'strong_pair', 'mid_pair',
                       'big_cards', 'broadway_suited', 'suited_ace',
                       'suited_connector'}
        # Widen iso-raise vs loose/limpy opponents to keep initiative + fold equity.
        _conf = opponent_model.get("confidence", 0.0)
        if _conf > 0.20 and opponent_model.get("vpip", 0.0) > 0.55:
            iso_buckets.add('small_pair')
        raise_amount = choose_raise(
            state["min_raise_action"],
            my_chips,
            state["my_round_bet"],
        ...
        if hand_cat in iso_buckets and raise_amount is not None:
            return raise_amount
        return 0  # check (BB option)

    elif spot_info['preflop_spot'] == 'bb_vs_raise':
        pot_odds_pf = to_call / (to_call + state['pot']) if to_call > 0 else 0.0
        hand_cat = classify_preflop_hand(req['my_cards'])
        # Value 3bet: premiums (QQ+) and big cards (AK/AQ/AJo) only.
        # NOTE: estimate_preflop_strength saturates 88-AA to 1.0, so the old
        # >=0.60 gate value-3bet 88 like AA. The hand-class bucket fixes this:
        # 88/99 (mid_pair) and TT/JJ (strong_pair) now call instead of 3-bet.
        ...
        # Bluff 3bet: suited connectors, suited aces, small pairs (~22% freq)
        if hand_cat in ('suited_connector', 'suited_ace', 'small_pair') and not trash_hand:
            ...
            if bluff_roll < 0.22:
                ...
                if raise_amount is not None:
                    return raise_amount
        # Call with strong/mid pairs (TT-JJ, 77-99), broadway suited, playable hands
        if hand_cat in ('strong_pair', 'mid_pair', 'playable', 'broadway_suited'):
            return 0
        ...
        # Limp-reraise with TRUE premiums (QQ+) and big cards (AK/AQ) only.
        # Same saturation fix as bb_vs_raise: old >=0.58 gate reraised 55+/KQs/JTs.
        if hand_cat_iso in ('premium', 'big_cards') and not trash_hand:
            raise_amount = choose_raise(
                state['min_raise_action'], my_chips, state['my_round_bet'],
                to_call, state['pot'], max(win_rate, preflop_strength),
            ...
            if raise_amount is not None:
                return raise_amount
            return 0  # Call if reraise sizing fails
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        if round_idx >= 2 and line_profile['line_label'] == 'value_heavy':
            if (value_profile is None or value_profile['tier'] not in ('strong', 'nut')) and draw_strength < 0.18 and not anti_lock_jam_continue:
                return -1
        # Board-texture-aware fold for non-nut hands facing all-in.
        _allin_fold = _allin_board_texture_fold(
            round_idx, made_strength, draw_strength,
            value_profile, board_texture, nutted_risk, to_call, pot,
        )
        if _allin_fold == -1 and not anti_lock_jam_continue:
            return -1
    ...
    if to_call >= my_chips:
        ...
        if round_idx >= 2 and line_profile['line_label'] == 'value_heavy':
            if (value_profile is None or value_profile['tier'] not in ('strong', 'nut')) and draw_strength < 0.18 and not anti_lock_shove_continue:
                return -1
        # Board-texture-aware fold for non-nut hands facing near-all-in.
        _allin_fold = _allin_board_texture_fold(
            round_idx, made_strength, draw_strength,
            value_profile, board_texture, nutted_risk, to_call, pot,
        )
        if _allin_fold == -1 and not anti_lock_shove_continue:
            return -1
    ...
    if to_call > 0:
        # Structural commitment gate — fires BEFORE must_continue_vs_raise to
        # break the historical 0% postflop fold rate. Folds marginal made hands
        # facing large commitment bets on turn/river using new axes (commit
        # ratio, pot ratio, SPR, board scariness).
        _gate = _spr_commitment_gate(
            round_idx, to_call, pot, my_chips, made_strength,
