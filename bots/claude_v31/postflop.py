...
def postflop_fold_incentive(made_strength, draw_strength, pair_profile, round_idx, spot_info, board_texture):
    if round_idx <= 0 or not spot_info.get('facing_postflop_aggression', False): return 0.0
    sz, ob, inc = bet_size_bucket(spot_info.get('last_raise_pot_ratio', 0.0)), spot_info.get('opp_postflop_bet_count', 0), 0.0; p = pair_profile or {}
    if made_strength < 0.18 and draw_strength < 0.08: inc += {1: {'small': 0.020, 'medium': 0.035, 'large': 0.050}.get(sz, 0.020), 2: {'small': 0.035, 'medium': 0.050, 'large': 0.050}.get(sz, 0.035), 3: 0.050}.get(round_idx, 0.0)
    if p.get('made_class') == 1 and p.get('pair_type') in ('middle_pair','bottom_pair','underpair','board_pair') and draw_strength < 0.10 and p.get('pair_rank', 14) < 11: inc += {2: 0.025 if sz in ('medium','large') else (0.020 if ob >= 2 else 0.0), 3: 0.035 if sz in ('medium','large') else 0.015}.get(round_idx, 0.0)
    if ob >= 2 and made_strength < 0.35 and draw_strength < 0.14: inc += 0.020
    if board_texture and not board_texture.get('dynamic', True) and made_strength < 0.30: inc += 0.010
    return min(max(inc, 0.0), 0.08)
