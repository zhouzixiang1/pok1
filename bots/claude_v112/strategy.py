...
def choose_raise(
    ...
    # CROSSOVER (from v91): value-tier sizing floor — ensures pot-building bets
    # on nut/strong made hands. Prevents downward modifiers from pushing value
    # bets below street-aware minimums (fixes global underbetting bias on value).
    # This is an OFFENSIVE value-extraction feature (not a defensive guard), so
    # it complements v111's offensive preflop bucket structure.
    # MUTATION: raise floors ~15% (0.50->0.58, 0.55->0.62, 0.60->0.68) to push
    # harder for value extraction on nutted hands. Memory directive: offensive
    # sizing floors/tiers remain permitted and v107-111 exhausted defensive
    # guards; this mutation leans into the OFFENSE pivot.
