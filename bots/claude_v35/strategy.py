...
from state import (
    ...
    preflop_hand_profile, classify_preflop_gap_hand,
)
...
from opponent import build_opponent_model, analyze_current_spot, classify_opponent_archetype, archetype_river_fold_gate
...
def realized_postflop_equity(
    ...
    if air_hand:
        ...
        # Mutation: opponent-model-aware EQR for air hands vs heavy barrelers
        # When confirmed heavy barreler (freq >= 0.60), reduce EQR further —
        # air hands realize even less equity vs opponents who barrel turns and rivers
...
def choose_raise(
    ...
    thin_cap = None
    if value_plan.get("thin_control", False) and value_profile.get("tier") not in ("nut", "strong"):
    ...
    # v34: river value sizing cap for strong (non-nut) hands
    # Prevent oversized raises that become de facto all-ins;
    # cap at 0.75x pot for strong tier, allowing nut hands to overbet freely
    if round_idx == 3 and value_profile is not None:
        tier = value_profile.get('tier', 'none')
        if tier == 'strong' and not overbet_mode and not pressure_line:
            river_value_cap = 0.75
            ratio = min(ratio, river_value_cap)
        elif tier == 'thin' and not overbet_mode:
            river_thin_cap = 0.55
            ratio = min(ratio, river_thin_cap)
    ...
    # Value raise floor: strong postflop raises should be at least 50% pot
    if round_idx > 0 and not semi_bluff and not blocker_bluff and not probe_mode:
        if value_profile is not None and value_profile.get("tier") == "strong":
            low_ratio = max(low_ratio, 0.50)
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        ...
        # Crossover from v21: gap broadway hands (KTo, QJo) limp instead of raise
        # to avoid building big pots with dominated holdings
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # Structural gate: skip bluff 3bet vs calling stations (they don't fold enough)
        can_bluff_3bet = opp_archetype != 'calling_station'
        if can_bluff_3bet and BB_BLUFF_3BET_LOW <= preflop_strength <= BB_BLUFF_3BET_HIGH and not trash_hand:
        ...
        if preflop_strength >= BB_CALL_THRESHOLD or win_rate >= pot_odds_pf - 0.02:
            return 0
        return -1
        ...
        # Crossover from v21: handle limped pots facing a raise
        # We showed weakness by limping, so play tight
...
def should_barrel_turn(round_idx, to_call, spot_info, value_profile, board_texture,
                       made_strength, draw_strength, draw_info, opponent_model,
                       win_rate, pot, my_chips, state, opp_archetype='unknown'):
    """Decide whether to barrel the turn after flop c-bet was called.
    Returns raise amount (int) or None to defer to existing logic."""
    if round_idx != 2 or to_call != 0:
        return None
    # Only barrel when we were the flop aggressor (opponent didn't raise postflop)
    if spot_info.get("opp_prior_postflop_raise_count", 0) > 0:
        return None
    # Don't barrel with nuts — use induce/trap instead
    if value_profile is not None and value_profile["tier"] == "nut":
        return None
    # Need minimum equity to barrel
    has_value = made_strength >= 0.35
    has_draw = draw_strength >= 0.14
    has_equity = win_rate >= 0.45
    if not (has_value or has_draw or has_equity):
        return None
    # Structural gate: calling stations don't fold to barrels — only barrel with value
    if opp_archetype == 'calling_station' and not has_value:
        return None
    # Extract opponent model confidence once for reuse
    opp_conf = opponent_model.get('confidence', 0.0) if opponent_model else 0.0
    # Structural gate: LAG opponents call light — skip pure bluff barrels
    if opp_archetype == 'lag' and not has_value:
        return None
    # Opponent-model fold equity gate (requires confidence >= 0.15)
    if opp_conf >= 0.15:
        ftr = opponent_model.get('fold_to_raise', 0.50)
        # Skip pure bluff barrels vs non-folding opponents
        if ftr < 0.38 and not has_value and not has_draw:
            return None
    # Board texture factors
    wetness = board_texture.get("wetness", 0.0) if board_texture else 0.0
    is_dynamic = board_texture.get("dynamic", False) if board_texture else False
    # Barrel sizing: 40-62% pot depending on hand and texture
    barrel_ratio = 0.42
    # Opponent-model sizing adjustments
    if opp_conf >= 0.15:
        ftr = opponent_model.get('fold_to_raise', 0.50)
        opp_barrel = opponent_model.get('barrel_freq', 0.40)
        if opp_archetype == 'nit':
            barrel_ratio += 0.05  # bigger vs overfolders
        elif ftr >= 0.55:
            barrel_ratio += 0.04  # bigger vs folders
        elif ftr <= 0.42:
            barrel_ratio -= 0.03  # smaller vs callers
        if opp_barrel >= 0.55 and not has_value:
            barrel_ratio -= 0.03  # heavy barrelers hero-call bluffs
    if has_value:
        barrel_ratio += 0.10 * min(1.0, max(0, made_strength - 0.3) / 0.3)
    if has_draw and not has_value:
        barrel_ratio = 0.40  # semi-bluff sizing
    if is_dynamic or wetness >= 0.35:
        barrel_ratio += 0.05  # bigger on wet/scary boards
    if value_profile is not None and value_profile["tier"] == "strong":
        barrel_ratio += 0.04  # value barrel bigger
    # Cap barrel sizing
    barrel_ratio = min(barrel_ratio, 0.62)
    min_raise = state.get("min_raise_action", state.get("round_raise", 100))
    amount = max(min_raise, int(pot * barrel_ratio))
    amount = min(amount, my_chips - 1)
    if amount >= my_chips or amount < min_raise:
        return None
    return amount
...
def get_action(req, requests):
    ...
    opp_archetype = opponent_model.get('opp_archetype', 'unknown')
    ...
    if to_call > 0:
        ...
        if should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class=street_texture["class"]):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # v34: archetype-aware river fold gate
        if round_idx == 3 and not anti_lock_call_continue and not strong_made_continue:
            if archetype_river_fold_gate(
                opp_archetype, opponent_model.get('confidence', 0.0),
                made_strength, draw_strength,
                bet_size_bucket(spot_info['last_raise_pot_ratio']),
                spot_info.get('opp_current_round_bet_count', 0),
            ):
                return -1
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            if not anti_lock_call_continue and not strong_made_continue:
        ...
        if realized_rate < call_threshold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # Crossover from v21: trap fold with strength check
        # Fold very weak hands vs repeated raises on medium/large sizing, but call with draws or reasonable hands
    ...
    # ── Turn barrel continuation ──────────────────────────────────────────────
    # After flop c-bet was called, barrel turn with value/draw hands
    barrel = should_barrel_turn(
        round_idx, to_call, spot_info, value_profile, board_texture,
        made_strength, draw_strength, draw_info, opponent_model,
        win_rate, pot, my_chips, state, opp_archetype=opp_archetype
    )
