...
def archetype_river_fold_gate(opp_archetype, opp_confidence, made_strength, draw_strength, size_bucket, opp_bets):
    """Archetype-aware river fold decision.

    Returns True if we should fold based on opponent behavioral archetype.
    Only active when confidence >= 0.15 (enough data to classify).
    """
    if opp_confidence < 0.15:
        return False
    ...
    if opp_archetype == 'lag':
        # LAGs barrel wide; their river bets are polarized nuts/bluffs.
        # Fold weak pairs (middle/bottom/under) to medium+ sizing
        if size_bucket in ('medium', 'large') and made_strength < 0.42:
            return True
        # Fold marginal pairs to any repeated river bet
        if opp_bets >= 2 and made_strength < 0.38:
            return True
    elif opp_archetype == 'nit':
        # Nits rarely bet river; when they do, it's the nuts.
        # Fold everything below strong pair to any sizing
        if size_bucket in ('medium', 'large') and made_strength < 0.50:
            return True
        if opp_bets >= 2 and made_strength < 0.45:
            return True
    elif opp_archetype == 'calling_station':
        # Calling stations bet erratically; don't fold marginal pairs
        # Only fold complete air
        return False
    elif opp_archetype == 'tag':
        # TAGs are balanced; fold only very weak hands to big bets
        if size_bucket == 'large' and made_strength < 0.35:
            return True
    return False
