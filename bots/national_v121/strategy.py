...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, texture_class="none"):
    ...
    if texture_class == "dry":
        ...
        # boards ranges are narrow, so defending wider is correct. Deepening the
        # discount to -0.040 (from v117's -0.025) calls ~1.5pp wider, reducing
        # the over-fold tendency that nemeses exploited.
        margin -= 0.040
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        # Polarized jam equity override for marginal hands vs confirmed polarized jammers.
        # When opponent shoves all-in and is a confirmed polarized shover, recompute
        # equity vs {value-jam, bluff-jam} range instead of using generic win_rate.
        if round_idx >= 2 and not anti_lock_pressure:
            ...
            if _pj_tier in ("thin", "none") and 0.20 <= made_strength < 0.55:
                _pj_shove = opponent_model.get("shove_rate", 0.08)
                _pj_pol = opponent_model.get(
                    "turn_polarity" if round_idx == 2 else "river_polarity", 0.0
                )
                if _pj_shove > 0.12 or _pj_pol > 0.15:
                    _pj_eq = polarized_jam_equity(
                        my_cards, public_cards, combos, weights,
                        opponent_model, iterations=200,
                    )
                    _pj_fold = _pj_eq < jam_odds
                    sys.stderr.write(
                        f"POLARIZED_JAM_EQUITY fired=1 round={round_idx} "
                        f"tier={_pj_tier} made={made_strength:.3f} "
                        f"polarized_eq={_pj_eq:.3f} generic_wr={win_rate:.3f} "
                        f"jam_odds={jam_odds:.3f} shove_rate={_pj_shove:.3f} "
                        f"street_pol={_pj_pol:+.3f} "
                        f"decision={'fold' if _pj_fold else 'call'}\n"
                    )
                    if _pj_fold:
                        return -1
