...
# River bluff admission is keyed by a salt distinct from the flop/turn 0xB10FF
# mix so the river subset is decorrelated from the street-bluff subset: a hand
# drawn into one channel never carries a fixed bias into the other.
RIVER_BLUFF_SALT = 0x51F3E3B1FF
...
RIVER_BLUFF_MIN_TERMINAL_WEIGHT = 0.20


def _bluff_allowed(context, equity):
    """Permit deterministic low-frequency semi-bluffs, never fixed air spam.

    On the river there is no draw to semi-bluff, so admission is earned only
    from tracked terminal evidence: the reducer's ``terminal_response``
    posterior over ``river_overcall``, already blended toward its prior by the
    published reach-scaled adaptation weight.  A prior-level or thin-evidence
    tracker stays below the weight floor and the channel stays closed, exactly
    ...
    if _number(betting.get("to_call"), 0.0) > 0:
        return False
    if street == "river":
        cards = context.get("cards", {}) or {}
        hole = _card_ids(cards.get("hole", ()))
        ...
        if (
            ...
            or _made_hand_tier(hole, board) not in {"air", "weak"}
        ):
            return False
        posterior = _opponent_posterior(context)
        if posterior["terminal_weight"] < RIVER_BLUFF_MIN_TERMINAL_WEIGHT:
            return False
        # ``_candidate_raise_fractions`` reaches this function again through
        # ``_polarized_raise_fraction``, so the breakeven must not recompute
        # the candidate list here; it uses the smallest base size the EV loop
        # prices, which is the cheapest bluff the channel can publish.
        smallest = 0.38
        non_call = 1.0 - posterior["river_overcall"]
        if non_call < smallest / (1.0 + smallest):
            return False
        return _deterministic_mix(context, RIVER_BLUFF_SALT) < 0.30
