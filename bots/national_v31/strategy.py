...
def _spr_commitment_gate(round_idx, made_strength, value_profile, win_rate, pot_odds, to_call, pot, my_chips, anti_lock_pressure, anti_lock_call_continue, strong_made_continue):
    ...
    # Graduated SPR ceiling: turn allows wider band (one card to come),
    # river stays tight (equity fully realized).
    spr_ceiling = 4.0 if round_idx == 2 else 2.5
    if spr > spr_ceiling:
    ...
    if tier in ("strong", "nut"):
        ...
        return None
    made_ceiling = 0.50 if round_idx == 2 else 0.55
    ...
    if win_rate < pot_odds:
        sys.stderr.write(
            ...
            f"win={win_rate:.3f} pot_odds={pot_odds:.3f} spr={spr:.2f} to_call={to_call}\n"
        )
        return -1
    sys.stderr.write(
