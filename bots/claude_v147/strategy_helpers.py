...
def _spr_commitment_gate(round_idx, my_chips, made_strength, value_profile,
    ...
    """
    # River-only — preflop/turn commitment math is complex, defer to existing logic.
    if round_idx != 3 or anti_lock_pressure:
        return False
    ...
    if made_strength >= 0.55:
        return False
    # Preserve line-aware value: a value_heavy line with a live draw may still improve.
...
if __name__ == "__main__":
    ...
    # v147 _spr_commitment_gate fixture: 6 cases
    vp_thin = {"tier": "thin"}
    vp_strong = {"tier": "strong"}
    # F1: turn (round_idx=2) -> False (gate is river-only)
    g = _spr_commitment_gate(2, 5000, 0.42, vp_thin, 5000, 5000, 0.30, False, 'balanced', 0.05)
    assert g is False, f"F1 fail: {g}"
    ...
    # F3: river, tier=thin, made=0.42, to_call=5000, pot=5000, win=0.30 -> True
    #   (pot_odds=0.5, win 0.30 << 0.53, clear fold)
    g = _spr_commitment_gate(3, 5000, 0.42, vp_thin, 5000, 5000, 0.30, False, 'balanced', 0.05)
    assert g is True, f"F3 fail: {g}"
    # F4: river, tier=thin, made=0.50, to_call=5000, pot=5000, win=0.55 -> False
    #   (win 0.55 > pot_odds+0.03=0.53, equity sufficient)
    g = _spr_commitment_gate(3, 5000, 0.50, vp_thin, 5000, 5000, 0.55, False, 'balanced', 0.05)
    assert g is False, f"F4 fail: {g}"
    # F5: river, anti_lock_pressure=True -> False (preserve match-lock defense)
    g = _spr_commitment_gate(3, 5000, 0.42, vp_thin, 5000, 5000, 0.30, True, 'balanced', 0.05)
    assert g is False, f"F5 fail: {g}"
    # F6: river, tier=none, made=0.35, to_call=10000, pot=2000, win=0.20 -> True
    #   (overbet allin, pot_odds=0.83, win 0.20 << 0.86, clear fold)
    g = _spr_commitment_gate(3, 10000, 0.35, None, 10000, 2000, 0.20, False, 'balanced', 0.05)
    assert g is True, f"F6 fail: {g}"
    print("v147 _spr_commitment_gate fixture: 6/6 PASS")
    ...
    print(f"v147 _spr_commitment_gate dispatch-reachability + placement: PASS ({n_refs} refs, call before return)")
