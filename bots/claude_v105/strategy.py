...
def _dry_board_value_barrel(
    ...
    sizing_exploit_delta,
):
    """Enhanced value barrel for strong/nut hands on dry boards.

    On dry boards (low wetness, not dynamic, not paired), opponents rarely
    connect with the board.  The default choose_raise sizing (~0.60x flop)
    under-extracts from opponents who float with weak pairs or overcards.
    This function returns a larger barrel sizing to maximize value.

    TRAP PRESERVATION: On the flop, if the check-raise trap would fire
    (_should_checkraise_trap returns True), returns None so the caller
    falls through to the trap dispatch (return 0 = check to trap).

    Returns a raise-to-total amount (int) or None.
    """
    # --- Guard: only for postflop strong/nut hands on dry boards ---
    if round_idx == 0:
        return None
    if value_profile is None or value_profile.get("tier") not in ("strong", "nut"):
        return None
    ...
    # --- Trap-preservation guard (flop only) ---
    # On the flop, if the check-raise trap qualifies, return None so the
    # trap path handles the spot instead (check to induce bluffs/shoves).
    ...
    # --- Compute dry-board value barrel sizing ---
    wetness = board_texture.get("wetness", 0.0)
    tier = value_profile.get("tier", "none")

    # Base ratio: larger on later streets where board has clarified
    if round_idx == 1:  # flop
        base_ratio = 0.72
    ...
    # Nut hands bet slightly bigger than strong hands
    if tier == "nut":
        base_ratio += 0.05

    # Slightly bigger when opponent has shown willingness to call
    fold_to_raise = opponent_model.get("fold_to_raise", 0.45)
    if fold_to_raise < 0.42:
        base_ratio += 0.04  # opponent calls more → bet bigger
    elif fold_to_raise > 0.55:
        base_ratio -= 0.04  # opponent folds a lot → slightly smaller

    # Apply exploit sizing delta from opponent model
    base_ratio += sizing_exploit_delta

    # Clamp to reasonable bounds
    base_ratio = clamp(base_ratio, 0.55, 1.10)
...
def get_action(req, requests):
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or exploit['should_barrel']:
        ...
        # Dry-board value barrel: extract more value on dry boards where
        # opponents rarely connect. Returns None for non-qualifying spots
        # (and for flop traps, which are handled above).
        dry_barrel = _dry_board_value_barrel(
            value_profile, round_idx, board_texture, opponent_model,
            made_strength, draw_strength, my_cards, public_cards,
            to_call, pot, my_chips, state["min_raise_action"],
            state["my_round_bet"], sizing_delta + exploit_sizing,
        )
        if dry_barrel is not None:
            return dry_barrel
        raise_amount = choose_raise(
            state["min_raise_action"],
            my_chips,
