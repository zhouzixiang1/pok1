...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if not preflop_defensive_only and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit):
            ...
            if raise_amount is not None and raise_amount > to_call:
                if round_idx == 3 and raise_amount > 0.70 * my_chips and (value_profile is None or value_profile.get('tier') not in ('nut',)):
                    return 0  # River near-all-in raise with non-nut hand → just call
                return raise_amount
        return 0
    ...
    if round_idx == 3 and to_call == 0 and made_strength >= 0.35:
        ...
        if extract_ratio > 0.0:
            ...
            if extract_amount > 0:
                extract_amount = min(extract_amount, my_chips - 1)
                if extract_amount >= state['min_raise_action']:
                    if (value_profile is None or value_profile.get('tier') != 'nut') and extract_amount > 0.70 * my_chips:
                        return 0  # Near-all-in value bet with non-nut hand → just check
