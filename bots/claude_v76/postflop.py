...
def river_showdown_extraction(made_strength, value_profile, opponent_model, board_texture, round_idx, to_call, opp_archetype='unknown'):
    ...
    if made_strength < 0.35 or made_strength >= 0.85:
        return 0.0
    ...
    # Graduated value tier: strong-but-not-nut hands (0.62-0.85)
    # Prevents fall-through to aggressive general raise path
    if made_strength >= 0.62 and value_profile is not None and value_profile.get('tier') in ('strong', 'thin'):
        base_ratio = 0.60 + 0.20 * min(1.0, (made_strength - 0.62) / 0.23)
        confidence = opponent_model.get('confidence', 0.0)
        if confidence >= 0.20:
            call_freq = 1.0 - opponent_model.get('fold_to_raise', 0.44)
            if call_freq >= 0.60:
        ...
        if opp_archetype == 'nit':
            ...
            base_ratio += 0.05
        return clamp(base_ratio, 0.55, 0.80)
