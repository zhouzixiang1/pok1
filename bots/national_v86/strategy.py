...
def sanitize_action(action, state, my_chips):
    ...
    if action > 0:
        min_raise_total = state.get("min_raise_total")
        if min_raise_total is None:
            min_raise_delta = state.get("min_raise_action", state["round_raise"])
            min_raise_total = state.get("my_round_bet", 0) + min_raise_delta
            if state.get("round") == 0:
                min_raise_total = max(min_raise_total, 2 * BIG_BLIND)
        action_total = action
        if action_total < min_raise_total:
            delta_total = state.get("my_round_bet", 0) + action
            action_total = delta_total if delta_total >= min_raise_total else min_raise_total
        needed = max(0, action_total - state.get("my_round_bet", 0))
        if needed >= my_chips:
            return -2
        if action_total <= state["to_call"]:
            return 0
        return action_total
...
def main():
    sample_req = {
        ...
        "dealer_id": 0,
        "my_id": 0,
        "my_chips": INITIAL_CHIPS - SMALL_BLIND,
        "my_cards": [48, 44],
        "public_cards": [],
        "history": [],
        "hand": 0,
    ...
    action = get_action(sample_req, [sample_req])
    print(f"strategy self-test passed, action={int(action)}")
