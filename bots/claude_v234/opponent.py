    ...
    for req in hand_requests:
        ...
        for record in history:
            ...
            if action_type == "raise" and action > 0:
                ...
                high_bet = max(round_bets.values())
                round_bets[pid] = max(round_bets[pid], high_bet)
                # v195 QUALITY FIX (is_allin recording bug): the v193/v194
                # betsize-magnitude sample was ONLY recorded inside the
                # `if action_type == "raise"` branch above, so the
                # `action_type == "allin"` discriminator was ALWAYS False and
                # shove_rate was permanently 0.0. Record the allin here with
                # is_allin=True so _opp_betsize_polarity can classify shovers.
                # The exact allin amount is hidden (action=-2), so we use a
                # conservative lower-bound ratio (>=1.0 => 'overbet' bucket);
                # this is sufficient for the shove/overbettor classification.
                if pid == opponent_id and round_idx > 0:
                    pot_estimate = prior_rounds_pot + round_bets[my_id] + round_bets[opponent_id]
                    if pot_estimate > 0:
                        _betsize_magnitude_samples.append(
...
def _multibarrel_line_fold(om, spot_info, round_idx, to_call, pot,
    ...
    Fires when ALL hold:
      ...
      - made_strength < 0.48 (v224: weak one-pair through upper two-pair
        bluff-catcher; trips+ at 0.58 preserved)
    ...
    if spot_info.get('opp_current_round_bet_count', 0) < 1:
        return False
    # Guard 3: weak bluff-catcher band. v224 MUTATION (crossover v195xv178):
    # raised ceiling 0.42 -> 0.48 (+14.3%). Rationale: the experience pool's
    # AUTHORITATIVE over-call leak band is 0.20 <= made < 0.45 vs value-heavy
    # shovers (the #1 documented -20k stack-off leak). v195's 0.42 ceiling let
    # the UPPER two-pair band (0.42-0.48, e.g. top-two-pair / overpair+kicker
    # stack-offs) escape the fold gate on confirmed multi-barrel lines — exactly
    # v195's losses (v178 0.429 / v194 0.417 / v200 0.440, all value-heavy
    # shovers). The tier guard below still protects tier=='nut' (never folds)
    # and strong ONE-PAIR (tier=='strong' & made<0.35 — overpair/TPTK, made
    # ~0.22), and the loose-caller carve-out (ftb<0.35) exempts calling
    # stations, so over-fold risk is contained. Trips+ (0.58) stays excluded.
    # This is the same mutation v215 battle-validated (0.42->0.48 beat both
    # parents in n=20).
    ...
    # two-pair (~0.40); trips+ (0.58) already excluded by made_strength<0.48.
...
def _estimate_bluff_frequency(om):
    ...
    Signals read (with smooth_rate priors from build_opponent_model):
      ...
      - v195 MUTATION: betsize_polarity from _opp_betsize_polarity() —
        underbettor (condensed thin-value range) floor at 0.30 so the
        polarized discount does NOT over-fold vs thin-value bettors.
    """
    if om is None:
        return 0.25
    conf = om.get('confidence', 0.0)
    ...
    # v195 MUTATION (crossover fold-side payoff): betsize_polarity calibration.
    # Experience pool mandate: "v193 betsize_polarity ... apply to FOLD-SIDE
    # CALIBRATION (underbettor=condensed/thin-value -> AVOID over-fold vs
    # thin-value bettors on turn/river to_call>0), NOT the exhausted value-lead
    # axis." An underbettor's bets are condensed thin-value (small sizing with
    # medium-strength hands), NOT polarized nuts-or-air. The standard 0.65
    # polarized discount UNDERESTIMATES our bluff-catcher equity vs such a
    # range, so the v188/v189 fold framework can OVER-FOLD. Flooring
    # bluff_freq at 0.30 raises the discount to 0.70 -> higher calibrated_equity
    # -> smaller equity_gap -> less aggressive folding. This is the fold-side
    # PAYOFF for the v193 _opp_betsize_polarity structural axis.
    # Confidence gate (>=0.25 ~= 3+ betsize samples) prevents misclassification
    # from tiny samples.
...
if __name__ == '__main__':
    ...
    # v224 MUTATION: UPPER two-pair band (0.42 < made < 0.48) now fires too.
    # At the old 0.42 ceiling this made=0.45 hand was PROTECTED (return False);
    # the raised ceiling covers the full documented over-call leak band
    # (0.20 <= made < 0.45). Catches top-two-pair stack-offs vs value-heavy shovers.
    assert _multibarrel_line_fold(_mb_om_value, _mb_spot_fire, 2, 400, 600, 0.45, {'tier':'strong'}, 0.05) is True, \
        'v224: upper two-pair (0.45) MUST fold on confirmed multi-barrel line'
    assert _multibarrel_line_fold(_mb_om_value, _mb_spot_fire, 3, 400, 600, 0.47, {'tier':'strong'}, 0.05) is True, \
        'v224: last foldable boundary 0.47 (just under the 0.48 ceiling) MUST fold'
    # v224: ceiling boundary 0.48 itself is PROTECTED (>= 0.48); trips (0.58) too.
    assert _multibarrel_line_fold(_mb_om_value, _mb_spot_fire, 2, 400, 600, 0.48, {'tier':'strong'}, 0.05) is False, \
        'v224: made=0.48 (>= ceiling) protected — trips/set boundary'
    assert _multibarrel_line_fold(_mb_om_value, _mb_spot_fire, 2, 400, 600, 0.58, {'tier':'strong'}, 0.05) is False, \
        'trips (0.58 >= 0.48 ceiling) stays protected'
    ...
    # v195 MUTATION: _estimate_bluff_frequency betsize_polarity fold-side calibration.
    # Underbettor (condensed thin-value) with sufficient confidence -> bluff_freq
    # floored at 0.30 (higher discount -> less over-fold vs thin-value bettors).
    # Test 1: standard opponent WITHOUT betsize_polarity -> unchanged (0.25 base).
    _ebf_om_std = {'confidence': 0.5, 'postflop_aggr': 0.36, 'large_bet_ratio': 0.32,
                   'vpip': 0.50, 'pfr': 0.40, 'fold_to_raise': 0.44,
                   'victory_momentum_index': 0.50}
    _ebf_std = _estimate_bluff_frequency(_ebf_om_std)
    assert 0.24 <= _ebf_std <= 0.26, (
        'standard opp without polarity must stay ~0.25, got %.4f' % _ebf_std)
    # Test 2: underbettor with confidence >= 0.25 -> floored at 0.30.
    _ebf_om_under = dict(_ebf_om_std)
    _ebf_om_under['betsize_polarity'] = {
        'tendency': 'underbettor', 'confidence': 0.40, 'samples': 5,
        'small_rate': 0.60, 'overbet_rate': 0.0, 'shove_rate': 0.0,
    }
    _ebf_under = _estimate_bluff_frequency(_ebf_om_under)
    assert _ebf_under >= 0.299, (
        'v195 underbettor must floor bluff_freq at 0.30, got %.4f' % _ebf_under)
    # Test 3: underbettor with LOW confidence (<0.25) -> no floor (unchanged).
    _ebf_om_low = dict(_ebf_om_std)
    _ebf_om_low['betsize_polarity'] = {
        'tendency': 'underbettor', 'confidence': 0.15, 'samples': 2,
    }
    _ebf_low = _estimate_bluff_frequency(_ebf_om_low)
    assert abs(_ebf_low - _ebf_std) < 0.01, (
        'v195 low-confidence underbettor must NOT floor (unchanged), '
        'got %.4f vs std %.4f' % (_ebf_low, _ebf_std))
    # Test 4: overbettor polarity -> NOT floored (unchanged; overbettor exempt
    # handled separately in _river_potodds_equity_margin step 7).
    _ebf_om_over = dict(_ebf_om_std)
    _ebf_om_over['betsize_polarity'] = {
        'tendency': 'overbettor', 'confidence': 0.40, 'samples': 5,
        'small_rate': 0.0, 'overbet_rate': 0.60, 'shove_rate': 0.0,
    }
    _ebf_over = _estimate_bluff_frequency(_ebf_om_over)
    assert abs(_ebf_over - _ebf_std) < 0.01, (
        'v195 overbettor must NOT trigger underbettor floor, '
        'got %.4f vs std %.4f' % (_ebf_over, _ebf_std))
    print('_estimate_bluff_frequency v195 fold-side PASS: '
          'std=%.3f under=%.3f low_conf=%.3f over=%.3f'
          % (_ebf_std, _ebf_under, _ebf_low, _ebf_over))
