...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        # v234 CROSSOVER (from v221): direct river pot-odds-vs-equity fold.
        # The margin-additive dispatch inside _postflop_response_margin is
        # INERT vs polarized value ranges (monte_carlo realized_rate
        # overestimates equity for weak made hands). Fold directly when
        # calibrated_equity (made*discount) < pot_odds. The gate enforces
        # its own conditions: river only, bet>=0.4x pot, made[0.20,0.50),
        # tier!=strong/nut, draw<0.15, overbettor-exempt.
        if round_idx == 3:
