...
# [v111] Turn SPR-commitment fold: low-SPR turn calls imply river commitment.
# When effective SPR < 4.0 on the turn, marginal thin/none-tier hands below
# pot_odds + buffer should fold rather than call and arrive river-committed.
...
def _opp_bet_size_adaptation(spot_info, opponent_model, round_idx):
    """Continuous call_margin adjustment based on how far the opponent's
    CURRENT bet deviates from their OWN historical average raise size on
    this street.

    Signal > 0 (bet larger than opponent's usual) -> tighten calls (positive margin).
    Signal < 0 (bet smaller than usual) -> slightly loosen (negative margin).
    Continuous, no deadzone, confidence-gated. M5/M6 compliant.

    Opponent model fields (already tracked by opponent.py build_opponent_model):
      avg_flop_raise_bb, avg_turn_raise_bb, avg_river_raise_bb
    These default to DEFAULT_FLOP_RAISE_BB=3.0 etc. when no data.
    spot_info['last_raise_bb'] = current opponent raise-to-total / BIG_BLIND.
    """
    opp_conf = opponent_model.get('confidence', 0.0)
...
def _spr_commitment_gate(round_idx, value_profile, made_strength, win_rate, pot_odds, spr=999.0):
    """[v69 Mandatory Fix #4 + v111 extension] SPR-commitment fold gate.

    River (round_idx==3): folds thin/none-tier marginal hands when real
    range-based monte-carlo equity (win_rate) is strictly below pot_odds.

    Turn (round_idx==2, v111 extension): when SPR < SPR_TURN_COMMIT_THRESHOLD,
    folds thin/none-tier marginal hands when equity < pot_odds + buffer.
    Low-SPR turn calls implicitly commit to river barrels; folding early
    saves more chips than calling twice with a marginal holding.
    """
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
...
def get_action(req, requests):
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            # [v111] Continuous opponent bet-size deviation adaptation:
            # tighten when current bet exceeds opponent's avg raise size on
            # this street, loosen when smaller. Confidence-gated, no deadzone.
...
def _self_test_spr_commitment_gate():
    """[v69 Mandatory Fix #4 + v111 extension] Verify SPR-commitment fold gates."""
...
if __name__ == "__main__":
    ...
    # [v111] M5/M6 self-test: bet-size adaptation must be non-INERT
