...
def choose_raise(
    ...
    elif (_tier == 'thin' and round_idx >= 2 and not semi_bluff and not blocker_bluff
            and not probe_mode and not inducing_value and thin_cap is None
            and not board_texture.get('dynamic', False)):
        _thin_floor = 0.42 if round_idx == 2 else 0.45
        if _thin_floor > low_ratio:
            low_ratio = _thin_floor
