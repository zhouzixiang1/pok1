...
def exploit_dispatch(opponent_model, round_idx, value_profile, made_strength):
    ...
    if round_idx >= 2 and is_value:
        ...
        # Sticky caller: high call-down OR low fold-to-bet (OR gate, not AND)
        if call_down_ft >= 0.45 or fold_turn <= 0.40:
            result['value_boost'] = 0.06 * confidence
    if round_idx == 3 and is_value:
        ...
        if call_down_tr >= 0.40 or fold_river <= 0.38:
            result['value_boost'] = max(result['value_boost'], 0.08 * confidence)
    if round_idx == 1:
