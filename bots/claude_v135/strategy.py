...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        _river_raise_guard_skip = (
            round_idx == 3
            and to_call >= my_chips * 0.25
            and not _river_stackoff_guard(
                round_idx, made_strength, value_profile, nutted_risk['risk'],
                blocker_profile, opponent_model,
            )
        )
        if (not _river_raise_guard_skip
                and not preflop_defensive_only
                and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit)):
            raise_amount = choose_raise(
                state["min_raise_action"],
                my_chips,
                state["my_round_bet"],
    ...
    if round_idx == 3 and to_call == 0:
        _river_ship_ok = _river_stackoff_guard(
            round_idx, made_strength, value_profile, nutted_risk['risk'],
            blocker_profile, opponent_model,
        )
        if _river_ship_ok:
