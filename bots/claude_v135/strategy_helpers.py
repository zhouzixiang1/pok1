...
def _river_stackoff_guard(
    round_idx, made_strength, value_profile, nutted_risk_score,
    blocker_profile, opponent_model,
):
    """River pot-commitment guard. Returns True iff committing >25% of
    effective stack on the river is justified by hand strength.

    Targets the recurring -20000 swing pattern where v134 ships non-nut
    hands (two-pair, top-pair-weak-kicker, one-pair) on the river and
    loses full stack to opponent nut hands. Spotlight evidence:
    G0H32/G0H52/G6H31/G3H25 (all -20000 losses).

    NOT a fold gate (fold axis exhausted per audit). This guard caps
    raise sizing - it does NOT fold, does NOT subtract from call_margin,
    does NOT touch thresholds. When guard denies: open-bet path falls
    through to river_value_raise_tier (capped 0.85x pot); facing-raise
    path falls through to call.
    """
    if round_idx != 3:
        return True
    # Bluff-catcher with blocker: allow ship as bluff (made_hand < 0.20)
    if (blocker_profile is not None
            and blocker_profile.get('eligible', False)
            and made_strength < 0.20):
        return True
    # True nuts with low nutted-risk
    if (value_profile is not None
            and value_profile.get('tier') == 'nut'
            and nutted_risk_score <= 0.05):
        return True
    # Near-nut made strength (top set, full house, nut flush, top two-pair on dry board)
    if made_strength >= 0.88:
        return True
    # Strong tier vs confirmed extreme calling station (preserves v134 value
    # extraction: vm_index>=0.80 + confidence>=0.30 = opp rarely folds AND
    # calls down consistently, so wider value ship is correct)
    if (value_profile is not None
            and value_profile.get('tier') == 'strong'
            and opponent_model.get('value_maximizer_index', 0.0) >= 0.80
            and opponent_model.get('confidence', 0.0) >= 0.30):
        return True
    return False
