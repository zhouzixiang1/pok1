...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    # BB vs preflop raise — defend wide in HU (fixes 66% fold rate)
        ...
        if preflop_strength >= 0.60:
            ...
            if preflop_strength >= 0.62 and not trash_hand:
                ...
                if raise_amount is not None:
                    return raise_amount
            return 0
        # Defend wide: K-high, any pair, suited connectors down to 54s
        if preflop_strength >= 0.42:
            return 0
        # Fold the very weakest hands
        if preflop_strength < 0.32:
            return -1
        # 0.32–0.42 gap: fall through to general calling logic
        return None
        ...
        if preflop_strength >= 0.60:
            ...
            if not trash_hand:
                ...
                if raise_amount is not None:
                    return raise_amount
            return 0
        # Defend with decent hands
        if preflop_strength >= 0.46:
            return 0
        # Fold weak hands
        return -1
...
def get_action(req, requests):
    ...
    # Runs early in the postflop decision path — before all-in/shove/equity comparisons.
    ...
    if state["opponent_allin"]:
        ...
        # This must execute BEFORE any style_deltas or threshold adjustments.
