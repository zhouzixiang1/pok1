...
# Terminal-response facing-action posteriors (fold_to_raise / fold_to_jam /
# river_overcall) may diverge a published intent from the sanitized parent
# baseline only once the reducer's posterior confidence clears this floor.
# Below it the sanitized-action counterfactual wins, so sparse terminal
# evidence stays near the parent baseline rather than stretching a thin
# facing-action sample into a stack off.
TERMINAL_RESPONSE_CONFIDENCE_FLOOR = 0.30
# Showdown-range posteriors (revealed-hole bucket rates / tightness) are
# selected evidence: hands reach showdown conditional on strength.  They may
# diverge a published intent from the sanitized parent baseline only once the
# reducer's posterior confidence clears this floor and the reached-showdown-only
# selection guard is present.  Below it the sanitized-action counterfactual
# wins, so sparse showdown evidence stays near the parent baseline rather than
# tilting a thin reached-showdown-only sample into a range exploit.
SHOWDOWN_RANGE_CONFIDENCE_FLOOR = 0.30
# Per-candidate raise fold-response slope cap.  The terminal_response
# fold-to-raise scalar (confidence-gated through adaptation_weight) is the base;
# this bounded slope shifts it by each candidate raise's own realized pressure,
# so a larger raise projects proportionally more fold equity and a minimal raise
# stays near the prior, while the slope cannot over-fold a small raise or
# over-value a shove.
RAISE_FOLD_SLOPE_CAP = 0.6
...
def _terminal_response_signal(context):
    """Confidence-scaled terminal-response posteriors from the reducer snapshot.

    ``fold_to_raise`` / ``fold_to_jam`` / ``river_overcall`` are reducer-owned
    facing-action posteriors; ``confidence`` and ``adaptation_weight`` bound how
    far they may move a decision.  Sparse or missing terminal evidence collapses
    to the neutral prior, so the candidate never exploits a tracker that has not
    yet observed the relevant facing-action opportunities.  This is the sole
    confidence-gated read of ``decision_context.opponent.terminal_response`` and
    the anchor for the sanitized-action counterfactual; it owns no state and
    performs no candidate tracking, history scan, or I/O.
    """
    ...
    confidence = _bounded(terminal.get("confidence"), 0.0, 1.0, 0.0)
    weight = _bounded(
        terminal.get(
            "adaptation_weight", confidence * MAX_ADAPTATION_WEIGHT
        ),
        0.0,
    ...
    return {
        ...
        "weight": weight,
        "samples": max(0.0, _number(terminal.get("samples"), 0.0)),
        "fold_to_raise": _bounded(
            terminal.get("fold_to_raise"), 0.0, 1.0, FOLD_TO_RAISE_PRIOR
        ),
        "fold_to_jam": _bounded(
            terminal.get("fold_to_jam"), 0.0, 1.0, FOLD_TO_JAM_PRIOR
        ),
        "river_overcall": _bounded(
            terminal.get("river_overcall"), 0.0, 1.0, RIVER_OVERCALL_PRIOR
        ),
    }


def _showdown_range_signal(context):
    """Confidence-scaled showdown-range posterior from the reducer snapshot.

    Revealed-hole bucket rates and tightness are sampled conditional on reaching
    showdown, so they are admitted only under the reducer's exact reach-rate
    selection guard and remain bounded by their adaptation weight.
    ``confidence`` and ``adaptation_weight`` bound how far the showdown posterior
    may move a decision.  Sparse, unguarded, or missing showdown evidence
    collapses to a neutral signal, so the candidate never exploits a
    reached-showdown-only sample that has not cleared the selection guard.  This
    is the sole confidence-gated read of ``decision_context.opponent.showdown_range``
    for the sanitized-action counterfactual; it owns no state and performs no
    candidate tracking, history scan, or I/O.
    """
    ...
    confidence = _bounded(showdown.get("confidence"), 0.0, 1.0, 0.0)
    weight = _bounded(
        showdown.get(
            "adaptation_weight", confidence * MAX_ADAPTATION_WEIGHT
        ),
    ...
    return {
        "confidence": confidence,
        "weight": weight,
        "samples": max(0.0, _number(showdown.get("samples"), 0.0)),
        "selection_guarded": selection_guarded,
    }


def _sanitized_opponent_context(context, *, reset_terminal=True, reset_showdown=True):
    """Shallow-copy context with terminal-response/showdown evidence reset.

    The counterfactual removes only the reducer's terminal facing-action and/or
    showdown-range evidence (per the scoped flags), leaving every card, betting,
    hand, line, legal, and deadline field identical to the live context.  Diffing
    a decision made here from the real-context decision isolates the selected
    opponent-evidence effect and bounds sparse-evidence drift back toward the
    parent baseline.  This is a read-only projection of the reducer snapshot: it
    builds fresh opponent sub-dicts so the live context is never mutated, owns no
    state, and performs no history scan or I/O.
    """

    opponent = context.get("opponent") or {}
    opponent = opponent if isinstance(opponent, dict) else {}
    sanitized_opponent = dict(opponent)
    if reset_terminal:
        terminal = dict(opponent.get("terminal_response") or {})
        terminal["fold_to_raise"] = FOLD_TO_RAISE_PRIOR
        terminal["fold_to_jam"] = FOLD_TO_JAM_PRIOR
        terminal["river_overcall"] = RIVER_OVERCALL_PRIOR
        terminal["confidence"] = 0.0
        terminal["adaptation_weight"] = 0.0
        sanitized_opponent["terminal_response"] = terminal
    if reset_showdown:
        showdown = dict(opponent.get("showdown_range") or {})
        showdown["adaptation_weight"] = 0.0
        showdown["confidence"] = 0.0
        sanitized_opponent["showdown_range"] = showdown
    sanitized = dict(context)
    sanitized["opponent"] = sanitized_opponent
    return sanitized
...
def _opponent_posterior(context):
    ...
    terminal_signal = _terminal_response_signal(context)
    terminal_weight = terminal_signal["weight"]
...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    # Evidence admission for sub-guard raise candidates.  The 0.56 guard is
    # street-generic and opponent-blind, and the semi-bluff permit is False on
    # every preflop spot, so a strong terminal facing-action posterior could
    # never even score a steal.  Admit one raise candidate below the guard only
    # when the reducer-owned ``opponent.terminal_response`` posterior (the
    # match-memory snapshot owned by the system tracker, never recomputed
    # here) clears the terminal confidence floor, carries a positive terminal
    # adaptation weight, and folds to raises at least 0.20 above the neutral
    # prior.  That weight is exactly what the sanitized-evidence counterfactual
    # zeroes below the floor, so at or below the prior, or under the floor,
    # nothing is admitted and the published intent stays byte-identical to the
    # parent baseline.  The admitted candidate is still decided by the
    # unchanged size-sloped fold-EV scoring below.
    terminal_signal = _terminal_response_signal(context)
    admit_fold_evidence = bool(
        terminal_signal["weight"] > 0.0
        and terminal_signal["confidence"] >= TERMINAL_RESPONSE_CONFIDENCE_FLOOR
        and terminal_signal["fold_to_raise"] - FOLD_TO_RAISE_PRIOR >= 0.20
    )
    for fraction in _candidate_raise_fractions(context, safe_equity):
        if (
            safe_equity < 0.56
            and not bluff_allowed
            and not admit_fold_evidence
        ):
            continue
        ...
        # Size-dependent fold response: the confidence-gated
        # terminal_response fold-to-raise scalar is the base, shifted by this
        # candidate raise's own realized pressure (matched risk over pot plus
        # risk).  A larger raise projects proportionally more fold equity and a
        # minimal raise stays near the prior; the bounded slope cannot
        # over-fold a small raise or over-value a shove.  Both the fold-equity
        # term and the (1 - fold) called-EV term now vary per raise size.
        fold_for_size = _bounded(
            raise_fold_rate
            + RAISE_FOLD_SLOPE_CAP * pressure * (1.0 - raise_fold_rate),
            0.0,
            1.0,
            raise_fold_rate,
        )
...
def _apply_terminal_response_counterfactual(
    context, decision, equity, confidence, samples
):
    """Bound terminal-response adaptation by a sanitized-action counterfactual.

    Consumes the reducer-owned ``opponent.terminal_response`` posteriors
    (fold_to_raise / fold_to_jam / river_overcall) together with their
    confidence, then re-evaluates the same equity against a context whose
    terminal facing-action evidence has been reset to the neutral
    prior.  The adapted intent may diverge from that sanitized parent baseline
    only once the posterior confidence clears the floor; below it the
    counterfactual intent wins.  The comparison is the consumed telemetry: it
    records whether terminal-response evidence moved the published intent and
    caps the move so sparse evidence cannot stretch a thin facing-action sample
    into a stack off.  No candidate-owned tracking, history scan, or I/O occurs.
    """

    signal = _terminal_response_signal(context)
    if signal["confidence"] >= TERMINAL_RESPONSE_CONFIDENCE_FLOOR:
        return decision
    counterfactual_context = _sanitized_opponent_context(
        context, reset_terminal=True, reset_showdown=False
    )
    counterfactual = _decision_from_equity(
        counterfactual_context,
        _bounded(equity, 0.0, 1.0),
        confidence,
        samples,
    )
    if counterfactual == decision:
        return decision
    return counterfactual


def _apply_showdown_range_counterfactual(
    context, decision, equity, confidence, samples
):
    """Bound showdown-range adaptation by a sanitized-action counterfactual.

    Consumes the reducer-owned ``opponent.showdown_range`` posterior (reached-
    showdown-only bucket rates / tightness) together with its confidence and
    reach-rate selection guard, then re-evaluates the same equity against a
    context whose showdown-range evidence has been reset to the neutral prior.
    The adapted intent may diverge from that sanitized parent baseline only once
    the posterior confidence clears the floor and the selection guard is present;
    below it the counterfactual intent wins.  The comparison is the consumed
    telemetry: it records whether showdown-range evidence moved the published
    intent and caps the move so a thin reached-showdown-only sample cannot tilt
    a range exploit.  No candidate-owned tracking, history scan, or I/O occurs.
    """

    signal = _showdown_range_signal(context)
    if (
        signal["selection_guarded"]
        and signal["confidence"] >= SHOWDOWN_RANGE_CONFIDENCE_FLOOR
    ):
        return decision
    counterfactual_context = _sanitized_opponent_context(
        context, reset_terminal=False, reset_showdown=True
    )
    counterfactual = _decision_from_equity(
        counterfactual_context,
        _bounded(equity, 0.0, 1.0),
...
def iter_decisions(context, baseline, deadline):
    ...
    while samples < MAX_EQUITY_SAMPLES:
        ...
        decision = _apply_terminal_response_counterfactual(
            context, decision, blended_equity, confidence, samples
        )
        decision = _apply_showdown_range_counterfactual(
            context, decision, blended_equity, confidence, samples
        )
