...
def _sb_open_bucket_action(hand_cat, opponent_model, trash_hand):
    ...
    # CROSSOVER v121 (from v117): include 'broadway_suited' (KQs/KJs/QJs/QTs/JTs)
    # in the implied-odds bucket. These hands flop playable draws / two-pair+
    # often enough to play vs opens with implied odds. v104 grouped them with
    # offsuit 'playable' which folds them by default vs tight opens.
...
def _bb_vs_raise_bucket_action(hand_cat, opponent_model, pot_odds, preflop_strength, win_rate, trash_hand):
    ...
    if hand_cat in ('premium', 'big_cards') and not trash_hand:
        return 'value_raise'
    # CROSSOVER v121 (from v117/v116/v109): broadway_suited (KQs/KJs/QJs/QTs/JTs)
    # gets a dedicated call/fold decision with implied-odds reasoning. v104 lacked
    # this branch and grouped suited broadways with offsuit 'playable' which folds
    # vs tight opens. H2H evidence (v117 vs v104): v117 wins vs newer aggressive
    # openers where v104 loses — v102 +13.5pp (0.60 vs 0.465), v107 +9.5pp, v109
    # +6.7pp, v116 +8.5pp. The 0.36 pot_odds gate (NOT 0.40 — v116 reverted v115's
    # 0.40 widen because it over-defended into iso-raises) keeps the call grounded
    # in direct price; the win_rate fallback defends hands with positive equity.
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if preflop_strength >= 0.58 and not trash_hand:
            ...
            if raise_amount is not None:
                return raise_amount
        # CROSSOVER v121 (from v117): Call with most limp-range hands, including
        # broadway suited (KQs/KJs/QJs/QTs/JTs) which flop playable draws /
        # two-pair+ often enough to justify the call vs limp. v104 lacked this
        # branch and folded suited broadways vs limps.
...
def _single_reraise_stackoff_guard(round_idx, value_profile, made_strength, spot_info, spr):
    ...
    river, our hand is sub-nut (tier != 'nut'), and SPR is low (< 3.5, meaning
    ...
    MUTATION v121 (a — threshold widening, +16.7%): SPR cutoff 3.0 -> 3.5 extends
    protection to medium-SPR (3.0-3.5) spots where stack-off commitment is still
    high (a 3.5-SPR turn shove commits ~28% of effective stacks) but the prior
    3.0 cutoff let the bot initiate all-in shoves with sub-nut hands. H2H
    evidence: v104 (this guard at 3.0) wins vs older pool (v50 +18pp, v93 +11pp)
    but loses vs newer aggressive re-raisers (v102 0.465, v107 0.471, v109 0.50)
    whose turn/river 3-bet lines exploit medium-SPR shoves. Widening to 3.5
    keeps the guard firing through that range, complementing the offensive
    broadway_suited crossover import. Within the 10-20% mutation budget; the
    made_strength < 0.70 + tier != 'nut' + single-raise grounding unchanged.
    """
    if round_idx < 2:
        return None
