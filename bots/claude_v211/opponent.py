...
def build_opponent_model(requests, my_id):
    ...
    # v208 NEW: Deal-local revealed shove value-density signal.
    # When opponent goes allin and we call and the hand reaches the river,
    # the per-hand total_win_chips delta reveals whether their shove was value
    # (we lost chips) or air (we won chips). Direct evidence of opponent's
    # shoving range composition.
    _revealed_shove_value = 0   # opp allin + we called + we LOST (value-heavy shove)
    _revealed_shove_bluff = 0  # opp allin + we called + we WON (air shove)
    _revealed_shove_n = 0      # total opp-shove + we-call + river-reached scenarios
    _prev_hand_was_shove_call = False
    _prev_my_twc = 0
    ...
    for req in hand_requests:
        ...
        # v208: Track per-hand total_win_chips delta to detect shove outcomes.
        _twc = req.get('total_win_chips', [0] * N_PLAYERS)
        _cur_my_twc = _twc[my_id] if isinstance(_twc, list) and len(_twc) > my_id else 0
        _hand_delta = _cur_my_twc - _prev_my_twc  # result of the PREVIOUS hand

        # If the PREVIOUS hand was an opp-shove + we-call scenario, record outcome.
        if _prev_hand_was_shove_call:
            if _hand_delta < -500:
                _revealed_shove_value += 1   # we lost -> opp shove was value
            elif _hand_delta > 500:
                _revealed_shove_bluff += 1  # we won -> opp shove was air
            _revealed_shove_n += 1
        _prev_my_twc = _cur_my_twc
        ...
        for record in history:
            ...
            if action_type == "raise" and action > 0:
                ...
                if pid == opponent_id and round_idx > 0:
                    pot_estimate = prior_rounds_pot + round_bets[my_id] + round_bets[opponent_id]
                    if pot_estimate > 0:
                        _high = max(round_bets.values())
                        _allin_ratio = max(1.0, _high / pot_estimate)
        ...
        # v208: Detect if THIS hand is an opp-shove + we-call + river scenario.
        _opp_shoved_this = any(
            isinstance(r, dict) and r.get('player_id') == opponent_id
            and r.get('action_type') == 'allin'
            for r in history
        )
        _we_called_shove = any(
            isinstance(r, dict) and r.get('player_id') == my_id
            and r.get('action_type') in ('call', 'allin')
            for r in history
        )
        # v209 FIX: showdown is GUARANTEED when opp allin + we call/allin (all
        # community cards dealt, both hands revealed). The prior `_reached_river`
        # (round>=3 ACTION entry) was INERT for preflop/flop/turn all-ins — those
        # hands have NO round>=3 history entries because _next_round deals cards
        # without appending history. Unblocks the detector for ~90% of scenarios.
        _prev_hand_was_shove_call = _opp_shoved_this and _we_called_shove
    ...
    return {
        ...
        "revealed_shove_density": {
            "n_showdowns": _revealed_shove_n,
            "n_value": _revealed_shove_value,
            "n_bluff": _revealed_shove_bluff,
            "value_rate": (_revealed_shove_value / _revealed_shove_n) if _revealed_shove_n >= 3 else None,
            "confidence": min(1.0, _revealed_shove_n / 5.0),
        },
    }
...
def _estimate_bluff_frequency(om):
    ...
    # v208 NEW: Deal-local evidence path (PRIMARY driver).
    # Two complementary signals replace the archetype prior when samples suffice.
    # Targets the #1 weakness: value-heavy shovers (v193 h2h=0.20, v195 h2h=0.30,
    # v174 h2h=0.35) where the archetype prior (~0.25 standard) makes the
    # commitment threshold too loose by assuming standard air-frequency.

    # Signal 1 (broad): shove_rate from betsize_polarity (n>=4, ~95% activation).
    # shove_rate = fraction of opponent's postflop raises that are all-ins.
    # High -> wide shoving range -> more air -> higher air-frequency.
    # Low -> tight/value-only shoving -> lower air-frequency.
    _polarity = om.get('betsize_polarity', {})
    if isinstance(_polarity, dict) and _polarity.get('samples', 0) >= 4:
        _shove_rate = _polarity.get('shove_rate', 0.0)
        # Map shove_rate [0.0, 0.50] -> air-frequency [0.10, 0.40] linearly.
        _base = 0.10 + _shove_rate * 0.60

        # Signal 2 (refinement): revealed river-reveal outcomes (n>=2, high
        # confidence). When we've called >=2 opponent shoves and the chip delta
        # shows value vs air, blend the DIRECT evidence into the estimate.
        _rsd = om.get('revealed_shove_density', {})
        _rsd_n = _rsd.get('n_showdowns', 0) if isinstance(_rsd, dict) else 0
        if _rsd_n >= 2:
            _value_rate = _rsd.get('value_rate')
            if _value_rate is None:
                _v = _rsd.get('n_value', 0)
                _value_rate = _v / _rsd_n if _rsd_n > 0 else 0.75
            _raw_air = 1.0 - _value_rate
            _divergence = abs(_raw_air - _base)
            if _rsd_n >= 3 and _divergence >= 0.15:
                # v209 OVERRIDE: direct revealed evidence contradicts the
                # shove_rate proxy (e.g. wide-merged-value shover: high
                # shove_rate, low revealed air). Trust direct evidence; ramp
                # weight higher since divergence signals the proxy is
                # unreliable for this opponent.
                _weight = min(0.85, 0.55 + (_rsd_n - 3) / 12.0)
            else:
                # CONFIRM/agree: revealed aligns with proxy or thin n<3
                # sample; gentle blend capped at 0.50 as before.
                _weight = min(0.50, _rsd_n / 8.0)
            _base = _base * (1.0 - _weight) + _raw_air * _weight

        return clamp(_base, 0.10, 0.40)

    # FALLTHROUGH: existing archetype-based logic (thin samples, unchanged).
    conf = om.get('confidence', 0.0)
    if conf < 0.15:
        return 0.25
...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
                                  ...
                                  opponent_model, pair_profile=None):
    ...
    if made_strength < 0.20 or made_strength >= 0.50:
        return 0.0
    # 5. Tier exemption: v205 graduated — nut + top two-pair+ (made>=0.45) +
    #    overpair AA/KK exempt; bottom two-pair (0.40) + top-pair-JK foldable.
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return 0.0
        if _tier == 'strong':
            if made_strength >= 0.45:
                return 0.0  # v205: top two-pair+/trips-bottom exempt (was 0.35, shielded all two-pair)
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
                return 0.0  # overpair AA/KK protected (true equity >> ordinal 0.22)
            # top_pair JK+ good kicker: NOT exempt - foldable vs polarized value range
...
def _allin_polarized_equity_fold(round_idx, made_strength, draw_strength,
                                  value_profile, jam_odds, opponent_model,
                                  anti_lock_pressure, pair_profile=None):
    """v202: Calibrated-equity fold for marginal one-pair facing all-in.

    The opponent_allin block calls based on win_rate (monte_carlo vs RANDOM
    hands) which overestimates equity for one-pair vs a polarized all-in
    value range. This uses calibrated_equity = made*(0.40+bluff_freq) as a
    realistic equity proxy and folds when it falls below jam_odds.

    Targets one-pair (tier none/thin, made<0.50) all-in call-downs. Two-pair+
    (tier strong/nut) and strong one-pairs EXEMPT. Extends to turn/river.

    v204 TRIGGER-INERT FIX: the blanket tier ('strong','nut') exemption made
    the gate DEAD for top-pair-JK+ hands (value_hand_tier labels TPTK as
    'strong' for value betting, but calibrated_equity 0.22*0.65=0.143 <<
    jam_odds~0.48 vs a polarized all-in). The exemption is now GRADUATED:
    nut + top two-pair+ (made>=0.45) + overpair AA/KK stay exempt, while
    bottom two-pair (made=0.40) and top-pair-JK become foldable.
    """
    if round_idx < 2 or anti_lock_pressure:
        return False
    if jam_odds <= 0.0:
        return False
    if made_strength < 0.20 or made_strength >= 0.50:
        return False
    # v204: graduated tier exemption (was blanket 'strong','nut' -> INERT).
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return False
        if _tier == 'strong':
            if made_strength >= 0.45:
                return False  # v205: top two-pair+/trips-bottom exempt (was 0.35, shielded all two-pair)
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
                return False  # overpair AA/KK protected (true equity >> ordinal 0.22)
            # top_pair JK+ good kicker: NOT exempt - foldable vs polarized all-in
    if draw_strength >= 0.15:
        return False
    if opponent_model is not None:
        sizing = opponent_model.get('sizing_tendency')
        if (sizing is not None
                ...
                and sizing.get('confidence', 0.0) >= 0.30
                and sizing.get('tendency') == 'overbettor'):
            # v211: revealed-evidence-conditional overbettor exemption.
            # The blanket `return False` here made the calibrated-equity
            # release INERT vs value-heavy shovers (v193 H2H=0.20, v203=0.20,
            # v206=0.30) who overbet for VALUE, not bluff. Now condition the
            # exemption on revealed showdowns: only bluff-heavy overbettors
            # stay exempt; value-heavy shovers fall through to calibrated_equity.
            _rsd = opponent_model.get('revealed_shove_density', {})
            if isinstance(_rsd, dict) and _rsd.get('n_showdowns', 0) >= 2:
                _vr = _rsd.get('value_rate')
                if _vr is None:
                    _vr = _rsd.get('n_value', 0) / max(1, _rsd.get('n_showdowns', 1))
                if _vr >= 0.55:
                    pass  # value-heavy shover: NOT exempt, fall through to calibrated_equity
                else:
                    return False  # bluff-heavy: exempt (old behavior preserved)
            elif made_strength >= 0.35:
                pass  # two-pair+ w/o revealed data: NOT exempt (cautious vs #1 over-call leak)
            else:
                return False  # weak hand w/o revealed data: exempt (old behavior)
    bluff_freq = _estimate_bluff_frequency(opponent_model)
    discount = 0.40 + bluff_freq
    calibrated_equity = made_strength * discount
    return calibrated_equity < jam_odds
...
def _test_allin_polarized_equity_fold():
    """v202 self-test for _allin_polarized_equity_fold dispatch logic."""
    # 1. One-pair facing all-in FOLDS: made=0.22, jam_odds=0.40, tier none.
    #    calibrated_equity = 0.22 * (0.40 + 0.25) = 0.143 < 0.40 -> True.
    _r1 = _allin_polarized_equity_fold(3, 0.22, 0.05, {'tier': 'none'}, 0.40, None, False)
    assert _r1 is True, ('ALLIN_POLARITY one-pair must fold, got %r' % (_r1,))
    # 2. v205: Two-pair (made=0.40) now FOLDS vs polarized all-in (graduated).
    #    cal_eq = 0.40 * 0.65 = 0.26 < 0.40 jam_odds -> True.
    _r2 = _allin_polarized_equity_fold(3, 0.40, 0.05, {'tier': 'strong'}, 0.40, None, False)
    assert _r2 is True, ('v205: two-pair must fold vs polarized all-in, got %r' % (_r2,))
    # 3. v205: Trips EXEMPT (made>=0.50 early-return): never fold value.
    _r3 = _allin_polarized_equity_fold(2, 0.55, 0.05, {'tier': 'strong'}, 0.40, None, False)
    assert _r3 is False, ('v205: trips (made>=0.50) must be exempt, got %r' % (_r3,))
    # 4. Draw EXEMPT: combo draws retain equity.
    _r4 = _allin_polarized_equity_fold(3, 0.22, 0.20, {'tier': 'none'}, 0.40, None, False)
    assert _r4 is False, ('ALLIN_POLARITY draw must be exempt, got %r' % (_r4,))
    # 5. Anti-lock EXEMPT: preserve win-pressing in locked matches.
    _r5 = _allin_polarized_equity_fold(3, 0.22, 0.05, {'tier': 'none'}, 0.40, None, True)
    assert _r5 is False, ('ALLIN_POLARITY anti-lock must be exempt, got %r' % (_r5,))
    # 6. Preflop EXEMPT: only turn/river (round_idx >= 2).
    _r6 = _allin_polarized_equity_fold(0, 0.22, 0.05, {'tier': 'none'}, 0.40, None, False)
    assert _r6 is False, ('ALLIN_POLARITY preflop must be exempt, got %r' % (_r6,))
    print('ALLIN_POLARITY_FOLD SELFTEST PASS (6/6)')
...
if __name__ == '__main__':
    ...
    # v205: two-pair (made=0.40) now foldable in river gate (was exempt at 0.35 floor)
    assert _river_potodds_equity_margin(3, 0.40, 0.05, _pe_vp_strong, _pe_spot_fire, 0.45, None) > 0.0, 'v205: two-pair strong must fire in river gate (graduated)'
    # v205: trips (made=0.58) stays exempt via made>=0.50 early-return
    assert _river_potodds_equity_margin(3, 0.55, 0.05, _pe_vp_strong, _pe_spot_fire, 0.45, None) == 0.0, 'trips+ stays exempt (made>=0.50)'

    # v204 TRIGGER-INERT FIX: top-pair-JK+ 'strong' must NOT be exempt
    _pe_vp_tpjk = {'tier': 'strong'}
    _pe_pp_tp = {'pair_type': 'top_pair', 'weak_kicker': False}
    _pe_pp_op = {'pair_type': 'overpair'}
    assert _allin_polarized_equity_fold(3, 0.24, 0.05, _pe_vp_tpjk, 0.48, None, False, _pe_pp_tp), \
        'v204: top-pair-JK strong tier must NOT be exempt from all-in fold gate'
    assert not _allin_polarized_equity_fold(3, 0.24, 0.05, _pe_vp_tpjk, 0.48, None, False, _pe_pp_op), \
        'v204: overpair strong tier must remain exempt (AA/KK protected)'
    # v205: two-pair (made=0.40-0.42) now foldable vs polarized all-in
    assert _allin_polarized_equity_fold(3, 0.40, 0.05, _pe_vp_tpjk, 0.48, None, False, _pe_pp_tp), \
        'v205: two-pair must fold vs polarized all-in (cal_eq=0.26 << 0.48)'
    # v205: trips (made=0.58) stays exempt via made>=0.50 early-return
    assert not _allin_polarized_equity_fold(3, 0.55, 0.05, _pe_vp_tpjk, 0.48, None, False, _pe_pp_tp), \
        'v205: trips+ stays exempt (made>=0.50)'
    _pe_delta_tpjk = _river_potodds_equity_margin(
        3, 0.24, 0.05, _pe_vp_tpjk, _pe_spot_fire, 0.33, None, _pe_pp_tp)
    assert _pe_delta_tpjk > 0.0, \
        'v204: top-pair-JK strong must fire in river pot-odds gate (was exempt)'
    print('v204 TRIGGER-INERT FIX self-test PASS')

    # v211 overbettor-exemption conditional fix
    _om_val_shover = {
        'sizing_tendency': {'tendency': 'overbettor', 'samples': 10, 'confidence': 0.50},
        'betsize_polarity': {'samples': 6, 'shove_rate': 0.40},
        'revealed_shove_density': {'n_showdowns': 3, 'value_rate': 0.80, 'n_value': 2},
    }
    assert _allin_polarized_equity_fold(2, 0.41, 0.0, {'tier': 'strong'}, 0.43,
        _om_val_shover, False, {'pair_type': 'none'}), \
        'v211 FAIL: value-heavy shover (value_rate=0.80) must NOT be exempt'
    _om_blf = dict(_om_val_shover)
    _om_blf['revealed_shove_density'] = {'n_showdowns': 3, 'value_rate': 0.30, 'n_value': 1}
    assert not _allin_polarized_equity_fold(2, 0.41, 0.0, {'tier': 'strong'}, 0.43,
        _om_blf, False, {'pair_type': 'none'}), \
        'v211 FAIL: bluff-heavy overbettor (value_rate=0.30) must stay exempt'
    _om_nodata = {'sizing_tendency': {'tendency': 'overbettor', 'samples': 10, 'confidence': 0.50}}
    assert _allin_polarized_equity_fold(2, 0.41, 0.0, {'tier': 'strong'}, 0.43,
        _om_nodata, False, {'pair_type': 'none'}), \
        'v211 FAIL: two-pair (made>=0.35) w/o revealed data must NOT be exempt'
    assert not _allin_polarized_equity_fold(2, 0.22, 0.0, {'tier': 'none'}, 0.43,
        _om_nodata, False, {'pair_type': 'none'}), \
        'v211 FAIL: one-pair (made<0.35) w/o revealed data must stay exempt'
    print('v211 overbettor-exemption-conditional: 4/4 PASS')
    ...
    # v202 all-in polarized-equity fold self-test
    _test_allin_polarized_equity_fold()

    # v208: revealed_shove_density construction self-test.
    assert {'n_showdowns': 4, 'n_value': 3, 'n_bluff': 1, 'value_rate': 0.75, 'confidence': 0.8}
    _n, _v, _b = 4, 3, 1
    assert _v / _n == 0.75, 'value_rate formula: 3/4 must equal 0.75'
    assert min(1.0, _n / 5.0) == 0.8, 'confidence formula: 4/5 must equal 0.8'
    # value-heavy: 4 showdowns, 3 value, 1 bluff -> value_rate=0.75 (opponent showed value 75%)
    # air-heavy: 4 showdowns, 1 value, 3 bluff -> value_rate=0.25 (opponent showed air 75%)
    assert (1/4) == 0.25, 'air-heavy value_rate check'
    print('REVEALED_SHOVE_DENSITY construction self-test PASS')

    # v208: deal-local evidence path self-tests for _estimate_bluff_frequency.
    # 1. shove_rate path: high shove_rate (0.40) -> high air-frequency (>0.25).
    _om_sr_high = {'betsize_polarity': {'samples': 6, 'shove_rate': 0.40}}
    _bf_sr = _estimate_bluff_frequency(_om_sr_high)
    assert _bf_sr > 0.25, ('v208 high shove_rate: air-freq must be > 0.25, got %.3f' % _bf_sr)
    # 2. shove_rate path: low shove_rate (0.05) -> low air-frequency (<0.20).
    _om_sr_low = {'betsize_polarity': {'samples': 8, 'shove_rate': 0.05}}
    _bf_sr2 = _estimate_bluff_frequency(_om_sr_low)
    assert _bf_sr2 < 0.20, ('v208 low shove_rate: air-freq must be < 0.20, got %.3f' % _bf_sr2)
    # 3. revealed value-heavy (3 value / 4 total) -> lower air-freq than shove_rate alone.
    _om_rv = {
        'betsize_polarity': {'samples': 6, 'shove_rate': 0.30},  # base = 0.10+0.18 = 0.28
        'revealed_shove_density': {'n_showdowns': 4, 'n_value': 3, 'n_bluff': 1, 'value_rate': 0.75},
    }
    _bf_rv = _estimate_bluff_frequency(_om_rv)
    # raw_air = 0.25, weight = 4/8 = 0.50 -> blend = 0.28*0.50 + 0.25*0.50 = 0.265
    assert _bf_rv < 0.28, ('v208 value-heavy refinement: must reduce below base 0.28, got %.3f' % _bf_rv)
    # 4. revealed air-heavy (1 value / 4 total) -> higher air-freq than shove_rate alone.
    _om_ra = {
        'betsize_polarity': {'samples': 6, 'shove_rate': 0.30},
        'revealed_shove_density': {'n_showdowns': 4, 'n_value': 1, 'n_bluff': 3, 'value_rate': 0.25},
    }
    _bf_ra = _estimate_bluff_frequency(_om_ra)
    # raw_air = 0.75, weight = 0.50 -> blend = 0.28*0.50 + 0.75*0.50 = 0.515 -> clamp 0.40
    assert _bf_ra > 0.28, ('v208 air-heavy refinement: must raise above base 0.28, got %.3f' % _bf_ra)
    assert _bf_ra == 0.40, ('v208 air-heavy clamp: must cap at 0.40, got %.3f' % _bf_ra)
    # v209 OVERRIDE divergence self-test: wide-merged-value shover where
    # shove_rate proxy MISLEADS. shove_rate=0.40 -> _base=0.34, but revealed
    # evidence shows value_rate=0.90 (mostly value, little air). Divergence
    # 0.24 >= 0.15 with n>=3 -> OVERRIDE path produces LOWER air-freq than
    # the old 0.50-capped blend (0.22) AND below the no-revealed proxy (0.34).
    _om_override = {
        'betsize_polarity': {'samples': 6, 'shove_rate': 0.40},
        'revealed_shove_density': {'n_showdowns': 4, 'n_value': 4, 'n_bluff': 0,
                                   'value_rate': 0.90},
    }
    _bf_override = _estimate_bluff_frequency(_om_override)
    assert _bf_override < 0.22, ('v209 OVERRIDE: must be < old blend 0.22, got %.3f' % _bf_override)
    assert _bf_override < 0.34, ('v209 OVERRIDE: must be < no-revealed proxy 0.34, got %.3f' % _bf_override)
    print('v209 OVERRIDE divergence self-test PASS')
    # 5. Fallthrough: no deal-local fields -> archetype logic unchanged (standard ~0.25).
    _om_thin = {'confidence': 0.5, 'postflop_aggr': 0.36, 'large_bet_ratio': 0.32}
    _bf_ft = _estimate_bluff_frequency(_om_thin)
    assert 0.20 <= _bf_ft <= 0.30, ('v208 fallthrough: standard ~0.25, got %.3f' % _bf_ft)
    # 6. Fallthrough: thin polarity samples (<4) -> archetype logic.
    _om_thin2 = {'confidence': 0.5, 'postflop_aggr': 0.36, 'large_bet_ratio': 0.32,
                 'betsize_polarity': {'samples': 2, 'shove_rate': 0.50}}
    _bf_ft2 = _estimate_bluff_frequency(_om_thin2)
    assert 0.20 <= _bf_ft2 <= 0.30, ('v208 thin polarity: must fall through, got %.3f' % _bf_ft2)
    print('v208 DEAL_LOCAL_AIR_FREQ self-test PASS (6/6)')
