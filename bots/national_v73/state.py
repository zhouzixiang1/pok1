...
def preflop_domination_penalty(my_cards):
    ...
    Crossover import from national_v37 (CROSS_GEN_PIVOT). Hands like
    KQo/KJo/QJo/ATo enter big pots, flop dominated top-pair-good-kicker, and
    stack off to AK/AQ/AJ overpairs and sets. Returns a penalty subtracted
    from preflop_strength in the bb_vs_raise / sb_vs_reraise call decisions so
    these hands fold preflop instead of calling.

    Mutation (national_v73): penalty widened 0.035 -> 0.040 (~14% increase).
    v57 (base) carries a wider calling range than v37 (looser
    SB_FACING_ALLIN_CALL 0.48 vs 0.55), so a slightly larger domination penalty
    compensates by shedding more dominated hands from the wider range.
    """
    profile = preflop_hand_profile(my_cards)
