...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    # Early-game wider defense: lower preflop thresholds when no opponent data.
    early_game_bonus = 0.03 if confidence < 0.10 else 0.0

    if spot_info["preflop_spot"] == "sb_open":
        # Lower SB open/limp thresholds to open ~75% and limp ~10% more.
        open_threshold = 0.41 + match_adjust + match_profile["open_delta"] - early_game_bonus
        limp_threshold = 0.28 + match_adjust - early_game_bonus
        ...
        if preflop_strength >= 0.60:
            ...
            if preflop_strength >= 0.62 and not trash_hand:
                ...
                if raise_amount is not None:
                    return raise_amount
            return 0
        # Widen BB defense: call with 0.38+ (was 0.42), fold below 0.26 (was 0.32).
