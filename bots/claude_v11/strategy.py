...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position):
    ...
    margin = 0.0
    air_hand = made_strength < 0.20 and draw_strength < 0.08
    weak_showdown = made_strength < 0.25
    size_bucket = bet_size_bucket(spot_info["last_raise_pot_ratio"])
    ...
    # Crossover from v8: CBet-based call margin adjustment
...
# Crossover from v8 (originally v6): Structured postflop folding logic
...
def get_action(req, requests):
    ...
    medium = 0.54 if round_idx == 0 else 0.50 if round_idx == 1 else 0.46
    ...
    # Mutation: CBet exploitation — if opponent folds to CBets frequently, lower flop bet threshold
    # Implements experience pool lesson: "If opp folds to cbets >60%, increase cbet frequency there"
    ...
    if to_call > 0:
        ...
        if fragile_pair_raise_fold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # Crossover from v8: should_fold_postflop structured fold logic
