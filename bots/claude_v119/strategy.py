...
def choose_raise(
    ...
    # CROSSOVER (v116 × v112 → v119): value-tier sizing floor imported from v112
    # (traces to v91's first offensive sizing gen). v116 LACKS this floor; H2H
    # attribution evidence (all ≥30g): v112 beats v116 where it matters most —
    # vs v96 0.522 vs 0.400 (+0.122), vs v104 0.533 vs 0.450 (+0.083), vs v113
    # 0.550 vs 0.467 (+0.083), vs v106 0.550 vs 0.500, vs v108 0.470 vs 0.400.
    # These newer aggressive bots punish underbetting; the floor prevents
    # downward modifiers from pushing nut/strong value bets below street-aware
    # minimums (fixes global underbetting bias on value). OFFENSIVE value-
    # extraction feature, NOT an exhausted defensive guard.
    # NOTE: we deliberately do NOT import v112's probe_mode thin-value extension
    # (v112 line 1420) — that is a known v101-era bug the v102 fix removed (it
    # bleeds value-hand sizing from 0.60-0.85x down to 0.33-0.41x). v116's
    # simpler corrected `probe_mode=check_probe or small_probe` is retained.
    # MUTATION (exactly one — opponent-aware floor boost): raise the value-tier
    # floor against CONFIRMED calling stations (passivity_score>=0.60 AND
    # confidence>=0.30) to extract maximum value from opponents who call down
    # with weak hands. Experience-pool heuristic: "low aggression/passivity may
    # indicate calling-station behavior" → don't bluff them, DO value-bet bigger.
    # This adds a NEW opponent signal (passivity_score) gating the floor — not a
    # pure constant tweak — and is distinct from passive_exploit (which handles
    # thin-value donks on checked lines; this floor governs raise sizing on
    # nut/strong hands reaching choose_raise).
    ...
    if (_tier in ('nut', 'strong') and not semi_bluff and not blocker_bluff
        ...
        _is_station = (confidence >= 0.30
                       and opponent_model.get('passivity_score', 0.5) >= 0.60)
        if round_idx == 1:
            _value_floor = 0.64 if _is_station else 0.58
        elif round_idx == 2:
            _value_floor = 0.70 if _is_station else 0.62
        elif round_idx == 3:
            _value_floor = 0.76 if _is_station else 0.68
        else:
