...
def _sb_mandatory_play(my_cards, preflop_strength):
    """HU SB should play almost every hand. Only fold absolute trash.

    In heads-up, the SB has excellent pot odds (completing to 1 BB in a 1.5 BB pot
    = 40% pot odds). Structural playability — suited, paired, connected — justifies
    playing far more hands than the raw strength metric suggests.
    """
    from state import preflop_hand_profile
    profile = preflop_hand_profile(my_cards)
    high, low = profile["high"], profile["low"]
    ...
    # Always play: pairs, suited hands, any ace, any king
    if pair or suited or high == 14 or high == 13:
        return True
    # Two broadway cards
    if high >= 11 and low >= 10:
        return True
    # Connected offsuit (gap <= 2) with low >= 7
    if gap <= 2 and low >= 7:
        return True
    # Two-gap offsuit with low >= 8 (e.g., J8o, Q9o — playable in HU SB)
    if gap == 3 and low >= 8:
        return True
    # Everything else: only play if strength not terrible
    return preflop_strength >= 0.22


def _sb_reraise_playable(my_cards, preflop_strength):
    """Structural check for hands worth defending vs a 3-bet from BB in HU.

    When facing a reraise in HU, many hands that look weak by raw strength
    still have enough playability (suited, connected, pair) to call profitably
    due to good pot odds and postflop realization.
    """
    from state import preflop_hand_profile
    profile = preflop_hand_profile(my_cards)
    high, low = profile["high"], profile["low"]
    ...
    # Always defend: pairs, suited aces/kings
    if pair:
        return True
    if suited and high >= 12:
        return True
    # Any ace — high card equity + blocker to premium hands
    if high == 14:
        return True
    # Suited connectors and one-gaps
    if suited and gap <= 2 and low >= 7:
        return True
    # Broadway offsuit
    if high >= 11 and low >= 10:
        return True
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        ...
        if preflop_strength <= limp_threshold - loose_bonus:
            # HU SB structural guard: play structurally playable hands even
            # if below the limp threshold — only fold absolute trash
            if not _sb_mandatory_play(req["my_cards"], preflop_strength):
                return -1
        return 0
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if can_bluff_3bet and 0.38 <= preflop_strength <= 0.54 and not trash_hand:
            ...
            if bluff_roll < 0.25:
                ...
                if raise_amount is not None:
                    return raise_amount
        # Call with playable hands — structural defense first, then numeric threshold
        # HU BB defense: check structural playability BEFORE numeric threshold
        # to avoid folding playable hands that the metric underestimates
        if _bb_defend_vs_raise(req['my_cards']):
            return 0
        if preflop_strength >= 0.32 or win_rate >= pot_odds_pf - 0.05:
            return 0
        # Structural pot-odds defense: when raw preflop equity directly exceeds
        # the computed pot odds, calling is mathematically +EV regardless of
        # hand classification — catches hands the numeric threshold misses
        if pot_odds_pf > 0 and preflop_strength >= pot_odds_pf:
            return 0
        return -1
        ...
        if preflop_strength >= 0.38:
            if win_rate >= pot_odds_sbr - 0.08:
                return 0
        # vs LAG: call wider (they 3-bet light)
        if opp_archetype == 'lag' and preflop_strength >= 0.38:
            return 0
        # Structural defense: play structurally playable hands vs non-allin reraise
        # HU opponents 3-bet wide — suited, paired, and connected hands have
        # enough postflop playability to call profitably
        if not state.get('opponent_allin', False) and _sb_reraise_playable(req['my_cards'], preflop_strength):
            return 0
