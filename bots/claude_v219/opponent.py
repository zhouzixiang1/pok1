...
def _estimate_bluff_frequency(om):
    ...
    # v219 CROSSOVER (ported from v195) + MUTATION: betsize_polarity fold-side
    # calibration. An underbettor's bets are condensed thin-value (small sizing
    # with medium-strength hands), NOT polarized nuts-or-air. The standard 0.65
    # polarized discount UNDERESTIMATES our bluff-catcher equity vs such a range,
    # so the v188/v189 fold framework can OVER-FOLD. Flooring bluff_freq at 0.30
    # raises the discount to 0.70 -> higher calibrated_equity -> smaller
    # equity_gap -> less aggressive folding. This is the fold-side payoff that
    # lets v195 beat condensed-betting opponents (v197/v206) where v204 over-folds.
    # MUTATION: confidence gate lowered 0.25 -> 0.20 (20% reduction) so the
    # anti-over-fold protection activates earlier in 70-hand matches with fewer
    # betsize samples (experience pool: opponent-gated fixes are inert if the
    # gate never fires against actual nemeses).
    _polarity = om.get('betsize_polarity', {})
    if (_polarity.get('confidence', 0.0) >= 0.20
...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
    ...
    if made_strength < 0.20 or made_strength >= 0.50:
        return 0.0
    # 5. Tier exemption: never fold nut/two-pair+ value.
    #    v204 TRIGGER-INERT FIX: graduated (was blanket 'strong','nut' -> INERT
    #    for top-pair-JK+). nut + two-pair+ (made>=0.35) + overpair AA/KK stay
    #    exempt; top-pair-JK 'strong' one-pairs become foldable.
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return 0.0
        if _tier == 'strong':
            if made_strength >= 0.35:
                return 0.0  # two-pair+ (HAND_CLASS_SCORE[2]=0.40+) genuinely strong
...
def _allin_polarized_equity_fold(round_idx, made_strength, draw_strength,
    ...
    nut + two-pair+ (made>=0.35) + overpair AA/KK stay exempt, while
    top-pair-JK 'strong' one-pairs become foldable.
    """
    ...
    if value_profile is not None:
        ...
        if _tier == 'strong':
            if made_strength >= 0.35:
                return False  # two-pair+ (HAND_CLASS_SCORE[2]=0.40+) genuinely strong
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
...
def _test_allin_polarized_equity_fold():
    ...
    # 2. Two-pair EXEMPT (tier strong): never fold value.
    _r2 = _allin_polarized_equity_fold(3, 0.40, 0.05, {'tier': 'strong'}, 0.40, None, False)
    assert _r2 is False, ('ALLIN_POLARITY two-pair (strong) must be exempt, got %r' % (_r2,))
    # 3. Overpair EXEMPT (tier strong, turn): never fold value.
    _r3 = _allin_polarized_equity_fold(2, 0.35, 0.05, {'tier': 'strong'}, 0.40, None, False)
    assert _r3 is False, ('ALLIN_POLARITY overpair (strong) must be exempt, got %r' % (_r3,))
...
if __name__ == '__main__':
    ...
    assert not _allin_polarized_equity_fold(3, 0.42, 0.05, _pe_vp_tpjk, 0.48, None, False, _pe_pp_tp), \
        'v204: two-pair strong (made>=0.35) must remain exempt'
