...
本目录独立实现项目原生 B 路线。当前提交完成 M0 调研/许可证冻结和 M3 小型博弈正确性底座；尚未实现国赛 HUNL 蓝图、神经叶值、在线搜索、对手后验或 native TCP Bot。
...
2. 用相同规则参数与独立参考实现交叉验证 Kuhn/Leduc 数值；
3. 将 M3 per-sample JSON delta 替换为经证明等价的压缩/并行 reducer 后，才扩大 CFR 状态；
4. 先交付 blueprint-only native Bot，再生成神经叶值大资产。

来源与许可证见 `reports/m0_research_license_matrix.md`。阶段结果见 `reports/m3_small_game_report.md`（测试完成后生成）。
