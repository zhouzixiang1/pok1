"""Betting decision helpers extracted from postflop.py and strategy.py to keep files under 1000 lines."""

from constants import BIG_BLIND
from card_utils import clamp
from postflop import bet_size_bucket, empty_draw_profile
...
def choose_raise(
    ...
    # River value raise floor: strong/nut hands bet at least 0.75x pot
    if round_idx == 3 and value_profile is not None and value_profile["tier"] in ("strong", "nut") and not semi_bluff and not blocker_bluff and not probe_mode:
        low_ratio = max(low_ratio, 0.75)
