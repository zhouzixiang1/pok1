...
from betting import (
    ...
    choose_raise, choose_overbet_river, choose_overbet_bluff_river,
)
...
def get_action(req, requests):
    ...
    strong = 0.69 if round_idx == 0 else 0.65 if round_idx == 1 else 0.61 if round_idx == 2 else 0.56
    medium = 0.54 if round_idx == 0 else 0.50 if round_idx == 1 else 0.48 if round_idx == 2 else 0.45
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            if spot_info["has_position"]:
                call_margin -= 0.06  # Widen BB defense range
        ...
        if anti_lock_attack is not None:
            return anti_lock_attack
        # River fold discipline: fold when facing large bet with weak equity
        river_weak_equity_fold = (
            round_idx == 3
            and to_call > 0
            and to_call / max(1, pot - to_call) > 0.50  # facing bet > 50% of pot
            and win_rate < 0.35
            and (value_profile is None or value_profile["tier"] not in ("strong", "nut"))
            and draw_strength < 0.12
            and not anti_lock_pressure
        )
        if river_weak_equity_fold:
    ...
    induce_nut_value = (
        round_idx in (1, 2)  # Only induce on flop/turn — river always value bet
        and to_call == 0
        and value_profile is not None
        and value_profile["tier"] == "nut"
        and board_texture is not None
