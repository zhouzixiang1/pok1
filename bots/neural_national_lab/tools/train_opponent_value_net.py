...
def build_sample(
    row: dict[str, Any], *, max_hist: int = 16, target_field: str = "delta_vs_rule"
) -> dict[str, Any]:
    ...
    if target_field == "delta_vs_rule":
        raw_targets = (
            row.get("delta_vs_rule")
            or row.get("targets")
            or row.get("raw_targets")
            or row.get("label_values")
        )
    else:
        raw_targets = row.get(target_field)
    target_masks = row.get("target_masks") or {}
    raw_mask = (
        target_masks.get(target_field)
        if isinstance(target_masks, dict)
        else None
    )
    raw_mask = raw_mask or row.get("target_mask") or row.get("legal_mask")
...
class OpponentAwareValueNet(nn.Module):
    ...
    def forward(self, state: torch.Tensor, profile: torch.Tensor,
                hist: torch.Tensor, hist_lengths: torch.Tensor,
                cross_hand: torch.Tensor | None = None) -> torch.Tensor:
        if hist.size(1) > 0 and bool((hist_lengths > 0).any()):
            safe_lengths = hist_lengths.clamp(min=1).to("cpu")
            ...
            _, h = self.gru(packed)
            opp_emb = h.squeeze(0) * (hist_lengths > 0).float().unsqueeze(1)
        else:
...
def collate(samples: list[dict[str, Any]], *, max_hist: int, device: str):
    ...
    hist_lengths = torch.zeros(B, dtype=torch.long, device=device)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--target-field",
        choices=("delta_vs_rule", "match_delta_vs_rule", "tail_delta_vs_rule"),
        default="delta_vs_rule",
        help="Counterfactual target scope. Match/tail targets capture downstream opponent-model feedback.",
    )
    ...
    train_samples = [
        build_sample(r, max_hist=args.max_hist, target_field=args.target_field)
        for r in train_rows
    ]
    val_samples = [
        build_sample(r, max_hist=args.max_hist, target_field=args.target_field)
    ...
    def run_epoch(samples, *, train: bool):
        ...
        for start in range(0, len(idx), args.batch_size):
            ...
            state_t, profile_t, cross_t, hist_t, hist_lengths, target_t, _, _ = collate(
            ...
            pred = model(state_t, profile_t, hist_t, hist_lengths, cross_t)
    ...
    def data_manifest(path: str | None, rows: list[dict[str, Any]]) -> dict[str, Any] | None:
        if not path:
            return None
        digest = hashlib.sha256(Path(path).read_bytes()).hexdigest()
        return {"path": str(path), "sha256": digest, "rows": len(rows)}

    training_meta = {
        "seed": args.seed,
        "epochs_requested": args.epochs,
        "best_epoch": best_epoch,
        "exported_checkpoint": "best_validation" if best_state is not None else "final_no_validation",
        ...
        "train_data": data_manifest(args.data, train_rows),
        "validation_data": data_manifest(args.val_data, val_rows),
    }

    # Export the best validation checkpoint to a deterministic JSON weight file.
    _export_json(model, args.out, state_dim, profile_dim, args.gru_hidden,
                 args.hidden, args.max_hist, best_val_metric, val_samples,
                 task=args.task, training_meta=training_meta)
...
def _export_json(model, out_path: str, state_dim: int, profile_dim: int,
                 ...
                 best_val_mae: float, val_samples, *, task: str = "regression",
                 training_meta: dict[str, Any] | None = None) -> None:
    model.eval()
    sd = model.state_dict()
    ...
    meta = {
        ...
        metric_key: float(best_val_mae) if math.isfinite(best_val_mae) else None,
