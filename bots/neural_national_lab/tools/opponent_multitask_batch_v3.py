...
def _encoded_rows(
    rows: Sequence[Mapping[str, Any]], *, response: bool
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("encoded_row_schema") != ENCODED_ROW_SCHEMA:
...
def collate_encoded_rows(
    ...
    *,
    ...
    encoded = _encoded_rows(rows, response=response)
    state = torch.tensor(
        [
    ...
    if response:
        ...
        if not bool((value_legal.sum(dim=1) > 0.0).all()):
            raise ValueError("every value row requires at least one legal action")
        if not bool(((inputs["rule_action"] * value_legal).sum(dim=1) == 1.0).all()):
