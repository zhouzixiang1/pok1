...
from cross_hand_sequence import MAX_CROSS_HANDS, summarize_native_hand
...
class NativeNationalBot:
    ...
    def _reset_match(self) -> None:
        ...
        self._cross_hand_sequence: list[list[float]] = []
        self._last_hand_settled = False
        self._last_hand_showdown = False
    ...
    def _request(self) -> dict:
        req = {
            ...
            "cross_hand_sequence": [
                list(row) for row in self._cross_hand_sequence
            ],
    ...
    def _finalize_previous_hand(self) -> None:
        if self._hand_num <= 0 or not self._last_hand_settled:
            return
        summary = summarize_native_hand(
            self._history,
            self._public_cards,
            opponent_id=self._opponent_id,
            hero_earned=self._last_earned,
            final_pot=self._pot,
            showdown=self._last_hand_showdown,
        )
        self._cross_hand_sequence.append(summary)
        self._cross_hand_sequence = self._cross_hand_sequence[-MAX_CROSS_HANDS:]
        self._last_hand_settled = False
    ...
    def handle(self, line: str, sock: socket.socket) -> None:
        ...
        if line.startswith("earnChips"):
            ...
            self._last_hand_settled = True
            self._in_allin_runout = False
            return
        if line.startswith("oppo_hands|"):
            self._last_hand_showdown = True
