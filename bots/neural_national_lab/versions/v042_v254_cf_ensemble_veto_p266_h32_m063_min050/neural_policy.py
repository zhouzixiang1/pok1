...
_INTERACTION_MODELS: list[dict[str, Any]] | None = None
...
def _interaction_models() -> list[dict[str, Any]]:
    global _INTERACTION_MODELS
    if _INTERACTION_MODELS is not None:
        return _INTERACTION_MODELS
    cfg = _config()
    raw_paths = cfg.get("interaction_weights_list")
    if not cfg.get("interaction_enabled", False):
        _INTERACTION_MODELS = []
        return _INTERACTION_MODELS
    if not raw_paths:
        model = _interaction_model()
        _INTERACTION_MODELS = [model] if model is not None else []
        return _INTERACTION_MODELS
    ...
    for raw_path in raw_paths:
        ...
        try:
            ...
            print(f"INTERACTION_ENSEMBLE_MODEL_ERROR {exc}", file=sys.stderr)
            _INTERACTION_MODELS = []
            return _INTERACTION_MODELS
    _INTERACTION_MODELS = models
    return _INTERACTION_MODELS
...
def _passes_interaction_gate(
    ...
    if conf < float(cfg.get("interaction_apply_min_conf", 0.0)):
        return True
    scores = [_predict_advantage(model, features) for model in models]
    mean_score = sum(scores) / max(1, len(scores))
    return (
        mean_score >= float(cfg.get("interaction_mean_min", cfg.get("interaction_min", 0.70)))
        and min(scores) >= float(cfg.get("interaction_member_min", 0.0))
    )
