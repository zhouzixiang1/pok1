...
from opponent import build_opponent_model, analyze_current_spot, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold, _semibluff_raise_construct, _river_value_raise_construct
from postflop import (
    ...
    _thin_value_extraction_sizing,
    _paired_board_weak_one_pair_fold,
    _tier_opp_sizing_directive,
)
...
def choose_anti_lock_pressure_action(
    ...
    # v241 FIX: suppress emergency all-in for trash preflop hands.
    # Evidence: 43.4% of v240 preflop all-ins are trash (pf_str < 0.40),
    # ALL in late-game hands 64-68, 9.2% win rate. Each risks 20000 chips.
    # Return None -> caller (L1325-1341) limps (call 50) instead of
    # open-shoving 200BB with J4o/T3o/72o.
    if round_idx == 0 and preflop_strength is not None and preflop_strength < 0.40:
        try:
            sys.stderr.write('ANTI_LOCK_TRASH_GATE pf=%.2f hl=%d\n'
                             % (preflop_strength, hands_left))
        except Exception:
            pass
        return None
...
def choose_raise(
    ...
    _tier_dir = _tier_opp_sizing_directive(value_profile, opponent_model, round_idx, wetness, to_call)
    ratio += _tier_dir['size_delta']
    if _tier_dir['ratio_cap'] is not None:
        ratio = min(ratio, _tier_dir['ratio_cap'])
    _inducing = _tier_dir['inducing'] or induce_mode
    if round_idx >= 2 and value_profile.get("tier") in ("nut", "strong") and not _inducing and not semi_bluff and not probe_mode and not blocker_bluff and not pressure_line:
    ...
    if _tier_dir['low_ratio_override'] is not None:
        low_ratio = max(low_ratio, _tier_dir['low_ratio_override'])
    if _inducing:
        low_ratio = min(low_ratio, 0.28)
    ...
    if street_fold_boost > 0.0 and not probe_mode and not blocker_bluff and not _inducing:
    ...
    # v194 crossover: SPR commitment gate -- cap marginal-hand raise
...
def _bb_vs_raise_bucket_action(hand_cat, opponent_model, pot_odds, preflop_strength, win_rate, trash_hand, my_cards):
    # v172 NEW: fold weak offsuit (A2o-A8o, K9o, 59o-type) vs raises; runs before steal-defense widen.
    if my_cards is not None and _is_offsuit_commitment_risk(my_cards):
        try:
            ...
            pass
        return 'fold'
    # v254 NEW preflop axis: dominated-non-pair floor inside 'playable' bucket (exempts pairs/big_cards/broadway_suited/suited_ace/suited_connector).
    _floor_fold = _bb_vs_raise_dominated_floor(hand_cat, preflop_strength)
    try:
        sys.stderr.write('PREFLOP_BB_RAISE_FLOOR hand_cat=%s str=%.3f decision=%s\n'
                         % (hand_cat, preflop_strength,
                            'fold' if _floor_fold else 'pass'))
    except Exception:
        pass
    if _floor_fold:
        return 'fold'
    ...
    # v152 NEW: Steal-defense widen — defend playable/mid pairs/speculative vs unknown/loose openers.
    ...
    if hand_cat in ('premium', 'big_cards') and not trash_hand:
        return 'value_raise'
    # broadway_suited (KQs/KJs/QJs/QTs/JTs): dedicated implied-odds call/fold
    # gate. pot_odds<=0.36 or win_rate>=pot_odds-0.02 -> call, else fold.
    # (v116 reverted v115's 0.40 widen; v102 probe_mode fix preserved upstream.)
    ...
    if hand_cat in ('strong_pair', 'mid_pair'):
        # v146: thin-value raise vs high-fold openers (mid pairs vs SB over-folds to 3bets).
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    ...
    # v137: Defense relaxation vs confirmed overbettors — placed BEFORE all
    # upstream fold gates (fixes dispatch-order shadow, project's #1 failure
    # mode). Fires when: sizing_tendency='overbettor', samples>=8, conf>=0.30,
    # size_bucket=='large', and NOT absolute air (made<0.15 with no draw).
...
def get_action(req, requests):
    ...
    # v193 gate-mandated behavior-preserving relocation of _river_stackoff_guard
    # (fix-3 placement_shadow): the call was inside `if to_call > 0:` AFTER this
    # `if to_call >= my_chips:` all-in block, making it a TRUE SHADOW for stack
    # all-ins. The all-in case is already covered by _spr_commitment_gate (below)
    # and _multibarrel_line_fold, so the guard only ever serves 0<to_call<my_chips.
    # The added `0 < to_call < my_chips` guard restricts to that SAME non-all-in
    # range -> byte-identical behavior, call-site now before the early-return.
    ...
    if to_call >= my_chips:
        ...
        if _spr_commitment_gate(round_idx, my_chips, made_strength, value_profile,
            ...
            print(f"SPR_FOLD round={round_idx} made={made_strength:.2f} "
                  ...
                  file=sys.stderr)
            return -1
        # v192: multi-barrel line-evidence fold for all-in call-downs. Dispatched
        # in BOTH all-in and to_call>0 blocks. Function's own guards safe.
        if round_idx >= 2 and _multibarrel_line_fold(
            ...
            return -1
        if _marginal_made_river_fold_gate(
                round_idx, made_strength, draw_strength, value_profile,
                spot_info, opponent_model, to_call, pot, paired_board_profile,
                site='allin_cover'):
            return -1
    ...
    if to_call > 0:
        # v197 CROSSOVER: removed v193 direct fold gates (over-folded vs wide
        # barrelers). All-in _multibarrel_line_fold kept; continuous calibration
        # via _postflop_response_margin (v197 mutation 0.48→0.42).
        ...
        marginal_gate_fold = _marginal_made_river_fold_gate(
            round_idx, made_strength, draw_strength, value_profile,
            spot_info, opponent_model, to_call, pot, paired_board_profile,
            site='gt0_after_margin')
        if marginal_gate_fold:
        ...
        if should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=opponent_model, spr=_spr):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # v247: paired-board weak one-pair cap fold (two-pair+ explicitly excluded)
        if _paired_board_weak_one_pair_fold(round_idx, made_strength, draw_strength, value_profile, pair_profile, paired_board_profile, spot_info, opponent_model, to_call, pot):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            if not anti_lock_call_continue and not strong_made_continue:
        ...
        _value_upsizing = _value_lead_upsizing_delta(
            ...
            board_texture, opponent_model)
        if round_idx >= 1 and to_call > 0 and not preflop_defensive_only:
            _sbr = _semibluff_raise_construct(
                opponent_model, round_idx, to_call, pot, made_strength,
                draw_info, draw_strength, board_texture, value_profile,
                spot_info, my_chips, state['min_raise_action'],
                state['my_round_bet'])
            if _sbr is not None:
                return _sbr
        if round_idx == 3 and to_call > 0 and not preflop_defensive_only:
            _rvr = _river_value_raise_construct(
                opponent_model, round_idx, to_call, pot, made_strength,
                value_profile, spot_info, my_chips, state['min_raise_action'],
                state['my_round_bet'], board_texture, pair_profile)
            if _rvr is not None:
                return _rvr
    ...
    induce_nut_value = (
        round_idx == 1  # was: round_idx > 0 — induce only on flop
        and to_call == 0
        and value_profile is not None
        and value_profile["tier"] == "nut"
        and board_texture is not None
    ...
    # v244 NEW OFFENSE: Thin-value extraction vs passive callers.
    # Bets 25-40% pot with marginal made (0.35-0.55) on turn/river checked-to-us
    # vs high-VPIP/low-fold/low-aggro opponents. Sited BEFORE bad_river_bluff_candidate
    # and thin_static_showdown_control which would force-check these hands.
    if to_call == 0 and round_idx in (2, 3):
        _tve = _thin_value_extraction_sizing(
            made_strength, opponent_model, board_texture,
            round_idx, to_call, pot, state['min_raise_action'], my_chips)
        if _tve is not None:
            return _tve
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or exploit['should_barrel']:
        ...
        raise_amount = choose_raise(
            ...
            # probe_mode simplification (v104 crossover): prevents value-hand sizing bleed.
            probe_mode=check_probe or small_probe,
            induce_mode=induce_nut_value,
            nutted_risk_score=nutted_risk["risk"],
            match_sizing_delta=match_profile["sizing_delta"],
