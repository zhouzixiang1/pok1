...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    # Terminal-response-conditioned bluff-jam.  The equity+SPR value-jam gate
    # above blocks a legitimate stack-off, but the bounded opponent posterior
    # may still expose a high, confident fold-to-jam rate; when it does, a
    # fold-equity-weighted allin is positive-EV even on a low-equity runout.
    # This consumes only the reducer-owned opponent.terminal_response through
    # the posterior already derived (jam_fold_rate and terminal_weight) and
    # reuses the matched-commitment / showdown-EV helpers; it never
    # reimplements tracking or memory.  A low-confidence posterior keeps
    # fold_to_jam near its 0.28 prior, below the floor, so the branch cannot
    # fire on sparse evidence.  The existing comparison below still owns the
    # pick, so allin is published only when this beats every alternative.
    value_jam_admitted = (
        "allin" in kinds
        and hero_stack > 0
        and safe_equity >= 0.62
    ...
    if (
        "allin" in kinds
        and not value_jam_admitted
        and hero_stack > 0
        and posterior["terminal_weight"] >= 0.40
        and jam_fold_rate >= 0.55
    ):
        bluff_jam_target = hero_stage + hero_stack
        bluff_commitments = _matched_showdown_commitments(
            context, bluff_jam_target
        )
        bluff_risk = bluff_commitments["hero_extra"]
        if bluff_risk > 0.0:
            # A caller's realized range is stronger than the unconditional
            # sample, so discount the called-branch equity exactly as the
            # value-jam path does before scoring the fold-equity-weighted EV.
            bluff_jam_equity = _bounded(safe_equity - 0.105, 0.0, 1.0)
            bluff_called_ev = _called_showdown_ev(
                context, bluff_jam_target, bluff_jam_equity
            )
            bluff_jam_score = (
                jam_fold_rate * pot
                + (1.0 - jam_fold_rate) * bluff_called_ev
                - uncertainty * bluff_risk * 0.15
            )
            candidates.append((bluff_jam_score, {"kind": "allin"}))
    if not candidates:
