...
def _flop_cbet_texture_delta(opponent_model, board_texture, made_strength,
                              round_idx, to_call, value_profile=None):
    """v163 NEW: Flop c-bet texture adaptation. [FLOP_CBET_TEXTURE]

    Continuous delta on flop c-bet/open-bet ratio (round_idx==1, to_call==0).
    Returns additive ratio delta in [-0.05, +0.08]. Returns 0.0 off-path.

    CONTINUOUS (M5/M6 compliant -- no thresholded deadzones):
      - fold_to_bet_flop: smooth_rate prior_mean=0.44 prior_weight=2.0 (LIVE)
      - At priors (fold_flop=0.44, dry_score=0.5, made=0.40) -> delta=0 (no edge)
      - DRY + fold-prone -> delta -> -0.05 (smaller c-bet, cheap fold equity)
      - WET + made hand -> delta -> +0.08 (bigger c-bet, charge draws)
    """
    if round_idx != 1 or to_call != 0:
        return 0.0
    fold_flop = float(opponent_model.get('fold_to_bet_flop', 0.44))
    confidence = float(opponent_model.get('confidence', 0.0))
    if board_texture is None:
        return 0.0
    wetness = float(board_texture.get('wetness', 0.0))
    flush_p = float(board_texture.get('flush_pressure', 0.0))
    straight_p = float(board_texture.get('straight_pressure', 0.0))
    paired = bool(board_texture.get('paired', False))
    # Continuous dry_score (high=dry/no draws; low=wet/dynamic)
    raw_dry = 0.50 + 0.50 * (0.45 - wetness) + 0.25 * (0.40 - flush_p) + 0.25 * (0.40 - straight_p)
    if paired:
        raw_dry -= 0.10
    dry_score = clamp(raw_dry, 0.0, 1.0)

    # Continuous signals (M5: no deadzone)
    fold_signal = clamp((fold_flop - 0.44) / 0.15, -1.0, 1.0)
    dry_signal = clamp((dry_score - 0.50) / 0.30, -1.0, 1.0)
    made_signal = clamp((made_strength - 0.40) / 0.30, -1.0, 1.0)
    conf_gate = clamp(confidence / 0.30, 0.0, 1.0)

    # Bidirectional continuous delta
    dry_fold_shrink = -0.050 * max(0.0, dry_signal) * max(0.0, fold_signal)
    wet_value_grow = +0.080 * max(0.0, -dry_signal) * max(0.0, made_signal)
    delta = conf_gate * (dry_fold_shrink + wet_value_grow)

    # Telemetry (M6: function-scope, unconditional, TOTAL delta + reason)
    reason = ('dry_fold_shrink' if dry_fold_shrink < 0
              else 'wet_value_grow' if wet_value_grow > 0
              else 'standard')
    try:
        sys.stderr.write(
            'FLOP_CBET_TEXTURE delta_milli=%+d fold_flop=%.3f dry_score=%.2f '
            'made=%.2f fold_sig=%+.2f dry_sig=%+.2f made_sig=%+.2f conf_gate=%.2f '
            'reason=%s\n'
            % (int(round(delta * 1000)), fold_flop, dry_score, made_strength,
               fold_signal, dry_signal, made_signal, conf_gate, reason)
        )
    except Exception:
        pass
    return delta
...
def _flop_cbet_texture_self_test():
    """M5/M6 fixture: verifies continuous non-zero delta when signals deviate."""
    cases = [
        # (opponent_model, target_dry_score, made_strength, expect_sign)
        ({"fold_to_bet_flop": 0.44, "confidence": 0.50}, 0.50, 0.40, 0),   # priors
        ({"fold_to_bet_flop": 0.60, "confidence": 0.50}, 0.95, 0.40, -1),  # dry+fold-prone
        ({"fold_to_bet_flop": 0.44, "confidence": 0.50}, 0.05, 0.70, +1),  # wet+made
        ({"fold_to_bet_flop": 0.25, "confidence": 0.50}, 0.95, 0.40, 0),   # call-station+dry (no shrink)
        ({"fold_to_bet_flop": 0.60, "confidence": 0.0},  0.95, 0.40, 0),   # conf_gate=0
    ]
    results = []
    for om, dry_target, made, expect_sign in cases:
        # Build board_texture that yields target dry_score via inverse of wetness formula:
        # dry ~ 0.5 + 0.5*(0.45-wet) => wet = 0.45 - 2*(dry-0.5)
        wet = 0.45 - 2.0 * (dry_target - 0.5)
        bt = {"wetness": clamp(wet, 0.0, 1.0), "flush_pressure": 0.0,
              "straight_pressure": 0.0, "paired": False}
        delta = _flop_cbet_texture_delta(om, bt, made, 1, 0, None)
        sign = 0 if abs(delta) < 1e-4 else (1 if delta > 0 else -1)
        results.append((sign == expect_sign, delta, expect_sign))
    return results
...
def turn_bluff_continuation_barrel(
    round_idx, to_call, my_id, history,
    made_strength, draw_strength, value_profile, pair_profile,
    board_texture, public_cards, opponent_model,
    pot, my_chips, min_raise, my_round_bet,
):
    """NEW v161 OFFENSE: Turn bluff continuation barrel (barrel-planning axis).

    Fires a turn BLUFF second-barrel when bot was the flop aggressor (c-bet)
    but missed, the turn brings a scare card, and opp over-folds to turn bets.
    Returns raise-to-total amount (int) or None.

    STRUCTURALLY NEW vs turn_second_barrel_planner (value-only, made_class>=1):
    this is the bluff companion for weak-made hands (air / draw-missed).
    Uses grep-verified-live signal fold_to_bet_turn (read in 4+ existing fns:
    _street_fold_exploit_sizing_boost, _turn_value_extraction_floor,
    turn_second_barrel_planner, exploit_dispatch).
    """
    if round_idx != 2 or to_call != 0:
        return None
    if len(public_cards) < 4 or board_texture is None:
        return None
    ...
    # 2. Weak made hand — value hands go to turn_second_barrel_planner.
    if made_strength >= 0.30:
        return None
    if value_profile is not None and value_profile.get('tier') in ('strong', 'nut'):
        return None

    # 3. Scare card detection: turn is overcard to flop high (T+),
    #    OR flush/straight pressure completed on turn, OR turn paired the board.
    flop_ranks = [card_number(c) for c in public_cards[:3]]
    turn_rank = card_number(public_cards[3])
    flop_high = max(flop_ranks)
    overcard_scare = turn_rank > flop_high and turn_rank >= 10
    flush_scare = board_texture.get('flush_pressure', 0.0) >= 0.75
    straight_scare = board_texture.get('straight_pressure', 0.0) >= 0.65
    paired_scare = len(set(flop_ranks + [turn_rank])) < 4
    if not (overcard_scare or flush_scare or straight_scare or paired_scare):
        return None

    # 4. Opponent over-folds to turn bets (grep-verified live signal).
    confidence = opponent_model.get('confidence', 0.0)
    if confidence < 0.20:
        return None
    fold_turn = opponent_model.get('fold_to_bet_turn', 0.44)
    ...
    # 5. Some equity fallback (overcard scare / draw equity / paired board).
    has_equity_fallback = (
        draw_strength >= 0.08
        or overcard_scare
        or paired_scare
    )
    if not has_equity_fallback:
        return None

    # 6. Sizing: 0.55-0.65x pot, scaled by how much opp over-folds.
    ratio = 0.55 + 0.10 * clamp((fold_turn - 0.50) / 0.30, 0.0, 1.0)
    target = int(pot * ratio)
    amount = max(min_raise, target - my_round_bet)
    amount = min(amount, my_chips - 1)
    if amount < min_raise or amount >= my_chips:
    ...
    try:
        print('TURN_BLUFF_CONTINUATION made=%.2f draw=%.2f '
              'fold_turn=%.2f conf=%.2f scare=%d%d%d%d pot=%d bet=%d'
              % (made_strength, draw_strength, fold_turn, confidence,
                 overcard_scare, flush_scare, straight_scare, paired_scare,
                 pot, amount), file=sys.stderr)
    except Exception:
        pass
    return amount
...
def _river_vs_station_value_raise(round_idx, to_call, made_strength, value_profile,
                                   pot, my_chips, min_raise, my_round_bet,
                                   opponent_model, archetype):
    """River direct sizing for strong/nut hands vs calling stations.

    Bypasses choose_raise to use station-specific pot-fraction sizing.
    Calling stations call 70%+ of river bets — standard sizing under-extracts.
    ARCHETYPE-GATED: first use of classify_archetype for bet sizing.
    """
    if round_idx != 3 or to_call != 0:
        return None
    ...
    if tier == 'nut':
        ...
        return None
    raw = int(pot * frac)
    raw = max(raw, min_raise)
    raw = min(raw, my_chips)
    if raw <= to_call:
        return None
    try:
        sys.stderr.write(
            'STATION_RIVER_RAISE tier=%s frac=%.2f pot=%d raw=%d '
            'made=%.2f conf=%.2f\n'
            % (tier, frac, pot, raw, made_strength, confidence))
    except Exception:
        pass
    return raw
...
if __name__ == "__main__":
    ...
    # v161 turn_bluff_continuation_barrel fixture: 4 scenarios.
    # Scenario setup: flop = [K(12), 7(7), 2(2)] (ranks), turn = A(14) (overcard scare).
    # Card integers: K♥=46, 7♥=22, 2♥=2, A♥=50. card_number: 46//4+2=13, 22//4+2=7,
    # 2//4+2=2, 50//4+2=14. So flop_high=13, turn_rank=14 → overcard_scare=True.
    _tbl_cards = [46, 22, 2, 50]  # K, 7, 2, A
    _tbl_bt = {'flush_pressure': 0.10, 'straight_pressure': 0.10,
               'dynamic': False, 'paired': False, 'wetness': 0.05}
    _tbl_hist_fire = [{'player_id': 0, 'round': 1, 'action_type': 'raise'}]
    _tbl_hist_noaggr = [{'player_id': 1, 'round': 1, 'action_type': 'raise'}]

    # F1: FIRES — bot was flop aggressor, air hand, overcard scare, opp over-folds.
    _tbl_opp_foldy = {'confidence': 0.50, 'fold_to_bet_turn': 0.60}
    _tbl_r1 = turn_bluff_continuation_barrel(
        2, 0, 0, _tbl_hist_fire,
        0.18, 0.05, {'tier': 'none'}, {'made_class': 0, 'pair_type': 'none'},
        _tbl_bt, _tbl_cards, _tbl_opp_foldy,
        1000, 5000, 100, 0,
    )
    assert _tbl_r1 is not None and _tbl_r1 >= 100 and _tbl_r1 < 5000, \
        f'F1 (fires) expected amount in [100, 4999], got {_tbl_r1}'

    # F2: NO-FIRE value hand — made_strength=0.55 goes to value planner.
    _tbl_r2 = turn_bluff_continuation_barrel(
        2, 0, 0, _tbl_hist_fire,
        0.55, 0.05, {'tier': 'thin'}, {'made_class': 1, 'pair_type': 'top_pair'},
        _tbl_bt, _tbl_cards, _tbl_opp_foldy,
        1000, 5000, 100, 0,
    )
    assert _tbl_r2 is None, f'F2 (value hand) expected None, got {_tbl_r2}'

    # F3: NO-FIRE low fold_to_bet_turn — do not bluff calling stations.
    _tbl_opp_call = {'confidence': 0.50, 'fold_to_bet_turn': 0.40}
    _tbl_r3 = turn_bluff_continuation_barrel(
        2, 0, 0, _tbl_hist_fire,
        0.18, 0.05, {'tier': 'none'}, {'made_class': 0, 'pair_type': 'none'},
        _tbl_bt, _tbl_cards, _tbl_opp_call,
        1000, 5000, 100, 0,
    )
    assert _tbl_r3 is None, f'F3 (calling station) expected None, got {_tbl_r3}'

    # F4: NO-FIRE no flop aggression — opp was the flop aggressor.
    _tbl_r4 = turn_bluff_continuation_barrel(
        2, 0, 0, _tbl_hist_noaggr,
        0.18, 0.05, {'tier': 'none'}, {'made_class': 0, 'pair_type': 'none'},
        _tbl_bt, _tbl_cards, _tbl_opp_foldy,
        1000, 5000, 100, 0,
    )
    assert _tbl_r4 is None, f'F4 (no flop aggression) expected None, got {_tbl_r4}'

    print('TURN_BLUFF_CONTINUATION fixture: PASS')

    # v165 _river_vs_station_value_raise tests
    cs_model = {'confidence': 0.40}
    nut_vp = {'tier': 'nut', 'size_bonus': 0.0}
    strong_vp = {'tier': 'strong', 'size_bonus': 0.0}
    thin_vp = {'tier': 'thin', 'size_bonus': 0.0}
    assert _river_vs_station_value_raise(3, 0, 0.70, nut_vp, 2000, 15000, 200, 0, cs_model, 'calling_station') is not None, 'nut vs station'
    assert _river_vs_station_value_raise(3, 0, 0.65, strong_vp, 2000, 15000, 200, 0, cs_model, 'calling_station') is not None, 'strong vs station'
    assert _river_vs_station_value_raise(3, 0, 0.50, thin_vp, 2000, 15000, 200, 0, cs_model, 'calling_station') is None, 'thin should not inflate'
    assert _river_vs_station_value_raise(3, 0, 0.70, nut_vp, 2000, 15000, 200, 0, cs_model, 'standard') is None, 'non-station should not inflate'
    assert _river_vs_station_value_raise(2, 0, 0.70, nut_vp, 2000, 15000, 200, 0, cs_model, 'calling_station') is None, 'turn should not fire'
    # Verify sizing: nut should be 0.90x pot
    result = _river_vs_station_value_raise(3, 0, 0.70, nut_vp, 2000, 15000, 200, 0, cs_model, 'calling_station')
    assert result == 1800, f'nut sizing expected 1800 got {result}'
    # Strong should be 0.75x pot
    result2 = _river_vs_station_value_raise(3, 0, 0.65, strong_vp, 2000, 15000, 200, 0, cs_model, 'calling_station')
    assert result2 == 1500, f'strong sizing expected 1500 got {result2}'
    print('_river_vs_station_value_raise: ALL TESTS PASS')
