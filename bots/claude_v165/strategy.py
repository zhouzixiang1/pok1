...
from opponent import build_opponent_model, analyze_current_spot, classify_archetype
...
from strategy_helpers import (
    ...
    turn_bluff_continuation_barrel,
    ...
    _flop_cbet_texture_delta,
    _river_vs_station_value_raise,
)
...
def choose_raise(
    ...
    turn_value_floor_delta=0.0,
    flop_cbet_texture_delta=0.0,
    ...
    # v163 NEW: Flop c-bet texture adaptation. Continuous bidirectional delta
    # (dry+fold-prone -> shrink, wet+made -> grow). Applied to flop open-bet
    # ratio pre-clamp. At priors returns 0.0 (no exploitable edge).
    if round_idx == 1 and flop_cbet_texture_delta != 0.0:
        ratio += flop_cbet_texture_delta
    ...
    # Added AFTER thin_cap clamp (same pattern as street_fold_boost) so the delta
    # survives the 0.30 thin_cap. Fires only on turn (round_idx==2) and only when
    # the opponent is a calling station (fold_to_bet_turn <= 0.42).
    if turn_value_floor_delta > 0.0 and not probe_mode and not blocker_bluff and not inducing_value:
        ratio += turn_value_floor_delta
...
def get_action(req, requests):
    ...
    archetype = classify_archetype(opponent_model)
    sys.stderr.write(f'ARCHETYPE type={archetype} conf={opponent_model.get("confidence",0):.2f}\n')
    ...
    # v162 PLACEMENT FIX: _river_stackoff_guard relocated from ~L1173 (inside
    # to_call>0 block, UNREACHABLE when opponent_allin returns at ~L1123 or
    # to_call>=my_chips returns at ~L1166 first). Now fires on ALL river
    # to_call>0 decisions INCLUDING all-ins and stack-covering bets. NOT a new
    # gate, NOT tuning - PLACEMENT FIX of EXISTING code. Targets 0%-fold river leak (-20k swings).
    if round_idx == 3 and not anti_lock_pressure and to_call > 0:
        if _river_stackoff_guard(made_strength, value_profile, spot_info,
                                 opponent_model, my_chips, pot, to_call):
            print(f"STACKOFF_GUARD_RELOCATED made={made_strength:.2f} "
                  f"tier={value_profile.get('tier','none') if value_profile else 'none'} "
                  f"to_call={to_call} pot={pot} "
                  f"allin={spot_info.get('facing_allin',False)}",
                  file=sys.stderr)
            return -1
    ...
    if to_call > 0:
        # v162: _river_stackoff_guard relocated to ~L1050 (before opponent_allin block).
        if round_idx == 0:
        ...
        turn_value_floor = _turn_value_extraction_floor(opponent_model, round_idx, value_profile, board_texture, made_strength)
        flop_cbet_texture_delta_value = _flop_cbet_texture_delta(
            opponent_model, board_texture, made_strength, round_idx, to_call, value_profile)
        if not preflop_defensive_only and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit):
            raise_amount = choose_raise(
                ...
                turn_value_floor_delta=turn_value_floor,
                flop_cbet_texture_delta=flop_cbet_texture_delta_value,
            )
    ...
    if to_call == 0 and round_idx in (2, 3) and archetype != 'calling_station':
    ...
    # v165 NEW: River value sizing vs calling stations. Inflates strong/nut
    # bets to 0.75-0.90x pot when opponent is a calling station. First use of
    # archetype classification for BET SIZING (v164 only used it for bluff
    # suppression). Bypasses choose_raise for station-specific sizing.
    if round_idx == 3 and to_call == 0:
        _station_river = _river_vs_station_value_raise(
            round_idx, to_call, made_strength, value_profile,
            pot, my_chips, state['min_raise_action'], state['my_round_bet'],
            opponent_model, archetype,
        )
        if _station_river is not None:
            return _station_river
    ...
    if round_idx == 2 and to_call == 0:
        ...
        if _turn_barrel is not None:
            return _turn_barrel
        # v161 NEW: Turn bluff continuation barrel. When bot c-bet the flop
        # but missed (air/draw-missed) and the turn brings a scare card,
        # fire a bluff second-barrel vs over-folding opponents. Orthogonal to
        # turn_second_barrel_planner (value-only) — this is the bluff companion.
        # v164: Suppress vs calling-stations (bluffs are -EV when opp never folds).
        if archetype != 'calling_station':
            _turn_bluff = turn_bluff_continuation_barrel(
                round_idx, to_call, my_id, req.get('history', []),
                made_strength, draw_strength, value_profile, pair_profile,
                board_texture, public_cards, opponent_model,
                pot, my_chips, state['min_raise_action'], state['my_round_bet'],
            )
            if _turn_bluff is not None:
                return _turn_bluff
    ...
    if donk["eligible"]:
        ...
        _flop_cbet_delta_donk = _flop_cbet_texture_delta(
            opponent_model, board_texture, made_strength, round_idx, to_call, value_profile)
        raise_amount = donk_probe_sizing(
            donk["ratio"] + _donk_fold_boost + _flop_cbet_delta_donk, to_call, pot,
            state["min_raise_action"], my_chips, state["my_round_bet"],
        )
        if raise_amount is not None:
            return raise_amount
    ...
    if probe["eligible"]:
        ...
        _flop_cbet_delta_probe = _flop_cbet_texture_delta(
            opponent_model, board_texture, made_strength, round_idx, to_call, value_profile)
        raise_amount = donk_probe_sizing(
            probe["ratio"] + _probe_fold_boost + _flop_cbet_delta_probe, to_call, pot,
            state["min_raise_action"], my_chips, state["my_round_bet"],
        )
        if raise_amount is not None:
            return raise_amount
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or exploit['should_barrel']:
        ...
        if is_pure_value_raise and archetype != 'rock' and _should_checkraise_trap(value_profile, round_idx, board_texture, opponent_model, my_cards, public_cards):
