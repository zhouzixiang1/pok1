...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position):
    ...
    if spot_info["facing_postflop_aggression"]:
        ...
        if spot_info.get("opp_postflop_bet_count", 0) >= 2:
            margin += 0.032 if size_bucket == "small" else 0.020
...
def realized_postflop_equity(
    ...
    if air_hand:
        eqr = 0.55 if has_position else 0.42
    ...
    if pair_profile is not None and pair_profile["made_class"] == 1:
        ...
        if pair_type in ("middle_pair", "bottom_pair", "underpair", "board_pair"):
            eqr = 0.78 if has_position else 0.68
...
def should_fold_postflop(
    ...
    if round_idx == 1 and air_hand and not has_blocker:
        ...
        if size_bucket == "small" and board_texture is not None:
            ...
            if board_texture["wetness"] >= effective_wetness:
                return True
        if size_bucket == "small" and board_texture is not None and board_texture.get("paired", False):
            return True
    ...
    effective_cat4_made = 0.38 + aggr_delta * 0.5
    ...
    _cat5_made_threshold = 0.47 if spot_info.get("opp_postflop_bet_count", 0) >= 3 else 0.40
    ...
    effective_cat6_upper = 0.34 + aggr_delta * 0.5
    ...
    # Extended to medium bets when opponent shows multi-street aggression.
    if (round_idx == 2
            and pair_profile is not None and pair_profile["made_class"] == 1
            and pair_profile["pair_type"] == "middle_pair"
            and size_bucket in ("medium", "large") and (size_bucket == "large" or spot_info.get("opp_current_round_bet_count", 0) >= 2)
