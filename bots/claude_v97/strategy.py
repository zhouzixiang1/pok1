...
def _sb_open_bucket_action(hand_cat, opponent_model, trash_hand):
    ...
    sticky = conf >= 0.20 and fold_open < 0.34  # sticky-open-defense boundary: below prior fold-to-open/fold-to-raise range.
    steal = conf >= 0.20 and fold_open > 0.52 and threebet < 0.20  # steal boundary: reuses existing fold-to-raise steal line; low-3bet guard has no named prior.
    pressure = conf >= 0.20 and threebet > 0.24  # pressure boundary: no existing threebet-vs-open aggression prior.
    if hand_cat in ('premium', 'strong_pair', 'mid_pair', 'big_cards'):
        return 'raise'
    if hand_cat in ('small_pair', 'suited_ace', 'suited_connector', 'playable'):
        if pressure and hand_cat in ('playable', 'suited_connector'):
            return 'call'
        if sticky and hand_cat == 'playable':
            return 'call'
        return 'raise' if steal or not sticky else 'call'
    return 'fold' if trash_hand or sticky or pressure else 'call'
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if hand_cat in ('suited_connector', 'suited_ace', 'small_pair') and not trash_hand:
            ...
            if bluff_roll < 0.22:
                ...
                if raise_amount is not None:
                    return raise_amount
        # Call with strong/mid pairs (TT-JJ, 77-99) and playable hands
        if hand_cat in ('strong_pair', 'mid_pair', 'playable'):
            return 0
