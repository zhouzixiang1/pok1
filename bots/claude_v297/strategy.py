    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")):
        ...
        if raise_amount is not None:
            ...
            if round_idx == 3 and raise_amount >= my_chips * 0.60:
                tier = value_profile.get("tier", "none") if value_profile else "none"
                if tier != "nut":
                    # [v297 mutation] Calling-station-aware thin-value cap.
                    # Experience pool: "Unconditional value caps (e.g. 0.85x)
                    # risk leaving chips vs calling-stations; gate on
                    # fold_to_raise>0.45." vs v284/v46/v45 (calling-station
                    # cluster, WR<0.45), the 0.75x thin cap forfeits value.
                    # When opp folds < 0.40 to raises, widen thin cap to 0.85x
                    # so we extract an extra street of thin value from callers.
                    ftr = opponent_model.get("fold_to_raise", PRIOR_FOLD_TO_RAISE)
                    if tier == "strong":
                        cap_ratio = 1.0
                    elif ftr < 0.40:
                        cap_ratio = 0.85  # calling station: widen thin/none cap
                    else:
                        cap_ratio = 0.75
