...
def opponent_pressure_adjustment(opponent_model, spot_info, round_idx):
    # [v15 base] Wider clamp bounds for more reactive opponent modeling
...
def realized_postflop_equity(
    ...
    # [v15 feature] Per-street opponent-model-aware EQR for air hands
    air_hand = made_strength < 0.18 and draw_strength < 0.08
    if round_idx <= 0:
        return win_rate
...
def choose_raise(
    ...
    # [MUTATION] Calling-station value sizing: inflate bets vs passive callers
    if round_idx > 0 and not semi_bluff and not blocker_bluff:
        tier = value_profile.get("tier", "none")
        if tier in ("strong", "nut") and confidence >= 0.25 and fold_to_raise < 0.38:
            ratio += 0.06
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    # [CROSSOVER] v18's enhanced version with SPR + opponent-model fold awareness
    if round_idx <= 0:
        return False
    ...
    # [v18] SPR commitment: fold weak uncommitted hands on late streets
    ...
    # [v18] Opponent-model-aware fold: value-heavy opponents
    ...
    # [v18] River multi-barrel fold: very weak hands even vs small bets
...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if fragile_pair_raise_fold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # [CROSSOVER] v18: pass opponent_model and SPR to enhanced should_fold_postflop
        ...
        if realized_rate < pot_odds + call_margin:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # [CROSSOVER] v18: smarter repeated-raise-trap — fold weak hands instead of always calling
