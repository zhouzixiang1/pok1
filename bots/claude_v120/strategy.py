...
def choose_raise(
    ...
    # CROSSOVER v120 (v116 × v93): value-tier sizing floor (OFFENSIVE axis absent
    # from v116; imported from v93 where it was unconditional). H2H evidence:
    # v93 beats v116 head-to-head 16-14 and wins vs newer aggressive bots where
    # v116 loses (v96 0.50 vs 0.425; v97 0.515 vs 0.467; v117 0.55 vs 0.40). v116
    # underbets made hands (global ~0.4x pot bias) leaving value on the table vs
    # aggressive value-bettors who extract every chip. The floor ensures nut/strong
    # hands bet at least street-aware minimums.
    # NOT byte-identical to v93: adds NEW passivity_score-aware boost gating
    # (per PARAMETER_TUNING exemption for offensive value-tier floor imports with
    # NEW opponent signal). vs passive opponents (passivity_score >= 0.58 AND
    # confidence >= 0.20) who call down wider, raise floors to extract more value.
    ...
    if (_tier in ('nut', 'strong') and not semi_bluff and not blocker_bluff
        ...
        _passivity = opponent_model.get('passivity_score', 0.5)
        _passive_boost = (_passivity >= 0.58 and confidence >= 0.20)
        if round_idx == 1:
            _value_floor = 0.56 if _passive_boost else 0.50
        elif round_idx == 2:
            _value_floor = 0.60 if _passive_boost else 0.55
        elif round_idx == 3:
            _value_floor = 0.65 if _passive_boost else 0.60
        else:
...
def _sb_open_bucket_action(hand_cat, opponent_model, trash_hand):
    ...
    # MUTATION v120 (a — threshold adjustment, ~12% lower): widen pressure_bb
    # detection threebet >= 0.26 -> >= 0.23 so marginally light 3betters (who
    # 3bet at 0.23-0.25 frequency) are also classified as pressure. When facing
    # a pressure BB, implied-odds hands (small pairs, suited connectors, suited
    # aces, broadway suited) CALL instead of RAISE, keeping the pot small and
    # avoiding being 3bet off the hand. Reduces preflop chip bleed against the
    # newer aggressive light-3betting opponents (v96/v97/v117) where v116 loses.
    pressure_bb = has_open_read and threebet >= 0.23
