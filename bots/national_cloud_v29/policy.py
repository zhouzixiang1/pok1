...
def _bluff_allowed(context, equity):
    ...
    base_freq = 0.30 if draw > 0.0 else 0.10 if line_permission else 0.0
    # Consume the reducer-owned terminal-response posterior: a demonstrated
    # over-folder (terminal_response.fold_to_raise above the prior) admits more
    # low-equity flop/turn raise candidates for the EV scorer, while a caller
    # suppresses them.  The leaf defaults to the prior on a sparse profile, so
    # the neutral path is unchanged.
    opponent = context.get("opponent", {}) or {}
    opponent = opponent if isinstance(opponent, dict) else {}
    terminal = opponent.get("terminal_response", {}) or {}
    terminal = terminal if isinstance(terminal, dict) else {}
    ...
    # 0-1 terminal confidence derived exactly as the posterior consumer derives
    # its terminal weight, so sparse evidence cannot move the gate off the
    # prior-centered credit and spike frequency.
    terminal_weight = _bounded(
        terminal.get(
            "adaptation_weight",
            _bounded(terminal.get("confidence"), 0.0, 1.0)
    ...
    confidence = terminal_weight / MAX_ADAPTATION_WEIGHT
    credit_raw = _bounded(
        1.0 + 2.0 * (fold_to_raise - FOLD_TO_RAISE_PRIOR), 0.5, 1.5
    )
    # Pull the credit toward 1.0 by confidence: at the prior credit_raw is
    # exactly 1.0 for any confidence, so the gate is byte-identical to the
    # fixed-frequency baseline; only a confident over-folder raises frequency
    # and only a confident caller lowers it.
    credit = 1.0 + confidence * (credit_raw - 1.0)
    frequency = base_freq * credit
...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    # A low-SPR jam at the equity floor is only +EV with real fold equity.
    # Against a sticky caller (modeled fold_to_jam at/under the prior) the
    # calling range is stronger, so require a higher raw-equity bar rather
    # than stacking off on thin fold equity the native opponent does not
    # concede.  A demonstrated over-folder keeps the original floor.
    jam_equity_floor = 0.66 if jam_fold_rate < 0.30 else 0.62
    if (
        "allin" in kinds
        and hero_stack > 0
        and safe_equity >= jam_equity_floor
