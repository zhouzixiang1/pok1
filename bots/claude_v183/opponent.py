...
def _aggro_bluffcatcher_should_fold(om, round_idx, to_call, pot,
    ...
    """v183 NEW: Bluff-catcher fold discipline vs confirmed aggro archetype.
    ...
    Folds when pot-odds demand >10% more equity than the hand has.

    This is an EV-integrated continuous fold (NOT a dead binary postflop fold):
    the 0.10 margin ensures only clearly -EV calls are released.

    Guards: fires ONLY on turn/river (round_idx>=2) + aggro archetype (conf>=0.15)
    + bluff-catcher band (made 0.20-0.45) + no draw + tier not nut/strong
    + bet is significant (pot_odds >= 0.33, i.e. half-pot+).
    """
    if round_idx < 2 or to_call <= 0:
        return False
    archetype, conf = classify_archetype(om)
    if archetype != 'aggro' or conf < 0.15:
        return False
    ...
    if pot_odds < 0.33:
        return False
    # EV gate: fold when pot-odds exceed made-strength by >0.10 (clearly -EV call).
