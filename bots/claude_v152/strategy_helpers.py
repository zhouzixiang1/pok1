...
def _preflop_steal_defense_widen(hand_cat, opponent_model, pot_odds,
    ...
    if not facing_3bet:
        unknown_or_loose = (confidence < 0.20) or (pfr >= 0.30 or vpip >= 0.55)
        ...
        if hand_cat in ('mid_pair', 'strong_pair'):
            if 0.32 < pot_odds <= 0.42 and win_rate >= pot_odds - 0.04:
                return 'call'
            return None
        if hand_cat in ('suited_connector', 'suited_ace'):
            if pot_odds <= 0.34 or win_rate >= pot_odds - 0.02:
                return 'call'
            return None
        return None
    ...
    if hand_cat == 'small_pair':
        return 'call' if pot_odds <= 0.32 else None
    if hand_cat in ('suited_connector', 'suited_ace', 'broadway_suited'):
        return 'call' if pot_odds <= 0.30 else None
...
if __name__ == "__main__":
    ...
    print("v152 _preflop_steal_defense_widen: 5/5 fixture cases PASS")
