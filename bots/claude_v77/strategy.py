...
from state import (
    ...
    is_preflop_3bet_candidate, is_preflop_trash_hand, preflop_hand_profile,
)
...
from opponent import build_opponent_model, analyze_current_spot, classify_opponent_archetype, exploit_dispatch
...
def choose_raise(
    ...
    overbet_mode=False,
    sizing_hint=None,  # pot-fraction from evaluate_turn_barrel; overrides round_idx base
    exploit_sizing_boost=0.0,  # offensive value sizing boost from exploit_dispatch
):
    ...
    # Caller-provided sizing hint overrides round_idx default base ratio.
    # All subsequent adjustments (value floor, protective floor, clamp) still apply.
    if sizing_hint is not None and sizing_hint > 0:
        ratio = sizing_hint
    ...
    # Offensive exploit boost — increase value sizing vs over-calling streets
    ratio += exploit_sizing_boost
    ratio = clamp(ratio, low_ratio, max_ratio)
    ...
    # ── Value sizing floor: prevent underbetting strong/nut hands on flop/turn/river ──
    if (round_idx >= 1
        ...
        elif round_idx == 1:  # flop — charge draws, extract value
...
def evaluate_turn_barrel(spot_info, opponent_model, board_texture, made_strength,
    ...
    if draw_quality >= 0.16 and confidence >= 0.15 and fold_to_raise > (0.46 - barrel_boost):
    ...
    if (confidence >= 0.20
            and fold_to_raise > (0.52 - barrel_boost)
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # ── Mutation (experience pool): Premium 5-bet-jam for QQ+ when facing 4-bet ──
        # Experience pool: "BB vs 4-bet QQ+ (strength >=0.72) falls into a call-only path
        # with no 5-bet-jam exception, unlike sb_vs_reraise (line 702, strength >=0.78
        # premium jam preserved)." This mirrors that structure.
        # opp_preflop_raises >= 2 means SB raised, BB 3-bet, SB 4-bet → we face a 4-bet.
        # Uses preflop_hand_profile to identify QQ+ (rank>=12 pairs) and AK directly,
        # since estimate_preflop_strength saturates to 1.0 for ALL pocket pairs.
        if (not trash_hand
                and spot_info.get('opp_preflop_raises', 0) >= 2
                and my_chips > 0):
            _pf = preflop_hand_profile(req['my_cards'])
            _is_qq_plus = _pf['pair'] and _pf['high'] >= 12
            _is_ak = _pf['high'] == 14 and _pf['low'] == 13
            if _is_qq_plus or _is_ak:
                return -2  # All-in jam with QQ+, KK, AA, AK
        ...
        # ── Crossover from v19: SB limped then faces BB raise ──────────────────
        # v75 was MISSING this entire spot — it fell through to generic call/fold
        # logic, misidentified as sb_vs_reraise. Limp range is fundamentally weaker
        # than open-raise range, so the strategy is mostly fold/call with trap-raise
        # for premiums (limp-raise) and implied-odds calls for speculative hands.
        pot_odds_sl = to_call / (to_call + state['pot']) if to_call > 0 else 0.0
        # Trap-raise (limp-raise) with premiums — capitalizes on BB's iso-raise
        if preflop_strength >= 0.60 and not trash_hand:
            raise_amount = choose_raise(
                state['min_raise_action'], my_chips, state['my_round_bet'],
                to_call, state['pot'], max(win_rate, preflop_strength),
                0, 'sb_limp_vs_raise', preflop_strength,
                False, opponent_model,
                match_sizing_delta=match_profile['sizing_delta'],
            )
            if raise_amount is not None:
                return raise_amount
        # Call with playable hands — limp range has implied odds when hitting flops
        if preflop_strength >= 0.28 or win_rate >= pot_odds_sl - 0.02:
            return 0
        return -1
...
def get_action(req, requests):
    ...
    exploit = exploit_dispatch(opponent_model, round_idx)
    to_call = state["to_call"]
    pot = max(1, state["pot"])
    ...
    # ── v73: Opponent-aware commitment gate ──────────────────────────────────
    # Folds marginal hands to high-commitment bets on turn/river.
    # Evidence: 0% postflop fold across 589 H2H pairs; replay shows
    # -19999 losses from calling all-ins with one-pair hands.
    if round_idx >= 2 and to_call > 0 and not anti_lock_pressure:
        stack_commit = to_call / my_chips if my_chips > 0 else 1.0
        if stack_commit >= 0.35:
            is_marginal = (
                (value_profile is None or value_profile.get("tier") not in ("strong", "nut"))
                and made_strength < 0.50
                and draw_strength < 0.18
            )
            if is_marginal and opp_archetype != 'lag':
                return -1
    ...
    if to_call > 0:
        ...
        if not preflop_defensive_only and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit):
            raise_amount = choose_raise(
                ...
                match_sizing_delta=match_profile["sizing_delta"],
                exploit_sizing_boost=exploit['value_sizing_boost'],
            )
    ...
    if (round_idx == 2 and to_call == 0 and was_flop_aggressor
        ...
        barrel = evaluate_turn_barrel(
            ...
            draw_strength, draw_info, value_profile, was_flop_aggressor,
            spot_info['has_position'], barrel_boost=exploit['barrel_freq_boost'],
        )
        if barrel['barrel']:
            barrel_amount = choose_raise(
                state["min_raise_action"], my_chips, state["my_round_bet"],
                to_call, pot, win_rate, round_idx, spot_info["preflop_spot"],
                preflop_strength, spot_info["has_position"], opponent_model,
                semi_bluff=(barrel['barrel_type'] == 'semi_bluff'),
                value_profile=value_profile,
                value_plan=value_plan,
                board_texture=board_texture,
                draw_info=draw_info,
                sizing_hint=barrel['sizing_hint'],
                nutted_risk_score=nutted_risk["risk"],
                match_sizing_delta=match_profile["sizing_delta"],
            )
    ...
    river_blocker_bluff = (
        ...
        and blocker_profile["eligible"]
        and allow_low_frequency_blocker_bluff(req, my_cards, public_cards, blocker_profile, round_idx)
        and not exploit['bluff_suppress']
    )
    if opp_archetype == 'calling_station':
