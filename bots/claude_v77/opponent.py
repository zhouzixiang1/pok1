    ...
    if state["round"] == 0:
        if not history and info["my_is_sb"]:
            ...
            if history[-1]["action_type"] in ("raise", "allin"):
                # Crossover from v19: distinguish limp-then-face-raise from open-then-face-3bet.
                # When SB limped (call) and BB raises, the strategic situation is entirely
                # different from SB opening and facing a 3-bet — limp range is much weaker.
                sb_first = None
                for rec in history:
                    if rec["player_id"] == my_id and rec["round"] == 0:
                        sb_first = rec["action_type"]; break
