...
def nutted_risk_profile(hole_cards, public_cards, pair_profile=None, board_texture=None, value_profile=None, paired_board_profile=None):
    ...
    if hand_class == 6:
        ...
        if score[1] < max(board_ranks):
            risk += 0.02
    # v128: Flush-board vulnerability for two-pair/set — opponent may have a flush.
    # Closes G0H28/G4H12 leak: two-pair on 4-flush board called all-in (-19999).
    # Existing logic only adds risk when board is PAIRED; this extends to FLUSH boards.
