...
def check_raise_pressure(spot_info, opponent_model):
    ...
    Delivers the check_raise_freq directive (undelivered 11+ generations) via a
    NEW opponent-line signal: opp_current_round_check_count AND
...
def barrel_pressure_profile(spot_info, opponent_model, round_idx):
    ...
    Birth requirements satisfied: NEW detector, NEW signal
    (opp_postflop_bet_count + bet_size_bucket, not used in check_raise_pressure),
    >=3 wired reachable sites, >=3 cited replays (G0H28/G4H12/G2H51),
    confidence gate, firing logs.
