...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    ...
    # v127: Check-raise pressure fold gate (delivers check_raise_freq mandate).
    # Folds marginal hands to a LIVE opponent check-raise trap
    # (opp_current_round_check_count>=1 AND opp_current_round_bet_count>=1).
    # Targets the 0%-postflop-fold leak — directly eliminates river CR stack-offs
    # (G4H41 -16567, G8H64 -9325, G4H63 -8136 patterns). made_strength bands:
    # one-pair ~0.22-0.23, two-pair ~0.40-0.41, air <0.18. Threshold 0.42 river
    # catches one-pair+air, preserves two-pair+; 0.30 turn catches air+weak-one-pair.
    if opponent_model is not None:
        ...
        if cr_active:
            ...
            if round_idx == 3 and made_strength < river_threshold and not has_draw:
                import sys
                print(f"FOLD_GATE_FIRE check_raise_river ms={made_strength:.3f} sev={cr_severity:.3f} thr={river_threshold:.3f}", file=sys.stderr)
                return True
            if round_idx == 2 and made_strength < turn_threshold and not has_draw:
                import sys
                print(f"FOLD_GATE_FIRE check_raise_turn ms={made_strength:.3f} sev={cr_severity:.3f} thr={turn_threshold:.3f}", file=sys.stderr)
                return True

    # v128: barrel_pressure fold gate — REGULAR barrel lines (non-trap).
    # Closes the 0%-fold leak: folds weak-one-pair/air facing single+ barrel
    # from frequent barreler. Distinct from check_raise_pressure (trap only).
    if opponent_model is not None:
        bp_active, bp_severity = barrel_pressure_profile(spot_info, opponent_model, round_idx)
        if bp_active:
            # River: fold weak-one-pair and worse (~0.20-0.32). Preserves strong one-pair+.
            # Turn: fold air and weak one-pair (~0.18-0.24).
            river_thr = 0.32 - bp_severity
            turn_thr = 0.24 - bp_severity
            if round_idx == 3 and made_strength < river_thr and not has_draw:
                import sys
                print(f"FOLD_GATE_FIRE barrel_river ms={made_strength:.3f} sev={bp_severity:.3f} thr={river_thr:.3f}", file=sys.stderr)
                return True
            if round_idx == 2 and made_strength < turn_thr and not has_draw:
                import sys
                print(f"FOLD_GATE_FIRE barrel_turn ms={made_strength:.3f} sev={bp_severity:.3f} thr={turn_thr:.3f}", file=sys.stderr)
                return True
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        if preflop_strength is not None and preflop_strength < 0.42:
            jam_buffer += 0.02
        # v127: Check-raise pressure reduces all-in call willingness.
        # jam_buffer = required win_rate margin; higher = tighter call.
        cr_active, cr_severity = check_raise_pressure(spot_info, opponent_model)
        if cr_active:
            jam_buffer += cr_severity
            import sys
            print(f"FOLD_GATE_FIRE check_raise_jam round={round_idx} ms={made_strength:.3f} sev={cr_severity:.3f}", file=sys.stderr)
        # v128: barrel_pressure tightens all-in call willingness (parallel to check_raise).
        bp_active, bp_severity = barrel_pressure_profile(spot_info, opponent_model, round_idx)
        if bp_active:
            jam_buffer += bp_severity
            import sys
            print(f"FOLD_GATE_FIRE barrel_jam round={round_idx} ms={made_strength:.3f} sev={bp_severity:.3f}", file=sys.stderr)
        if anti_lock_pressure:
            jam_buffer -= 0.10
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            if round_idx == 3 and paired_board_profile is not None and paired_board_profile["fold_to_raise"]:
                call_margin += 0.05
            # v127: Check-raise pressure tightens call threshold (more folds).
            # call_margin is the required safety margin; higher = harder to call.
            cr_active, cr_severity = check_raise_pressure(spot_info, opponent_model)
            if cr_active:
                call_margin += cr_severity
                import sys
                print(f"FOLD_GATE_FIRE check_raise_call_margin round={round_idx} ms={made_strength:.3f} sev={cr_severity:.3f}", file=sys.stderr)
            # v128: barrel_pressure tightens call threshold (parallel to check_raise).
            bp_active, bp_severity = barrel_pressure_profile(spot_info, opponent_model, round_idx)
            if bp_active:
                call_margin += bp_severity
                import sys
                print(f"FOLD_GATE_FIRE barrel_call_margin round={round_idx} ms={made_strength:.3f} sev={bp_severity:.3f}", file=sys.stderr)
