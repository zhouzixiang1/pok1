...
def _spr_commitment_gate(round_idx, to_call, pot, my_chips, made_strength,
    ...
    """CROSSOVER (imported from v91): structural fold gate for large commitment bets.

    Distinct decision point from the other postflop defensive paths:
      - _single_reraise_stackoff_guard: fires only at exactly 1 re-raise (returns 0/call)
      - facing_barrel_continuation: nudges call_margin by <=0.06 (never forces fold)
      - THIS gate: returns -1 to fold marginal made hands facing LARGE bets on new
        axes — commit_ratio (bet/stack), pot_ratio (bet/pot), SPR, board scariness.

    Fires BEFORE the call_margin logic inside the to_call>0 block to intercept
    spots where made_strength alone would keep us calling a near-all-in polarized
    bet with a second-best hand.

    MUTATION: GATE 2 river strength_cap with elevated nutted_risk tightened from
    v91's 0.45 to 0.40 — a ~11% relative reduction. Marginal made hands on the
    river facing a >=75% pot bet when opponent is likely nutted (set+) now fold
    slightly more readily, bleeding less in the worst-case polarization spot.
    """
    ...
    # MUTATION: elevated-risk cap tightened 0.45 -> 0.40 (see docstring).
    if round_idx == 3 and pot_ratio >= 0.75:
        strength_cap = 0.40 if risk >= 0.08 else 0.50
        if made_strength < strength_cap and not has_draw:
            return -1
...
def get_action(req, requests):
    ...
    if to_call > 0:
        # CROSSOVER (from v91): structural commitment gate — fires BEFORE the
        # call_margin logic to fold marginal made hands facing large commitment
        # bets on turn/river using axes distinct from the other defensive paths
        # (commit ratio, pot ratio, SPR, board scariness). Targets the blind
        # spot where made_strength alone keeps us calling a near-all-in
        # polarized bet with a second-best hand.
