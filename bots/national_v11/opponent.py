...
def classify_opp_open_sizing(opponent_model):
    ...
    Distinct from strategy._classify_open_raise_size (immediate raise) and
    classify_opponent_archetype (VPIP/aggression). Returns one of:
    ...
    gives ~50% weight to observed data).
    """
    confidence = opponent_model.get('confidence', 0.0)
...
def build_opponent_model(requests, my_id):
    ...
    for req in hand_requests:
        ...
        for record in history:
            ...
            if pid == my_id and round_idx == 3 and action_type in ("raise", "allin"):
            ...
            if pending_my_river_bet and pid == opponent_id and round_idx == 3:
                if action_type == "call":
...
if __name__ == '__main__':
    # M5/M6 self-test: live-pool defaults must classify as 'standard' or 'unknown',
    # NOT silently as 'large'/'xl'/'small' (which would mean false-positive
    # exploitation vs an unknown opponent).
    _defaults = {
        'confidence': 0.5, 'raise_samples': 0,
        'avg_raise_bb': DEFAULT_AVG_RAISE_BB,
    }
    _b1 = classify_opp_open_sizing(_defaults)
    assert _b1 == 'unknown', f'no-samples must be unknown, got {_b1}'
    _b2 = classify_opp_open_sizing({'confidence': 0.5, 'raise_samples': 8, 'avg_raise_bb': 5.0})
    assert _b2 == 'large', f'5BB avg over 8 samples should be large, got {_b2}'
    _b3 = classify_opp_open_sizing({'confidence': 0.5, 'raise_samples': 8, 'avg_raise_bb': 2.0})
    assert _b3 == 'small', f'2BB avg over 8 samples should be small, got {_b3}'
    _b4 = classify_opp_open_sizing({'confidence': 0.5, 'raise_samples': 8, 'avg_raise_bb': 2.8})
    assert _b4 == 'standard', f'2.8BB avg should be standard, got {_b4}'
    print('opponent.py classify_opp_open_sizing self-test OK')
