...
def spr_commitment_gate(my_chips, pot, value_profile, pair_profile, made_hand_class, board_texture=None):
    ...
    # 0.625 factor tightens TPTK SPR threshold 4 -> 2.5 on dynamic boards.
    _TPTK_DYNAMIC_TIGHTEN_FACTOR = 0.625
    _TPTK_DYNAMIC_FLOOR_SPR = 1.0  # never tighten below SPR 1 (chip-commit floor)
    ...
    if made_hand_class == 1 and pair_profile is not None and pair_profile.get('made_class') == 1:
        ...
        if key is not None:
            ...
            label = key
    # ── Dynamic-board TPTK tightening (crossover: board-texture awareness) ──────
    # On wet/connected boards (flush_draw / OESD / high wetness -> dynamic=True),
    # TPTK loses equity vs sets/flushes/straights; require a LOWER SPR to be
    # auto-committed so the downstream fold gates can still rescue us when the
    # board texture demands it. No-op when board_texture is None (legacy caller).
    if (
        label == 'tptk'
        and threshold > 0
        and board_texture is not None
        and bool(board_texture.get('dynamic', False))
    ):
        tightened = threshold * _TPTK_DYNAMIC_TIGHTEN_FACTOR
        if tightened < _TPTK_DYNAMIC_FLOOR_SPR:
            tightened = _TPTK_DYNAMIC_FLOOR_SPR
        threshold = tightened
        label = 'tptk_dynamic'
...
if __name__ == '__main__':
    ...
    _wet_board = {'wetness': 0.85, 'flush_pressure': 0.75, 'straight_pressure': 0.65,
    ...
    # 11. TPTK on DYNAMIC board, SPR=4 — NOT committed (tightened threshold 2.5)
    assert spr_commitment_gate(800, 200, FakeVal('strong'), FakePair(1,'top_pair'), 1, _wet_board)[0] is False, \
        'TPTK dynamic SPR=4 above tightened 2.5 threshold'
    # 12. TPTK on DYNAMIC board, SPR=2 — committed (within tightened threshold)
    assert spr_commitment_gate(400, 200, FakeVal('strong'), FakePair(1,'top_pair'), 1, _wet_board)[0] is True, \
        'TPTK dynamic SPR=2 committed (within tightened 2.5)'
    # 13. TPTK on DYNAMIC board, SPR=3 — NOT committed (above tightened threshold)
    assert spr_commitment_gate(600, 200, FakeVal('strong'), FakePair(1,'top_pair'), 1, _wet_board)[0] is False, \
        'TPTK dynamic SPR=3 above tightened 2.5 threshold'
