...
def _river_reraise_tighten(state, spot_info, made_strength, value_profile,
    ...
    """v169 NEW: Continuous call-margin delta for strong-but-not-nut hands
    facing polarized river re-raises. Targets the 12-gen -20k stack-off leak
    where tier='strong' hands (trips/two-pair/low flush) NEVER fold because
    _river_stackoff_guard and _spr_commitment_gate both exempt them.

    GENUINELY NEW AXIS: uses opponent re-raise ratio (opp raise-to-total /
    bot raise-to-total on current street) — no existing gate reads this.
    ...
    if not spot_info.get('facing_postflop_aggression', False):
        return 0.0
    # 3. Hand-strength gate: only strong-but-not-nut (0.55 <= made < 0.80)
    #    This is the specific band that ALL existing gates exempt.
    if made_strength < 0.55 or made_strength >= 0.80:
        return 0.0
    ...
    # 7. Scale by made_strength: lower strong hands (0.55-0.65) more vulnerable
    #    than higher (0.70-0.80)
    strength_factor = 1.0 - clamp((made_strength - 0.55) / 0.25, 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    # v169 _river_reraise_tighten self-test: 4 scenarios.
    ...
    print(f'_river_reraise_tighten self-test PASS: high={_d1:.4f} low={_d2} nut={_d3} safe={_d4}')
