...
def should_fold_postflop(
    ...
    """Board-texture-aware postflop fold detection layer.

    Identifies three categories of holdings that should fold to postflop bets:
    1. Air hands (no made equity, no draws) on turn/river facing bets
    2. Weak made hands (bottom pair, underpair, etc.) facing large bets with bad pot odds
    3. Non-nut holdings facing extreme multi-street aggression on wet boards

    Returns True if the hand should fold, False otherwise.
    This layer sits above equity comparison to catch hands that are
    structurally unprofitable regardless of small equity tweaks.
    """
    if round_idx <= 0:
        return False
    ...
    if round_idx == 1 and air_hand and not has_blocker:
        if size_bucket in ("medium", "large"):
            return True
        if size_bucket == "small" and board_texture is not None and board_texture["wetness"] >= 0.35:
            return True
    ...
    if round_idx >= 2 and spot_info.get("opp_current_round_bet_count", 0) >= 2:
        if board_texture is not None and board_texture["dynamic"] and size_bucket == "large":
            ...
            has_strong_draw = draw_strength >= 0.16
    ...
    if round_idx == 3 and made_strength < 0.25 and draw_strength < 0.08:
    ...
    # raise threshold by +0.05 to avoid over-folding to pressure.
    _cat6_made_threshold = 0.45 if spot_info.get("opp_postflop_bet_count", 0) >= 3 else 0.40
    if (round_idx == 3 and pair_profile and pair_profile["made_class"] == 1
            ...
            and not has_blocker and made_strength < _cat6_made_threshold
    ...
    if (round_idx == 2 and 0.18 <= made_strength < 0.26 and draw_strength < 0.10
