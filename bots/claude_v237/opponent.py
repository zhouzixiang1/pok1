...
def _estimate_bluff_frequency(om):
    ...
    # v237 CROSSOVER (port v235's underbettor betsize_polarity calibration
    # from v206×v235) + MUTATION (VPIP gate tightened 0.55→0.60).
    # An underbettor's bets are CONDENSED thin-value (small sizing with
    # medium-strength hands), NOT polarized nuts-or-air. The standard
    # discount (0.40+base) UNDERESTIMATES our bluff-catcher equity vs such
    # a condensed range. Flooring bluff_freq at 0.30 raises the discount
    # to 0.70 → higher calibrated_equity → less aggressive folding in
    # _allin_polarized_equity_fold / _river_potodds_equity_margin.
    #
    # This fires ONLY when _opp_bluff_prone has NOT already floored (narrow
    # window: underbettor + moderate aggression + decent fold_to_bet).
    #
    # MUTATION: VPIP gate tightened from v235's 0.55 to 0.60. Aligns with
    # the experience-pool standard "VPIP>0.60" for wide open-shovers.
    # v235's 0.55 gate likely false-triggered vs marginal-VPIP value-heavy
    # opponents (v200/v208/v205), explaining v235's -22%/-35%/-20%
    # regression there vs v206. At 0.60, the gate fires only for TRULY
    # wide opponents (v182/v167/v233), preserving v206's fold discipline
    # against the value-heavy tier while retaining v235's wide-underbettor
    # edge. Experience pool: "Gate value/fold aggression on deal-local
    # opp fields (VPIP>0.60) from the start."
