...
def bluff_heavy_call_widen(line_profile, value_profile, made_strength, draw_strength, round_idx, opponent_model):
    ...
    CROSSOVER+MUTATION v126 (v118 base; option (b) — heuristic from
    experience_pool.md BLUFF_CALIBRATION: "low aggression/passivity alone may
    indicate calling-station behavior"). The `bluff_heavy` line label may fire
    from PASSIVE-opponent signals in line_reading.py (post_aggr<=0.28 AND
    barrel_freq<=0.30, contributing +0.18 to bluff_opportunity). But a passive
    opponent's bets skew toward value with worse hands, NOT bluffs — widening
    calls vs a calling-station burns chips. v118's H2H losses to v92 (0.467)
    and v96 (0.457) — where v108 (BLUFF_OPP=0.55) wins by ACCIDENT because
    the label fires less often — trace to this conflation. We target the root
    cause directly: if the opponent is passive, do NOT widen calls regardless
    of line label. The label may still inform our bluffs/raises elsewhere
    (correct response vs fold-prone opponents), just not call-widening.
    Adds NEW opponent-signal gating to an existing function — per experience
    pool PARAMETER_TUNING, this counts as NEW (not exhausted-axis work)."""
    ...
    if confidence < 0.15:
        return 0.0
    # Calling-station guard: passive opponents' bets are value-weighted, not
    # bluffs. Mirror the passive-signal condition in line_reading.py
    # (post_aggr<=0.28 AND barrel_freq<=0.30) to neutralize exactly the call-
    # widening driven by those same passive signals.
    post_aggr = opponent_model.get('postflop_aggr', 0.36)
    barrel_freq = opponent_model.get('barrel_freq', 0.45)
    if post_aggr <= 0.28 and barrel_freq <= 0.30:
        return 0.0
