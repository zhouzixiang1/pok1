...
def _semibluff_raise_construct(om, round_idx, to_call, pot, made_strength,
                                ...
                                min_raise, my_round_bet):
    """v233 NEW OFFENSE: Semi-bluff raise construction on turn/river.

    Fires when facing a bet (to_call > 0) with a legit draw (8+ outs) and a
    non-made hand (made_str 0.15-0.42) vs opponents who over-fold to raises.
    Constructs raise_to = to_call * 2.8 to exploit fold equity. DISTINCT from
    existing semi_bluff path (generic choose_raise sizing) and
    _board_texture_bluff_raise (to_call==0 with air).
    """
    import sys as _sys
    if round_idx < 2 or to_call <= 0:
        return None
    # Must have a legitimate draw (8+ outs equivalent)
    if draw_info is None or not draw_info.get('semi_bluff', False):
        return None
    if draw_strength < 0.15:
        return None
    # Non-made to marginal hand — drawing territory
    if made_strength < 0.15 or made_strength > 0.42:
        return None
    # Don't fire with strong value hands (they should raise for value, not bluff)
    if value_profile is not None and value_profile.get('tier') in ('strong', 'nut'):
        return None
    # Board must be coordinated (draws more likely to hit, more credible raise)
    if board_texture is None:
        return None
    ...
    if not (wetness >= 0.30 or has_flush or has_combo):
        return None
    # Opponent bet size gate: 25-65% pot (mid-range, not tiny or all-in)
    opp_bet_ratio = spot_info.get('last_raise_pot_ratio', 0.0)
    if opp_bet_ratio < 0.25 or opp_bet_ratio > 0.65:
        return None
    # Opponent must over-fold to raises
    fold_to_raise = om.get('fold_to_raise', 0.44)
    if fold_to_raise <= 0.50:
        return None
    # Confidence gate — need some data
    conf = om.get('confidence', 0.0)
    if conf < 0.20:
        return None
    # SPR gate — need room to maneuver (not committed)
    spr = my_chips / max(1, pot)
    if spr <= 3.0:
        return None
    # NOTE: call_to_raise < 0.55 is implicitly satisfied since
    # fold_to_raise > 0.50 means non-fold rate < 0.50 < 0.55

    # Construct raise-to-total: 2.8x the call amount creates fold pressure
    raise_to_total = int(to_call * 2.8) + my_round_bet
    # Validate legality
    if raise_to_total < min_raise:
        raise_to_total = min_raise
    if raise_to_total > my_chips:
        return None  # would be all-in, not a semi-bluff raise
    if raise_to_total <= to_call + my_round_bet:
        return None  # must be an actual raise
    # +EV check: fold equity must exceed the pot fraction we risk
