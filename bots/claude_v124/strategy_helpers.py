...
def bluff_heavy_call_widen(line_profile, value_profile, made_strength, draw_strength, round_idx, opponent_model):
    ...
    # MUTATION v124 (option a — slope ~20% lower): reduce boost slope 0.05 -> 0.04
    # to dampen call-widening magnitude when the 'bluff_heavy' label fires.
    # Compounds the v124 crossover (BLUFF_OPPORTUNITY_THRESHOLD restored to
    # 0.55, aligning with the 0.55 baseline below): now the label fires exactly
    # when the ramp begins, so the original 0.05 slope would yield boost =
    # 0.03 + 0.05*(bluff_opp-0.55) at the moment of label activation. Reducing
    # to 0.04 adds a safety margin against over-calling even when the label
    # correctly fires. Effect at typical operating points:
    #   bluff_opp=0.70, conf=0.30: 0.0375*0.30=0.01125 -> 0.036*0.30=0.0108 (-4%)
    #   bluff_opp=0.85, conf=0.50: 0.0525*0.50=0.0263 -> 0.048*0.50=0.0240 (-9%)
    #   bluff_opp=1.00, conf=1.00: 0.0525 -> 0.048 (-8.6%)
    # Cap 0.08 never binds (max boost 0.048 << 0.08), so cap is INERT and
    # slope reduction is the only effective lever. Per experience pool, this
    # is a threshold tweak on the call-widening boost amplitude (NOT a
    # defensive-axis fold gate, NOT a new primitive).
    boost = 0.03 + 0.04 * max(0.0, bluff_opp - 0.55)
    boost *= confidence
