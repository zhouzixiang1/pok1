...
def opponent_bet_size_adjust(opponent_model, spot_info, round_idx):
    """Adjust postflop thresholds based on opponent bet sizing.
    Returns (call_adjust, fold_adjust) deltas.
    Positive call_adj = harder to call; negative = easier to call.
    Positive fold_adj = more likely to fold; negative = less likely."""
    if round_idx <= 0 or not spot_info.get("facing_postflop_aggression", False):
        return 0.0, 0.0
    conf = opponent_model.get("confidence", 0.0)
    if conf < 0.10:
        return 0.0, 0.0
    ratio = spot_info.get("last_raise_pot_ratio", 0.0)
    aggr = opponent_model.get("postflop_aggr", 0.36)
    call_adj = fold_adj = 0.0
    if ratio < 0.40:
        # Small bet: opponent weak/blocking -> call wider, fold less
        call_adj = -0.015 * conf
        fold_adj = -0.020 * conf
        if aggr > 0.45:
            call_adj -= 0.010 * conf
    elif ratio > 0.70:
        # Large bet: polarized (nuts or bluff) -> call tighter, fold more
        call_adj = 0.020 * conf
        fold_adj = 0.015 * conf
    return call_adj, fold_adj
