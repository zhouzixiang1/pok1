...
from state import (
    ...
    estimate_preflop_strength, is_preflop_3bet_candidate,
...
from betting import (
    ...
    big_pot_safety_guard, must_continue_vs_raise, should_fold_postflop,
)
...
def get_action(req, requests):
    ...
    pot = max(1, state["pot"])
    spr = my_chips / max(1, pot) if round_idx > 0 else 99.0
    ...
    # SPR: tighter in big pots, looser in small
    if round_idx > 0:
        _sa = 0.025 if spr < 5 else (-0.015 if spr > 12 else 0.0)
        strong += _sa; medium += _sa * 0.7
    bet_call_adj, bet_fold_adj = opponent_bet_size_adjust(opponent_model, spot_info, round_idx)
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            if pot > 5000 and made_strength < 0.40 and draw_strength < 0.15:
                call_margin += 0.02
            if spr < 5 and made_strength < 0.40 and draw_strength < 0.15:
                call_margin += 0.020
            elif spr > 12 and (made_strength >= 0.25 or draw_strength >= 0.10):
                call_margin -= 0.010
            call_margin += bet_call_adj
            ...
            if spr < 5 and realized_rate < 0.45 and made_strength < 0.40:
                realized_rate -= 0.015
        if anti_lock_pressure:
            call_margin -= 0.07
        ...
        if anti_lock_attack is not None:
            return anti_lock_attack
        if round_idx > 0 and not anti_lock_pressure and should_fold_postflop(made_strength, value_profile, board_texture, spot_info, draw_info, pot_odds, to_call, pot, round_idx) and bet_fold_adj >= -0.01:
            return -1
    ...
    if opp_double_barrel_then_river_check and weak_pair_river:
        return 0
    if bad_river_bluff_candidate or weak_bottom_pair_barrel or weak_pair_after_raise_barrel or bad_river_value_bet or bad_stackoff_overpair:
        return 0
