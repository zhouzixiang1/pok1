...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    ...
    # eliminating river CR stack-offs.
    #
    # REWIRE (v131, PATH A): the ORIGINAL fixed-threshold formula
    # `0.42 - cr_severity` / `0.30 - cr_severity` was INERT vs mid-pair hands
    # because the constant baseline absorbed the small severity signal
    # (e.g. river made=0.45, cr_severity=0.07 -> thr=0.35 < made=0.45 -> no
    # fold). Pot-odds-grounded formula `pot_odds_required + cr_severity`
    # couples the continue threshold to the actual price the pot is laying,
    # so any hand whose made_strength is below the break-even equity
    # (adjusted upward by the trap-line severity) becomes a fold.
    if opponent_model is not None:
        cr_active, cr_severity = check_raise_pressure(spot_info, opponent_model)
        if cr_active:
            size_ratio_cr = spot_info.get('last_raise_pot_ratio', 0.0)
            pot_odds_required_cr = size_ratio_cr / (1.0 + size_ratio_cr) if size_ratio_cr > 0 else 0.0
            continue_threshold_cr = pot_odds_required_cr + cr_severity
            if round_idx >= 2 and made_strength < continue_threshold_cr and not has_draw:
                return True
    ...
    # barreler. Capped by has_draw and not strong-tier upstream; preserves
    # strong one-pair+.
    #
    # REWIRE (v131, PATH A): the ORIGINAL fixed-threshold formula
    # `0.36 - bp_severity` / `0.24 - bp_severity` was INERT because the
    # baseline constants (0.36 river / 0.24 turn) already lay well below
    # typical mid-one-pair made_strength (~0.32-0.40), so the small severity
    # subtraction (0.07-0.14) could never push the threshold above
    # made_strength. Pot-odds-grounded formula `pot_odds_required + bp_severity`
    # couples the continue threshold to the actual bet size, so a 0.5x pot
    # bet demands ~0.33 equity to continue; adding bp_severity lifts the bar
    # above mid-one-pair, converting the leak hands (made ~0.30-0.40 facing
    # frequent barreler) into folds while preserving strong-pair+ calls.
    if opponent_model is not None:
        bp_active, bp_severity = barrel_pressure_profile(spot_info, opponent_model, round_idx)
        if bp_active:
            size_ratio_bp = spot_info.get('last_raise_pot_ratio', 0.0)
            pot_odds_required_bp = size_ratio_bp / (1.0 + size_ratio_bp) if size_ratio_bp > 0 else 0.0
            continue_threshold_bp = pot_odds_required_bp + bp_severity
            if round_idx >= 2 and made_strength < continue_threshold_bp and not has_draw:
                return True
