...
def build_opponent_model(requests, my_id):
    ...
    for req in hand_requests:
        ...
        _reached_river = any(
            isinstance(r, dict) and r.get('round', 0) >= 3
            for r in history
        )
        _prev_hand_was_shove_call = _opp_shoved_this and _we_called_shove and _reached_river
...
def _estimate_bluff_frequency(om):
    ...
    if isinstance(_polarity, dict) and _polarity.get('samples', 0) >= 4:
        ...
        if _rsd_n >= 2:
            ...
            # Shrinkage: sampling bias exists (we only call with decent hands),
            # so cap weight at 0.50 and ramp with sample size.
