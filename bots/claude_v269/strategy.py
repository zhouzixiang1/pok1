...
def choose_raise(
    ...
    # SPR commitment gate: cap marginal-hand (0.50-0.62) raise so projected
    # SPR-if-called stays >= 4. Premium (>0.70) and trash (<0.45) bypass.
    if round_idx == 0 and preflop_strength is not None:
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        hand_cat = classify_preflop_hand(req['my_cards'])
        # Preflop shove/4bet defense — self-gates on opp_allin/opp_pf_raises>=2/raise>=12BB.
        ...
        if hand_cat_sbr == 'premium' or is_aks:
            ...
            return 0
        # Multi-site 4bet/5bet defense dispatch — self-gates; folds dominated hands.
        if _preflop_shove_defense_fold(
                req['my_cards'], preflop_strength, state, spot_info, opponent_model):
            return -1
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        # Fold dominated preflop hands vs all-in before Monte-Carlo overestimates equity.
        ...
        if round_idx >= 2 and line_profile['line_label'] == 'value_heavy':
            if (value_profile is None or value_profile['tier'] not in ('strong', 'nut')) and draw_strength < 0.18 and not anti_lock_jam_continue:
                return -1
        # SPR commitment gate — relocated from dead to_call>=my_chips block.
        # Folds marginal hands (made<0.55, thin/none) facing all-in when
        # pot_odds > win_rate+0.03; preserves draws per street.
        if _spr_commitment_gate(round_idx, my_chips, made_strength, value_profile,
            ...
            print(f"SPR_FOLD round={round_idx} made={made_strength:.2f} "
                  ...
                  file=sys.stderr)
            return -1
        # River weak-made-hand all-in gate: catches 0.55-0.60 band SPR gate misses.
    ...
    # _river_stackoff_guard relocated before to_call>=my_chips block (was a
    # placement shadow); restricted to 0<to_call<my_chips (non-all-in range).
    ...
    if to_call > 0:
        # Aggro-archetype bluff-catcher fold: kills -20k turn/river call-downs
        ...
        # Multi-barrel line-evidence fold: keys on direct in-hand history (opp bet
        # prior street AND bets this), not classify_archetype.
        if round_idx >= 2 and _multibarrel_line_fold(
                opponent_model, spot_info, round_idx, to_call, pot,
                made_strength, value_profile, draw_strength):
            return -1

        # Direct river pot-odds-vs-equity fold: margin-additive dispatch is INERT
        # vs polarized ranges; fold when calibrated_equity (made*discount) < pot_odds.
        if round_idx == 3:
