...
def _estimate_bluff_frequency(om):
    ...
    Signals read (with smooth_rate priors from build_opponent_model):
      ...
      - v269: betsize_polarity underbettor floor REMOVED (BANNED/EXHAUSTED axis
        per experience pool); the structural polarity signal is still recorded
        and consumed by _river_potodds_equity_margin directly.
    """
    if om is None:
        return 0.25
    conf = om.get('confidence', 0.0)
    ...
    # v269 MUTATION (option-c): REMOVED the underbettor betsize_polarity floor
    # that v234 carried unconditionally. The experience pool marks this exact
    # axis as BANNED/EXHAUSTED ("Fold-side GENERIC nudges BANNED (closed,
    # exhausted axis -- do not reopen): _estimate_bluff_frequency underbettor
    # floors ... FORBADE & dead") and RECENT_LESSONS (v254) warns that
    # "unconditional gates turn -EV fast" -- v234's version fired for EVERY
    # underbettor read with no VPIP/pfr conditioning, risking over-call vs
    # value-heavy underbettors. The structural _opp_betsize_polarity axis is
    # preserved (still recorded + exposed in opponent_model); only this dead
    # fold-side floor is removed.
...
if __name__ == '__main__':
    ...
    # v269 MUTATION: _estimate_bluff_frequency betsize_polarity floor REMOVED.
    # The underbettor floor (v195/v234) is BANNED/EXHAUSTED per the experience
    # pool. These tests now assert the floor is GONE: underbettor polarity no
    # longer changes bluff_freq vs a standard opponent.
    ...
    # Test 2: underbettor with confidence >= 0.25 -> NO LONGER floored (v269).
    ...
    assert abs(_ebf_under - _ebf_std) < 0.01, (
        'v269 underbettor floor REMOVED: must equal standard, '
        'got %.4f vs std %.4f' % (_ebf_under, _ebf_std))
    ...
    assert abs(_ebf_low - _ebf_std) < 0.01, (
        'low-confidence underbettor must NOT floor (unchanged), '
        'got %.4f vs std %.4f' % (_ebf_low, _ebf_std))
    # Test 4: overbettor polarity -> NOT floored (unchanged).
    ...
    assert abs(_ebf_over - _ebf_std) < 0.01, (
        'overbettor must NOT trigger floor, '
        'got %.4f vs std %.4f' % (_ebf_over, _ebf_std))
    print('_estimate_bluff_frequency v269 (floor removed) PASS: '
