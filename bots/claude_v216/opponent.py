...
def _estimate_bluff_frequency(om):
    ...
    # v216 MUTATION (crossover v193 x v204): VPIP-aware bluff boost.
    # Experience pool directive: "vs WIDE open-shovers (VPIP>0.60) -> fold
    # LESS". Wide-loose opponents carry far more bluffs/semi-bluffs in their
    # betting and shoving range than archetype alone captures. Linear ramp
    # VPIP 0.55->0.75 adds up to +0.15 to bluff_freq, raising calibrated_equity
    # (made*(0.40+bluff_freq)) at the margin so top-pair/two-pair hands fold
    # less vs mixed-aggression opponents. Addresses v204's over-fold weakness
    # (v204 lost to v211 WR=0.43, v206 WR=0.35 where v193 won WR=0.62/0.57 by
    # calling down more). Standard-tight opponents (VPIP<=0.55) unaffected.
    vpip = om.get('vpip', 0.58)
    base += max(0.0, (vpip - 0.55) / 0.20) * 0.15
