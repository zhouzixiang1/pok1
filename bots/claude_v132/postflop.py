...
def nutted_risk_profile(hole_cards, public_cards, pair_profile=None, board_texture=None, value_profile=None, paired_board_profile=None):
    ...
    if hand_class == 6:
        ...
        if score[1] < max(board_ranks):
            risk += 0.02
    # Crossover from v130 (imported from v128 lineage): Flush-board vulnerability
    # for two-pair/set — opponent may have a flush. Closes the G0H28/G4H12 leak
    # where two-pair on a 4-flush board called all-in (-19999) against a made
    # flush. The existing paired-board logic below only adds risk when the
    # board is PAIRED; this extends vulnerability detection to FLUSH boards.
    # Placed BEFORE the paired-board branch (mutually exclusive elif chain) so
    # the larger flush risk dominates when the board is BOTH paired AND 4-flush.
    # Genuinely orthogonal to the exhausted defensive fold-gate axis: this is a
    # POSTFLOP_RISK assessment feeding value_profile, not a fold gate.
