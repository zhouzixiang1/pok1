...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    ...
    if opponent_model is not None and opponent_model.get('confidence', 0) >= 0.15:
        ...
        if opp_value_heavy:
            # MUTATION (option b — heuristic from experience_pool.md):
            # Apply the v131 INERTNESS RESCUE PATTERN to this gate. The
            # original v118 fixed thresholds `0.28`/`0.34` are uncoupled from
            # pot odds — they fire identically regardless of whether the
            # opponent bets 0.3x pot or 1.5x pot. Rewire to
            # `pot_odds_required + severity` so the continue threshold couples
            # to the actual price the pot is laying. For a 0.5x pot bet the
            # old threshold 0.28 becomes ~0.37 (pot_odds 0.33 + sev 0.04);
            # for a 1.0x pot bet it becomes ~0.54, catching mid-one-pair
            # hands that over-call vs value-heavy barrelers (v114/v110/v102
            # all beat v118 with H2H <=48% across 110-170g). Severity 0.04 is
            # a small uplift mirroring the check_raise_pressure lower bound;
            # raised to 0.06 on the river to reflect tighter value shading.
            # The size_bucket gating is preserved to keep small-bet cases
            # conservative (no fold to probe-sized bets).
            if round_idx >= 2 and not has_draw and size_bucket in ('medium', 'large'):
                size_ratio_v132 = spot_info.get('last_raise_pot_ratio', 0.0)
                pot_odds_req_v132 = size_ratio_v132 / (1.0 + size_ratio_v132) if size_ratio_v132 > 0 else 0.0
                sev_v132 = 0.06 if round_idx == 3 else 0.04
                continue_thr_v132 = pot_odds_req_v132 + sev_v132
                if made_strength < continue_thr_v132:
                    return True
            if round_idx == 3 and opp_bets >= 2 and not has_draw:
                size_ratio_mb = spot_info.get('last_raise_pot_ratio', 0.0)
                pot_odds_req_mb = size_ratio_mb / (1.0 + size_ratio_mb) if size_ratio_mb > 0 else 0.0
                if made_strength < pot_odds_req_mb + 0.06:
                    return True
