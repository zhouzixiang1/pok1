...
def _bluff_allowed(context, equity):
    ...
    if street == "river":
        # River air-bluff gate.  The parent hard-banned the river, so every
        # showdown-valueless rivered holding was forced into fold/pass.  The
        # frozen opponent concedes a strong share of the raises it faces, so
        # authorize a bounded, deterministic-subset bluff only on true air and
        # only when the memory-backed terminal fold-to-raise posterior, weighted
        # by its adaptation weight, clears a fold-equity floor.  Unknown
        # opponents stay neutral: the prior plus a low adaptation weight keep the
        # weighted term below the floor until terminal evidence accumulates.
        cards = context.get("cards", {}) or {}
        hole = _card_ids(cards.get("hole", ()))
        board = _card_ids(cards.get("board", ()))
        if (
            ...
            return False
        # Reuse the existing tier helper.  It is already on the live decision
        # path through _strategy_adjustments, so this introduces no new
        # evaluator call.  Only high-card ("air") holdings are eligible.
        if _made_hand_tier(hole, board) != "air":
            return False
        pot = max(1.0, _number(betting.get("pot"), 1.0))
        to_call = _number(betting.get("to_call"), 0.0)
        # Bound the bluff commitment to a small pot fraction; larger facing
        ...
        # Fold-equity floor: fr*w >= 0.20 gates the exploit on confidence.
        if fold_to_raise * adaptation_weight < 0.20:
            return False
        # A distinct salt decorrelates the river subset from the flop/turn
        # semi-bluff subset; the low frequency keeps the air bluff bounded.
        return _deterministic_mix(context, 0x51B0FF) < 0.25
...
def _decision_from_equity(context, equity, confidence, samples, return_margin=False):
    ...
    # Fold-equity admission (check: terminal_response_adaptation, contract
    # 3a77c1c070ecb8366a9da35bc3fea63e27671fc06587990b1d9fef5cc5ef84ce).
    # The reducer-owned opponent.terminal_response posterior alone admits the
    # jam when its confidence-weighted fold rate clears the breakeven for the
    # exact matched jam risk.  The posterior already blends fold_to_jam toward
    # the 0.28 prior under the capped adaptation weight (memory=opponent
    # snapshot from the match OpponentTracker, confidence-capped at
    # MAX_ADAPTATION_WEIGHT), so thin match memory stays near the prior and
    # cannot clear a ~0.50 breakeven.  Card strength keeps its own disjunct
    # unchanged: this adds an evidence-gated exploit rather than re-tuning the
    # equity/SPR thresholds.  Reads only this decision_context; no file,
    # network, or history work, and no change to the raise_to sizing path.
    jam_breakeven = jam_commitments["hero_extra"] / max(
        1.0, jam_commitments["contest_pot"] + jam_commitments["hero_extra"]
    )
    if (
        "allin" in kinds
        and hero_stack > 0
        and (
            (safe_equity >= 0.62 and (spr <= 2.5 or safe_equity >= 0.78))
            or jam_fold_rate >= jam_breakeven
        )
    ):
