...
def _opponent_sizing_raise_boost(opponent_model, round_idx):
    """v171 NEW OFFENSE: DOWN-sizing delta vs calling-stations on flop/turn.

    COMPLEMENTARY inverse of _turn_probe_sizing (UP vs weak) and
    _street_fold_exploit_sizing_boost (UP vs folders). This DOWN-sizes
    vs calling-stations to keep pots moderate -- attacking -20k stack-offs
    at the pot-CONSTRUCTION stage (not the forbidden river fold stage).

    M5/M6: continuous (no deadzone), OR-gate (not compound AND),
    unconditional telemetry at function scope, fires at live-pool defaults.
    """
    delta = 0.0
    reason = 'not_eligible'
    vmi = 0.0
...
def _turn_oop_pot_control(
    ...
    stop being geometric commitments. Distinct from _river_stackoff_guard
    (fires AT the all-in call). Orthogonal to choose_raise() sizing knobs.
    """
    # 1. Hard eligibility gates
    if round_idx != 2 or to_call != 0:
        return None
    # Heads-up contract: dealer_id=SB, bb=1-dealer_id; BB acts first postflop
    # (OOP). OOP on turn = BB via my_is_bb, NOT has_position (= my_id==sb, IP).
...
if __name__ == "__main__":
    ...
    base_spot = {"my_is_bb": True, "has_position": False}
    ip_spot = {"my_is_bb": False, "has_position": True}
    ...
    # v170 _turn_probe_sizing self-test: verifies LIVE POOL DEFAULTS produce a
    # non-zero continuous delta (M5 INERTNESS HARD RULE: no deadzone gap once
    # scenario gates pass), and that non-eligible scenarios return 0.0.
    ...
    _tp_delta = _turn_probe_sizing(2, 0, _test_tp_spot, _test_tp_model,
                                   0.45, _test_tp_vp, _test_tp_bt, 0.05)
    ...
    # Non-eligible: not turn (flop)
    assert _turn_probe_sizing(1, 0, _test_tp_spot, _test_tp_model,
                              0.45, _test_tp_vp, _test_tp_bt, 0.05) == 0.0
    # Non-eligible: to_call > 0
    assert _turn_probe_sizing(2, 500, _test_tp_spot, _test_tp_model,
                              0.45, _test_tp_vp, _test_tp_bt, 0.05) == 0.0
    # Non-eligible: opponent strong (no weakness)
    _tp_strong_opp = {'confidence': 0.30, 'flop_aggr': 0.60}
    assert _turn_probe_sizing(2, 0, {'last_opp_action_type': 'raise'},
                              _tp_strong_opp, 0.45, _test_tp_vp,
                              _test_tp_bt, 0.05) == 0.0
    print('TURN_PROBE_SIZING self-test PASS: delta=%.4f' % _tp_delta)

    # v171 _opponent_sizing_raise_boost self-test: M5 compliant (non-zero
    # at live-pool defaults).
    _test_st_model_def = {'confidence': 0.15, 'value_maximizer_index': 0.43,
                          'fold_to_bet_turn': 0.44, 'fold_to_bet_flop': 0.44}
    ...
    _test_st_model_station = {'confidence': 0.30, 'value_maximizer_index': 0.65,
                              'fold_to_bet_turn': 0.28, 'fold_to_bet_flop': 0.28}
    ...
    _test_st_model_folder = {'confidence': 0.30, 'value_maximizer_index': 0.30,
                             'fold_to_bet_turn': 0.60, 'fold_to_bet_flop': 0.60}
