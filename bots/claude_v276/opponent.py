...
def build_opponent_model(requests, my_id):
    ...
    # v249 STRUCTURAL: per-street mean bet-to-pot ratio (continuous signal).
    # Derived from the SAME per_street_sizing_samples that feed
    # classify_sizing_tendency, but as a continuous mean instead of categorical
    # buckets. Consumed by _tier_opp_sizing_directive in postflop.py.
    per_street_mean_bet_ratio = {}
    for _ridx in (1, 2, 3):
        _street = [r for ri, r in per_street_sizing_samples
                   if ri == _ridx and r == r and 0.05 < r < 5.0]
        per_street_mean_bet_ratio[_ridx] = sum(_street) / len(_street) if _street else 0.50
    ...
    return {
        ...
        "per_street_mean_bet_ratio": per_street_mean_bet_ratio,
    }
...
def _multibarrel_line_fold(om, spot_info, round_idx, to_call, pot,
    ...
    ftb = om.get('fold_to_bet')
    if ftb is None:
        # v276 REGRESSION-REPAIR (0W-1L vs v274, native TCP 70-hand): the prior
        # repair DROPPED a lone river-only fold_to_bet reading, which over-folded
        # vs loose callers (ftb < 0.35 => carve-out should fire but couldn't) —
        # the exact matchup v274 won. v274 trusts every available street reading
        # via min(); the over-correction here regressed. Restore river-only trust
        # so the loose-caller carve-out fires on river data alone; both streets
        # present => min() corroboration; turn-only => trust; river-only =>
        # trust; neither => None.
        _turn_ftb = om.get('fold_to_bet_turn')
        _river_ftb = om.get('fold_to_bet_river')
        if _turn_ftb is not None and _river_ftb is not None:
            ftb = min(_turn_ftb, _river_ftb)
        elif _turn_ftb is not None:
            ftb = _turn_ftb
        elif _river_ftb is not None:
            ftb = _river_ftb
    # v276 REGRESSION-REPAIR (0W-1L vs v274, native TCP 70-hand): prior
    # attempts added a confidence floor (0.25, then 0.15) to screen thin-
    # sample noise on the loose-caller carve-out. But the floor BLOCKED the
    # carve-out through the early-mid phase of the 70-hand match
    # (confidence = clamp((total_actions-5)/35.0); 0.15 needs ~11 postflop
    # actions), forcing weak-bluff-catcher folds vs sticky opponents — the
    # exact matchup v274 (no gate) won. The per-street branching above is
    # structurally explicit but provably identical to v274's min() across
    # all 5 cases (both/turn-only/river-only/neither), so the floor was the
    # SOLE behavioral delta vs v274. A lone low fold_to_bet (<0.35) is
    # itself the most reliable stickiness signal on a thin 70-hand sample;
    # layering a sample-count gate on top screens the very signal it must
    # trust. The _allin_polarized_equity_fold gate (restored in v274)
    # already handles the polarized-value-barrel case, bounding the
    # carve-out's false-positive cost. Restore v274's ungated condition.
...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
    ...
    if made_strength < 0.20 or made_strength >= 0.50:
        return 0.0
    # 5. Tier exemption (v274 MUTATION — v204 graduated exemption, ported from
    #    v235). The blanket tier ('strong','nut') exemption made the gate DEAD
    #    for top-pair-JK+ hands: value_hand_tier labels TPTK as 'strong' for
    #    value betting, but calibrated_equity 0.22*0.65=0.143 << pot_odds~0.33
    #    vs a polarized value-betting range. The exemption is now GRADUATED:
    #    nut + top two-pair+ (made>=0.45) + overpair AA/KK stay exempt, while
    #    bottom two-pair (made=0.40) and top-pair-JK become foldable. Targets
    #    the documented over-call leak vs elite value-heavy bots (v242/v243).
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return 0.0
        if _tier == 'strong':
            if made_strength >= 0.45:
                return 0.0  # top two-pair+/trips-bottom exempt
...
def _allin_polarized_equity_fold(round_idx, made_strength, draw_strength,
    ...
    """v274 CROSSOVER (ported from v235/v202): Calibrated-equity fold for
    marginal one-pair facing all-in.
    ...
    v274 rationale: v254 dropped this gate (relied on _spr_commitment_gate +
    _river_weak_made_hand_gate), but H2H data shows v254 LOSES to elite
    value-heavy bots (v242 0.461, v243 0.388, v240 0.412) where v235
    (which kept this gate) WINS (v242 0.533, v243 0.533, v240 0.495).
    Restoring this calibrated polarized-all-in fold closes the documented
    -20k over-call leak vs polarized value ranges (exp-pool v269 hint).
    """
    ...
    if value_profile is not None:
        ...
        if _tier == 'strong':
            if made_strength >= 0.45:
                return False  # top two-pair+/trips-bottom exempt
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
    ...
    if opponent_model is not None:
        ...
        if (sizing is not None
                ...
                and sizing.get('tendency') == 'overbettor'):
            return False
    # v276 REGRESSION-REPAIR (0W-1L vs v274, native TCP 70-hand): no shove-
    # density blend or carve-out here. Prior attempts wired a small-sample
    # revealed_shove_density consumer into calibrated_equity; it misfired on
    # thin 70-hand samples (binomial FP ~31% at n=4) and the tracking itself
    # had no live consumer, so both were removed. Reopening needs a NEW
    # independent mechanism + >=30g paired net-chips H2H evidence.
    bluff_freq = _estimate_bluff_frequency(opponent_model)
    discount = 0.40 + bluff_freq
    calibrated_equity = made_strength * discount
...
def analyze_current_spot(req, state):
    ...
    # Authoritative heads-up contract: dealer_id is the small blind, and the
    # big blind is 1 - dealer_id. SB acts first preflop; BB acts first postflop
    # (out of position). SB/dealer is in position postflop.
    sb = dealer_id
    bb = 1 - dealer_id
    history = req["history"]
    ...
    info = {
        "my_is_sb": my_id == sb,
        "my_is_bb": my_id == bb,
        # SB/dealer is in position postflop; BB is out of position.
        "has_position": my_id == sb,
...
if __name__ == '__main__':
    ...
    # v274 graduated exemption: strong tier made>=0.45 (top two-pair+/trips) exempt...
    assert _river_potodds_equity_margin(3, 0.45, 0.05, _pe_vp_strong, _pe_spot_fire, 0.33, None) == 0.0, 'v274: strong tier made>=0.45 (top two-pair+) exempt'
    # ...and strong tier with overpair AA/KK exempt...
    _pe_pp_overpair = {'pair_type': 'overpair'}
    assert _river_potodds_equity_margin(3, 0.40, 0.05, _pe_vp_strong, _pe_spot_fire, 0.33, None, _pe_pp_overpair) == 0.0, 'v274: strong overpair AA/KK exempt (graduated)'
    # ...but strong tier made<0.45 WITHOUT overpair (TPTK) is foldable -> fires.
    _pe_delta_tpjk = _river_potodds_equity_margin(3, 0.40, 0.05, _pe_vp_strong, _pe_spot_fire, 0.45, None)
    assert _pe_delta_tpjk > 0.0, ('v274: strong tier made<0.45 w/o overpair (TPTK) must fire, got %.4f' % _pe_delta_tpjk)

    # v274 CROSSOVER self-test: _allin_polarized_equity_fold graduated exemption.
    _pe_pp_tp = {'pair_type': 'top_pair', 'weak_kicker': False}
    _pe_pp_op = {'pair_type': 'overpair'}
    assert _allin_polarized_equity_fold(3, 0.24, 0.05, _pe_vp_strong, 0.48, None, False, _pe_pp_tp), \
        'v274: top-pair-JK strong tier must NOT be exempt from all-in fold gate'
    assert not _allin_polarized_equity_fold(3, 0.24, 0.05, _pe_vp_strong, 0.48, None, False, _pe_pp_op), \
        'v274: overpair strong tier must remain exempt (AA/KK protected)'
    # two-pair (made=0.40) foldable vs polarized all-in
    assert _allin_polarized_equity_fold(3, 0.40, 0.05, _pe_vp_strong, 0.48, None, False, _pe_pp_tp), \
        'v274: two-pair must fold vs polarized all-in (cal_eq=0.26 << 0.48)'
    # trips+ (made>=0.50) stays exempt via made>=0.50 early-return
    assert not _allin_polarized_equity_fold(3, 0.55, 0.05, _pe_vp_strong, 0.48, None, False, _pe_pp_tp), \
        'v274: trips+ stays exempt (made>=0.50)'
    # nut tier exempt
    assert not _allin_polarized_equity_fold(3, 0.30, 0.05, {'tier': 'nut'}, 0.48, None, False, None), \
        'v274: nut tier exempt from all-in fold gate'
    # strong tier made>=0.45 exempt (top two-pair+)
    assert not _allin_polarized_equity_fold(3, 0.45, 0.05, _pe_vp_strong, 0.48, None, False, None), \
        'v274: strong tier made>=0.45 exempt (top two-pair+/trips-bottom)'
    # anti_lock_pressure disables the gate
    assert not _allin_polarized_equity_fold(3, 0.24, 0.05, _pe_vp_strong, 0.48, None, True, _pe_pp_tp), \
        'v274: anti_lock_pressure disables all-in fold gate'
    print('v274 CROSSOVER _allin_polarized_equity_fold self-test PASS')
    ...
    # v276: include realistic 'confidence' (always populated by build_opponent_model)
    # so the fixture mirrors real OM shape.
    _mb_om_sticky = {'large_bet_ratio': 0.65, 'postflop_aggr': 0.20,
                     'fold_to_bet': 0.20, 'confidence': 0.6}
    ...
    # v276: include realistic 'confidence' (always populated by build_opponent_model)
    # so the fixture mirrors real OM shape.
    _mb_om_loose = {'fold_to_bet': 0.20, 'large_bet_ratio': 0.2,
                    'postflop_aggr': 0.6, 'confidence': 0.6}
