...
def _bb_3bet_polarization(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype):
    """Polarized 3-bet from BB vs raise: premium / thin-value / polar-bluff tiers."""
    ...
    # ── Tier 1: Premium value 3-bet (>= 0.60) ──
    # AA, KK, QQ, JJ, AK — unchanged from current logic
    if preflop_strength >= 0.60 and not is_preflop_trash_hand(req['my_cards'], preflop_strength):
    ...
    # ── Tier 2: Thin value 3-bet (0.50 - 0.60) ──
    # AQ, AJs, KQs, 99, 88 — only vs confirmed wide opponents
    if 0.50 <= preflop_strength < 0.60:
        wide_opp = (
            (confidence >= 0.20 and opp_pfr >= 0.40)
            or (confidence >= 0.25 and opp_pfr >= 0.32 and fold_to_raise >= 0.48)
            or (opp_archetype == 'lag')
        )
        if wide_opp and opp_archetype != 'calling_station':
            raise_amount = choose_raise(
                state['min_raise_action'], my_chips, state['my_round_bet'],
                to_call, state['pot'], max(win_rate, preflop_strength),
                0, 'bb_vs_raise', preflop_strength,
                True, opponent_model,
                match_sizing_delta=match_profile['sizing_delta'] - 0.05,
            )
            if raise_amount is not None:
                return raise_amount

    # ── Tier 3: Polar bluff 3-bet (0.34 - 0.50) ──
    # Opponent-model-aware frequency replacing fixed 25%
    can_bluff = opp_archetype not in ('calling_station', 'unknown')
    if can_bluff and 0.34 <= preflop_strength <= 0.50:
        base_freq = 0.22
        if opp_archetype == 'nit':
            ...
            base_freq = 0.15
        if confidence >= 0.20:
            base_freq += confidence * max(0.0, fold_to_raise - 0.44) * 0.8
        base_freq = clamp(base_freq, 0.10, 0.40)
        bluff_roll = (hash(tuple(req['my_cards'])) % 100) / 100.0
        if bluff_roll < base_freq:
            raise_amount = choose_raise(
                state['min_raise_action'], my_chips, state['my_round_bet'],
                to_call, state['pot'], max(win_rate, preflop_strength),
                0, 'bb_vs_raise', preflop_strength,
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        return _bb_3bet_polarization(
            req, state, spot_info, opponent_model,
            preflop_strength, win_rate, match_profile, opp_archetype,
        )
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        # Strong/nut hands auto-call all-in — never fold sets, flushes, two pair to shoves
    ...
    if to_call >= my_chips:
        ...
        # Strong/nut hands auto-call effective shove
        if value_profile is not None and value_profile.get('tier') in ('strong', 'nut'):
            return -2
        if hard_repressure_fold or paired_board_stackoff["severe"]:
    ...
    if to_call > 0:
        ...
        if should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class=street_texture["class"], opp_archetype=opp_archetype, pot_odds=pot_odds):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            if not anti_lock_call_continue and not strong_made_continue:
