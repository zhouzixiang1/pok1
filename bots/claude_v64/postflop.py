...
def river_showdown_extraction(made_strength, value_profile, opponent_model, board_texture, round_idx, to_call, opp_archetype='unknown'):
    """Identify river spots where a value bet extracts from weaker holdings.

    Returns sizing ratio or 0.0 if not applicable.
    - Strong tier (made_strength ≥ 0.50): calibrated 0.50-0.75x pot
    - Medium tier (0.35-0.62, non-strong): thin value 0.20-0.40x pot
    Nut hands return 0.0 (handled by overbet/induce paths in strategy.py).
    """
    ...
    if value_profile is None or value_profile.get('tier') == 'none':
        return 0.0
    # Nut hands use overbet/induce paths in strategy.py
    if value_profile.get('tier') == 'nut':
        return 0.0

    # Strong hands: calibrated river value sizing (0.50-0.75x pot)
    if value_profile.get('tier') == 'strong' and made_strength >= 0.50:
        base_ratio = 0.55 + 0.15 * min(1.0, (made_strength - 0.50) / 0.20)
        # Opponent calling tendency
        confidence = opponent_model.get('confidence', 0.0)
        if confidence >= 0.20:
            call_freq = 1.0 - opponent_model.get('fold_to_raise', 0.44)
            if call_freq >= 0.60:
                base_ratio += 0.05  # call-happy opponent → bet bigger
        ...
        if board_texture is not None:
            ...
            if board_texture.get('paired', False):
                base_ratio += 0.03
        # Nit opponent: less value (they fold everything except monsters)
        if opp_archetype == 'nit':
            base_ratio -= 0.08
        # Calling station: max value (they call with worse)
        elif opp_archetype == 'calling_station':
            base_ratio += 0.05
        return clamp(base_ratio, 0.50, 0.75)

    # Don't bet air or strong/nut already handled above
    if value_profile.get('tier') == 'strong':
        return 0.0
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opp_archetype='unknown', pot_odds=0.0):
    if round_idx <= 0:
        return False
    ...
    # Pot-odds gate: if getting good odds, don't fold regardless of hand weakness
    if pot_odds > 0 and eff_made >= pot_odds - 0.08:
        return False  # getting correct odds to continue
    ...
    if opp_postflop_bets >= 2 and not has_draw and tier not in ('strong', 'nut'):
        ...
        if round_idx == 3 and eff_made < 0.38 and size_bucket in ('medium', 'large'):
            return True
    # Ultra-weak fold: any bet size on turn/river with near-zero showdown
    # Current branches only cover medium/large sizing — this closes the small-bet gap
    if round_idx >= 2 and eff_made < 0.15 and not has_draw:
        return True
    # Turn small-bet fold: weak hands (below medium-pair strength) vs any sizing
    if round_idx == 2 and eff_made < 0.22 and not has_draw and tier not in ('strong', 'nut'):
        return True
    return False
