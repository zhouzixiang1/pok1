...
from opponent import build_opponent_model, analyze_current_spot, build_action_sequence_profile
...
def preflop_call_adjustment(opponent_model, preflop_strength, is_bb=True):
    """Tighten preflop calling range based on opponent profile.

    Tight opponents (low PFR) have stronger ranges — fold more marginal hands.
    Loose opponents (high PFR) have wider ranges — call more.
    Returns adjustment to ADD to the calling threshold (positive = tighter).
    """
    confidence = opponent_model.get('confidence', 0.0)
    if confidence < 0.10:
        return 0.0  # No data, use default

    opp_pfr = opponent_model.get('pfr', PRIOR_PFR)
    opp_vpip = opponent_model.get('vpip', PRIOR_VPIP)

    adjustment = 0.0

    # Tight raiser: their range is strong, fold more
    if opp_pfr < 0.20:
        adjustment += confidence * (0.20 - opp_pfr) * 0.30
    elif opp_pfr < 0.28:
        adjustment += confidence * (0.28 - opp_pfr) * 0.15

    # Loose raiser: their range is wide, call more
    elif opp_pfr > 0.38:
        adjustment -= confidence * (opp_pfr - 0.38) * 0.12

    # Very tight player overall: even more cautious
    if opp_vpip < 0.45:
        adjustment += confidence * (0.45 - opp_vpip) * 0.08

    # Marginal hands (0.37-0.45) get the most adjustment
    if 0.37 <= preflop_strength <= 0.45:
        adjustment *= 1.5
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        iso_threshold -= confidence * max(0.0, opponent_model["vpip"] - 0.58) * 0.04
        ...
        if preflop_strength >= BB_VALUE_3BET_THRESHOLD and not trash_hand:
            raise_amount = choose_raise(
        ...
        if BB_BLUFF_3BET_LOW <= preflop_strength <= BB_BLUFF_3BET_HIGH and not trash_hand:
            hand_idx = req.get("hand", 0)
            bluff_roll = ((hash(tuple(req['my_cards'])) * 31 + hash((hand_idx, my_chips))) % 100) / 100.0
            if bluff_roll < BB_BLUFF_3BET_FREQ:
                raise_amount = choose_raise(
                    state['min_raise_action'], my_chips, state['my_round_bet'],
                    to_call, state['pot'], max(win_rate, preflop_strength),
                    0, 'bb_vs_raise', preflop_strength,
                ...
                if raise_amount is not None:
                    return raise_amount
        # Call with playable hands — filter out structurally weak hands
        _profile = preflop_hand_profile(req["my_cards"])
        _gap = _profile["high"] - _profile["low"]
        _unsuited_disconnected = not _profile["suited"] and _gap >= 4 and _profile["high"] <= 11
        _call_threshold = BB_CALL_THRESHOLD + preflop_call_adjustment(opponent_model, preflop_strength)
        if preflop_strength >= _call_threshold and not _unsuited_disconnected:
            return 0
        if win_rate >= pot_odds_pf - 0.02 and not _unsuited_disconnected:
            return 0
        return -1
...
# ── v50 Bluff-Catch Detection ────────────────────────────────────────────────
# Detects when opponent's aggression pattern suggests bluffing rather than value.
# Counter-exploits high barrel-frequency opponents (v30, v21, v32).

def detect_bluff_catch_signal(action_sequence, opponent_model, made_strength, spot_info, board_texture, round_idx):
    """Detect when opponent aggression pattern suggests bluffing.
    Returns (should_bluff_catch: bool, signal_strength: float)."""
    if round_idx < 2 or not spot_info.get('facing_postflop_aggression'):
        return False, 0.0
    if made_strength < 0.28 or made_strength >= 0.55:
        return False, 0.0
    ...
    # High barrel frequency: opponent barrels often → more bluffs in range
    barrel = opponent_model.get('barrel_freq', 0.40)
    if barrel >= 0.55:
        signal += 0.30 * min(1.0, (barrel - 0.55) / 0.20)

    # Triple barrel: 3 consecutive aggression streets is polarized
    if action_sequence.get('is_triple_barrel'):
        signal += 0.25
    elif action_sequence.get('is_double_barrel'):
        signal += 0.15

    # Sizing escalation: small flop bet → large later bet = bluff polarization
    flop_raise_bb = opponent_model.get('avg_flop_raise_bb', 3.0)
    river_raise_bb = opponent_model.get('avg_river_raise_bb', 5.5)
    turn_raise_bb = opponent_model.get('avg_turn_raise_bb', 4.5)
    if round_idx == 3 and river_raise_bb >= 2.0 * flop_raise_bb:
        signal += 0.20
    elif round_idx == 2 and turn_raise_bb >= 1.8 * flop_raise_bb:
        signal += 0.15

    # Dry board texture: fewer value combos → more bluffs
    if board_texture is not None:
        if not board_texture.get('dynamic', False) and board_texture.get('wetness', 1.0) <= 0.25:
            signal += 0.15
        elif board_texture.get('paired'):
            signal += 0.10

    # Opponent aggression intensity
    aggr_intensity = action_sequence.get('aggression_intensity', 0.0)
    if aggr_intensity >= 0.80:
        signal += 0.10

    should_catch = signal >= 0.45 and confidence >= 0.15
    return should_catch, min(1.0, signal)
...
def get_action(req, requests):
    ...
    action_sequence = build_action_sequence_profile(req, state, my_id)
    ...
    if to_call > 0:
        ...
        # ── v51 Bluff-catch override: gated on confidence + hand floor ──
        if round_idx >= 2 and to_call > 0 and not state['opponent_allin'] and not anti_lock_pressure:
            _bc_tier = value_profile.get('tier', 'none') if value_profile else 'none'
            if _bc_tier not in ('strong', 'nut') and draw_strength < 0.14:
                _bc_should, _bc_conf = detect_bluff_catch_signal(
                    action_sequence, opponent_model, made_strength,
                    spot_info, board_texture, round_idx)
                if _bc_should and _bc_conf >= 0.60 and made_strength >= 0.38:
                    return 0  # bluff-catch: only vs confirmed barreling archetypes

        # ── Inline river equity fold: bypass should_fold_postflop (0% fold rate) ──
        # Uses made_strength (absolute hand metric) instead of simulation win_rate
        # because simulation overestimates equity for weak hands vs wide ranges.
        if round_idx == 3 and to_call > 0 and not state["opponent_allin"]:
            _rv_tier = value_profile.get("tier", "none") if value_profile else "none"
            _rv_has_draw = draw_strength >= 0.14
            if _rv_tier not in ("strong", "nut") and not _rv_has_draw:
                _rv_jam_odds = to_call / (pot + to_call)
                _rv_safety = 0.08
                if not spot_info["has_position"]:
                    _rv_safety += 0.04
                _rv_size = bet_size_bucket(spot_info["last_raise_pot_ratio"])
                if _rv_size in ("medium", "large"):
                    _rv_safety += 0.06
                if spot_info.get("opp_current_round_bet_count", 0) >= 2:
                    _rv_safety += 0.04
                if not anti_lock_pressure and made_strength < _rv_jam_odds + _rv_safety:
                    return -1

        # ── Opponent action-sequence river fold ──────────────────────────────
        # Structural alternative to threshold tuning: use opponent's betting
        # pattern across streets to make exploitively correct river folds.
        if round_idx == 3 and to_call > 0 and not state['opponent_allin'] and not anti_lock_pressure:
            _as_tier = value_profile.get('tier', 'none') if value_profile else 'none'
            if _as_tier not in ('strong', 'nut') and draw_strength < 0.14:
                # Triple barrel from opponent: very credible, fold weak hands
                if action_sequence['is_triple_barrel'] and made_strength < 0.50:
                    return -1
                # Double barrel + large river bet: credible aggression
                if (action_sequence['is_double_barrel']
                    and bet_size_bucket(spot_info['last_raise_pot_ratio']) in ('medium', 'large')
        ...
        if realized_rate < call_threshold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # [v51] Smarter repeated-raise-trap: fold weak+medium hands vs aggression
        if repeated_raise_trap and (value_profile is None or value_profile["tier"] != "nut"):
            ...
            if made_strength < 0.42 and draw_strength < 0.14 and trap_size in ("medium", "large"):
                return -1
            if trap_size == "large" and made_strength < 0.50 and draw_strength < 0.14:
                return -1
            return 0
