...
def reconstruct_state(req):
    ...
    for record in req["history"]:
        ...
        if action_type in ("call", "check"):
            ...
            target = record.get("stage_bet", action)
            try:
                target = int(target)
            except (TypeError, ValueError):
                target = int(action)
            target = max(round_contrib[pid], target)
            add = max(0, min(target - round_contrib[pid], stacks[pid]))
            stacks[pid] -= add
            committed[pid] += add
            round_contrib[pid] += add
            round_bet = max(round_bet, round_contrib[pid])
            last_raise_total = round_contrib[pid]
    ...
    if last_raise_total is not None:
        min_raise_total = last_raise_total * 2 + 1
    elif round_idx == 0:
        min_raise_total = 2 * BIG_BLIND
    else:
        min_raise_total = BIG_BLIND
    min_raise_total = max(min_raise_total, my_round_bet + 1)
    if round_bet > my_round_bet:
        min_raise_total = max(min_raise_total, round_bet + 1)
    min_raise_delta = max(0, min_raise_total - my_round_bet)
    judge_round_raise = last_raise_total if last_raise_total is not None else BIG_BLIND
    ...
    return {
        ...
        "min_raise_action": min_raise_delta,
        "min_raise_delta": min_raise_delta,
