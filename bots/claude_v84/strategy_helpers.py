...
def bluff_heavy_raise_to_extract(
    line_profile, value_profile, made_strength, draw_strength,
    to_call, pot, my_chips, min_raise, my_round_bet, round_idx, opponent_model
):
    """v84: Wire dead bluff_heavy label into OFFENSIVE value extraction.

    When opponent line is classified bluff_heavy (passive opp, high fold-to-river-bet,
    low barrel freq, dry board -- a high-confidence multi-signal trigger) AND bot holds
    catcher-strength (thin/strong tier = real pair+), make a SIZED raise (0.45-0.65x
    pot) capped well below all-in. This extracts value from the perceived weak/bluff
    range AND enforces sizing discipline (no polarized shove with marginal hands).
    Returns raise-increment int or None."""
    if line_profile is None or line_profile.get('line_label') != 'bluff_heavy':
        return None
    if round_idx < 2:  # turn/river only -- flop needs more nutted hands
        return None
    if value_profile is None or value_profile.get('tier') not in ('thin', 'strong'):
        return None  # need real catcher-strength, not air nor nut
    if made_strength < 0.30:  # below thin-pair floor
        return None
    confidence = opponent_model.get('confidence', 0.0)
    if confidence < 0.20:  # need opponent data
        return None
    pot_after_call = pot + to_call
    base = 0.50 if value_profile['tier'] == 'thin' else 0.60
    ratio = base + 0.10 * max(0.0, made_strength - 0.45)
    ratio = max(0.45, min(ratio, 0.70))
    target = int(to_call + pot_after_call * ratio)
    amount = max(min_raise, target)
    stack_cap = int(my_chips * 0.55)  # sizing discipline: never all-in from this path
    amount = min(amount, stack_cap, my_chips - 1)
