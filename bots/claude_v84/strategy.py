...
from strategy_helpers import (
    ...
    sizing_exploit_adjustment, bluff_heavy_raise_to_extract,
)
...
def get_action(req, requests):
    ...
    # v84: offensive value extraction via dead bluff_heavy branch.
    # When opp line is bluff_heavy (passive/folds-to-bets) AND we hold catcher-strength,
    # raise to a SIZED amount to extract value -- NOT all-in, NOT check-back.
    if round_idx >= 2 and line_profile.get('line_label') == 'bluff_heavy':
        extract_raise = bluff_heavy_raise_to_extract(
            line_profile, value_profile, made_strength, draw_strength,
            to_call, pot, my_chips, state['min_raise_action'],
            state['my_round_bet'], round_idx, opponent_model,
        )
        if extract_raise is not None:
            return extract_raise
