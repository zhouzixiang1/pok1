...
from opponent import build_opponent_model, analyze_current_spot, classify_archetype, _aggro_bluffcatcher_should_fold, _rock_value_bet_fold, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold
...
def choose_raise(
    ...
    # v212 CROSSOVER from v201: SPR commitment gate -- cap marginal-hand raise
    # size so projected SPR-if-called stays >= 4 (pot control), capping -20k
    # stack-off exposure. Premium (>0.70) and trash (<0.50) bypass.
    if round_idx == 0 and preflop_strength is not None:
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        hand_cat = classify_preflop_hand(req['my_cards'])
        # v212 CROSSOVER from v201: preflop shove defense -- fold marginal hands
        # vs all-in shove (VPIP-aware mutation: wider mid-pair call vs wide shovers)
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        # v212 CROSSOVER from v201: preflop shove defense (2nd dispatch site).
        # Fold marginal hands vs preflop shove before entering the jam_odds
        # call/fold calc (VPIP-aware mutation for wide-shover exploitation).
