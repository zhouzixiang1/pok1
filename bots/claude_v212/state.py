...
def _preflop_spr_commitment_gate(raise_to, my_chips, already_invested, pot,
    ...
    """Preflop SPR commitment gate (v194 NEW axis, crossover from v201).
...
def _preflop_shove_defense_fold(my_cards, preflop_strength, state, spot_info, opponent_model=None):
    """Preflop shove/4-bet defense gate (v201 axis, crossover from v201).
    ...
    Mid pairs (77-99) continue only with cheap odds (pot_odds < threshold).
    Everything else folds. Returns True=fold, False=continue.

    MUTATION (v212): VPIP-aware mid-pair continuation threshold. Against WIDE
    open-shovers (VPIP>0.60), the base 0.35 threshold is widened to 0.42 (+20%)
    so mid-pairs continue more often — experience pool: "vs WIDE open-shovers
    (VPIP>0.60) → fold LESS (premiums-only exploitable; folding 55 @pot_odds
    ~0.50 is -EV)". Against value-heavy/standard shovers, the tight 0.35
    ...
    if hand_cat == 'mid_pair':
        # MUTATION: widen continuation vs wide open-shovers
        mid_threshold = 0.35
        if opponent_model is not None:
            opp_vpip = opponent_model.get('vpip', 0)
            if opp_vpip > 0.60:
                mid_threshold = 0.42  # +20%: exploit wide shovers
        if pot_odds < mid_threshold:
            return False
    return True
...
def _verify_preflop_shove_defense():
    """Self-test: _preflop_shove_defense_fold tier + VPIP mutation logic."""
    def c(r, s):
        return (r - 2) * 4 + s
    ...
    # MUTATION test: wide shover (VPIP>0.60) widens mid-pair continuation
    state_mid_odds = {'opponent_allin': True, 'to_call': 700, 'pot': 1800}
    # pot_odds = 700/2500 = 0.28 < 0.35 -> both standard and wide continue
    nines = [c(9, 0), c(9, 1)]
    assert not _preflop_shove_defense_fold(nines, 0.60, state_mid_odds, spot1, None), '99 continues cheap (no model)'
    # pot_odds between 0.35 and 0.42 -> standard folds, wide continues
    state_border = {'opponent_allin': True, 'to_call': 900, 'pot': 1800}
    # pot_odds = 900/2700 = 0.333 -> just under 0.35
    state_border2 = {'opponent_allin': True, 'to_call': 1000, 'pot': 2000}
    # pot_odds = 1000/3000 = 0.333 -> under both
    state_wide_odds = {'opponent_allin': True, 'to_call': 1100, 'pot': 2000}
    # pot_odds = 1100/3100 = 0.355 -> standard folds, wide (0.42) continues
    opp_standard = {'vpip': 0.50}
    opp_wide = {'vpip': 0.65}
    assert _preflop_shove_defense_fold(nines, 0.60, state_wide_odds, spot1, opp_standard), '99 folds vs standard (0.355>0.35)'
    assert not _preflop_shove_defense_fold(nines, 0.60, state_wide_odds, spot1, opp_wide), '99 continues vs wide (0.355<0.42)'
