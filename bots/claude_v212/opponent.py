    ...
    for req in hand_requests:
        ...
        for record in history:
            ...
            if action_type == "raise" and action > 0:
                ...
                high_bet = max(round_bets.values())
                round_bets[pid] = max(round_bets[pid], high_bet)
                # v212 CROSSOVER FIX from v201: record betsize-polarity sample
                # for all-ins. action=-2 hides the exact amount; approximate
                # ratio via high_bet/pot_estimate (lower bound, capped at 1.0
                # since all-ins cover >= pot). is_allin=True unblocks
                # _opp_betsize_polarity shove_rate (was permanently 0 because
