...
from postflop import (
    ...
    check_probe_resistance_margin, must_continue_vs_raise, barrel_plan,
)
...
def choose_raise(
    ...
    if barrel_ratio_override is not None:
        ratio = clamp(barrel_ratio_override, low_ratio, 1.45)
...
def get_action(req, requests):
    ...
    _barrel_spr = my_chips / pot if pot > 0 else 999.0
    _barrel_plan = barrel_plan(
        value_profile, board_texture, pair_profile, nutted_risk,
        opponent_model, spot_info, round_idx, _barrel_spr,
    ) if round_idx > 0 else {"active": False, "line": "none", "target_ratio": 0.0}
    ...
    sizing_delta = sizing_exploit_adjustment(opponent_model, round_idx)
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or _barrel_plan["active"]:
        ...
        raise_amount = choose_raise(
            ...
            match_sizing_delta=match_profile["sizing_delta"],
            sizing_exploit_delta=sizing_delta,
            barrel_ratio_override=_barrel_plan["target_ratio"] if _barrel_plan["active"] else None,
        )
        if raise_amount is not None:
            return raise_amount
    return 0
