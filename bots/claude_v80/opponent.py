...
def analyze_current_spot(req, state):
    ...
    for record in history:
        ...
        # Track my postflop bets and PFR status for barrel sequencing
        if record["player_id"] == my_id:
            if record["round"] == 0 and record["action_type"] in ("raise", "allin"):
                info["was_preflop_raiser"] = True
            if (record["round"] > 0 and record["round"] < state["round"]
                    and record["action_type"] in ("raise", "allin")):
                info["my_postflop_bet_count"] += 1
