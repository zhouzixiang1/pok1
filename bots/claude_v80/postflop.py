...
def barrel_plan(value_profile, board_texture, pair_profile, nutted_risk,
                opponent_model, spot_info, round_idx, spr):
    """OFFENSIVE multi-street barrel sequencing. Commits to 2-3 street value
    or bluff lines to extract max value and apply sustained pressure.
    Re-derived each street from current state (stateless consistency).
    NO fold/call logic — purely drives bet sizing and continuation.
    Returns: {"active": bool, "line": str, "target_ratio": float}
    """
    result = {"active": False, "line": "none", "target_ratio": 0.0}
    if round_idx < 1 or round_idx > 3:
        return result

    tier = value_profile.get("tier", "none") if value_profile else "none"
    streets_barreled = spot_info.get("my_postflop_bet_count", 0)
    wet = board_texture["wetness"] if board_texture else 0.0
    dynamic = board_texture["dynamic"] if board_texture else False
    risk = nutted_risk.get("risk", 0.0) if nutted_risk else 0.0
    fold_to_raise = opponent_model.get("fold_to_raise", 0.44)
    confidence = opponent_model.get("confidence", 0.0)

    # Abort barrel if opponent raised back more than us postflop (strength shown)
    if spot_info.get("opp_prior_postflop_raise_count", 0) > streets_barreled:
        return result

    # VALUE BARREL: strong/nut hands commit 2-3 streets with escalating sizing
    if tier in ("strong", "nut"):
        if risk > 0.12 and tier != "nut":
            return result
        commit = 3 if (spr < 5 and not dynamic and risk < 0.04) else 2
        if streets_barreled >= commit:
            return result
        base = {1: 0.55, 2: 0.70, 3: 0.85}
        ratio = base.get(round_idx, 0.65)
        if dynamic:
            ratio += 0.05 + 0.04 * wet  # charge draws on dynamic boards
        if tier == "nut" and round_idx == 3:
            ratio = max(ratio, 0.90)  # river nut = big bet
        return {"active": True, "line": f"value_{commit}barrel", "target_ratio": ratio}

    # BLUFF CONTINUATION: already barreled, air hand, high fold equity
    made = pair_profile.get("made_class", -1) if pair_profile else -1
    if (streets_barreled >= 1 and tier == "none" and made < 1
            and confidence >= 0.30 and fold_to_raise > 0.52 and not dynamic):
        commit = 3 if fold_to_raise > 0.62 else 2
        if streets_barreled >= commit:
            return result
        ratios = {2: 0.65, 3: 0.85}
        return {"active": True, "line": f"bluff_{commit}barrel",
                "target_ratio": ratios.get(round_idx, 0.65)}

    # DELAYED C-BET: PFR who checked flop, now bets turn/river for value
    if (round_idx >= 2 and streets_barreled == 0
            and spot_info.get("was_preflop_raiser", False)
            and tier in ("thin", "strong") and not dynamic):
        ratio = 0.55 if round_idx == 2 else 0.65
        return {"active": True, "line": "delayed_cbet", "target_ratio": ratio}
