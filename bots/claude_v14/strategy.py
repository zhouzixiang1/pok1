...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if repeated_raise_trap and (value_profile is None or value_profile["tier"] != "nut"):
            # Fold genuinely weak hands (no made hand, no draw, expensive call)
            if made_strength < 0.15 and draw_strength < 0.12 and pot_odds > 0.20:
                if not anti_lock_call_continue:
                    return -1
            # Non-value hands: call (avoid bluff-raising into heavy aggression)
            if value_profile is None or value_profile["tier"] == "none":
                return 0
            # thin/strong tier: fall through to raise logic below
