...
def facing_barrel_continuation(spot_info, round_idx, made_strength, draw_strength, value_profile, opponent_model, pot, to_call):
    ...
    CROSSOVER v122 (v116 base × v113 donor): restored from v113. v116 actively
    REMOVED this gene (v101 lineage) in favor of the broadway_suited preflop
    tightening + line_reading bluff_opp relaxation. H2H evidence (≥30g samples)
    shows v113 wins matchups where v116 loses: v117 (v113 0.60 vs v116 0.375,
    +22.5pp), v93 (0.57 vs 0.48), v107 (0.542 vs 0.483), v110 (0.533 vs 0.475),
    v97 (0.489 vs 0.440), v104 (0.508 vs 0.467). These opponents fire multi-
    street barrels (v107×v87 loss half: river allin 22%, 6 big losses incl.
    -17248 per battle_experience.md) — exactly the scenario this gate targets.
    Direct H2H v116×v113: v113 wins 21/40. Restoring the gene is justified by
    concrete "B wins where A loses" evidence, not byte-identical re-import.
    ...
    if round_idx == 2:  # turn barrel
        ...
        signal = 0.035 + 0.085 * min(equity_deficit, 0.20)

    # MUTATION v122 (option a, ~13% threshold bump): deep-barrel amplifier
    # 1.15 -> 1.30 specifically on barrel_depth >= 3. Targets the exact
    # multi-street-barrel matchups where v116 bleeds chips (v117/v93/v107/v110
    # all barrel aggressively per battle_experience.md). Bounded to the
    # already-imported defensive gene (not a new gate), within the 10-20%
    # threshold-adjustment budget, and capped at 0.08 downstream so the
    # amplification cannot force a fold — only tightens the call nudge.
    if barrel_depth >= 3:
        signal *= 1.30
