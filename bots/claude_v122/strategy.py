...
def get_action(req, requests):
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            # CROSSOVER v122 (v116 × v113): barrel-continuation fold signal
            # restored from v113. Tighten call margin vs multi-street barrels
            # (barrel_depth >= 2 across postflop streets). Grounded in pot-odds
            # — only fires when equity is meaningfully below the call price, and
            # capped at 0.08 so it never forces a fold (only nudges). Targets
            # the multi-street-barrel matchups where v116 loses to v113: v117,
            # v93, v107, v110, v97, v104 (all >=30g H2H evidence).
