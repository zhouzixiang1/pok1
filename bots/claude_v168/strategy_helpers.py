...
def _river_value_raise_reduce(round_idx, to_call, made_strength, value_profile,
                               opponent_model, pot):
    """v168: Offensive river value raise sizing reduction vs large-bet-heavy opps.

    When the bot is value-raising on the river (to_call==0, tier thin/strong/nut),
    reduce sizing vs opponents who frequently make large bets. These opponents
    are likely calling-stations who will call smaller bets with worse hands.

    Returns negative delta to add to raise ratio (e.g., -0.15 reduces 0.70x to 0.55x).
    Range: [-0.18, 0.0].
    """
    if round_idx != 3 or to_call != 0:
        return 0.0
    if value_profile is None or value_profile.get('tier') not in ('thin', 'strong', 'nut'):
        return 0.0
    if opponent_model is None:
        return 0.0
    confidence = opponent_model.get('confidence', 0.0)
    if confidence < 0.10:
        return 0.0
    large_ratio = opponent_model.get('large_bet_ratio', 0.32)
    # Continuous delta: negative when large_ratio > 0.28 (typical pool mean)
    delta = -confidence * max(0.0, large_ratio - 0.28) * 0.60
    delta = max(-0.18, delta)
    try:
        sys.stderr.write(
            'RIVER_VALUE_REDUCE delta_milli=%+d large_ratio=%.3f conf=%.2f '
            'tier=%s reason=%s\n'
            % (round(delta * 1000), large_ratio, confidence,
               value_profile.get('tier', 'none'),
               'reduced' if delta < -0.005 else 'no_reduce')
        )
    except Exception:
        pass
    return delta
...
def _river_stackoff_guard(made_strength, value_profile, spot_info, opponent_model,
    ...
    if pot_odds_required <= made_strength + 0.03:
        return False
...
if __name__ == "__main__":
    ...
    # v168 regression: _river_stackoff_guard fires at tighter buffer (0.03).
    # With made_strength=0.48 and to_call=5500: pot_odds=5500/10500=0.524,
    # made + 0.03 = 0.51 → 0.524 > 0.51 → fires (fold).
    guard_tight = _river_stackoff_guard(
        made_strength=0.48,
        value_profile={'tier': 'thin', 'size_bonus': 0.0},
        spot_info={'last_raise_pot_ratio': 0.80, 'facing_allin': False},
        opponent_model={'confidence': 0.30, 'sizing_tendency': None},
        my_chips=10000, pot=5000, to_call=5500)
    assert guard_tight is True, \
        'v168 tighter buffer should fire at pot_odds 0.524 > made 0.48+0.03'
    print('v168 _river_stackoff_guard tighter buffer: PASS (0.524 > 0.51 fires)')

    # v168 regression: borderline hand that should NOT fire (equity sufficient).
    # made=0.50, to_call=4000: pot_odds=4000/9000=0.444, made+0.03=0.53 → safe.
    guard_safe = _river_stackoff_guard(
        made_strength=0.50,
        value_profile={'tier': 'thin', 'size_bonus': 0.0},
        spot_info={'last_raise_pot_ratio': 0.80, 'facing_allin': False},
        opponent_model={'confidence': 0.30, 'sizing_tendency': None},
        my_chips=10000, pot=5000, to_call=4000)
    assert guard_safe is False, \
        'v168 tighter buffer should NOT fire at pot_odds 0.444 <= made 0.50+0.03'
    print('v168 _river_stackoff_guard tight-but-safe: PASS (0.444 <= 0.53 no fire)')

    # v168 regression: old buffer (0.10) would NOT have fired, new (0.03) does.
    # made=0.45, to_call=5000: pot_odds=0.50, old 0.45+0.10=0.55 no fire,
    # new 0.45+0.03=0.48 → 0.50 > 0.48 → fires.
    guard_old_miss = _river_stackoff_guard(
        made_strength=0.45,
        value_profile={'tier': 'thin', 'size_bonus': 0.0},
        spot_info={'last_raise_pot_ratio': 0.80, 'facing_allin': False},
        opponent_model={'confidence': 0.30, 'sizing_tendency': None},
        my_chips=10000, pot=5000, to_call=5000)
    assert guard_old_miss is True, \
        'v168: hand that old buffer missed (0.50 <= 0.55) now fires (0.50 > 0.48)'
    print('v168 _river_stackoff_guard old-buffer-miss now fires: PASS')

    # v168 regression: _river_value_raise_reduce fires for standard pool.
    rrr_standard = _river_value_raise_reduce(
        round_idx=3, to_call=0, made_strength=0.62,
        value_profile={'tier': 'strong', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.30, 'large_bet_ratio': 0.32},
        pot=5000)
    assert rrr_standard < 0, f'Expected negative delta for standard pool, got {rrr_standard}'
    print(f'v168 _river_value_raise_reduce standard: delta={rrr_standard:.3f} PASS')

    # v168: zero for non-river or to_call > 0
    rrr_flop = _river_value_raise_reduce(
        round_idx=1, to_call=0, made_strength=0.62,
        value_profile={'tier': 'strong', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.30, 'large_bet_ratio': 0.32},
        pot=5000)
    assert rrr_flop == 0.0, f'Expected 0 for flop, got {rrr_flop}'
    print('v168 _river_value_raise_reduce non-river: PASS')

    # v168: zero when facing bet (to_call > 0)
    rrr_face = _river_value_raise_reduce(
        round_idx=3, to_call=500, made_strength=0.62,
        value_profile={'tier': 'strong', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.30, 'large_bet_ratio': 0.32},
        pot=5000)
    assert rrr_face == 0.0, f'Expected 0 for to_call>0, got {rrr_face}'
    print('v168 _river_value_raise_reduce to_call>0: PASS')

    # v168: high large-bet opp produces larger reduction
    rrr_high = _river_value_raise_reduce(
        round_idx=3, to_call=0, made_strength=0.62,
        value_profile={'tier': 'strong', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.50, 'large_bet_ratio': 0.50},
        pot=5000)
    assert rrr_high < rrr_standard, f'High large-bet should produce more reduction: {rrr_high} vs {rrr_standard}'
    print(f'v168 _river_value_raise_reduce high large-bet: delta={rrr_high:.3f} PASS')

    # v168: low confidence -> 0
    rrr_low_conf = _river_value_raise_reduce(
        round_idx=3, to_call=0, made_strength=0.62,
        value_profile={'tier': 'strong', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.05, 'large_bet_ratio': 0.50},
        pot=5000)
    assert rrr_low_conf == 0.0, f'Expected 0 for low confidence, got {rrr_low_conf}'
    print('v168 _river_value_raise_reduce low confidence: PASS')

    # v168: thin tier eligible
    rrr_thin = _river_value_raise_reduce(
        round_idx=3, to_call=0, made_strength=0.50,
        value_profile={'tier': 'thin', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.30, 'large_bet_ratio': 0.40},
        pot=5000)
    assert rrr_thin < 0, f'Expected negative delta for thin tier, got {rrr_thin}'
    print(f'v168 _river_value_raise_reduce thin tier: delta={rrr_thin:.3f} PASS')

    # v168: none tier -> 0 (not eligible)
    rrr_none = _river_value_raise_reduce(
        round_idx=3, to_call=0, made_strength=0.35,
        value_profile={'tier': 'none', 'size_bonus': 0.0},
        opponent_model={'confidence': 0.30, 'large_bet_ratio': 0.40},
        pot=5000)
    assert rrr_none == 0.0, f'Expected 0 for none tier, got {rrr_none}'
    print('v168 _river_value_raise_reduce none tier: PASS')

    print('v168 _river_value_raise_reduce: 7/7 PASS')
