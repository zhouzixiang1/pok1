...
def delayed_cbet_strategy(round_idx, to_call, made_strength, draw_strength,
                           value_profile, board_texture, opponent_model,
                           was_pfr, checked_flop, spot_info,
                           texture_class='none', opp_archetype='unknown'):
    """Turn delayed c-bet: checked flop as PFR, now bet the turn.

    Returns: {'action': 'bet'|'check', 'ratio': float, 'reason': str}
    Only active on round_idx==2 when to_call==0, was PFR, and checked flop.
    """
    if round_idx != 2 or to_call != 0 or not was_pfr or not checked_flop:
        return {'action': 'check', 'ratio': 0.0, 'reason': 'not_eligible'}
    ...
    # Value bet: strong/nut hands — bet for value
    if tier in ('strong', 'nut'):
        if texture_class == 'dry':
            return {'action': 'bet', 'ratio': 0.45, 'reason': 'delayed_value_dry'}
        return {'action': 'bet', 'ratio': 0.55, 'reason': 'delayed_value_wet'}

    # Thin value: decent made hand on dry board
    if tier == 'thin' and made_strength >= 0.40 and texture_class in ('dry', 'paired'):
        return {'action': 'bet', 'ratio': 0.35, 'reason': 'delayed_thin_dry'}

    # Semi-bluff: draw on wet/dynamic board
    if has_draw and draw_strength >= 0.14:
        if wetness >= 0.25 or texture_class in ('draw_heavy', 'semi_connected'):
            return {'action': 'bet', 'ratio': 0.45, 'reason': 'delayed_semibluff'}

    # Bluff: weak hand on dry board vs folding opponent
    if made_strength < 0.25 and texture_class == 'dry':
        if opp_archetype == 'calling_station':
            return {'action': 'check', 'ratio': 0.0, 'reason': 'no_bluff_cs'}
        if confidence >= 0.15 and fold_to_raise >= 0.45:
            return {'action': 'bet', 'ratio': 0.35, 'reason': 'delayed_bluff_dry'}
