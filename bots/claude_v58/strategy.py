...
from postflop import (
    ...
    flop_cbet_strategy, delayed_cbet_strategy,
)
...
def choose_anti_lock_pressure_action(
    ...
    if amount >= my_chips * 0.72:
        tier = value_profile.get('tier', 'none') if value_profile is not None else 'none'
        if round_idx == 3 and tier not in ('strong', 'nut') and made_strength < 0.55:
            return None  # skip all-in with weak river hands
        return -2
...
def _was_preflop_raiser(req, my_id):
    for record in req.get('history', []):
        if (record.get('player_id') == my_id
            and record.get('round') == 0
            and record.get('action_type') in ('raise', 'allin')):
            return True
    return False


def river_commitment_protection(round_idx, raise_amount, my_chips, value_profile, made_strength, to_call):
    """Gate river raises that commit >50% of remaining stack with non-strong hands.

    Returns the capped raise amount, or 0 (check) if the raise is too risky.
    Only active on river (round_idx == 3) when we are betting (to_call == 0).
    """
    if round_idx != 3 or raise_amount is None or raise_amount <= 0:
        return raise_amount
    if to_call > 0:  # only gate when WE are betting (no call to match)
        return raise_amount

    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    # Strong/nut hands are allowed to commit fully
    if tier in ('strong', 'nut'):
        return raise_amount

    commit_ratio = raise_amount / max(1, my_chips)

    # Weak hands (tier none/thin): never commit >50% of stack
    if commit_ratio > 0.50 and tier in ('none', 'thin'):
        capped = int(my_chips * 0.50)
        if capped < raise_amount and capped > 0:
            return capped
        return 0  # check instead of massive overcommit
...
def get_action(req, requests):
    ...
    # ── Flop c-bet strategy by board texture ───────────────────────
    was_pfr = _was_preflop_raiser(req, my_id)
    if (round_idx == 1 and to_call == 0 and was_pfr
        and not spot_info.get('facing_postflop_aggression')
        and board_texture is not None):
        cbet = flop_cbet_strategy(
            made_strength, draw_strength, draw_info, value_profile,
            board_texture, opponent_model, spot_info['has_position'],
            texture_class=street_texture['class'],
            opp_archetype=opp_archetype,
        )
        if cbet['action'] == 'bet' and cbet['ratio'] > 0:
            cbet_amount = max(state['min_raise_action'], int(pot * cbet['ratio']))
    ...
    # ── Turn delayed c-bet (checked flop as PFR, bet turn) ────────────
    if round_idx == 2 and to_call == 0 and was_pfr and not was_flop_aggressor:
        dcbet = delayed_cbet_strategy(
            round_idx, to_call, made_strength, draw_strength,
            value_profile, board_texture, opponent_model,
            was_pfr, True, spot_info,
            texture_class=street_texture['class'],
            opp_archetype=opp_archetype,
        )
        if dcbet['action'] == 'bet' and dcbet['ratio'] > 0:
            dcbet_amount = max(state['min_raise_action'], int(pot * dcbet['ratio']))
            if dcbet_amount < my_chips and dcbet_amount > 0:
                return dcbet_amount
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")):
        ...
        if raise_amount is not None:
            raise_amount = river_commitment_protection(
                round_idx, raise_amount, my_chips, value_profile, made_strength, to_call,
            )
            if raise_amount is not None and raise_amount > 0:
                return raise_amount
    return 0
