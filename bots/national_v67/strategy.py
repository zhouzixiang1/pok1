...
# ── [v67] Domination-aware preflop pruning ────────────────────────────────────
# v66 loses 0-20% WR vs tight opponents (battle evidence ev_879d250dcc620a21,
# ev_02cc70f70621cee1). Root cause: estimate_preflop_strength saturates AQo/AJo/
# ATo/KQo/KJo/QJo at 1.000, so three preflop call sites ALWAYS fire for these
# hands vs tight openers whose range (AA/KK/AK) dominates them (reverse-implied-
# odds leak). These helpers add a continuous, confidence-gated domination penalty
# so dominated offsuit broadway is pruned vs CONFIRMED tight ranges while leaving
# AKo, suited broadway, and pairs untouched.
def _domination_prone_offsuit_broadway(my_cards):
    """True for offsuit broadway hands dominated by tight value ranges.

    Covers ATo/AJo/AQo/KQo/KJo/QJo/KTo/QTo/JTo (excludes AKo, suited, pairs).
    Vs tight openers (AA/KK/QQ/AK heavy), these suffer reverse implied odds:
    top pair dominated, second pair kicks weakly.
    """
    r1, r2 = sorted((card_number(c) for c in my_cards), reverse=True)
    s1, s2 = card_suit(my_cards[0]), card_suit(my_cards[1])
    if s1 == s2 or r1 == r2:
        return False
    if r2 < 10:  # only broadway (both cards >= T)
        return False
    if r1 == 14 and r2 == 13:  # AKo is fine (top of offsuit range)
        return False
    return True


def _domination_strength_penalty(my_cards, opponent_model):
    """Continuous penalty for offsuit broadway vs confirmed tight openers.

    Returns 0.0 for: suited/paired hands, non-broadway, AKo, or loose opponents.
    Returns up to -0.20 for KQo/KJo/QJo/ATo/AJo/AQo/KTo/QTo/JTo when BOTH
    opp_pfr <= 0.25 AND opp_vpip <= 0.40 with confidence >= 0.40.
    [v67-repair] Cap reduced -0.60 -> -0.20 and tight_signal clamped to [0,1]:
    the original -0.60 (effectively -0.75 unclamped) dropped AQo/KQo from
    saturated 1.0 to 0.25-0.55, folding playable broadway vs tight opponents
    and causing the bot to blind off in 70-hand national TCP matches.
    """
    if not _domination_prone_offsuit_broadway(my_cards):
        return 0.0
    ...
    # Continuous tight signals; 15-20pp bands per PARAMETER_TUNING guidance
    tight_pfr = max(0.0, 0.25 - opp_pfr) / 0.15      # 0..1, fires pfr<=0.25
    tight_vpip = max(0.0, 0.40 - opp_vpip) / 0.20    # 0..1, fires vpip<=0.40
    tight_signal = min(1.0, min(tight_pfr, tight_vpip))  # clamp; BOTH must signal
    conf_gate = min(1.0, conf / 0.40)                 # require n>=~5 actions
    penalty = -0.20 * tight_signal * conf_gate
    if penalty < 0:  # [v67-repair] gate telemetry to fire-only to reduce TCP I/O
        r1, r2 = sorted((card_number(c) for c in my_cards), reverse=True)
        sys.stderr.write(
            f"DOMINATION_PENALTY r1={r1} r2={r2} "
            f"opp_pfr={opp_pfr:.2f} opp_vpip={opp_vpip:.2f} conf={conf:.2f} "
            f"penalty={penalty:.2f} reason=fired\n"
        )
    return penalty
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        eff_bluff_freq = BB_BLUFF_3BET_FREQ + _def_adj['bluff_freq_delta']
        eff_call_threshold = BB_CALL_THRESHOLD + _def_adj['call_delta']
        # [v67] Domination-aware value 3bet: prune dominated offsuit broadway
        # (KQo/KJo/QJo/ATo/AJo/AQo/KTo/QTo) vs confirmed tight openers.
        dom_penalty = _domination_strength_penalty(req['my_cards'], opponent_model)
        # Value 3bet: TT+, AK, AQs (strength >= BB_VALUE_3BET_THRESHOLD)
        if preflop_strength + dom_penalty >= eff_value_3bet and not trash_hand:
            raise_amount = choose_raise(
        ...
        if can_bluff_3bet and eff_bluff_low <= preflop_strength <= eff_bluff_high and not trash_hand:
            ...
            if bluff_roll < eff_bluff_freq:
                ...
                if raise_amount is not None:
                    return raise_amount
        # [v67] Call with playable hands. dom_penalty mildly prunes dominated
        # offsuit broadway vs confirmed tight openers. [v67-repair] Removed the
        # compounding +0.05 equity_floor raise — the penalty already reduces
        # preflop_strength, so raising the win_rate bar too double-counts RIO.
        if preflop_strength + dom_penalty >= eff_call_threshold or win_rate >= pot_odds_pf - 0.02:
            return 0
        return -1
        ...
        sbr_dom_penalty = _domination_strength_penalty(req['my_cards'], opponent_model)
        # Non-all-in: call with strong hands if pot odds are reasonable
        if preflop_strength + sbr_dom_penalty >= SB_VS_RERAISE_CALL and win_rate >= pot_odds_sbr - SB_VS_RERAISE_POT_SLACK:
            return 0
...
def _self_test_domination_penalty():
    """Verify domination penalty fires vs tight opps, returns 0 for standard priors."""
    tight_opp = {'pfr': 0.10, 'vpip': 0.25, 'confidence': 0.50}
    standard_opp = {'pfr': PRIOR_PFR, 'vpip': PRIOR_VPIP, 'confidence': 0.50}
    kqo = [44, 41]    # K suit=0, Q suit=1 (offsuit)
    aks = [48, 44]    # A K same suit=0
    pair_qq = [40, 41]
    ako = [48, 46]    # A suit=0, K suit=2 (offsuit, top of range)
    # KQo vs tight: mild penalty (cap -0.20, clamped signal)
    p1 = _domination_strength_penalty(kqo, tight_opp)
    assert p1 <= -0.05, f"KQo vs tight should penalize, got {p1}"
    # AKs vs tight: no penalty (suited)
    p2 = _domination_strength_penalty(aks, tight_opp)
    assert p2 == 0.0, f"AKs should not be penalized (suited), got {p2}"
    # QQ vs tight: no penalty (pair)
    p3 = _domination_strength_penalty(pair_qq, tight_opp)
    assert p3 == 0.0, f"QQ should not be penalized (pair), got {p3}"
    # AKo vs tight: no penalty (top of offsuit range)
    p4 = _domination_strength_penalty(ako, tight_opp)
    assert p4 == 0.0, f"AKo should not be penalized, got {p4}"
    # KQo vs standard: no penalty (opp not tight)
    p5 = _domination_strength_penalty(kqo, standard_opp)
    assert p5 == 0.0, f"Standard opp should not trigger, got {p5}"
    print(f"DOMINATION_SELF_TEST OK: tight_KQo={p1:.2f} AKs={p2} QQ={p3} AKo={p4} std_KQo={p5}")
