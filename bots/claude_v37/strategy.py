"""
Strategy engine: v7 (Alpha) base + v8 (Beta) improvements.
Crossover: v7's looser jam buffers + v8's pot-size EQR, big_pot_safety_guard,
           must_continue_vs_raise with draw_strength, overbet bluff, thin_static_showdown round>=1.
"""
...
def realized_postflop_equity(
    ...
    """v7 base + v8 pot-size adjustments for more accurate equity realization in big pots."""
    air_hand = made_strength < 0.18 and draw_strength < 0.08
    if round_idx <= 0:
        return win_rate
    ...
    if air_hand:
        # v8's tighter air EQR (0.68/0.56 vs v7's 0.72/0.62)
    ...
    if pair_profile is not None and pair_profile["made_class"] == 1:
        ...
        if pair_type in ("middle_pair", "bottom_pair", "underpair", "board_pair"):
            # v8's tighter weak pair EQR (0.84/0.73 vs v7's 0.86/0.78)
            ...
            if round_idx == 3:
                eqr -= 0.06
            # v8's big pot penalty for weak pairs
            if pot > 3000:
                eqr -= 0.03
...
def get_action(req, requests):
    ...
    # Compute anti-bot_4 adjustments
    ...
    # River overbet bluff (from v8 - actual implementation)
    ...
    if state["opponent_allin"]:
        ...
        # v7's higher jam buffer (0.04 thin, 0.14 cap) - proven effective
    ...
    if to_call > 0:
        ...
        # v8's must_continue_vs_raise with draw_strength
    ...
    if big_pot and round_idx == 3 and (value_profile is None or value_profile["tier"] not in ("strong", "nut")):
        if blocker_profile is None or not blocker_profile["eligible"]:
            return 0
    # v8's thin_static_showdown_control from round >= 1 (was >= 2 in v7)
