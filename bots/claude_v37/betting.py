"""Betting decision functions. Cross of v7 (jam buffers) + v8 (modular, safety guards, overbet bluff)."""
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        # MUTATION: open_threshold lowered from 0.49 to 0.47 for wider steal range
...
def choose_overbet_bluff_river(
    ...
    """River overbet bluff with strong blockers on dry boards (from v8)."""
    if to_call != 0:
        return None
...
def big_pot_safety_guard(pot, my_chips, value_profile, made_strength, round_idx, to_call, draw_strength):
    ...
    Prevents catastrophic stack-offs with thin value in big pots (from v8)."""
    if round_idx < 2:
        return False
    if to_call > 0:
        return False
...
def must_continue_vs_raise(value_profile, made_strength, pot_odds, nutted_risk, board_texture, draw_strength=0.0):
    ...
    if tier == "strong" and pot_odds <= 0.36 and risk <= 0.05:
        return True
    # Protect strong combo draws facing aggression (from v8)
