...
def _marginal_made_river_fold_gate(round_idx, made_strength, draw_strength,
    ...
    """v242: Marginal-made river fold gate vs value-heavy opponents.

    Broadened from trips_vulnerable-only to cover the full over-call leak
    band: one-pair (~0.22) and weak two-pair (~0.40) facing LARGE river
    bets/shoves. Targets the -14696/-19846/-15322 blowouts that game-count
    W-L masks.

    Returns True (force fold) iff ALL hold:
      - round_idx == 3 (river only)
      - to_call > 0 AND to_call >= 0.50 * pot (large bet/shove; NOT small probes)
      - made_strength < 0.45 (one-pair/weak-two-pair; PROTECT trips ~0.58+)
      - draw_strength < 0.19 (no combo/flush draws; river rarely has draws)
      - opponent_model present with confidence >= 0.25
      - value-heavy: value_maximizer_index > 0.40 OR fold_to_bet_turn < 0.40

    Additional trigger: trips_vulnerable two-pair from paired board fires
    alongside the marginal-made condition (redundant since trips_vulnerable
    hands have made ~0.40 < 0.45, but kept explicit for audit trail).

    NEVER unconditional — always requires opponent value signal. Vs
    bluffers/maniacs (vmi <= 0.40 AND ftb_turn >= 0.40) it still calls down.
    """
    # River-only — no flop/turn/preflop folds
    if round_idx != 3:
        return False
    if to_call <= 0 or pot <= 0:
        return False
    # Large-bet gate — do NOT fold to small probes or min-raises
    if to_call < 0.50 * pot:
        return False
    # Hand-strength ceiling — protect trips (0.58) and strong two-pair (>=0.45)
    if made_strength >= 0.45:
        return False
    # Strong-draw protection — do not fold combo/flush/straight draws
    if draw_strength >= 0.19:
        return False
    # Nut/strong tier protection (belt-and-suspenders with made_strength gate)
    if value_profile is not None and value_profile.get('tier') in ('strong', 'nut'):
        return False
    # Opponent model required — NEVER unconditional
    ...
    # Value-heavy signal MANDATORY — vs bluffers/maniacs still call down
    if not (vmi > 0.40 or ftb_turn < 0.40):
        return False
    return True
