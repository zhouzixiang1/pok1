...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        # v242: marginal-made river fold gate — DIRECT fold (no suppression).
        # Broadened from trips_vulnerable-only to full over-call leak band
        # (one-pair/weak-two-pair made<0.45 facing to_call>=0.5x pot river).
        # Opponent-signal gated: NEVER unconditional. Vs bluffers still calls.
        if _marginal_made_river_fold_gate(
                round_idx, made_strength, draw_strength, value_profile,
                spot_info, opponent_model, to_call, pot, paired_board_profile):
            return -1
