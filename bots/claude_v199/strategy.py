...
from opponent import build_opponent_model, analyze_current_spot, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold
...
def get_action(req, requests):
    ...
    if to_call > 0:
        # v197 CROSSOVER (v193 × v179): REMOVED the three direct binary fold
        # gates that v193 added (_aggro_bluffcatcher_should_fold, _rock_value_bet_fold,
        # and the non-all-in _multibarrel_line_fold dispatch). v179 — which BEATS
        # v177 (0.575) and v159 (0.567) where v193 LOSES (0.300 / 0.400) — relies
        # ENTIRELY on the realized_rate < pot_odds + call_margin comparison with NO
        # pre-emptive direct folds for non-all-in bets. The direct gates over-folded
        # weak bluff-catchers vs wide-barreling opponents (v177/v159 barrel multi-
        # street with blended ranges, not just value). The all-in _multibarrel_line_fold
        # dispatch (~L1577) is KEPT for true stack-covering -20k leak protection, and
        # the continuous _river_potodds_equity_margin inside _postflop_response_margin
        # still provides river fold calibration (v197 mutation: threshold 0.48→0.42).
...
def _v199_polarity_selftest():
    ...
    _std = dict(_base, betsize_polarity={'tendency': 'standard', 'confidence': 0.50})
    _under = dict(_base, betsize_polarity={'tendency': 'underbettor', 'confidence': 0.50})
    _over = dict(_base, betsize_polarity={'tendency': 'overbettor', 'confidence': 0.50})
    d_std = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _std)
    d_under = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _under)
    d_over = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _over)
    assert d_std > 0.0, 'standard polarity must fire (M5/M6 LIVE)'
    assert d_under > d_std, 'underbettor must up-size more than standard'
    assert d_over < d_std, 'overbettor must NOT up-size via polarity'
