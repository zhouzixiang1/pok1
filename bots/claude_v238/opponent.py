...
def _multibarrel_line_fold(om, spot_info, round_idx, to_call, pot,
    ...
    if draw_strength >= 0.15:
        return False
    # Guard 6: bet is significant (pot_odds >= 0.28, bet >= ~40% pot).
    # v238 MUTATION: lowered 0.33 -> 0.28 (15% reduction). Extends the
    # multi-barrel fold gate to cover MEDIUM-SIZED bets (~40-50% pot) where
    # the same over-call leak applies but the prior 0.33 floor missed.
    # H2H rationale: v235 (parent A, base) loses v208 (0.20) / v205 (0.35) /
    # v184 (0.35) where v206 wins — opponents who likely thin-value bet medium
    # sizing on confirmed multi-barrel lines. All OTHER guards (multi-barrel
    # evidence, made<0.42, tier protection, no-draw, loose-caller carve-out)
    # remain intact, so the gate only fires in genuinely -EV call-downs.
    # NOT the exhausted underbettor/made_strength axis — this is the BET-SIZE
    # BAND axis (previously uncovered: 0.28-0.33 pot_odds).
    pot_odds = to_call / max(1, pot + to_call)
    if pot_odds < 0.28:
        return False
    return True
...
def _estimate_bluff_frequency(om):
    ...
    # v235 CROSSOVER (port v221's underbettor betsize_polarity calibration)
    # + MUTATION (VPIP-gate). An underbettor's bets are CONDENSED thin-value
    # (small sizing with medium-strength hands), NOT polarized nuts-or-air.
    # The standard discount (0.40+base) UNDERESTIMATES our bluff-catcher
    # equity vs such a condensed range. Flooring bluff_freq at 0.30 raises
    # the discount to 0.70 -> higher calibrated_equity -> less aggressive
    # folding in _allin_polarized_equity_fold / _river_potodds_equity_margin.
    # This is the fold-side PAYOFF for the v193 _opp_betsize_polarity axis.
    #
    # MUTATION: VPIP-gate (v221's floor was unconditional -> experience pool
    # warns "unconditional gates turn -EV within one generation"). Only apply
    # when VPIP > 0.55 — wide opponents' underbets include more bluffs/weak
    # hands, so the condensed-range assumption holds. Tight underbettors
    # (VPIP <= 0.55) betting small may still have value, so the standard
    # discount applies, preserving v206's edge vs value-heavy opponents
    # (v195/v200/v178/v196) while gaining v221's edge vs wide underbettors
    # (v194/v207/v213).
    _polarity = om.get('betsize_polarity', {})
    _opp_vpip = om.get('vpip', 0.0)
    if (_polarity.get('confidence', 0.0) >= 0.25
            and _polarity.get('tendency') == 'underbettor'
            and _opp_vpip > 0.55):
        base = max(base, 0.30)
