...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
    ...
    '''
    # v189: Range-polarization discount. made_strength is a hand-class ordinal
    # (pair=0.22, two-pair=0.40), NOT equity vs a polarized betting range.
    # A pair wins ~22% vs a value-heavy range, but made_strength=0.30 (medium
    # pair) still exceeds typical pot_odds (~0.27), making the raw gap check
    # dead for the 0.28-0.50 band. Discount made_strength to approximate
    # equity vs a polarized range before computing the gap.
    POLARIZATION_DISCOUNT = 0.65  # LOCAL CONSTANT: tune 0.60-0.70 if over/under-folding
    # v189: Leak-band floor lowered from 0.50 to 0.40 to catch the uncovered
    # medium-bet band where the same over-calling leak applies. _river_stackoff_guard
    # (binary, >=0.75x) already covers the top end; this catches 0.40-0.75x.
    ...
    calibrated_equity = made_strength * POLARIZATION_DISCOUNT
    ...
    try:
        sys.stderr.write(
            ...
            'pot_odds=%.3f gap=%.3f bet_ratio=%.2f reason=fired\n'
            % (round(delta * 1000), made_strength, calibrated_equity,
               pot_odds, equity_gap, bet_ratio))
    except Exception:
        pass
    return delta
