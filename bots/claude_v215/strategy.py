...
from opponent import (
    build_opponent_model, analyze_current_spot,
    classify_archetype, _aggro_bluffcatcher_should_fold,
    _rock_value_bet_fold, _multibarrel_line_fold,
)
...
def choose_anti_lock_pressure_action(
    ...
    # v214 RESTORED: Deep-stack trash-jam self-leak gate (v210 fix lost in v212
    # crossover). Open-jamming all-in preflop with trash when deep-stacked
    # (>15BB) is massively -EV (76.9% empirical loss rate). Convert to
    # limp/fold. Short-stacked (<=15BB) trash jams PRESERVED (fold equity).
    if (round_idx == 0
            and my_cards is not None
            and preflop_strength is not None
            and is_preflop_trash_hand(my_cards, preflop_strength)
            and my_chips > 15 * BIG_BLIND):
        try:
            sys.stderr.write('TRASH_JAM_GATE hand=%s pf=%.2f chips=%d '
                             'tc=%d -> %s\n'
                             % (classify_preflop_hand(my_cards), preflop_strength,
                                my_chips, to_call,
                                'limp' if to_call <= BIG_BLIND else 'fold'))
        except Exception:
            pass
...
def choose_raise(
    ...
    if round_idx == 0 and preflop_strength is not None:
        if spot_name == "sb_open":
            ...
            # CROSSOVER: 0.58->0.48 to match v187 recalibrated strength scale
            ...
            # CROSSOVER: 0.60->0.50 to match v187 recalibrated strength scale
    ...
    # CROSSOVER (imported from v212/v201): SPR commitment gate -- cap marginal-hand
    # raise size so projected SPR-if-called stays >= 4 (pot control), capping
    # -20k stack-off exposure. Premium (>0.70) and trash (<0.50) bypass.
    if round_idx == 0 and preflop_strength is not None:
...
def _should_4bet_light(my_cards, preflop_strength, opponent_model, state, my_chips):
    ...
    # CROSSOVER: estimate_preflop_strength recalibrated to real HU equity (v187).
    # Pairs 0.50-0.85, non-pairs 0.33-0.67. Band [0.30, 0.55) admits small pairs
    # 22(0.50)/33(0.53) as intended set-mine candidates; 0.55 excludes 55+ (0.59+).
...
def get_action(req, requests):
    ...
    # v214: Relocated river stack-off guard BEFORE to_call>=my_chips early-return
    # to resolve the TRUE placement-shadow flagged by the static gate (the old
    # call-site inside `if to_call > 0:` was unreachable for stack-covering
    # all-ins). Guard internals UNCHANGED. The stack-covering all-in fold case
    # is also covered downstream by _spr_commitment_gate (v147).
    ...
    if to_call >= my_chips:
        ...
        if round_idx >= 2 and line_profile['line_label'] == 'value_heavy':
            if (value_profile is None or value_profile['tier'] not in ('strong', 'nut')) and draw_strength < 0.18 and not anti_lock_shove_continue:
                return -1
        # CROSSOVER v195: Multi-barrel line-evidence fold in the stack-covering
        # block. Two-pair bluff-catchers (made~0.40) facing a confirmed multi-barrel
        # all-in from a non-loose opponent fold. Dispatch #1 of 3 (birth-mandate).
    ...
    if to_call > 0:
        # CROSSOVER v195: Aggro-archetype bluff-catcher fold. Kills -20k turn/river
        # call-downs vs value-heavy aggressors. Dispatch #2 of 3 (birth-mandate).
        if round_idx >= 2 and _aggro_bluffcatcher_should_fold(
                opponent_model, round_idx, to_call, pot,
                made_strength, value_profile, draw_strength):
            return -1
        # CROSSOVER v195: Rock-archetype value-bet fold (sibling on separate axis).
        # Rocks bet value-heavy, so weak made bluff-catchers facing their bets are -EV.
        if round_idx >= 2 and _rock_value_bet_fold(
                opponent_model, round_idx, to_call, pot,
                made_strength, value_profile, draw_strength):
            return -1
        # CROSSOVER v195: Multi-barrel line-evidence fold. Keys on DIRECT in-hand
        # history (opponent bet prior street AND this street). Dispatch #3 of 3.
        if round_idx >= 2 and _multibarrel_line_fold(
                opponent_model, spot_info, round_idx, to_call, pot,
                made_strength, value_profile, draw_strength):
            return -1
        if round_idx == 0:
            ...
            # v177: Weak one-pair river margin — pot_odds-scaled call-margin for 0.20-0.55 band
            call_margin += _weak_one_pair_river_margin(
