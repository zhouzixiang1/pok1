...
# ============================================================================
# CROSSOVER from v195: Archetype-aware postflop fold discipline.
# These functions give v195 its edge over v214 (H2H 0.60). They provide
# direct fold paths for marginal bluff-catchers facing value-heavy barrels,
# attacking the #1 documented multi-site -20k stack-off leak.
# ============================================================================

def classify_archetype(om):
    """CROSSOVER v195: Classify opponent into a strategic archetype bucket.
    ...
    (no hard thresholds) so classification degrades gracefully with thin data.
    """
...
def _opp_bluff_prone(om):
    """CROSSOVER v195: detect opponents whose bets include many bluffs.

    Suppresses bluff-catcher folds vs these opponents — calling wider is +EV.
    Fires if EITHER (low large_bet_ratio + high postflop_aggr) barreling profile
    OR loose-caller profile (low fold-to-bet) holds.
    """
...
def _aggro_bluffcatcher_should_fold(om, round_idx, to_call, pot,
    ...
    """CROSSOVER v195: Bluff-catcher fold discipline vs confirmed aggro archetype.

    Vs a value-heavy aggressor, a bluff-catcher (weak one-pair) only beats bluffs.
    Aggro opponents under-deliver bluffs -> the call is -EV. Folds when pot-odds
    demand >7% more equity than the hand has. Protected by bluff-awareness guard.

    Fires ONLY on turn/river (round_idx>=2) + NOT bluff-prone + aggro archetype
    (conf>=0.12) + bluff-catcher band (made 0.20-0.50) + no draw + tier not
    nut/strong + bet is significant (pot_odds >= 0.33).
    """
...
def _rock_value_bet_fold(om, round_idx, to_call, pot,
                          made_strength, value_profile, draw_strength):
    """CROSSOVER v195: Thin-value-call suppression vs confirmed rock archetype.

    Rocks (tight, high fold-to-raise, low aggression) only bet/raise with strong
    hands. A marginal made hand facing a rock's aggression is dominated. COMPLEMENT
    of _aggro_bluffcatcher_should_fold on a separate archetype axis.

    Fires ONLY on turn/river + NOT bluff-prone + rock archetype (conf>=0.12) +
    marginal band (made 0.20-0.50) + no draw + tier not nut/strong + significant bet.
    """
    if round_idx < 2 or to_call <= 0:
        return False
    if _opp_bluff_prone(om):
...
def _multibarrel_line_fold(om, spot_info, round_idx, to_call, pot,
                            made_strength, value_profile, draw_strength):
    """CROSSOVER v195: Line-evidence multi-barrel bluff-catcher fold.

    Keys on DIRECT IN-HAND HISTORY EVIDENCE (not archetype): opponent bet a prior
    postflop street AND bets this street — confirmed multi-barrel line. No archetype
    dependency, so fires in the 'standard' case where archetype siblings are dead.

    MUTATION (v215 crossover): made_strength ceiling raised 0.42 -> 0.48 (~14%) to
    cover the full documented over-call leak band (0.20-0.45 per experience pool),
    catching strong two-pair variants (made ~0.40-0.46) that escape the original gate.
    ...
    Fires when ALL hold:
      ...
      - made_strength < 0.48 (MUTATION: was 0.42; catches two-pair + thin trips)
      - tier=='nut' NEVER folds; tier=='strong' folds ONLY on TWO-PAIR band
        (made >= 0.35); protected on ONE-PAIR band (made < 0.35)
      - draw_strength < 0.15 (no live draw)
      - pot_odds >= 0.33 (bet >= ~half-pot — significant)
    """
    if round_idx < 2 or to_call <= 0:
        return False
    ftb = om.get('fold_to_bet')
    ...
    if spot_info.get('opp_current_round_bet_count', 0) < 1:
        return False
    # MUTATION: ceiling raised from 0.42 -> 0.48
