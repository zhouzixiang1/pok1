...
def _estimate_bluff_frequency(om):
    ...
    # v221 CROSSOVER (port v195's betsize_polarity fold-side calibration).
    # An underbettor's bets are CONDENSED thin-value (small sizing with
    # medium-strength hands), NOT polarized nuts-or-air. The standard polarized
    # discount (0.40+0.25=0.65) UNDERESTIMATES our bluff-catcher equity vs such a
    # condensed range. Flooring bluff_freq at 0.30 raises the discount to 0.70 ->
    # higher calibrated_equity -> less aggressive folding in the fold gates
    # (_allin_polarized_equity_fold / _river_potodds_equity_margin). This is the
    # fold-side PAYOFF for the v193 _opp_betsize_polarity structural axis, ported
    # verbatim from v195. Confidence gate (>=0.25 ~= n>=4 betsize samples per
    # _opp_betsize_polarity) prevents misclassification from tiny samples.
