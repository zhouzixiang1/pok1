...
def _preflop_shove_defense_fold(my_cards, preflop_strength, state, spot_info, opponent_model=None):
    ...
    Mid pairs (77-99) continue only with cheap odds (pot_odds < 0.42).
    Everything else folds. Returns True=fold, False=continue.

    v221 MUTATION: pot_odds continuation threshold 0.35 -> 0.42 (+20%). v195
    lacks this gate entirely (calls down with everything) and beats v204's
    nemesis v206 (WR 0.525 vs 0.350) -- a preflop-sizing bot that raises big
    preflop. Relaxing the mid-pair continuation threshold lets 77-99 continue
    against moderately-priced preflop aggression (pot_odds 0.35-0.42), gaining
    v195's preflop call-down edge. The threshold stays conservative enough that
    expensive all-ins still fold (preserving value-heavy-shover defense vs
    v196/v200/v212), and mid-pairs retain ~35-45% equity vs a balanced shove
    range, making pot_odds up to ~0.42 approximately break-even.
    """
    ...
    if hand_cat == 'mid_pair' and pot_odds < 0.42:
        return False
    return True
