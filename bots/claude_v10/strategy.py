...
# --- Crossover from Parent B (v2): 3bet/4bet dedicated preflop logic ---
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile):
    ...
    # Crossover from v2: 3bet logic when BB facing a preflop raise
    if spot_info["preflop_spot"] == "bb_vs_raise":
        fold_to_raise = opponent_model.get("fold_to_raise", 0.44)

        # Value 3bet: strong hands (JJ+, AK, AQs)
        ...
        if preflop_strength >= call_threshold:
            return 0
        # Fold weak hands facing big raises
        if preflop_strength < 0.35 and to_call > BIG_BLIND * 3:
            return -1
        return 0

    # Crossover from v2: 4bet logic when SB facing a 3bet
...
def get_action(req, requests):
    ...
    if round_idx == 0 and preflop_strength is not None:
        ...
        if spot_action is not None:
            if anti_lock_pressure and spot_action <= 0:
                if not is_preflop_trash_hand(my_cards, preflop_strength):
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            # --- Mutation: CBet-aware postflop call adjustments ---
            # Wire opponent CBet rate into decision: if opp CBets air frequently,
            # we're more willing to call; if opp only CBets strong, fold more.
            cbet_rate = opponent_model.get("cbet_rate", 0.55)
            if round_idx == 1 and spot_info["facing_postflop_aggression"]:
                if cbet_rate > 0.65:
                    # Opponent CBets air frequently -> more willing to call down
                    call_margin -= 0.02
                elif cbet_rate < 0.40:
    ...
    if big_pot and round_idx == 3 and (value_profile is None or value_profile["tier"] not in ("strong", "nut")):
        if blocker_profile is None or not blocker_profile["eligible"]:
            return 0
    # Thin static showdown: don't bluff with thin value on dry boards
    ...
    # Mutation: increase probe frequency when opponent has high fold_to_cbet
    # (opponent folds to flop bets a lot -> probe bet more frequently)
    fold_to_cbet = opponent_model.get("fold_to_cbet", 0.40)
    if fold_to_cbet > 0.55 and round_idx == 1 and made_strength < 0.40 and draw_strength < 0.12:
        if not small_probe:
            small_probe = opponent_model["confidence"] >= 0.20 and opponent_model["fold_to_raise"] > probe_fold_threshold + 0.05
