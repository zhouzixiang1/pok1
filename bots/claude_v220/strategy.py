...
from opponent import build_opponent_model, analyze_current_spot, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold, _river_potodds_equity_margin, _allin_polarized_equity_fold
...
def _value_lead_upsizing_delta(round_idx, to_call, made_strength, value_profile,
    ...
    # v198 NEW: calling-station arm. Gates on value_maximizer_index (calling
    # stickiness) instead of fold_to_bet. Only fires for value hands (made >= 0.45);
    # bluffs vs calling stations must NOT be up-sized. OR-gated with fit_or_fold
    # so the function fires for EITHER archetype.
    calling_station_factor = 0.0
    if made_strength >= 0.45:
        _vmi = opponent_model.get('value_maximizer_index', 0.43) if opponent_model else 0.43
        calling_station_factor = clamp((_vmi - 0.50) / 0.25, 0.0, 1.0)
    # v199 NEW: betsize_polarity arm — wires v193's INERT _opp_betsize_polarity
    # into the LIVE value-sizing path. Underbettor (small_rate>=0.45) = condensed
    # thin-value range -> CAPPED on later streets -> bot value relatively
    # stronger -> UP-size. Standard = baseline calling opp -> moderate up-size.
    # Overbettor (polarized nuts-or-air) -> NO up-size (avoid value-owning vs
    # nuts). Captures RANGE-CAP signal from bet-size MAGNITUDE, orthogonal to
    # fold_to_bet (fit_or_fold) and value_maximizer_index (calling_station).
    # M5/M6 compliant: fires for the ~97% 'standard' majority at live-pool
    # defaults (non-zero delta), stronger for underbettor, excluded for overbettor.
    _polarity = opponent_model.get('betsize_polarity', {}) if opponent_model else {}
    _pol_tendency = _polarity.get('tendency', 'unknown')
    _pol_conf = _polarity.get('confidence', 0.0)
    polarity_factor = 0.0
    if _pol_tendency == 'underbettor' and _pol_conf >= 0.15:
        polarity_factor = clamp((_pol_conf - 0.15) / 0.30, 0.0, 1.0)
    elif _pol_tendency == 'standard' and _pol_conf >= 0.25:
        polarity_factor = 0.5 * clamp((_pol_conf - 0.25) / 0.30, 0.0, 1.0)
    # 'overbettor' and 'unknown' -> polarity_factor stays 0.0 (no up-size).
    effective_factor = max(fit_or_fold_factor, calling_station_factor, polarity_factor)
    ...
    delta = 0.25 * effective_factor * (0.40 + 0.60 * dryness) * value_factor * confidence_gate
    delta = clamp(delta, 0.0, 0.30)

    try:
        sys.stderr.write(
            'VALUE_UPSIZING delta_milli=%+d round=%d made=%.2f tier=%s '
            'ftb=%.3f conf=%.2f fof=%.2f csf=%.2f pol=%.2f(pten=%s,pc=%.2f) '
            'dry=%.2f vf=%.2f\n'
            % (round(delta * 1000), round_idx, made_strength, tier,
               fold_to_bet, confidence, fit_or_fold_factor, calling_station_factor,
               polarity_factor, _pol_tendency, _pol_conf,
               dryness, value_factor)
        )
    except Exception:
        pass
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        if _river_weak_made_hand_gate(round_idx, pot, my_chips, made_strength,
            ...
            print(f"RIVER_WEAK_FOLD made={made_strength:.2f} "
                  ...
                  file=sys.stderr)
            return -1
        # v220 CROSSOVER (ported from v219/v202): calibrated-equity fold for
        # marginal one-pair vs polarized all-in. win_rate (monte_carlo vs random)
        # overestimates one-pair equity vs a polarized all-in value range; this
        # uses calibrated_equity = made*(0.40+bluff_freq) and folds when below
        # jam_odds. VPIP-aware mutation: folds LESS vs wide shovers (VPIP>0.60).
    ...
    if to_call > 0:
        # v220 CROSSOVER (ported from v219/v200): DIRECT river pot-odds-vs-equity
        # fold. v197 had removed ALL direct fold gates here to stop over-folding vs
        # wide-barreling opps, relying only on the margin-additive dispatch inside
        # _postflop_response_margin. But that is too weak vs POLARIZED value ranges
        # (monte_carlo realized_rate overestimates weak-made equity). v219 BEATS the
        # value-heavy shovers v199 loses to (v212/v198/v205/v204) precisely because
        # it folds directly when calibrated_equity (made*discount) < pot_odds. The
        # graduated tier exemption (ported above) protects true value (nut/two-pair+/
        # overpair AA-KK), so only TPTK-and-worse fold — no over-fold vs wide barrelers.
        if round_idx == 3:
    ...
    if donk["eligible"]:
        ...
        _donk_station_delta = _opponent_sizing_raise_boost(opponent_model, round_idx, made_strength, value_profile)
    ...
    if probe["eligible"]:
        ...
        _probe_station_delta = _opponent_sizing_raise_boost(opponent_model, round_idx, made_strength, value_profile)
    ...
    _station_sizing_delta = _opponent_sizing_raise_boost(opponent_model, round_idx, made_strength, value_profile)
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")) or exploit['should_barrel']:
        ...
        _tp_delta_main = _turn_probe_sizing(
            round_idx, to_call, spot_info, opponent_model,
            made_strength, value_profile, board_texture, draw_strength)
        # v198: wire _value_lead_upsizing_delta into the MAIN value-raise path
        # (was only on donk/probe paths). Lifts value bet sizing vs calling
        # stations + fit-or-fold opponents on the to_call==0 betting-lead line.
        _main_value_upsizing = _value_lead_upsizing_delta(
            round_idx, to_call, made_strength, value_profile,
            board_texture, opponent_model)
        raise_amount = choose_raise(
            state["min_raise_action"],
            ...
            value_upsizing_delta=_main_value_upsizing,
        )
        if raise_amount is not None:
...
def _v199_polarity_selftest():
    _bt, _vp = {'wetness': 0.5}, {'tier': 'strong'}
    _base = {'confidence': 0.30, 'fold_to_bet_turn': 0.44, 'value_maximizer_index': 0.43}
    _pol = lambda t: dict(_base, betsize_polarity={'tendency': t, 'confidence': 0.50})
    d_std = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _pol('standard'))
    d_under = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _pol('underbettor'))
    d_over = _value_lead_upsizing_delta(2, 0, 0.60, _vp, _bt, _pol('overbettor'))
    assert d_std > 0.0 and d_under > d_std and d_over < d_std, 'polarity ordering'
    print('v199 polarity self-test PASS: std=%.4f under=%.4f over=%.4f' % (d_std, d_under, d_over))

if __name__ == '__main__':
    _v199_polarity_selftest()
