    ...
    for req in hand_requests:
        ...
        for record in history:
            ...
            if action_type == "raise" and action > 0:
                ...
                high_bet = max(round_bets.values())
                round_bets[pid] = max(round_bets[pid], high_bet)
                # v199 FIX: record opponent allin in betsize-polarity samples
                # (was missing -> shove_rate permanently 0, misclassifying
                # shove-heavy opps as 'standard'). action=-2 hides exact amount;
                # ratio=2.0 sentinel (>=1.0 overbet threshold) is safe — only
                # the is_allin=True flag increments shove_rate; the ratio value
                # feeds only unread avg_fraction (v196 disproved corruption).
                if pid == opponent_id and round_idx > 0:
                    _betsize_magnitude_samples.append((round_idx, 2.0, True))
...
def _river_potodds_equity_margin(round_idx, made_strength, draw_strength,
    ...
    #    v220 CROSSOVER (ported from v219/v204): graduated (was blanket
    #    'strong','nut' -> INERT for top-pair-JK+). nut + two-pair+ (made>=0.35)
    #    + overpair AA/KK stay exempt; top-pair-JK 'strong' one-pairs foldable.
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return 0.0
        if _tier == 'strong':
            if made_strength >= 0.35:
                return 0.0  # two-pair+ genuinely strong
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
                return 0.0  # overpair AA/KK protected
            # top_pair JK+ good kicker: NOT exempt - foldable vs polarized value
    ...
    #    continues. v220 CROSSOVER: restore TARGET_THRESHOLD = 0.48 (v197 had
    #    lowered to 0.42 to reduce over-folding, but the graduated tier exemption
    #    above now protects true value hands, so the lower threshold is no longer
    #    needed and 0.48 is the validated value that lets TPTK fold vs polarized
    #    value bets — the edge v219 holds over v199 vs value-heavy shovers).
...
def _allin_polarized_equity_fold(round_idx, made_strength, draw_strength,
    ...
    hands) which overestimates equity for one-pair vs a polarized all-in value
    range. This uses calibrated_equity = made*(0.40+bluff_freq) as a realistic
    equity proxy and folds when it falls below jam_odds.

    Targets one-pair (tier none/thin, made<0.50) all-in call-downs. Two-pair+
    (made>=0.35 strong / nut) and overpair AA/KK EXEMPT. Extends to turn/river.

    v220 MUTATION (experience pool: 'vs WIDE open-shovers VPIP>0.60 -> fold LESS;
    premiums-only exploitable; folding 55 @pot_odds~0.50 is -EV'): wide shovers
    jam many worse hands, so medium one-pairs (made 0.35-0.50) have positive
    call equity vs their RANGE. Narrow the fold ceiling from 0.50 to 0.35 when
    VPIP>0.60 so we only fold the very weakest, preserving v199's robust
    call-down vs wide aggression while still folding trash vs value-heavy shovers.
    """
    if round_idx < 2 or anti_lock_pressure:
        return False
    if jam_odds <= 0.0:
        return False
    if made_strength < 0.20:
        return False
    # v220 MUTATION: VPIP-aware fold ceiling.
    _vpip = opponent_model.get('vpip', 0.58) if opponent_model else 0.58
    _fold_ceiling = 0.35 if _vpip > 0.60 else 0.50
    if made_strength >= _fold_ceiling:
        return False
    # Graduated tier exemption: nut + two-pair+ (made>=0.35) + overpair AA/KK
    # stay exempt; top-pair-JK 'strong' one-pairs become foldable.
    if value_profile is not None:
        _tier = value_profile.get('tier')
        if _tier == 'nut':
            return False
        if _tier == 'strong':
            if made_strength >= 0.35:
                return False  # two-pair+ genuinely strong
            if pair_profile is not None and pair_profile.get('pair_type') == 'overpair':
                return False  # overpair AA/KK protected
...
if __name__ == '__main__':
    ...
    # v199 FIX verification: allin samples must now classify as overbettor
    _bsp_shove = [(1, 0.25, False), (2, 0.20, False), (3, 0.30, False),
                  (1, 2.0, True), (2, 2.0, True)]
    _bsp_shove_r = _opp_betsize_polarity(_bsp_shove)
    assert _bsp_shove_r['shove_rate'] > 0.0, 'allin samples must increment shove_rate (v199 fix)'
    assert _bsp_shove_r['tendency'] == 'overbettor', 'shove-heavy must be overbettor'
    print('v199 allin-recording fix self-test PASS')
