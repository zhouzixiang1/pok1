...
def _opponent_sizing_call_tighten(spot_info, opponent_model, round_idx, has_position):
    """v166: Continuous call-margin delta from opponent bet-sizing pattern.
    Tightens when facing large bets from large-bet-heavy opponents.
    M5/M6: continuous, non-zero for standard bucket, unconditional telemetry.
    """
    if round_idx <= 0:
        return 0.0
    ...
    if confidence < 0.10:
        return 0.0
    large_ratio = opponent_model.get('large_bet_ratio', 0.32)
    delta = confidence * (large_ratio - 0.28) * 0.12
    if round_idx == 3:
        delta *= 1.4
    if bet_ratio >= 0.80:
        delta *= 1.2
    delta = clamp(delta, 0.0, 0.045)
    try:
        sys.stderr.write(
            'SIZING_TELL_ADJ delta_milli=%+d large_ratio=%.3f conf=%.2f '
            'bet_ratio=%.2f round=%d reason=%s\n'
            % (round(delta * 1000), large_ratio, confidence, bet_ratio,
               round_idx, 'fired' if delta > 0 else 'below_threshold')
        )
    except Exception:
        pass
    return delta
...
def _weak_one_pair_river_margin(round_idx, made_strength, draw_strength,  # v177
                                 spot_info, pot_odds):
    if (round_idx != 3 or not spot_info.get('facing_postflop_aggression', False)
            or made_strength < 0.20 or made_strength >= 0.55
            or draw_strength >= 0.15):
        return 0.0
    delta = clamp(0.18 * clamp((0.55 - made_strength) / 0.35, 0.0, 1.0)
                  * clamp(pot_odds / 0.25, 0.8, 1.5), 0.0, 0.06)
    try:  # telemetry: unconditional, M5/M6 compliant
        sys.stderr.write('WOP_MARGIN delta_milli=%+d made=%.2f pot_odds=%.3f draw=%.2f round=%d reason=%s\n' % (round(delta * 1000), made_strength, pot_odds, draw_strength, round_idx, 'fired' if delta > 0 else 'threshold'))
    except Exception: pass
    return delta

def _river_reraise_tighten(state, spot_info, made_strength, value_profile,
                            round_idx, board_texture, pair_profile):
    """v169 + v176 CROSSOVER MUTATION: Continuous call-margin delta for
    marginal-to-strong-but-not-nut hands facing polarized river re-raises.

    v176 crossover mutation (v172×v169): widened the made-strength band from
    0.55-0.80 down to 0.40-0.80. The experience pool identifies POSTFLOP
    over-calling vs value raises as the DOMINANT exploitable weakness (WR 0.20
    vs v169, 0.30 vs v165). The 0.40-0.55 band (weak one-pair / marginal
    two-pair) was completely exempt from this defense — now covered.

    NOT a binary fold gate (FOLD-SIDE RULE compliant): returns continuous
    delta [0, +0.08] added to call_margin.

    State keys verified in state.py reconstruct_state():
      - state['round_raise']   = last_raise_to (max raise-to-total this street)
      - state['my_round_bet']  = bot's own contribution this street
    When the bot raised first to X and opp re-raised to Y > X, at the
    decision point my_round_bet == X and round_raise == Y, so ratio = Y/X.
    """
    # 1. Street gate: river only
    if round_idx != 3:
        return 0.0
    # 2. Must be facing a raise
    if not spot_info.get('facing_postflop_aggression', False):
        return 0.0
    # 3. Hand-strength gate: marginal-to-strong-but-not-nut (0.40 <= made < 0.80)
    #    v176: lowered from 0.55 to 0.40 to cover the over-calling leak band.
    if made_strength < 0.40 or made_strength >= 0.80:
        return 0.0
    if value_profile is not None and value_profile.get('tier') == 'nut':
        return 0.0
    # 4. Board risk gate: only fire on boards with nutted risk
    board_risk = False
    if board_texture is not None:
        if board_texture.get('paired', False):
            board_risk = True
        if board_texture.get('flush_pressure', 0.0) >= 0.50:
            board_risk = True
        if board_texture.get('straight_pressure', 0.0) >= 0.50:
            board_risk = True
    if not board_risk:
        return 0.0
    # 5. Re-raise ratio gate: compute from state.
    #    state['round_raise'] = max raise-to-total this street (updated on each raise).
    #    state['my_round_bet'] = bot's own contribution this street.
    #    ratio = opp_raise / bot_raise when opp re-raised after bot raised.
    bot_bet = state.get('my_round_bet', 0)
    if bot_bet <= BIG_BLIND:
        return 0.0  # not a re-raise situation
    round_raise = state.get('round_raise', 0)
    if round_raise <= bot_bet:
        return 0.0  # opponent called or min-raised, not a re-raise
    ratio = round_raise / bot_bet
    if ratio < 2.0:
        return 0.0  # standard raise, not polarized
    # 6. Continuous delta: linear from 0.0 at ratio=2.0 to 0.08 at ratio=3.0+
    delta = 0.08 * clamp((ratio - 2.0) / 1.0, 0.0, 1.0)
    # 7. Scale by made_strength: lower hands (0.40-0.55) more vulnerable
    #    than higher (0.70-0.80). v176: band widened from 0.55-0.80 to 0.40-0.80.
    strength_factor = 1.0 - clamp((made_strength - 0.40) / 0.40, 0.0, 1.0)
    delta *= (0.60 + 0.40 * strength_factor)
    try:
        sys.stderr.write(
            'RERAISE_TIGHTEN ratio=%.2f made=%.2f tier=%s delta_milli=%+d '
            'board_risk=%s reason=%s\n'
            % (ratio, made_strength,
               value_profile.get('tier', 'none') if value_profile else 'none',
               int(delta * 1000), board_risk,
...
def _emit_turn_probe_telemetry(delta, reason, round_idx, to_call, flop_aggr, confidence):
    """v170 M6: Hoisted telemetry at function scope — prints unconditionally
    so the daemon can verify firing via reachability_test (NO `if delta != 0`
    gate, per M5 INERTNESS HARD RULE).
    """
    try:
        sys.stderr.write(
            'TURN_PROBE_SIZING delta_milli=%+d reason=%s round=%d to_call=%d '
            'flop_aggr=%.3f conf=%.2f\n'
            % (round(delta * 1000), reason, round_idx, to_call,
               flop_aggr, confidence)
        )
    except Exception:
        pass


def _turn_probe_sizing(round_idx, to_call, spot_info, opponent_model,
                       made_strength, value_profile, board_texture,
                       draw_strength=0.0):
    """v170 NEW OFFENSE: Turn probe sizing delta.

    FIRST turn-probe-sizing module in the entire lineage (0 prior generations).
    Fires on the turn (round_idx==2) when to_call==0 and the opponent showed
    weakness (checked to us, or has low flop aggression indicating frequent
    flop check-backs). Returns a CONTINUOUS additive sizing delta [+0.10, +0.22]
    that lifts the bot's turn raise from the 0.40x baseline toward 0.55-0.65x
    pot — denying free cards to draws and extracting value from capped ranges.

    M5/M6 compliance: continuous delta (no deadzone gap), unconditional
    telemetry at function scope, self-test fixture at bottom of file.

    Gates use OR (not AND) to minimize inertness risk per experience pool.
    """
    delta = 0.0
    reason = 'not_turn_probe'

    # Gate 1: turn only, no bet to call.
    if round_idx != 2 or to_call != 0:
        _emit_turn_probe_telemetry(delta, reason, round_idx, to_call, 0.0, 0.0)
        return delta

    if opponent_model is None:
        _emit_turn_probe_telemetry(delta, 'no_model', round_idx, to_call, 0.0, 0.0)
        return delta

    confidence = opponent_model.get('confidence', 0.0)

    # Gate 2: opponent showed weakness.
    # Primary signal: opponent checked to us this street (direct action, not
    # smoothed — avoids the smooth_rate prior-saturation inertness trap).
    # Secondary signal: opponent has low flop aggression (checks back flop often).
    opp_checked_this_street = spot_info.get('last_opp_action_type') == 'check'
    flop_aggr = opponent_model.get('flop_aggr',
                                   opponent_model.get('postflop_aggr', 0.36))
    opp_checks_flop_often = flop_aggr < 0.42

    if not (opp_checked_this_street or opp_checks_flop_often):
        _emit_turn_probe_telemetry(delta, 'no_weakness', round_idx, to_call,
                                    flop_aggr, confidence)
        return delta

    # Gate 3: must have a bettable hand (thin value or viable draw).
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    has_thin_or_better = tier in ('thin', 'strong', 'nut') or made_strength >= 0.30
    has_viable_draw = draw_strength >= 0.10

    if not has_thin_or_better and not has_viable_draw:
        _emit_turn_probe_telemetry(delta, 'hand_too_weak', round_idx, to_call,
                                    flop_aggr, confidence)
        return delta

    # CONTINUOUS delta — NO deadzone, NO thresholded elif chain.
    # Strength factor: 0 at made_strength=0.30, 1 at made_strength=0.62.
    strength_factor = clamp((made_strength - 0.30) / 0.32, 0.0, 1.0)
    # Base delta [0.10, 0.18] scaled by hand strength.
    delta = 0.10 + 0.08 * strength_factor

    # Board wetness bonus: wetter boards = deny draws harder (up to +0.03).
    wetness = board_texture.get('wetness', 0.0) if board_texture else 0.0
    delta += 0.03 * wetness

    # Draw bonus: if we have a draw ourselves, size up slightly to build pot.
    if has_viable_draw and not has_thin_or_better:
        delta += 0.02

    # Confidence scaling: with thin opponent data, use 70% of delta (prior-weighted)
    # rather than hard-blocking — keeps the module LIVE even with few samples.
    if confidence < 0.15:
        delta *= 0.70

    delta = clamp(delta, 0.10, 0.22)
    reason = 'turn_probe_fired'
    _emit_turn_probe_telemetry(delta, reason, round_idx, to_call, flop_aggr, confidence)
    return delta


def _opponent_sizing_raise_boost(opponent_model, round_idx,
                                 made_strength=None, value_profile=None):
    """v171/v198: DOWN-sizing delta vs calling-stations on flop/turn.
    v198 FIX: value hands (made>=0.55 or tier strong/nut) SKIP the down-size —
    calling stations pay off with worse, so value bets should be BIGGER, not
    smaller. Only down-size marginal/bluff bets (pot control vs stations).
    M5/M6: continuous (no deadzone), OR-gate, telemetry at function scope.
    """
    delta = 0.0
    reason = 'not_eligible'
    vmi = 0.0
    ...
    if round_idx in (1, 2) and opponent_model is not None:
        confidence = opponent_model.get('confidence', 0.0)
        vmi = opponent_model.get('value_maximizer_index', 0.43)
        street_key = {1: 'fold_to_bet_flop', 2: 'fold_to_bet_turn'}[round_idx]
        fold_to_bet = opponent_model.get(street_key, 0.44)

        # CONTINUOUS station factor -- OR-gate of two independent signals
        vmi_factor = clamp((vmi - 0.40) / 0.30, 0.0, 1.0)
        fold_factor = clamp((0.40 - fold_to_bet) / 0.15, 0.0, 1.0)
        station_factor = max(vmi_factor, fold_factor)

        if station_factor > 0.0:
            delta = -0.12 * station_factor
            if confidence < 0.15:
                delta *= 0.70
            delta = clamp(delta, -0.12, -0.01)
            reason = 'station_downsize'
        else:
            reason = 'not_station'
    elif opponent_model is None:
        reason = 'no_model'

    # v198 FIX: value hands (made>=0.55 / tier strong,nut) SKIP the down-size —
    # calling stations pay worse hands, so value bets must be BIGGER not smaller.
    _is_value = (
        (made_strength is not None and made_strength >= 0.55)
        or (value_profile is not None and value_profile.get('tier') in ('strong', 'nut'))
    )
    if _is_value and delta < 0.0:
        delta = 0.0
        reason = 'value_hand_skip'

    try:
        sys.stderr.write(
            'STATION_SIZING delta_milli=%+d reason=%s round=%d vmi=%.3f '
            'ftb=%.3f conf=%.2f\n'
            % (round(delta * 1000), reason, round_idx, vmi,
               fold_to_bet, confidence)
        )
    except Exception:
        pass
...
if __name__ == "__main__":
    ...
    # v166 _opponent_sizing_call_tighten self-test: 6 scenarios.
    _std_opp = {'confidence': 0.50, 'large_bet_ratio': 0.32}
    _high_large_opp = {'confidence': 0.50, 'large_bet_ratio': 0.45}
    _facing = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.75}
    _no_aggr = {'facing_postflop_aggression': False, 'last_raise_pot_ratio': 0.75}
    _small_bet = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.30}
    _pos = True
    # 1. Standard pool model + large bet + river -> delta > 0
    _d1 = _opponent_sizing_call_tighten(_facing, _std_opp, 3, _pos)
    assert _d1 > 0, f'v166 std+riv: expected >0, got {_d1}'
    # 2. High large-bet model -> delta exceeds standard
    _d2 = _opponent_sizing_call_tighten(_facing, _high_large_opp, 3, _pos)
    assert _d2 > _d1, f'v166 high_large: expected >{_d1}, got {_d2}'
    # 3. Small bet -> delta == 0.0
    _d3 = _opponent_sizing_call_tighten(_small_bet, _std_opp, 3, _pos)
    assert _d3 == 0.0, f'v166 small_bet: expected 0.0, got {_d3}'
    # 4. No aggression -> delta == 0.0
    _d4 = _opponent_sizing_call_tighten(_no_aggr, _std_opp, 3, _pos)
    assert _d4 == 0.0, f'v166 no_aggr: expected 0.0, got {_d4}'
    # 5. River boost >= turn delta
    _d_turn = _opponent_sizing_call_tighten(_facing, _std_opp, 2, _pos)
    assert _d1 >= _d_turn, f'v166 river>=turn: riv={_d1} turn={_d_turn}'
    # 6. Preflop -> 0.0
    _d6 = _opponent_sizing_call_tighten(_facing, _std_opp, 0, _pos)
    assert _d6 == 0.0, f'v166 preflop: expected 0.0, got {_d6}'
    print(f'v166 _opponent_sizing_call_tighten: 6/6 PASS')

    # v169+v176 _river_reraise_tighten self-test: 5 scenarios (v176 added marginal band).
    _rt_state_high = {'my_round_bet': 300, 'round_raise': 900}  # ratio=3.0
    _rt_state_low = {'my_round_bet': 300, 'round_raise': 550}   # ratio=1.83
    _rt_state_med = {'my_round_bet': 300, 'round_raise': 700}   # ratio=2.33
    _rt_bt_paired = {'paired': True, 'flush_pressure': 0.0, 'straight_pressure': 0.0}
    _rt_bt_safe = {'paired': False, 'flush_pressure': 0.0, 'straight_pressure': 0.0}
    _rt_vp_strong = {'tier': 'strong'}
    _rt_vp_nut = {'tier': 'nut'}
    _rt_vp_thin = {'tier': 'thin'}
    _rt_facing = {'facing_postflop_aggression': True}
    _rt_not_facing = {'facing_postflop_aggression': False}

    # Scenario 1: high re-raise ratio (3.0x) + paired + strong hand → should fire
    _d1 = _river_reraise_tighten(_rt_state_high, _rt_facing, 0.62, _rt_vp_strong, 3, _rt_bt_paired, None)
    assert _d1 > 0, f'high ratio + paired + strong should fire: {_d1}'
    # Scenario 2: low ratio (1.83x) → should not fire (below 2.0 threshold)
    _d2 = _river_reraise_tighten(_rt_state_low, _rt_facing, 0.62, _rt_vp_strong, 3, _rt_bt_paired, None)
    assert _d2 == 0.0, f'low ratio should not fire: {_d2}'
    # Scenario 3: nut hand → exempt
    _d3 = _river_reraise_tighten(_rt_state_high, _rt_facing, 0.90, _rt_vp_nut, 3, _rt_bt_paired, None)
    assert _d3 == 0.0, f'nut hand should be exempt: {_d3}'
    # Scenario 4: safe board → should not fire
    _d4 = _river_reraise_tighten(_rt_state_high, _rt_facing, 0.62, _rt_vp_strong, 3, _rt_bt_safe, None)
    assert _d4 == 0.0, f'safe board should not fire: {_d4}'
    # Scenario 5 (v176): marginal hand (made=0.45, previously below 0.55 gate) now fires
    _d5 = _river_reraise_tighten(_rt_state_high, _rt_facing, 0.45, _rt_vp_thin, 3, _rt_bt_paired, None)
    assert _d5 > 0, f'v176 marginal 0.45 should fire with widened band: {_d5}'
    # Scenario 6 (v176): very weak hand (made=0.35) still exempt
    _d6 = _river_reraise_tighten(_rt_state_high, _rt_facing, 0.35, _rt_vp_thin, 3, _rt_bt_paired, None)
    assert _d6 == 0.0, f'made=0.35 below 0.40 floor should not fire: {_d6}'
    print(f'_river_reraise_tighten self-test PASS: high={_d1:.4f} low={_d2} nut={_d3} safe={_d4} marginal={_d5:.4f} too_weak={_d6}')

    # v170 _turn_probe_sizing self-test: non-zero at live-pool defaults (M5), 0.0 if ineligible.
    _test_tp_spot = {'last_opp_action_type': 'check'}
    _test_tp_model = {'confidence': 0.15, 'flop_aggr': 0.36}
    _test_tp_vp = {'tier': 'thin'}
    _test_tp_bt = {'wetness': 0.25}
    _tp_delta = _turn_probe_sizing(2, 0, _test_tp_spot, _test_tp_model, 0.45, _test_tp_vp, _test_tp_bt, 0.05)
    assert _tp_delta >= 0.10, ('TURN_PROBE expected >=0.10, got %.4f' % _tp_delta)
    assert _tp_delta <= 0.22, ('TURN_PROBE expected <=0.22, got %.4f' % _tp_delta)
    assert _turn_probe_sizing(1, 0, _test_tp_spot, _test_tp_model, 0.45, _test_tp_vp, _test_tp_bt, 0.05) == 0.0
    assert _turn_probe_sizing(2, 500, _test_tp_spot, _test_tp_model, 0.45, _test_tp_vp, _test_tp_bt, 0.05) == 0.0
    _tp_strong_opp = {'confidence': 0.30, 'flop_aggr': 0.60}
    assert _turn_probe_sizing(2, 0, {'last_opp_action_type': 'raise'}, _tp_strong_opp, 0.45, _test_tp_vp, _test_tp_bt, 0.05) == 0.0
    print('TURN_PROBE_SIZING self-test PASS: delta=%.4f' % _tp_delta)

    # v171 _opponent_sizing_raise_boost self-test: M5 compliant (non-zero at defaults).
    _test_st_model_def = {'confidence': 0.15, 'value_maximizer_index': 0.43, 'fold_to_bet_turn': 0.44, 'fold_to_bet_flop': 0.44}
    _st_def = _opponent_sizing_raise_boost(_test_st_model_def, 2)
    assert _st_def < 0.0, ('STATION_SIZING expected <0 at defaults, got %.4f' % _st_def)
    assert _st_def >= -0.12, ('STATION_SIZING expected >=-0.12, got %.4f' % _st_def)
    _test_st_model_station = {'confidence': 0.30, 'value_maximizer_index': 0.65, 'fold_to_bet_turn': 0.28, 'fold_to_bet_flop': 0.28}
    _st_station = _opponent_sizing_raise_boost(_test_st_model_station, 2)
    assert _st_station <= -0.05, ('STATION_SIZING expected <=-0.05 for station, got %.4f' % _st_station)
    assert _opponent_sizing_raise_boost(_test_st_model_station, 3) == 0.0  # river excluded
    _test_st_model_folder = {'confidence': 0.30, 'value_maximizer_index': 0.30, 'fold_to_bet_turn': 0.60, 'fold_to_bet_flop': 0.60}
    assert _opponent_sizing_raise_boost(_test_st_model_folder, 2) == 0.0  # not station
    print('STATION_SIZING self-test PASS: default=%.4f station=%.4f' % (_st_def, _st_station))

    # v198 value-hand-skip self-test
    _cs_model = {'confidence': 0.30, 'value_maximizer_index': 0.65, 'fold_to_bet_flop': 0.44}
    assert _opponent_sizing_raise_boost(_cs_model, 1, made_strength=0.60, value_profile={'tier':'strong'}) == 0.0  # value hand skipped
    assert _opponent_sizing_raise_boost(_cs_model, 1, made_strength=0.35, value_profile={'tier':'none'}) < 0.0   # marginal still down-sized
    assert _opponent_sizing_raise_boost(_cs_model, 3, made_strength=0.60) == 0.0  # river unchanged (round_idx not in (1,2))
    print('v198 value-hand-skip self-test: 3/3 PASS')
