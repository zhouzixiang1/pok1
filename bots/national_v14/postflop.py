...
def disciplined_opp_river_margin(opponent_model, value_profile, round_idx):
    ...
    river ranges. Apply a continuous negative call_margin delta, scaled by how far
...
if __name__ == '__main__':
    ...
    # Fixture B — disciplined elite opponent (vpip=0.45, pfr=0.18 at conf=0.9): delta MUST be <0
    elite_om = {"vpip": 0.45, "pfr": 0.18, "confidence": 0.9}
    elite_delta = disciplined_opp_river_margin(elite_om, std_vp, 3)
    assert elite_delta < 0.0, f"disciplined bucket must be <0, got {elite_delta}"
