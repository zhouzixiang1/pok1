...
def passive_opponent_exploit_bonus(opponent_model, to_call, round_idx):
    """Lower raise thresholds when bot has initiative vs passive opponents."""
    confidence = opponent_model["confidence"]
    if confidence < 0.20 or to_call > 0 or round_idx <= 0:
        return 0.0
    bonus = 0.0
    ftr = opponent_model.get("fold_to_raise", 0.44)
    if ftr > 0.48:
        bonus += confidence * (ftr - 0.48) * 0.18
    check_rate = opponent_model.get("postflop_check_rate", 0.42)
    if check_rate > 0.50:
        bonus += confidence * (check_rate - 0.50) * 0.12
    post_aggr = opponent_model.get("postflop_aggr", 0.36)
    if post_aggr < 0.30:
        bonus += confidence * (0.30 - post_aggr) * 0.10
    return clamp(bonus, 0.0, 0.08)
...
def get_action(req, requests):
    ...
    passive_bonus = passive_opponent_exploit_bonus(opponent_model, to_call, round_idx)
    strong -= passive_bonus
    medium -= passive_bonus
