...
def _seeded_deck_factory(hand_num: int):
    ...
    Honors POK_DECK_SEED_BASE so paired-block H2H matches can swap seats while
    sharing deck seeds, isolating skill from card luck. With no env override,
    falls back to the GameEngine default (random shuffle).
    """
    try:
