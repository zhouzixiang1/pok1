"""Regression tests for the one authoritative official raw-stream transport."""

import asyncio

import pytest

from sever.server.transport import NationalProtocolError, NationalTCPClient, pop_client_action
...
async def _receive_fragments(fragments, *, name=False, eof=False):
    ...
    try:
        return await asyncio.wait_for(
            client.recv_name(1.0) if name else client.recv_action(1.0),
            timeout=2.0,
        )
    finally:
        await feeder
...
def test_transport_sends_server_messages_without_newline():
    class MemoryWriter:
        def __init__(self):
            self.payloads = []
    ...
    async def scenario():
        ...
        await client.send_message("flop|<0,1><1,2><2,3>")
        with pytest.raises(NationalProtocolError, match="delimiter"):
            await client.send_message("call\n")
        await client.close()
        return writer.payloads

    assert asyncio.run(scenario()) == [b"flop|<0,1><1,2><2,3>"]
...
def test_transport_does_not_idle_flush_inside_utf8_codepoint():
    encoded = "国赛".encode("utf-8")
    result = asyncio.run(_receive_fragments([
        (encoded[:4], 0.06),  # complete 国 + first byte of 赛
        (encoded[4:], 0.04),
    ], name=True))
    assert result == "国赛"


def test_transport_does_not_treat_newline_as_name_delimiter():
    async def scenario():
        with pytest.raises(NationalProtocolError, match="control_character"):
            await _receive_fragments([(b"Team\n", 0.04)], name=True)
