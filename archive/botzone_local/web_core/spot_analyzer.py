...
def run_bot_scenario(bot_path: str, scenario: dict) -> dict:
    """Run a zero-authority legacy diagnostic inside the candidate sandbox."""
    ...
    entry = Path(bot_path).resolve()
    try:
        result = run_candidate_probe(
            entry.parent,
            _DIAGNOSTIC_BOT_PROBE,
            args=(entry.name,),
            input_text=payload,
            timeout=10,
        )
        ...
        return {
            ...
            "error": "timeout",
            "raw_stdout": "",
            "evidence_authority": "diagnostic_zero_authority",
        }
    except CandidateSandboxError as exc:
        return {
            "action": None,
            "error": f"sandbox infrastructure failure: {exc}",
            "raw_stdout": "",
            "evidence_authority": "diagnostic_zero_authority",
        }
