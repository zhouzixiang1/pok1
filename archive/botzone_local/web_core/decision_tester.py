...
def run_single_scenario(bot_path, scenario):
    ...
    try:
        bot_path_abs = Path(bot_path).resolve()
        bot_root = bot_path_abs.parent
        if not bot_path_abs.is_file() or bot_path_abs.parent != bot_root:
            return False, "Bot entry is unavailable"
        proc = run_candidate_probe(
            bot_root,
            _CANDIDATE_DECISION_PROBE,
            args=(bot_path_abs.name,),
        ...
        # An authoritative caller must classify missing isolation as trusted
        # infrastructure failure, never as an ordinary bot scenario miss.
        raise
