"""
Bot 3 - Multi-Style EXP3 Bot (bot_2 backbone + innovations):
1-7) All bot_2 improvements inherited
8) 5-style strategy portfolio (GTO / exploit_fold / exploit_call / exploit_maniac / exploit_nit)
9) EXP3 meta-learner for online style selection
10) Adaptive sizing table per round/hand-tier/style
11) River refinement (exact equity rules + blocker bluff)
12) Per-style gift tracking
13) Timing normalization
"""
import bisect
import itertools
import json
...
# ---------------------------------------------------------------
# Innovation 9: EXP3 Meta-Learner for Online Style Selection
# ---------------------------------------------------------------
class EXP3MetaLearner(object):
    """EXP3 bandit over N_STYLES strategy styles.

    Maintains per-style weights, probabilities, and per-style gift tracking.
    Updated after each hand with reward = clip(chip_delta / 20000, -1, 1).
    """

    def __init__(self):
        self.gamma = 0.10  # exploration rate
        self.weights = [1.0] * N_STYLES
        self.style_gifts = [0.0] * N_STYLES  # per-style cumulative gift
        self.last_style_idx = 0
        self.hand_count = 0
        self._prob_cache = None  # type: ignore
    ...
    def observe_reward(self, reward, style_idx=None):
        """Update weight for the style that was played, using EXP3 rule."""
        ...
        # EXP3 weight update: w <- w * exp(gamma * scaled_reward / N)
    ...
    def initialize_bias(self, initial_weights):
        """Set initial weight bias from opponent classification."""
...
# Module-level EXP3 instance (re-created per decide_action call in Botzone)
_exp3_learner = EXP3MetaLearner()


# ---------------------------------------------------------------
# Innovation 11: Opponent Style Classifier
# ---------------------------------------------------------------
def classify_opponent_style(opp_model):
    """Classify opponent into a style index for EXP3 initial bias.

    Uses VPIP, PFR, fold_to_raise, postflop_aggr to produce weight bias.
    Returns list of N_STYLES initial weights.
    """
    ...
    if confidence < 0.15:
        return weights  # Not enough data, stay uniform

    # Classify based on stats
    # Nit: low VPIP, low PFR, high fold_to_raise
    if vpip < 0.35 and fold_to_raise > 0.50:
        weights[4] = 3.0  # exploit_nit
        weights[0] = 2.0  # gto fallback
    # Maniac: high VPIP, high PFR, high postflop aggression
    elif vpip > 0.65 and pfr > 0.40 and postflop_aggr > 0.45:
        weights[3] = 3.0  # exploit_maniac
        weights[2] = 1.5  # exploit_call also viable
    # Calling station: high VPIP, low PFR, low fold_to_raise
    elif vpip > 0.55 and pfr < 0.20 and fold_to_raise < 0.38:
        weights[2] = 3.0  # exploit_call
        weights[0] = 2.0  # gto fallback
    # Fold-heavy: high fold_to_raise, moderate VPIP
    elif fold_to_raise > 0.52:
        weights[1] = 3.0  # exploit_fold
        weights[4] = 1.5  # exploit_nit also viable
    # Balanced / unknown: favor GTO
    else:
        weights[0] = 3.0  # gto
        weights[1] = 1.5
        weights[2] = 1.5
...
# ---------------------------------------------------------------
# Innovation 10: Adaptive Sizing Helper
# ---------------------------------------------------------------
def style_sizing_ratio(round_idx, tier, style_name, wetness):
    """Compute sizing ratio blended between base table and style tier.

    ratio = 0.70 * base_ratio + 0.30 * tier_ratio + wetness_correction
    """
    ...
    # Wet board: bump sizing for value, reduce for bluff
...
def choose_raise(
    ...
    # Innovation 10: Adaptive sizing blend from style
    if style_params is not None:
        ...
        # Blend: 0.80 original ratio + 0.20 style-derived ratio
...
# Improvement 7: Safe Exploitation Framework
def track_opponent_gift(requests, my_id):
    """Estimate opponent's cumulative 'gift' (losses beyond reasonable play).
    Positive gift_balance means opponent has been losing more than expected."""
    gift_balance = 0.0
    opponent_id = next_player(my_id, 1)
    hand_requests = collect_latest_requests_by_hand(requests)
    ...
    for req in hand_requests:
        ...
        # Estimate expected loss per hand (blinds are forced losses)
        # Gift = excessive losses beyond blind-based expectation
        if opp_chips < -200:  # Significant loss
...
def safe_exploitation_lambda(gift_balance, confidence, style_idx=None):
    """Compute exploitation interpolation factor.
    Low confidence -> close to GTO baseline.
    High gift_balance -> more exploitation.
    If style_idx is provided, uses per-style gift from EXP3 learner."""
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, style_params=None):
    ...
    # Innovation 8: Style parameter deltas
    ...
    if spot_info["preflop_spot"] == "sb_open":
        open_threshold = 0.49 + match_adjust + 0.02 + match_profile["open_delta"] + open_delta
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        iso_threshold = 0.57 + match_adjust - loose_bonus + match_profile["open_delta"] + open_delta
    ...
    if spot_info["preflop_spot"] == "bb_vs_raise":
        ...
        if preflop_strength >= threebet_value_threshold:
            ...
            if target > to_call and target >= state["round_raise"] and target < my_chips:
                return target
            # If sizing doesn't work, just 3bet to a reasonable amount
            target = max(state["round_raise"], int(to_call + state["pot"] * 0.75))
            if target > to_call and target < my_chips:
                return target
            return 0

        # 3bet bluff: widen range by style delta, adjust freq by bluff_mult
        ...
        if bluff_low <= preflop_strength <= bluff_high and confidence >= 0.25 and fold_to_raise > 0.45:
            # Frequency-based bluff using hand hash
            hand_idx = get_hand_index(req) or 0
            cards = req["my_cards"]
        ...
        # Default: call or fold based on strength
        call_threshold = 0.42 + match_adjust - loose_bonus + sp["call_threshold_delta"]
    ...
    if spot_info["preflop_spot"] == "sb_vs_reraise":
        # Improvement 6: 4bet logic
        # We opened, opponent 3bet us
        if preflop_strength >= 0.85:  # AA, KK, QQ
            # Value 4bet / 5bet jam
            pot_after_call = state["pot"] + to_call
        ...
        # Fold most hands facing 3bet unless very strong
        if preflop_strength >= 0.60 and to_call <= my_chips * 0.15:
            return 0  # Call with decent hands
        return -1
...
def get_action(req, requests):
    ...
    # -------------------------------------------------------
    # Innovation 9: EXP3 reward update + style selection
    # -------------------------------------------------------
    rng = random.Random(sum(my_cards) * 31 + (get_hand_index(req) or 0) * 17 + 7)

    # Detect hand boundary for reward update
    total_win_chips = req.get("total_win_chips", [0] * N_PLAYERS)
    opp_id = next_player(my_id, 1)
    current_hand_idx = get_hand_index(req)

    # Initialize EXP3 learner bias from opponent classification
    ...
    # Update EXP3 reward based on previous hand chip delta
    ...
    # Improvement 7: Safe exploitation lambda (now per-style)
    ...
    # Innovation 8: Style delta applied to thresholds
    strong += style_params["open_threshold_delta"]
    medium += style_params["open_threshold_delta"] * 0.5

    # Improvement 7: Safe exploitation threshold mixing
    # GTO baseline thresholds (before opponent exploitation adjustments)
    ...
    # Mix: (1-lambda) * gto + lambda * exploit
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            # Innovation 8: Style call threshold delta
            ...
            # Improvement 3: CBet-based call margin adjustment
            ...
            if round_idx == 1 and spot_info["facing_postflop_aggression"]:
                if cbet_rate > 0.65:
                    # Opponent CBets air frequently -> more willing to call
                    call_margin -= 0.02
                elif cbet_rate < 0.40:
                    # Opponent only CBets strong hands -> fold more
                    call_margin += 0.02
            # Improvement 3: Check-raise bluff when opponent folds to CBet
            if round_idx == 1 and fold_to_cbet > 0.48 and opponent_model["confidence"] >= 0.25:
                pass  # Handled in raise logic below via fold_to_raise
        if anti_lock_pressure:
            call_margin -= 0.07
        ...
        if trap_nut_slowplay:
            return 0
        # Innovation 8: Style-based trap – extend slowplay probability
        if (round_idx in (1, 2)
            and value_profile is not None
            and value_profile["tier"] == "nut"
            and board_texture is not None
            ...
            and style_params["trap_probability"] > 0.0
            and rng.random() < style_params["trap_probability"]):
            return 0
    ...
    # -------------------------------------------------------
    # Innovation 11: River Refinement (exact equity rules)
    # -------------------------------------------------------
    if round_idx == 3 and len(public_cards) == 5:
        # Compute exact equity for river decisions
        exact_wr = estimate_weighted_win_rate(my_cards, public_cards, combos, weights, 0)
        # Force raise with very strong hands facing a bet
        if exact_wr > 0.85 and to_call > 0:
            ...
            if raise_amount is not None and raise_amount > to_call:
                return raise_amount
            return 0
        # Fold very weak hands facing a bet (unless pot odds justify)
        if exact_wr < 0.15 and to_call > 0:
            min_call_odds = to_call / (pot + to_call) if to_call > 0 else 1.0
            if exact_wr < min_call_odds - 0.10:
                return -1
        # Blocker bluff on river: holding cards that block opponent strong hands
        if (exact_wr < 0.20
            and to_call == 0
            and blocker_profile is not None
            and blocker_profile["eligible"]
            and style_params["bluff_frequency_mult"] > 0.8
            and rng.random() < 0.25 * style_params["bluff_frequency_mult"]):
            blocker_bluff = True
...
def decide_action(payload):
    ...
    return int(action), _exp3_learner.to_dict()
