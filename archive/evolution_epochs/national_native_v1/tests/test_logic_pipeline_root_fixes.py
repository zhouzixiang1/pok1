import asyncio
import ast
"""Archived mixed-ABI pipeline regression suite from national_native_v1."""
...
def test_smoke_test_ignores_successful_battle_cleanup_broken_pipe(monkeypatch, tmp_path):
    ...
    def _fake_run(*_args, **_kwargs):
        return SimpleNamespace(
            returncode=0,
            stdout="Smoke test passed successfully.\n",
            stderr=cleanup_noise,
            trusted_completion=True,
        )

    monkeypatch.setattr(code_verification, "run_candidate_probe", _fake_run)
...
def test_master_runtime_heartbeat_does_not_touch_semantic_checkpoint(monkeypatch):
    import evolution_infra
    ...
    checkpoint = {
        ...
        "workflow_run_id": "wf-268",
    }
    heartbeat_calls = []

    monkeypatch.setattr(evolution_infra, "read_pipeline_checkpoint", lambda: checkpoint)

    def _fake_heartbeat(received, **kwargs):
        heartbeat_calls.append((received, kwargs))
        return True
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: (_ for _ in ()).throw(
            AssertionError("runtime heartbeat must not write pipeline_state")
        ),
    )
    monkeypatch.setattr(tool_planning, "log_system_event", lambda *a, **k: None)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "bots/national_v143 --control-id first_strict_control_v1 "
            "--acknowledge-one-time-first-strict-control --wait-if-busy"
        ),
        (
            "python3 ./scripts/official_certify.py bootstrap-first-strict "
            "bots/national_v143 --acknowledge-one-time-first-strict-control"
        ),
        (
            "/usr/bin/python3 /home/zzx/project/pok/scripts/official_certify.py "
            "bootstrap-first-strict bots/national_v143"
        ),
        (
            "python -c \"from official_bootstrap import select_first_strict_control; "
            "select_first_strict_control('first_strict_control_v1', 'bots/national_v143')\""
        ),
    ],
)
def test_orchestrator_guard_identifies_operator_only_official_bootstrap(
    command,
):
    import orchestrator_context

    assert (
        orchestrator_context._orchestrator_bash_targets_operator_only_bootstrap(
            command
        )
        is True
    )
...
def test_orchestrator_guard_denies_one_time_control_without_actionable_route(
    ...
    monkeypatch.setattr(
        evolution_infra, "PIPELINE_STATE_FILE", tmp_path / "pipeline_state.json"
    )
    ...
    output = asyncio.run(hook(
        ...
        "call_operator_bootstrap_guard",
        None,
    ))

    decision = output["hookSpecificOutput"]
    assert decision["permissionDecision"] == "deny"
    assert "operator-only" in decision["permissionDecisionReason"]
    assert "explicit external operator command" in decision[
...
@pytest.mark.parametrize(
    ...
    [
        (
            "POK_LLM_RESUME_EVIDENCE_DIGEST=abc python web/core/orchestrator.py "
            "--one-gen"
        ),
        (
            "python -c \"import llm_availability_store as s; "
            "s.reconcile_llm_pause()\""
        ),
        "python -c \"from llm_availability_store import persist_llm_pause\"",
        "cat web/core/results/llm_availability_pause.json",
    ],
)
def test_main_orchestrator_guard_denies_llm_pause_control_bash(command, monkeypatch):
    import asyncio
    import orchestrator_context
    import system_log

    monkeypatch.setattr(system_log, "log_system_event", lambda *_a, **_k: None)
    assert orchestrator_context._orchestrator_bash_targets_llm_availability_control(
        command
    )
    hook = orchestrator_context._make_bot_dir_guard_hook()["PreToolUse"][0].hooks[0]
    output = asyncio.run(hook(
        {"tool_name": "Bash", "tool_input": {"command": command}},
        "call_llm_pause_guard",
        None,
    ))
    decision = output["hookSpecificOutput"]
    assert decision["permissionDecision"] == "deny"
    assert "operator-owned" in decision["permissionDecisionReason"]
    assert "parent-process startup" in decision["permissionDecisionReason"]


@pytest.mark.parametrize(
    "command",
    [
        "export POK_LLM_RESUME_EVIDENCE_DIGEST=abc",
        (
            "python -c \"import llm_availability_store as s; "
            "s.consume_operator_resume_ack_from_env()\""
        ),
        "cat web/core/results/llm_availability_pause.json",
    ],
)
def test_subagent_guard_denies_llm_pause_control_bash(command, monkeypatch):
    import asyncio
    import llm_query
    import system_log

    monkeypatch.setattr(system_log, "log_system_event", lambda *_a, **_k: None)
    hook = llm_query._make_subagent_llm_availability_guard("WORKER 1")
    handler = hook["PreToolUse"][0].hooks[0]
    output = asyncio.run(handler(
        {"tool_name": "Bash", "tool_input": {"command": command}},
        "subagent_pause_guard",
        None,
    ))
    decision = output["hookSpecificOutput"]
    assert decision["permissionDecision"] == "deny"
    assert "operator-owned" in decision["permissionDecisionReason"]
    assert "parent launcher" in decision["permissionDecisionReason"]


@pytest.mark.parametrize(
    "command",
    [
        "python -c \"import strict_authority_workflow\"",
        (
            "python -c \"from strict_authority_workflow import "
            "accept_role_result; accept_role_result({}, role_result={}, "
            "parse_contract='x')\""
        ),
        "rg StrictRoleAccepted web/core/results/workflow/events.sqlite3",
        "python -c \"from strict_authority_workflow import complete_provider_call\"",
    ],
)
def test_main_and_subagent_guards_deny_strict_authority_bash(command, monkeypatch):
    import asyncio
    import llm_query
    import orchestrator_context
    import system_log
    ...
    monkeypatch.setattr(system_log, "log_system_event", lambda *_a, **_k: None)
    assert orchestrator_context._orchestrator_bash_targets_strict_authority_control(
        command
    )
    main_hook = orchestrator_context._make_bot_dir_guard_hook()["PreToolUse"][0].hooks[0]
    main_output = asyncio.run(main_hook(
        {"tool_name": "Bash", "tool_input": {"command": command}},
        "main_strict_authority_guard",
        None,
    ))
    assert main_output["hookSpecificOutput"]["permissionDecision"] == "deny"

    child_hook = llm_query._make_subagent_llm_availability_guard("MASTER")
    child_output = asyncio.run(child_hook["PreToolUse"][0].hooks[0](
        {"tool_name": "Bash", "tool_input": {"command": command}},
        "child_strict_authority_guard",
        None,
    ))
    assert child_output["hookSpecificOutput"]["permissionDecision"] == "deny"
...
def test_git_commit_bot_refuses_preexisting_blocking_staged_files(monkeypatch):
    ...
    def fake_git(*args, **_kwargs):
        ...
        if args == ("rev-parse", "--abbrev-ref", "HEAD"):
            return "main\n"
        if args == ("tag", "-l", "national-bot-v999"):
            return ""
...
def test_git_commit_bot_preserves_unrelated_staged_files(monkeypatch):
    ...
    def fake_git(*args, **_kwargs):
        ...
        if args == ("tag", "-d", "national-bot-v999"):
            return ""
        if args[:5] == ("tag", "-a", "national-bot-v999", "HEAD", "-m"):
            assert "official-certificate: cert-digest" in args[5]
            return ""
        raise AssertionError(args)
...
def test_git_commit_bot_binds_official_certificate_to_commit_and_tag(monkeypatch):
    ...
    def fake_git(*args, **_kwargs):
        ...
        if args[:5] == ("tag", "-a", "national-bot-v999", "HEAD", "-m"):
            assert "official-certificate: cert-digest" in args[5]
            assert "official-policy: official-full-v5" in args[5]
            return ""
        raise AssertionError(args)
    ...
    assert any(
        call[:5] == ("tag", "-a", "national-bot-v999", "HEAD", "-m")
        for call in calls
    )
    ...
    tag_index = next(
        ...
        if call[:5] == ("tag", "-a", "national-bot-v999", "HEAD", "-m")
    )
