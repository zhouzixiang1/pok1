"""Archived tests for the retired multi-file incremental candidate reset.
...
The strict national TCP policy epoch permits only ``policy.py`` as a Worker-owned
file and rejects helper creation, so this compatibility behavior is audit history,
not an active pipeline contract.
"""
