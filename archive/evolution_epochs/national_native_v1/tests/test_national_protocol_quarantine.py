"""Archived tests for the retired protocol-quarantine migration path."""
...
def test_quarantine_matches_legacy_runtime_semantics_after_comment_edit(tmp_path):
    import national_protocol_quarantine as quarantine

    renamed = tmp_path / "innocent_name"
    renamed.mkdir()
    source = quarantine.BOTS_DIR / "national_v142" / "national_bot.py"
    (renamed / "national_bot.py").write_text(
        source.read_text(encoding="utf-8")
        + "\n\n# A byte-only edit must not create an executable identity.\n",
        encoding="utf-8",
    )
...
def test_complete_v142_derivative_requires_current_runtime_rewrite(tmp_path):
    import national_protocol_quarantine as quarantine
    from national_native import check_native_contract, ensure_native_entry

    source = quarantine.BOTS_DIR / "national_v142"
    legacy_derivative = tmp_path / "legacy_derivative"
    shutil.copytree(
        source,
        legacy_derivative,
        ignore=shutil.ignore_patterns("__pycache__", "*.pyc"),
    )
    with (legacy_derivative / "national_bot.py").open(
        "a", encoding="utf-8"
    ) as handle:
        handle.write("\nLEGACY_NOOP_MARKER = None\n")

    assert "national_v142" in quarantine.quarantined_native_entry_sources(
        legacy_derivative
    )
    ...
    ensure_native_entry(rewritten, overwrite=True)
    assert check_native_contract(
        rewritten,
        require_current_stream_decoder=True,
        require_current_decision_runtime=True,
    ) == []
    assert quarantine.quarantined_native_entry_sources(rewritten) == ()


def test_system_runtime_identity_blocks_multi_file_legacy_derivative(tmp_path):
    import national_native
    import national_protocol_quarantine as quarantine

    source = quarantine.BOTS_DIR / "national_v142"
    derivative = tmp_path / "renamed_v142"
    shutil.copytree(
        source,
        derivative,
        ignore=shutil.ignore_patterns("__pycache__", "*.pyc"),
    )
    entry = derivative / "national_bot.py"
    ...
    with (derivative / "constants.py").open("a", encoding="utf-8") as handle:
        handle.write("\n# Non-entry byte drift cannot grant execution authority.\n")

    errors = quarantine.current_system_native_runtime_errors(derivative)
    assert errors and "identity_mismatch" in errors[0]
    with pytest.raises(ValueError, match="non_system_owned_native_runtime_forbidden"):
        national_native._prepare_native_spec(derivative.name, derivative)


def test_current_system_runtime_identity_is_exact_and_static(tmp_path):
    import national_protocol_quarantine as quarantine
    ...
    identity = quarantine.current_system_native_runtime_identity()

    assert identity["sha256"] == (
        "9ac5fd790ac3eda58a8842308bc00886913113815a8959197b549717e7a180d0"
    )
    assert quarantine.current_system_native_runtime_errors(bot_dir) == []
    with (bot_dir / "national_bot.py").open("a", encoding="utf-8") as handle:
        handle.write("\n# even a comment changes executable authority\n")
    assert quarantine.current_system_native_runtime_errors(bot_dir)


def test_current_system_runtime_rejects_candidate_stdlib_shadow(tmp_path):
    import national_protocol_quarantine as quarantine
    from national_native import ensure_native_entry
    ...
    bot_dir.mkdir()
    ensure_native_entry(bot_dir, overwrite=True)
    (bot_dir / "argparse.py").write_text(
        "raise SystemExit('candidate stdlib shadow executed')\n",
        encoding="utf-8",
    )

    assert quarantine.current_system_native_runtime_errors(bot_dir) == [
        "managed_bot_stdlib_shadow_forbidden:argparse.py:argparse"
    ]
...
def test_pending_publication_excludes_only_exact_self_during_recovery(monkeypatch):
    import national_protocol_quarantine as quarantine

    health = quarantine.protocol_quarantine_health(force_refresh=True)
    monkeypatch.setattr(
        quarantine, "protocol_quarantine_health", lambda **_kwargs: health
    )
    strict = []
    monkeypatch.setattr(
        quarantine,
    ...
    strict.append("national_v150")
    proof_payload = {
        "schema_version": quarantine.PENDING_PUBLICATION_SCHEMA_VERSION,
        "kind": quarantine.PENDING_PUBLICATION_KIND,
        "bot": "national_v150",
        "version": 150,
        "artifact_hash": "1" * 64,
        "tag": "national-bot-v150",
        "tag_object": "2" * 40,
        "commit_oid": "3" * 40,
        "completion_tree_oid": "4" * 40,
        "main_commit_oid": "5" * 40,
    }
    proof = {
        **proof_payload,
        "proof_digest": quarantine._canonical_digest(proof_payload),
    }
    monkeypatch.setattr(
        quarantine,
        "build_pending_local_publication_proof",
        lambda _path: dict(proof),
    )
    ...
    tampered = dict(proof)
    tampered["commit_oid"] = "9" * 40
    tampered["proof_digest"] = quarantine._canonical_digest({
        key: value for key, value in tampered.items()
        if key != "proof_digest"
    })
    errors = quarantine.validate_protocol_bootstrap_receipt(
        migration["receipt"],
        active_bots=["national_v150"],
        pending_local_publication=tampered,
    )
    assert "pending_local_publication_current_state_mismatch" in errors

    strict.append("national_v151")
    # The synthetic strict-publication list must carry a matching portable
    # identity too; otherwise selection closes earlier on the fixture's missing
    # directory and never reaches the exact-self exclusion assertion below.
    monkeypatch.setattr(
        quarantine,
        ...
        lambda label, **_kwargs: {
            ...
            "version": 151,
            "artifact_hash": "6" * 64,
            "tag": "national-bot-v151",
            "tag_object": "7" * 40,
            "completion_tree_oid": "8" * 40,
        } if label == "national_v151" else None,
    )
    errors = quarantine.validate_protocol_bootstrap_receipt(
        migration["receipt"],
        active_bots=["national_v150", "national_v151"],
        pending_local_publication=proof,
    )
    assert any("current_state_mismatch" in error for error in errors)
