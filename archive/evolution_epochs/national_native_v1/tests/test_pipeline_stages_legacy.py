"""Archived national_native_v1 end-to-end pipeline stage tests.
...
class TestPipelineCheckpointFsync:
    ...
    def test_checkpoint_replace_and_clear_fsync_parent_directory(
        ...
        checkpoint = json.loads(ckpt_file.read_text(encoding="utf-8"))
        assert synced == [tmp_path]

        assert evolution_infra.clear_pipeline_checkpoint(
            expected_workflow_run_id=checkpoint["workflow_run_id"],
            expected_next_v=11,
            expected_source_v=10,
    ...
    def test_checkpoint_directory_fsync_failure_is_not_reported_durable(
        self, tmp_path, monkeypatch
    ):
        import evolution_infra
...
class TestPipelineStateTransitions:
    ...
    def test_publication_intent_is_immutable_and_checkpoint_cas_bound(
        self, tmp_path, monkeypatch
    ):
        import evolution_infra
        ...
        verified = json.loads(ckpt_file.read_text())
        intent = build_publication_intent(
            ...
            certificate_relative_path="official_certificates/national_v11.json",
            certificate_file_sha256="c" * 64,
            certificate_attestation_digest="d" * 64,
            final_gate_ledger_digest="e" * 64,
            strategy_tag="frozen strategy",
            rating_info="",
            baseline_head="1" * 40,
        ...
        assert evolution_infra.write_pipeline_checkpoint(
            ...
            expected_checkpoint_revision=verified["checkpoint_revision"],
            expected_checkpoint_stage="verified",
            expected_workflow_run_id=verified["workflow_run_id"],
        )
        publishing = json.loads(ckpt_file.read_text())
        assert publishing["publication_intent"] == intent
        assert publishing["checkpoint_revision"] == verified["checkpoint_revision"] + 1

        # Same-stage liveness writes preserve the exact intent; replacement is
        # forbidden even if the attacker recomputes a new self-consistent digest.
        assert evolution_infra.write_pipeline_checkpoint(
            next_v=11,
            source_v=10,
            stage="publishing",
            expected_checkpoint_revision=publishing["checkpoint_revision"],
            expected_checkpoint_stage="publishing",
            expected_workflow_run_id=publishing["workflow_run_id"],
        )
        refreshed = json.loads(ckpt_file.read_text())
        assert refreshed["publication_intent"] == intent
        rewritten = dict(intent)
        rewritten["strategy_tag"] = "replacement"
        from publication_transaction import canonical_digest

        rewritten["publication_id"] = canonical_digest({
        ...
        assert not evolution_infra.write_pipeline_checkpoint(
            ...
            source_v=10,
            stage="publishing",
            publication_intent=rewritten,
            expected_checkpoint_revision=refreshed["checkpoint_revision"],
            expected_checkpoint_stage="publishing",
            expected_workflow_run_id=refreshed["workflow_run_id"],
        )

        assert not evolution_infra.clear_pipeline_checkpoint(
            expected_workflow_run_id=refreshed["workflow_run_id"],
            expected_next_v=11,
            expected_source_v=10,
            expected_checkpoint_revision=publishing["checkpoint_revision"],
...
class TestWorkerFailureCircuitBreaker:
    ...
    def _defer_worker_for_availability(self, tmp_path, monkeypatch, issue):
        """Create one real attempt-neutral deferred Worker effect."""
        import asyncio
        import tool_planning
        ...
        async def pause_worker(*_args, **_kwargs):
            raise LLMAvailabilityBlocked(issue, role="worker")

        async def _run():
            with patch.object(
                tool_planning,
                "_execute_workers",
                side_effect=pause_worker,
            ):
        ...
        first = json.loads(asyncio.run(_run())["content"][0]["text"])
        assert first["error"] == "LLM_AVAILABILITY_BLOCKED"
        checkpoint = json.loads(ckpt_file.read_text())
        workflow = WorkerWorkflow.for_checkpoint(checkpoint)
        assert workflow.state()["status"] == "availability_deferred"
        return ckpt_file, workflow
    ...
    def test_llm_availability_defers_worker_without_attempt_or_infra_failure(
        self, tmp_path, monkeypatch
    ):
        """A classified provider pause is an attempt-neutral durable outcome."""
        import asyncio
        import tool_planning
        ...
        from llm_availability_store import load_llm_pause
        from worker_workflow import WorkerWorkflow
        ...
        async def write_then_pause(*args, **_kwargs):
            worker_dir = Path(args[2])
            (worker_dir / "strategy.py").write_text("partial = True\n")
            raise LLMAvailabilityBlocked(issue, role="worker")
        ...
        effect = workflow.store.effect(state["effect_id"])

        assert data["error"] == "LLM_AVAILABILITY_BLOCKED"
        assert data["failure_class"] == "availability"
        assert data["action"] == "wait_for_llm_availability"
        assert data["claimed_attempt"] == 1
        assert data["restored_attempt"] == 0
        assert state["status"] == "availability_deferred"
        ...
        assert load_llm_pause()["active"] is True
        assert (next_dir / "strategy.py").read_bytes() == before

    def test_active_pause_blocks_claim_and_exact_resume_releases_deferred_effect(
        self, tmp_path, monkeypatch
    ):
        """Restart/re-entry waits durably, then resumes the same logical attempt."""
        import asyncio
        import tool_planning
        from llm_availability import (
            LLMAvailabilityBlocked,
        ...
        async def _first():
            with patch.object(
                ...
                return await tool_planning.execute_workers.handler({
                    "tasks": [], "next_v": 11, "source_v": 10,
                    "reviewer_feedback": "",
                })

        first = json.loads(asyncio.run(_first())["content"][0]["text"])
        assert first["error"] == "LLM_AVAILABILITY_BLOCKED"
        checkpoint = json.loads(ckpt_file.read_text())
        workflow = WorkerWorkflow.for_checkpoint(checkpoint)
        event_count = len(workflow.store.events(workflow.run_id))

        async def _still_paused():
            with patch.object(
                tool_planning,
                ...
                return result, run_effect

        (blocked_result, run_effect) = asyncio.run(_still_paused())
        blocked = json.loads(blocked_result["content"][0]["text"])
        assert blocked["error"] == "LLM_AVAILABILITY_BLOCKED"
        assert blocked["attempt"] == 0
        run_effect.assert_not_awaited()
        assert len(workflow.store.events(workflow.run_id)) == event_count

        monkeypatch.setenv(RESUME_ENV, issue.evidence_digest)
        consume_operator_resume_ack_from_env()
        resumed_marker = tool_planning._json_tool_result({"resumed": True})
        ...
        async def _resume():
            with patch.object(
                ...
                "_run_durable_worker_effect",
                new_callable=AsyncMock,
                return_value=resumed_marker,
            ) as resumed_effect:
                result = await tool_planning.execute_workers.handler({
                    "tasks": [], "next_v": 11, "source_v": 10,
                    "reviewer_feedback": "",
                })
                return result, resumed_effect

        (resume_result, resumed_effect) = asyncio.run(_resume())
        assert resume_result == resumed_marker
        resumed_effect.assert_awaited_once()
        resumed_state = workflow.state()
        resumed_row = workflow.store.effect(resumed_state["effect_id"])
        assert resumed_state["status"] == "requested"
        assert resumed_state["attempt"] == 0
        assert resumed_row["status"] == "retry"
        assert resumed_row["attempt"] == 0
        assert [
            event.event_type
            for event in workflow.store.events(workflow.run_id)
        ][-1] == "EffectResumed"

    def test_pause_publish_crash_restarts_attempt_neutral_without_lease_wait(
        self, tmp_path, monkeypatch
    ):
        """EffectDeferred precedes pause publication, closing its crash window."""
        import asyncio
        ...
        real_persist = pause_store.persist_llm_pause

        def persist_then_crash(value, **kwargs):
            real_persist(value, **kwargs)
            raise SystemExit("injected crash after atomic pause publish")
        ...
        with pytest.raises(SystemExit, match="after atomic pause publish"):
            asyncio.run(first_run())

        checkpoint = json.loads(ckpt_file.read_text())
        workflow = WorkerWorkflow.for_checkpoint(checkpoint)
        crashed_state = workflow.state()
        crashed_effect = workflow.store.effect(crashed_state["effect_id"])
        assert pause_store.load_llm_pause()["active"] is True
        assert crashed_state["status"] == "availability_deferred"
        assert crashed_state["attempt"] == 0
        assert crashed_effect["status"] == "deferred"
        assert crashed_effect["attempt"] == 0
        assert crashed_effect["lease_owner"] is None

        monkeypatch.setattr(pause_store, "persist_llm_pause", real_persist)
        monkeypatch.setenv(pause_store.RESUME_ENV, issue.evidence_digest)
        pause_store.consume_operator_resume_ack_from_env()
        marker = tool_planning._json_tool_result({"resumed": True})
        ...
        result, run_effect = asyncio.run(restarted_run())
        assert result == marker
        run_effect.assert_awaited_once()
        resumed_state = workflow.state()
        resumed_effect = workflow.store.effect(resumed_state["effect_id"])
        assert resumed_state["status"] == "requested"
        assert resumed_state["attempt"] == 0
        assert resumed_effect["status"] == "retry"
        assert resumed_effect["attempt"] == 0

    @pytest.mark.parametrize(
        ("receipt_case", "expected_receipt_error"),
        [
            ("missing", "global_pause_resume_receipt_missing"),
            ("wrong_digest", "global_pause_resume_receipt_evidence_digest_mismatch"),
            ("no_operator_receipt", "manual_pause_operator_receipt_missing"),
        ],
    )
    def test_manual_pause_resume_receipt_failures_preserve_deferred_effect(
        self,
        receipt_case,
        ...
        """No inactive/mismatched record can authorize a manual Worker resume."""
        import asyncio
        import tool_planning
        ...
        assert issue is not None
        _ckpt_file, workflow = self._defer_worker_for_availability(
            tmp_path,
            monkeypatch,
            issue,
        )
        ...
        if receipt_case == "missing":
            pause_store.pause_path().unlink()
        else:
            receipt = pause_store.load_llm_pause()
            receipt.update({
                "active": False,
                "resumed_at": "2026-07-14T00:00:00+00:00",
            })
            if receipt_case == "wrong_digest":
                wrong_digest = "f" * 64
                receipt.update({
                    "evidence_digest": wrong_digest,
                    "resume_source": "operator_evidence_digest",
                    "resume_evidence_digest": wrong_digest,
                })
            else:
                receipt.pop("resume_source", None)
                receipt.pop("resume_evidence_digest", None)
            pause_store.pause_path().write_text(
                json.dumps(receipt),
                encoding="utf-8",
            )
        ...
        result, run_effect = asyncio.run(_run())
        data = json.loads(result["content"][0]["text"])
        assert data["error"] == "WORKER_AVAILABILITY_RESUME_RECEIPT_INVALID"
        assert expected_receipt_error in data["receipt_errors"]
        run_effect.assert_not_awaited()
        assert workflow.state()["status"] == "availability_deferred"
        assert workflow.state()["attempt"] == 0
        assert len(workflow.store.events(workflow.run_id)) == event_count

    def test_transient_pause_cooldown_receipt_resumes_same_worker_effect(
        self, tmp_path, monkeypatch
    ):
        """A system-produced bounded-cooldown receipt authorizes transient resume."""
        import asyncio
        ...
        resumed = pause_store.reconcile_llm_pause(
            now=datetime.now(timezone.utc) + timedelta(minutes=3),
        )
        assert resumed["active"] is False
        assert resumed["resume_source"] == "bounded_cooldown_elapsed"
    ...
    def test_preexisting_pause_freezes_worker_input_without_claiming_effect(
        self, tmp_path, monkeypatch
    ):
        """A restart with an active pause may prepare, but never leases an LLM."""
        import asyncio
        import tool_planning
        from llm_availability import (
            LLMAvailabilityBlocked,
        ...
        assert issue is not None
        persist_llm_pause(LLMAvailabilityBlocked(issue, role="worker"))
        ...
        assert data["error"] == "LLM_AVAILABILITY_BLOCKED"
        assert data["worker_status"] == "prepared"
        assert data["attempt"] == 0
        run_effect.assert_not_awaited()
        assert state["status"] == "prepared"
        assert workflow.store.effect(state["effect_id"]) == {}
    ...
    def test_post_claim_hook_exception_never_leaves_running_effect(
        ...
        envelope = build_worker_envelope(
            ...
            worker_execution_context={"schema_version": 2},
    ...
    def test_successful_worker_edits_roll_back_when_checkpoint_write_fails(
        ...
        assert result["canonical_artifact_restored"] is True
        assert (next_dir / "strategy.py").read_bytes() == baseline
        from worker_workflow import WorkerWorkflow
    ...
    def test_worker_projection_cas_false_accepts_matching_concurrent_receipt(
        self, tmp_path, monkeypatch
    ):
        """A writer that committed the same receipt wins without rollback."""
        import asyncio
        import tool_planning

        ckpt_file = self._setup_checkpoint(
            tmp_path, monkeypatch, stage="master_planned"
        )
        next_dir = tmp_path / "claude_v11"
        real_write = tool_planning.write_pipeline_checkpoint

        async def successful_worker(*_args, **_kwargs):
            worker_dir = Path(_args[2])
            (worker_dir / "strategy.py").write_text("value = 333\n")
            return True, {}, []

        def concurrent_same_projection(*args, **kwargs):
        ...
        with patch.object(
            ...
            tool_planning,
            "write_pipeline_checkpoint",
            side_effect=concurrent_same_projection,
        ):
            raw = asyncio.run(tool_planning.execute_workers.handler({
                "tasks": [], "next_v": 11, "source_v": 10,
            }))

        result = json.loads(raw["content"][0]["text"])
        assert result["success"] is True
        assert result["durable_recovery"] == (
            "confirmed_concurrent_worker_projection"
        )
        assert (next_dir / "strategy.py").read_text() == "value = 333\n"
        assert json.loads(ckpt_file.read_text())["stage"] == "workers_done"

    def test_worker_projection_cas_true_does_not_complete_after_artifact_drift(
        self, tmp_path, monkeypatch
    ):
        import asyncio
        import tool_planning
        ...
        async def successful_worker(*_args, **_kwargs):
            worker_dir = Path(_args[2])
            (worker_dir / "strategy.py").write_text("value = 601\n")
            return True, {}, []

        def commit_then_drift(*args, **kwargs):
            assert real_write(*args, **kwargs) is True
            (next_dir / "strategy.py").write_text("value = 602\n")
            return True
        ...
        result = json.loads(raw["content"][0]["text"])
        assert result["error"] == "DURABLE_WORKER_POST_COMMIT_ARTIFACT_MISMATCH"
        assert result["action"] == "operator_reconcile"
        assert (next_dir / "strategy.py").read_text() == "value = 602\n"
        checkpoint = json.loads(ckpt_file.read_text())
        assert checkpoint["stage"] == "workers_done"
        workflow = WorkerWorkflow.for_checkpoint(checkpoint)
        assert workflow.state()["status"] == "output_ready"

    def test_worker_matching_concurrent_receipt_still_checks_workers_done_bytes(
        self, tmp_path, monkeypatch
    ):
        import asyncio
        import tool_planning
        ...
        async def successful_worker(*_args, **_kwargs):
            worker_dir = Path(_args[2])
            (worker_dir / "strategy.py").write_text("value = 611\n")
            return True, {}, []

        def concurrent_receipt_then_drift(*args, **kwargs):
            assert real_write(*args, **kwargs) is True
            (next_dir / "strategy.py").write_text("value = 612\n")
            return False
        ...
        with patch.object(
            ...
            tool_planning,
            "write_pipeline_checkpoint",
            side_effect=concurrent_receipt_then_drift,
        ):
            raw = asyncio.run(tool_planning.execute_workers.handler({
                "tasks": [], "next_v": 11, "source_v": 10,
            }))
        ...
        result = json.loads(raw["content"][0]["text"])
        assert result["error"] == (
            "DURABLE_WORKER_CONCURRENT_PROJECTION_ARTIFACT_MISMATCH"
        )
        assert result["action"] == "operator_reconcile"
        assert (next_dir / "strategy.py").read_text() == "value = 612\n"

    def test_worker_projection_cas_does_not_overwrite_concurrent_artifact_drift(
        self, tmp_path, monkeypatch
    ):
        """A different post-CAS tree is preserved for operator reconciliation."""
        import asyncio
        import tool_planning
        ...
        async def successful_worker(*_args, **_kwargs):
            worker_dir = Path(_args[2])
            (worker_dir / "strategy.py").write_text("value = 444\n")
            return True, {}, []

        def concurrent_drift(*_args, **_kwargs):
            (next_dir / "strategy.py").write_text("value = 555\n")
            return False
        ...
        result = json.loads(raw["content"][0]["text"])
        assert result["error"] == (
            "DURABLE_WORKER_OUTPUT_PROJECTION_CONCURRENT_DRIFT"
        )
        assert result["action"] == "operator_reconcile"
        assert (next_dir / "strategy.py").read_text() == "value = 555\n"

    def test_worker_projection_does_not_rollback_preexisting_same_output(
        self, tmp_path, monkeypatch
    ):
        """A no-op materialization has no ownership over concurrent output bytes."""
        import asyncio
        import shutil
        import tool_planning
        ...
        self._setup_checkpoint(tmp_path, monkeypatch, stage="master_planned")
        next_dir = tmp_path / "claude_v11"
        original_materialize = WorkerArtifactStore.materialize
        calls = 0

        async def successful_worker(*_args, **_kwargs):
            worker_dir = Path(_args[2])
            (worker_dir / "strategy.py").write_text("value = 733\n")
            return True, {}, []

        def concurrent_same_output(self, digest, destination, **kwargs):
            nonlocal calls
            calls += 1
            if calls == 1:
                destination = Path(destination)
                shutil.rmtree(destination)
                shutil.copytree(self.path_for(digest), destination)
                cache = destination / "__pycache__"
                cache.mkdir(exist_ok=True)
                (cache / "operator.dat").write_text("concurrent owner\n")
        ...
        result = json.loads(raw["content"][0]["text"])
        assert result["error"] == (
            "DURABLE_WORKER_OUTPUT_PREEXISTED_FAILED_CHECKPOINT_CAS"
        )
        assert (next_dir / "strategy.py").read_text() == "value = 733\n"
        assert (next_dir / "__pycache__" / "operator.dat").read_text() == (
            "concurrent owner\n"
        )
        assert calls == 1
    ...
    def test_rework_running_without_overlay_reuses_prepared_transaction(
        ...
        assert first["canonical_artifact_restored"] is True
        assert (next_dir / "strategy.py").read_bytes() == baseline
    ...
    def test_rework_prepare_crash_resumes_frozen_staging_without_repreparing(
        ...
        context_builder = MagicMock(
            return_value={"schema_version": 2, "frozen_context": "before-crash"}
        )
        monkeypatch.setattr(
            agent_workers,
            "build_worker_execution_context",
        ...
        assert work_item["projection_preimage_artifact_hash"]
        assert (
            work_item["projection_preimage_snapshot_hash"]
            == work_item["projection_preimage_artifact_hash"]
        )
        assert work_item["frozen_worker_input"]["schema_version"] == 3
        assert work_item["frozen_worker_input"][
        ...
        # Live runtime changes after the receipt must not rewrite this logical
        # activity.
        ...
        tampered = json.loads(json.dumps(interrupted))
        tampered["master_plan"]["work_item"][
            "projection_preimage_artifact_hash"
        ] = "0" * 64
        ckpt_file.write_text(json.dumps(tampered))
        tampered_result = asyncio.run(tool_planning.execute_workers.handler({
            "tasks": [],
            "next_v": 11,
            "source_v": 10,
        }))
        tampered_data = json.loads(tampered_result["content"][0]["text"])
        assert tampered_data["error"] == "DURABLE_REPAIR_PREPARATION_UNAVAILABLE"
        ckpt_file.write_text(json.dumps(interrupted))

        (next_dir / "strategy.py").write_text("drifted after receipt\n")
        drifted_result = asyncio.run(tool_planning.execute_workers.handler({
            "tasks": [],
            "next_v": 11,
            "source_v": 10,
        }))
        drifted_data = json.loads(drifted_result["content"][0]["text"])
        assert drifted_data["error"] == "DURABLE_REPAIR_PREPARATION_UNAVAILABLE"
        (next_dir / "strategy.py").write_bytes(canonical_before)
    ...
    def test_critic_rework_without_file_evidence_uses_changed_strategy_files(self, tmp_path, monkeypatch):
        ...
        ckpt = {
            ...
            "reviewer_feedback": (
                ...
                "weak historical matchups and misreads frozen diagnostics."
            ),
            "master_plan": {"strategy": "crossover", "tasks": []},
            "gate_results": {
                "critic": {
