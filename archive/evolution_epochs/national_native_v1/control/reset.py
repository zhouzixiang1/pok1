"""Archived national_native_v1 reset implementation (v1-v6 only).

The active strict epoch has no Web/API reset path. Epoch migration is an
explicit stopped-operator ceremony owned by scripts/reset_national_tcp_policy_epoch.py.
"""
...
def reset_evolution(keep_versions=None):
    ...
    # Step 6: Clear orchestrator logs
    result["deleted_orch_logs"] = _clear_orchestrator_logs()

    # Step 7: Ensure kept bots have .completed sentinel
