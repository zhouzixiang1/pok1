...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, texture_class="none"):
    ...
    if texture_class == "dry":
        # call_threshold = base + margin; bot folds when realized < threshold.
        # A more negative margin => lower threshold => wider calling. On dry
        # boards ranges are narrow, so defending wider is correct.
        # [Mutation v135] With the crossover v71 EQR model already correcting
        # over-folding via calibrated per-street equity realization, the deeper
        # dry-board discount (-0.040 from v117) becomes partially redundant.
        # Reduced by 15% to -0.034 to prevent double-counting the correction.
        margin -= 0.034
...
# [Crossover from national_v71 (Beta)] Calibrated equity realization model.
# v119 applied win_rate*0.6 then EQR*0.55-1.0 = triple discount, which over-folds
# marginal made hands vs value-heavy opponents. v71's model uses win_rate directly,
# applies EQR calibrated per street (river~1.0, turn small, flop moderate), and a
# lower safety margin, so thin value continues instead of folding.
# H2H evidence: v71 beats national_v111 (53.0% vs v119's 42.0%) and national_v52
# (58.3% vs v119's 44.2%) — opponents where v119's over-fold leak costs chips.
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opponent_model=None, spr=999.0, pot_odds=0.0, win_rate=0.0, has_position=True):
    ...
    # Use win_rate directly (no 0.6 discount) — the 0.6 discount triple-discounted
    # equity and caused over-folding of marginal made hands.
    ...
    # Per-street EQR calibration (crossover from v71):
    # River: all cards out, equity is fully realized.
