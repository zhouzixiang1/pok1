...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, texture_class="none"):
    ...
    # ── Betsize polarity adjustment (continuous, structurally new axis) ──
    # Opponent_model['betsize_polarity'] in [-1,+1]: + = polarized (mix of
    # large/small raises), 0 = uniform/unknown. vs polarized bettors: large
    # bets skew value-heavy (tighten), small bets skew bluff-heavy (loosen).
    polarity = opponent_model.get("betsize_polarity", 0.0)
    opp_conf = opponent_model.get("confidence", 0.0)
    if round_idx >= 2 and polarity > 0.0:
        if size_bucket == "large":
            polarity_delta = 0.022 * polarity * opp_conf
        elif size_bucket == "small":
            polarity_delta = -0.016 * polarity * opp_conf
        else:
            polarity_delta = 0.0
    ...
    # M6: function-scope UNCONDITIONAL telemetry (no !=0 gate, hoisted to
    # function scope — prints total margin and polarity contribution).
    sys.stderr.write(
        f"POLARITY_MARGIN final_milli={margin*1000:+.0f} "
        f"polarity_delta_milli={polarity_delta*1000:+.0f} "
        f"polarity={polarity:+.3f} opp_conf={opp_conf:.2f} "
        f"size_bucket={size_bucket} round={round_idx}\n"
    )
