...
from opponent import build_opponent_model, analyze_current_spot, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold, _semibluff_raise_construct, _river_value_raise_construct, _allin_polarized_equity_fold
...
def _should_4bet_light(my_cards, preflop_strength, opponent_model, state, my_chips):
    ...
    # v187 scale-shift: band [0.30, 0.55) admits small pairs (22=0.50/33=0.53)
    # as intended 4bet-light candidates; < 0.30 floor inert, 0.55 excludes premiums.
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        if _river_weak_made_hand_gate(round_idx, pot, my_chips, made_strength,
            ...
            print(f"RIVER_WEAK_FOLD made={made_strength:.2f} "
                  ...
                  file=sys.stderr)
            return -1
        # v274 CROSSOVER (from v235/v202): calibrated-equity fold for marginal
        # one-pair vs polarized all-in; win_rate overestimates vs value range.
    ...
    # v193: _river_stackoff_guard relocated before the all-in early-return
    # (fixes placement_shadow); guards 0 < to_call < my_chips only.
