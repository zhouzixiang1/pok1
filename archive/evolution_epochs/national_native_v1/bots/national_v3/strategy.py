...
from opponent import build_opponent_model, analyze_current_spot, _board_texture_bluff_raise, _turn_float_value_donk, _postflop_response_margin, _multibarrel_line_fold, _semibluff_raise_construct, _river_potodds_equity_margin
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, opponent_model=None, spr=999.0):
    ...
    # v137: defense relaxation vs confirmed polarized overbettors. Placed BEFORE
    # all upstream fold gates (avoids dispatch-order shadow — a trailing override
    # is a no-op if an upstream gate returns first). Fires only when:
    # sizing_tendency samples>=8, confidence>=0.30, tendency=='overbettor',
    # size_bucket=='large', and NOT absolute air (made<0.15 and no draw).
    if opponent_model is not None:
        ...
        if sizing_signal_valid and size_bucket == 'large':
            ...
            if not absolute_air:
                # Telemetry: SIZING_RELAX_FIRE proves reachability (avoid
                # recurring INERTNESS failure — dead fold gates ship undetected).
                try:
                    sys.stderr.write(
...
def get_action(req, requests):
    ...
    if to_call > 0:
        # v197 CROSSOVER (v193 × v179): REMOVED 3 direct binary fold gates
        # (_aggro_bluffcatcher_should_fold, _rock_value_bet_fold, non-allin
        # _multibarrel_line_fold). They over-folded weak bluff-catchers vs
        # wide-barreling opps (v177/v159). The all-in _multibarrel_line_fold
        # dispatch (~L1577) is KEPT for stack-covering leak protection; the
        # continuous _river_potodds_equity_margin provides river fold calibration.

        # v279 CROSSOVER (v241 × v234): direct river pot-odds-vs-equity fold,
        # ported from v234 which beat shared nemeses v208/v209/v197 (0.68/0.80/
        # 0.867 vs v241's 0.56/0.52/0.60 — aggressive value-heavy shovers). The
        # margin-additive dispatch inside _postflop_response_margin is INERT vs
        # polarized value ranges (realized_rate overestimates weak-made equity).
        # Gate enforces its own conditions: river only, bet>=0.4x pot, made
        # [0.20,0.50), tier!=strong/nut, draw<0.15, overbettor-exempt. This is
        # the SANCTIONED TARGETED DEFENSE EXCEPTION (not a generic floor; not
        # the 3 over-folding archetype gates v241's v197 crossover removed).
        # MUTATION (exp-pool v276 lesson): STDERR fire-rate telemetry so
        # reachability is verifiable in the daemon before trusting as progress.
        if round_idx == 3:
            ...
            if _river_pe_delta > 0:
                try:
                    sys.stderr.write(
                        'RIVER_PE_FOLD made=%.2f ds=%.2f po=%.3f delta=%.3f '
                        'tier=%s br=%.2f\n' % (
                            made_strength, draw_strength, pot_odds, _river_pe_delta,
