...
def spr_commitment_gate(my_chips, pot, value_profile, pair_profile, made_hand_class, board_texture=None):
    """SPR commitment gate (crossover from v11/PNLH). Return (committed, spr, threshold, label).
    ...
    Crossover extension (v39 board-texture awareness): when ``board_texture`` is
    ...
    [v39 repair] Rolled back dynamic-board TPTK factor 0.55 -> 0.625 (crossover
    baseline). The 0.55 tightening was based on the STALE polarized-jam advisory
    (no >=30g WR-lift evidence). At factor 0.55 the effective TPTK threshold on
    dynamic boards was 4*0.55=2.2; the SPR 2.2-2.5 gap allowed downstream fold
    gates to over-fold TPTK vs aggressive opponents (national_v2 0W-1L). Restoring
    0.625 (threshold 2.5) keeps TPTK committed through normal SPR ranges while
    preserving the board-texture awareness branch as a real code change.
    """
    ...
    _TPTK_DYNAMIC_TIGHTEN_FACTOR = 0.625  # repair: 0.55 -> 0.625 crossover baseline
