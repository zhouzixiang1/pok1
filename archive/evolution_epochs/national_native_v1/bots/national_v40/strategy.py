...
def _sb_open_opp_exploit_delta(opponent_model):
    """SB open threshold delta keyed to opponent fold_to_raise tendency.

    Structural axis completing the parallelism with bb_vs_limp (vpip/ftr terms)
    and bb_vs_raise (immediate sizing + historical sizing + tight-open PFR).
    SB is the most valuable steal seat in HU; fold_to_raise is the cleanest
    signal that a raise will win the blinds immediately.

    Continuous formula (no deadzone, M5/M6 compliant):
      delta = -0.05 * confidence * clamp((ftr - PRIOR_FOLD_TO_RAISE) / 0.18, -1, 1)
    Negative delta widens (steal more), positive delta tightens (vs stations).
    At confidence=0 or ftr=PRIOR_FOLD_TO_RAISE, delta is exactly 0.
    """
    confidence = opponent_model.get('confidence', 0.0)
    ftr = opponent_model.get('fold_to_raise', PRIOR_FOLD_TO_RAISE)
    raw_signal = (ftr - PRIOR_FOLD_TO_RAISE) / 0.18
    clamped_signal = max(-1.0, min(1.0, raw_signal))
    delta = -0.05 * confidence * clamped_signal
    if clamped_signal > 0.05:
        reason = 'ftr_high_widen'
    elif clamped_signal < -0.05:
        reason = 'ftr_low_tighten'
    else:
        reason = 'prior_no_delta'
    sys.stderr.write(
        f"SB_OPEN_OPP_EXPLOIT ftr={ftr:.3f} prior={PRIOR_FOLD_TO_RAISE:.3f} "
        f"conf={confidence:.2f} signal={clamped_signal:+.2f} "
        f"delta={delta:+.4f} reason={reason}\n"
    )
    return delta


# ── Crossover from v18: EV-based preflop jam defense ─────────────────────────
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "sb_open":
        _sb_opp_delta = _sb_open_opp_exploit_delta(opponent_model)
        open_threshold = SB_OPEN_THRESHOLD + match_adjust + SB_OPEN_MATCH_ADJ + match_profile["open_delta"] + _sb_opp_delta
        limp_threshold = SB_LIMP_THRESHOLD + match_adjust + _sb_opp_delta
        ...
        if not trash_hand and eff_preflop_strength >= open_threshold and raise_amount is not None:
            return raise_amount
        if eff_preflop_strength <= limp_threshold - loose_bonus:
            return -1
        return 0
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if not trash_hand and eff_preflop_strength >= iso_threshold and raise_amount is not None:
            return raise_amount
        return 0
        ...
        # eff_bluff_freq and eff_call_threshold set below after historical sizing adjustments
        # ── Crossover from v18: EV-based jam defense (early return) ──────────
        ...
        if state.get('opponent_allin', False) or (eff_stack_jam > 0 and raise_to_total_jam >= 0.50 * eff_stack_jam):
            ...
            if jam_decision is not None:
                return jam_decision
        # ── Crossover from v18: Historical sizing-tendency adjustment ──────
        # Distinct from _def_adj (immediate raise size). Tighten vs confirmed
        # large openers (polarized range); widen vs confirmed small openers.
        ...
        # ── Crossover from v18: NEMESIS_TIGHT_OPEN_PFR ─────────────────────
        # Targets opponent RANGE TIGHTNESS. Static BB_CALL_THRESHOLD calls
        # dominated hands vs tight ranges. Inverted delta for wide openers
        # defends wider.
        _opp_pfr = opponent_model.get('pfr', PRIOR_PFR)
        _tight_open_pfr_delta = 0.0
        if confidence >= 0.20:
            if _opp_pfr <= 0.20:
                _tight_open_pfr_delta = +0.030
            elif _opp_pfr >= 0.32:
                _tight_open_pfr_delta = -0.025
        sys.stderr.write(
            f"NEMESIS_TIGHT_OPEN_PFR pfr={_opp_pfr:.3f} conf={confidence:.2f} "
            f"pfr_call_delta={_tight_open_pfr_delta:+.3f}\n"
        )
        eff_bluff_freq = BB_BLUFF_3BET_FREQ + _def_adj['bluff_freq_delta'] + _hist_bluff_freq_delta
        eff_call_threshold = BB_CALL_THRESHOLD + _def_adj['call_delta'] + _hist_call_delta + _tight_open_pfr_delta
...
def pot_odds_call_threshold(pot_odds, has_position, round_idx, draw_info, spr):
    ...
    Adjustments:
    - Position: IP needs ~2.5% less equity (better realization)
    ...
    # Position adjustment — mutation: 0.02 → 0.025 (25% wider IP calling range)
    if has_position:
        threshold -= 0.025
...
def _spr_commitment_gate(round_idx, made_strength, value_profile, win_rate, pot_odds, to_call, pot, my_chips, anti_lock_pressure, anti_lock_call_continue, strong_made_continue):
    """Turn/River low-SPR commitment fold gate for marginal hands facing polarized bets.

    Closes the -14k/-16k stack-off leak vs trapping opponents (v16: 10% WR / 10g).
    Folds thin/none-tier marginal hands on the turn/river when
    SPR is commitment-low and realized equity (win_rate) is below pot_odds.
    Honors anti-lock endgame pressure and genuinely strong hands via continue overrides.
    NEW structural axis — distinct from should_fold_postflop (EQR-based) and the
    exhausted _river_stackoff_guard direction.
    Turn (round_idx==2): SPR<=4.0, made<0.50 — catches mid-pot turn stack-offs missed by river-only coverage.
    River (round_idx==3): SPR<=2.5, made<0.55 — original behavior preserved.
    """
    import sys
    if round_idx not in (1, 2, 3):
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_street round={round_idx}\n")
        return None
    if to_call <= 0:
        sys.stderr.write("SPR_COMMITMENT_GATE reason=skip_no_to_call\n")
        return None
    spr = my_chips / max(1, pot + to_call)
    # Graduated SPR ceiling: flop widest (two cards to come, most equity realization),
    # turn mid (one card to come), river tightest (equity fully realized).
    if round_idx == 1:
        spr_ceiling = 6.0
    elif round_idx == 2:
        spr_ceiling = 4.0
        ...
        spr_ceiling = 2.5
    if spr > spr_ceiling:
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_spr={spr:.2f} ceiling={spr_ceiling}\n")
        return None
    tier = value_profile.get("tier", "none") if value_profile else "none"
    if tier in ("strong", "nut"):
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_tier={tier}\n")
        return None
    if round_idx == 1:
        made_ceiling = 0.38
    elif round_idx == 2:
        ...
        made_ceiling = 0.55
    if made_strength >= made_ceiling:
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_made={made_strength:.2f} ceiling={made_ceiling}\n")
        return None
    if anti_lock_pressure and anti_lock_call_continue:
        sys.stderr.write("SPR_COMMITMENT_GATE reason=skip_anti_lock\n")
        return None
    if strong_made_continue:
        sys.stderr.write("SPR_COMMITMENT_GATE reason=skip_strong_continue\n")
        return None
    equity_margin = 0.06 if round_idx == 1 else 0.0
    if win_rate < pot_odds + equity_margin:
        sys.stderr.write(
            f"SPR_COMMITMENT_GATE reason=fired tier={tier} made={made_strength:.2f} "
            f"win={win_rate:.3f} pot_odds={pot_odds:.3f} margin={equity_margin:.2f} "
            f"round={round_idx} spr={spr:.2f} to_call={to_call}\n"
        )
        return -1
    sys.stderr.write(
        f"SPR_COMMITMENT_GATE reason=pass_equity_ok tier={tier} made={made_strength:.2f} "
        f"win={win_rate:.3f} pot_odds={pot_odds:.3f} spr={spr:.2f}\n"
    )
    return None
...
def get_action(req, requests):
    ...
    # Early-compute continue flags so the SPR commitment gate covers ALL facing-bet
    # paths (including the to_call>=my_chips stack-cover scenario that previously
    # early-returned before the to_call>0 block could fire).
    ...
    # === SPR COMMITMENT GATE (crossover from v11; turn/river never-fold axis) ===
    # Committed premium made hands (trips+) never fold regardless of pot-odds;
    # OR into strong_made_continue so downstream fold gates no-op for nutted hands.
    # Tightened from original crossover: two pair (class 2) and all pair types are
    # EXCLUDED — they are too exploitable by polarized jamming on coordinated boards.
    # The existing _spr_commitment_gate fold-side gate + must_continue_vs_raise still
    # govern two pair / pairs as in v32. Only trips/straight/flush/full/quads/strflush
    # (hand_class >= 3) get the absolute never-fold override at commitment SPR.
    if round_idx >= 2 and len(public_cards) >= 4:
        _made_hand_class = evaluate_best(my_cards + public_cards)[0]
        _spr_committed, _spr_val, _spr_thresh, _spr_label = spr_commitment_gate(
            my_chips, pot, value_profile, pair_profile, _made_hand_class,
            board_texture,
        )
        if _spr_committed and _made_hand_class >= 3:
            strong_made_continue = True
    _spr_gate_decision = _spr_commitment_gate(
        round_idx, made_strength, value_profile, win_rate, pot_odds,
        to_call, pot, my_chips,
        anti_lock_pressure, anti_lock_call_continue, strong_made_continue,
    )
    if _spr_gate_decision == -1:
        return -1
    ...
    if to_call > 0:
        ...
        if anti_lock_pressure:
            call_threshold -= 0.07
        # anti_lock_call_continue and strong_made_continue are computed above
        # (before the to_call>=my_chips block) so the SPR commitment gate can
        # cover all facing-bet paths.
...
if __name__ == '__main__':
    # Self-test: _preflop_jam_call_decision helper must return non-None for clear spots
    ...
    assert _preflop_jam_call_decision(0.47, 0.50, 0.40, 0.30) == 0, 'wide jammer slack=0.05 calls 0.47 vs 0.50'
    assert _preflop_jam_call_decision(0.45, 0.50, 0.40, 0.30) == 0, 'wide jammer slack=0.05 calls 0.45 (boundary-inclusive) vs 0.50'
    assert _preflop_jam_call_decision(0.43, 0.50, 0.40, 0.30) is None, 'wide jammer slack=0.05 defers 0.43 vs 0.50'
    assert _preflop_jam_call_decision(0.38, 0.50, 0.40, 0.30) == -1, 'wide jammer slack=0.05 folds 0.38 vs 0.50'

    # Self-test: _sb_open_opp_exploit_delta must satisfy M5/M6 telemetry-fidelity
    # Live pool defaults: ftr=0.44 (PRIOR_FOLD_TO_RAISE), confidence varies.
    assert abs(_sb_open_opp_exploit_delta({'fold_to_raise': 0.44, 'confidence': 0.5})) < 1e-9, \
        'prior ftr must yield zero delta'
    assert _sb_open_opp_exploit_delta({'fold_to_raise': 0.62, 'confidence': 0.0}) == 0.0, \
        'zero confidence must yield zero delta'
    d_high = _sb_open_opp_exploit_delta({'fold_to_raise': 0.62, 'confidence': 0.4})
    assert d_high < -0.005, f'high folder (ftr=0.62) must widen SB opens, got {d_high}'
    d_low = _sb_open_opp_exploit_delta({'fold_to_raise': 0.26, 'confidence': 0.4})
    assert d_low > 0.005, f'calling station (ftr=0.26) must tighten SB opens, got {d_low}'
    print('sb-open-opp-exploit self-test OK')
