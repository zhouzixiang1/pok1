...
def match_pressure_profile(req, my_id, remaining_hands):
    ...
    late_factor = clamp((ENDGAME_HANDS - hands) / ENDGAME_RAMP_HANDS, 0.0, 1.0)
    if late_factor <= 0.0:
        return profile
...
def choose_anti_lock_pressure_action(
    ...
    emergency_jam = (
        hands_left <= ANTI_LOCK_JAM_HANDS
        or (to_call > 0 and to_call / max(1, pot) >= 0.35)
        or (weak_showdown and high_fold_pressure and hands_left <= ANTI_LOCK_WEAK_SHOWDOWN_HANDS)
        or (win_rate < 0.18 and hands_left <= ANTI_LOCK_AIR_HANDS)
    )
    if tier in ("strong", "nut") or has_draw:
        emergency_jam = emergency_jam and hands_left <= ANTI_LOCK_JAM_HANDS

    if emergency_jam:
        if round_idx == 0:
            strength = preflop_strength if preflop_strength is not None else win_rate
            if hands_left <= 1 and strength >= ANTI_LOCK_PREFLOP_LAST_HAND_FLOOR:
                return -2
            if strength >= ANTI_LOCK_PREFLOP_JAM_FLOOR:
                return -2
        elif hands_left <= 1 or tier in ("strong", "nut") or has_draw or win_rate >= ANTI_LOCK_POSTFLOP_JAM_FLOOR:
            return -2
    ...
    if amount <= to_call or amount < state["round_raise"]:
        return -2 if hands_left <= 1 else None
    return amount
