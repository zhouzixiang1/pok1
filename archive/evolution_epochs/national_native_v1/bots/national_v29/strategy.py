...
def _spr_commitment_gate(round_idx, made_strength, value_profile, win_rate, pot_odds, to_call, pot, my_chips, anti_lock_pressure, anti_lock_call_continue, strong_made_continue):
    """River low-SPR commitment fold gate for marginal hands facing polarized bets.

    Closes the -14k/-16k stack-off leak vs trapping opponents (v16: 10% WR / 10g).
    Folds thin/none-tier marginal hands (made_strength < 0.55) on the river when
    ...
    import sys
    if round_idx != 3:
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_not_river round={round_idx}\n")
        return None
    if to_call <= 0:
    ...
    if spr > 2.5:
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_spr={spr:.2f}\n")
        return None
    tier = value_profile.get("tier", "none") if value_profile else "none"
    if tier in ("strong", "nut"):
    ...
    if made_strength >= 0.55:
        sys.stderr.write(f"SPR_COMMITMENT_GATE reason=skip_made={made_strength:.2f}\n")
        return None
