...
def _bb_defense_pressure_profile(opponent_model):
    ...
    Targets the v70 BB/oop over-defense leak (H2H v70_wr=0.20 vs v66/v69/v3/v7;
    ev_b3ca7b6abc8458f4 / ev_61e93510af89f765: aggressive BB orientation bleeds chips).
    M5/M6: continuous, no deadzone, non-zero at live-pool priors; telemetry at
    function scope prints TOTAL deltas every call.
    """
    pfr = opponent_model.get('pfr', PRIOR_PFR)
    ...
    # STANDARD arm (M5: non-zero at priors so the detector is never INERT for
    # the 96.9% standard bucket). Small unconditional preflop tightening.
    std_call = 0.020
    std_bluff = -0.030
    std_iso = 0.015
