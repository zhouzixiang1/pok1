...
# [v61] Tight-opponent raise suppression (crossover from v59)
...
# [v69] River SPR-commitment fold gate.
SPR_COMMITMENT_FOLD_MADE_MAX = 0.55


# BB Defense-Depth Matrix keyed to SB open-raise size.
...
def _tight_opp_raise_suppress(opponent_model, made_strength, draw_strength, round_idx, value_profile):
    """Suppress marginal postflop raises vs tight opponents."""
...
# [v55] Preflop jam defense via pot-odds-equity.
def _preflop_jam_call_decision(win_rate, pot_odds, opponent_allin_rate, confidence):
    """Return call/fold/defer for preflop jam spots."""
...
def _bb_defense_depth_adjustments(raise_to_total):
    """Return BB defense threshold deltas keyed to SB open size."""
...
def postflop_call_margin(spot_info, opponent_model, made_strength, draw_strength, round_idx, has_position, texture_class="none"):
    ...
    # Betsize polarity + shove defense.
    ...
    # Prefer per-street polarity when available; fallback to aggregate.
    if round_idx == 3:
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # Bluff 3bet; skip vs calling stations.
...
# Pot-odds-equity fold gate with calibrated EQR.
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opponent_model=None, spr=999.0, pot_odds=0.0, win_rate=0.0, has_position=True):
    ...
    # Equity estimation via win_rate and street-calibrated EQR.
...
def _spr_commitment_gate(round_idx, value_profile, made_strength, win_rate, pot_odds,
                         draw_strength=0.0, last_raise_pot_ratio=0.0):
    ...
    [v72] Turn branch (round_idx==2) extends the OPEN SPR-commitment axis with
    a strictly conservative parameter set: lower made ceiling, draw ceiling so
    real draws are preserved, small river-improvement equity credit, and a
    size-bucket floor so small stabs are unaffected. See module-local literals
    below — these are NOT constants.py edits.
    """
    # ---- TURN branch (round_idx == 2): OPEN SPR-commitment axis, conservative ----
    # Experience pool: 'SPR commitment thresholds are a structurally new turn/river
    # axis DISTINCT from CLOSED fold-side directions; TPTK/overpair/two_pair
    # commitment gating is OPEN.' This branch probes the turn side. Stricter than
    # river: lower made ceiling, no-draw gate, small river-improvement credit,
    # medium/large bet only.
    _TURN_FOLD_MADE_MAX = 0.50
    _TURN_FOLD_DRAW_MAX = 0.14
    _TURN_EQUITY_CREDIT = 0.04
    _TURN_MIN_RAISE_POT_RATIO = 0.30  # BET_SIZE_SMALL_MAX; only medium/large
    if round_idx == 2:
        tier = value_profile.get('tier', 'none') if value_profile else 'none'
        if tier not in ('thin', 'none'):
            return False
        if made_strength >= _TURN_FOLD_MADE_MAX:
            return False
        if draw_strength >= _TURN_FOLD_DRAW_MAX:
            return False
        if last_raise_pot_ratio < _TURN_MIN_RAISE_POT_RATIO:
            return False
        fold = win_rate < pot_odds + _TURN_EQUITY_CREDIT
        sys.stderr.write(
            f'SPR_FOLD round=2 fired={1 if fold else 0} '
            f'reason={"equity_below_potodds_credit" if fold else "pass"} '
            f'tier={tier} made={made_strength:.3f} draw={draw_strength:.3f} '
            f'win_rate={win_rate:.3f} pot_odds={pot_odds:.3f} '
            f'credit={_TURN_EQUITY_CREDIT:.3f} ratio={last_raise_pot_ratio:.3f}\n'
        )
        return fold
    # ---- RIVER branch (round_idx == 3): UNCHANGED ----
    if round_idx != 3:
        return False
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    if tier not in ('thin', 'none'):
    ...
    sys.stderr.write(
        f'SPR_FOLD round=3 fired={1 if fold else 0} '
        f'reason={"equity_below_potodds" if fold else "pass"} '
...
def get_action(req, requests):
    ...
    if to_call > 0 and _spr_commitment_gate(
        round_idx, value_profile, made_strength, win_rate, pot_odds,
        draw_strength=draw_strength,
        last_raise_pot_ratio=spot_info.get("last_raise_pot_ratio", 0.0),
    ):
...
def _self_test_spr_commitment_gate():
    ...
    [v72] Also verifies the turn (round_idx==2) branch with its strictly
    conservative parameter set (made<0.50, draw<0.14, ratio>=0.30,
    win_rate<pot_odds+0.04).
    """
    ...
    # ---- TURN branch (round_idx == 2) ----
    # Turn, thin tier, weak made, no draw, equity below pot_odds+credit -> fold
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.20, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.75) is True, \
        'thin turn no-draw equity<potodds+credit must fold'
    # Turn, thin tier, but WITH a meaningful draw -> never fold (draw gate)
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.20, 0.33,
                                draw_strength=0.15, last_raise_pot_ratio=0.75) is False, \
        'turn with meaningful draw (>=0.14) must pass'
    # Turn, thin tier, made>=0.50 -> never fold (made gate)
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.52, 0.20, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.75) is False, \
        'turn made>=0.50 must pass'
    # Turn, small bet (ratio<0.30) -> never fold (size gate)
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.20, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.20) is False, \
        'turn small bet must pass'
    # Turn, strong tier -> never fold (tier gate)
    assert _spr_commitment_gate(2, {'tier': 'strong'}, 0.40, 0.20, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.75) is False, \
        'turn strong tier must pass'
    # Turn, equity above pot_odds+credit -> no fold
    assert _spr_commitment_gate(2, {'tier': 'none'}, 0.40, 0.42, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.75) is False, \
        'turn equity>=potodds+credit must pass'
    # Turn boundary: equity exactly at pot_odds+credit (0.33+0.04=0.37) -> no fold (strict <)
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.37, 0.33,
                                draw_strength=0.05, last_raise_pot_ratio=0.75) is False, \
        'turn equity==potodds+credit must pass (strict less-than)'
