...
def disciplined_opp_river_margin(opponent_model, value_profile, round_idx):
    """Crossover from v37: sanctioned-defense river overcall penalty vs
    tight-disciplined opponents.

    vs tight-passive barrels, marginal thin-tier made hands (top_pair_weak_kicker,
    middle_pair, bottom_pair, underpair, low two_pair) are value-cut by disciplined
    value-heavy river ranges. Apply a continuous POSITIVE call_margin delta (raises
    call threshold -> fold marginal hands vs value-heavy ranges), scaled by how far
    the opponent sits below the standard priors (vpip=0.62, pfr=0.32).

    Uses only surviving opp fields (vpip/pfr/confidence) per experience_pool
    OPPONENT_MODELING lesson. Standard-bucket (vpip>=0.62, pfr>=0.32) returns
    exactly 0.0 by construction — long-tail H2H is unaffected.

    [v59 mutation] delta coefficient 0.035 -> 0.042 (+20%): strengthen fold signal
    vs confirmed tight-disciplined opponents. v54 loses 37-45% WR vs v34/v7
    (tight value-heavy cluster); v37 (which has this gate) wins 52-56% vs same.
    The +20% magnitude increase targets this leak without widening the firing band.
    """
    ...
    vpip_dev = max(0.0, 0.62 - vpip)
    pfr_dev = max(0.0, 0.32 - pfr)
    tightness = clamp((vpip_dev + pfr_dev) / 0.20, 0.0, 1.0)
    delta = 0.042 * tightness * confidence
...
def nash_river_sizing_tier(req, public_cards):
    """Nash-ward river sizing tier, partially independent of hand strength.

    Returns one of {0.50, 0.67} via a deterministic hand-seeded hash. Value
    hands and bluffs use the SAME tier, reducing sizing tells.
    The original v48 used {0.33, 0.50, 0.67, 1.0}, but the 0.33 tier leaked
    massive value from strong hands (underbet) and the 1.0 tier overbet bluffs
    while bypassing the non-nut river cap in strategy.py. Clamping to the two
    mid-range tiers preserves randomization (2 sizings, not 1) while keeping
    every override within a standard, safe band close to choose_raise output.
    """
    hole = req.get('my_cards', [])
    hand_idx = get_hand_index(req) or 0
    seed = (sum(hole) * 7 + sum(public_cards) * 11 + hand_idx * 13) % 2
    tiers = [0.50, 0.67]
    return tiers[seed]


def nash_river_bet_amount(tier_ratio, to_call, pot, min_raise, my_chips, req, public_cards):
    """Compute river bet amount using Nash tier with +/-5% deterministic noise.

    Returns raise-to-total amount or None if illegal.
    The noise prevents exact-integer sizing reads while staying within 5%.
    """
    pot_after_call = pot + to_call
    hole = req.get('my_cards', [])
    hand_idx = get_hand_index(req) or 0
    noise_seed = (sum(hole) * 3 + sum(public_cards) * 5 + hand_idx * 9 + int(tier_ratio * 1000)) % 100
    noise = (noise_seed - 50) / 1000.0  # range -0.05 to +0.05
    amount = int(to_call + pot_after_call * tier_ratio * (1.0 + noise))
...
if __name__ == '__main__':
    ...
    # Nash river sizing self-test
    _fake_req_a = {'my_cards': [0, 8], 'my_id': 0}
    _fake_req_b = {'my_cards': [4, 12], 'my_id': 0}
    _pub = [16, 20, 24, 28]
    _tier_a = nash_river_sizing_tier(_fake_req_a, _pub)
    _tier_b = nash_river_sizing_tier(_fake_req_b, _pub)
    assert _tier_a in (0.50, 0.67), f'invalid tier {_tier_a}'
    assert _tier_b in (0.50, 0.67), f'invalid tier {_tier_b}'
    # Tier must be deterministic for same input
    assert nash_river_sizing_tier(_fake_req_a, _pub) == _tier_a, 'tier must be deterministic'
    _amt = nash_river_bet_amount(0.67, 0, 1000, 200, 19000, _fake_req_a, _pub)
    assert _amt is None or (_amt >= 200 and _amt < 19000), f'invalid amount {_amt}'
    print('nash_river_sizing self-test PASSED')
