...
# Tight-opponent raise suppression (v61 crossover): convert marginal raises to calls vs tight ranges.
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if state.get('opponent_allin', False) or (eff_stack_jam > 0 and raise_to_total_jam >= 0.50 * eff_stack_jam):
            ...
            if jam_decision is not None:
                return jam_decision
        # Adjust BB thresholds by SB open size.
...
def _self_test_polarity_shove_margin():
    """M5/M6 self-test: verify polarity + shove defense fires non-zero on live-pool defaults."""
...
def _self_test_spr_commitment_gate():
    """Verify SPR-commitment fold gates for river and low-SPR turn."""
    ...
    # [v111] Turn SPR-commitment gate
    # Turn, low SPR, thin tier, equity below pot_odds+buffer -> fold
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.25, 0.33, spr=2.0) is True, 'thin turn low-SPR equity<potodds+buffer must fold'
    # Turn, low SPR, thin tier, equity above threshold -> no fold
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.45, 0.33, spr=2.0) is False, 'turn equity>=threshold must pass'
    # Turn, HIGH SPR (>=threshold) -> no fold regardless of equity
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.40, 0.25, 0.33, spr=5.0) is False, 'high SPR turn must not fire'
    # Turn, strong tier -> never fold
    assert _spr_commitment_gate(2, {'tier': 'strong'}, 0.40, 0.25, 0.33, spr=2.0) is False, 'strong tier unaffected on turn'
    # Turn, made_strength >= SPR_TURN_FOLD_MADE_MAX -> never fold
    assert _spr_commitment_gate(2, {'tier': 'thin'}, 0.55, 0.25, 0.33, spr=2.0) is False, 'made>=0.50 unaffected on turn'
    print('SPR_COMMITMENT_GATE self-test OK')


def _self_test_bb_flop_checkraise():
    """Verify _bb_flop_checkraise_exploit gates behave as designed."""
    om_high_folder = {'confidence': 0.50, 'fold_to_raise': 0.55}
    om_low_folder = {'confidence': 0.50, 'fold_to_raise': 0.40}
    board_dry = {'wetness': 0.15, 'dynamic': False}
    board_wet = {'wetness': 0.50, 'dynamic': True}
    value_strong = {'tier': 'strong'}
    blocker_eligible = {'eligible': True}

    # OOP BB, dry board, strong hand, high fold_to_raise -> True
    assert _bb_flop_checkraise_exploit(
        1,
        {'facing_postflop_aggression': True, 'has_position': False},
        om_high_folder, board_dry, value_strong, {}, 0.05, 0.55, blocker_eligible, False,
    ), 'OOP BB strong value vs high folder must fire'

    # fold_to_raise below threshold -> False
    assert not _bb_flop_checkraise_exploit(
        1,
        {'facing_postflop_aggression': True, 'has_position': False},
        om_low_folder, board_dry, value_strong, {}, 0.05, 0.55, blocker_eligible, False,
    ), 'fold_to-raise below threshold must not fire'

    # Wet/dynamic board -> False
    assert not _bb_flop_checkraise_exploit(
        1,
        {'facing_postflop_aggression': True, 'has_position': False},
        om_high_folder, board_wet, value_strong, {}, 0.05, 0.55, blocker_eligible, False,
    ), 'wet/dynamic board must not fire'

    # In-position (SB) -> False
    assert not _bb_flop_checkraise_exploit(
        1,
        {'facing_postflop_aggression': True, 'has_position': True},
        om_high_folder, board_dry, value_strong, {}, 0.05, 0.55, blocker_eligible, False,
    ), 'in-position must not fire'

    # Marginal made hand with blocker vs high folder -> True
    assert _bb_flop_checkraise_exploit(
        1,
        {'facing_postflop_aggression': True, 'has_position': False},
        om_high_folder, board_dry, {'tier': 'thin'}, {}, 0.05, 0.38, blocker_eligible, False,
    ), 'marginal blocker hand vs high folder must fire'

    print('BB_FLOP_CHECKRAISE self-test OK')


# M5/M6 self-test entry point.
if __name__ == "__main__":
    ...
    _self_test_bb_flop_checkraise()

    # M5/M6 self-test: bet-size adaptation must be non-INERT.
    _spot_normal = {'last_raise_bb': 5.5, 'facing_postflop_aggression': True}
    _spot_large = {'last_raise_bb': 11.0, 'facing_postflop_aggression': True}
    _spot_small = {'last_raise_bb': 2.5, 'facing_postflop_aggression': True}
    _om_standard = {'confidence': 0.50, 'avg_river_raise_bb': 5.5}
    _om_tight = {'confidence': 0.50, 'avg_river_raise_bb': 3.0}
    _adapt_normal = _opp_bet_size_adaptation(_spot_normal, _om_standard, 3)
    _adapt_large = _opp_bet_size_adaptation(_spot_large, _om_standard, 3)
    _adapt_small_vs_tight = _opp_bet_size_adaptation(_spot_small, _om_tight, 3)
    _sys.stderr.write(f'SELFTEST_BET_SIZE_ADAPT normal={_adapt_normal:.4f} large={_adapt_large:.4f} small_vs_tight={_adapt_small_vs_tight:.4f}\n')
    assert _adapt_large > 0, 'INERT: larger-than-avg bet should produce positive margin'
    assert _adapt_normal == 0.0, 'WRONG: bet=avg should produce zero delta'
    assert _adapt_small_vs_tight < 0, 'WRONG SIGN: smaller-than-avg bet should produce negative margin'
    print('SELFTEST_BET_SIZE_ADAPT_OK')
