...
from card_utils import clamp, evaluate_best, card_number, card_suit
from state import (
...
from opponent import build_opponent_model, analyze_current_spot, classify_opponent_archetype, classify_opp_open_sizing
from postflop import (
    ...
    polarized_jam_call_gate, spr_commitment_gate,
)
...
# Opponent-conditioned raise suppression (v49: reduce pot escalation vs tight opponents)
TIGHT_OPP_PFR_MAX = 0.22       # opponent pfr at/below this is 'tight' (prior=0.28)
TIGHT_OPP_RAISE_SUPPRESS_CONF = 0.40  # need >=5 hands for reliable read
TIGHT_OPP_RAISE_SUPPRESS_MS_LOW = 0.48
TIGHT_OPP_RAISE_SUPPRESS_MS_HIGH = 0.66
TIGHT_OPP_RAISE_SUPPRESS_DRAW_CEIL = 0.20

# v54 preflop tight-opponent defense gate (PFR-based; distinct from sizing/archetype axes)
# Fires when smoothed opp_pfr is low enough to indicate premium-only open/3bet range.
# smooth_rate(raises, opps, PRIOR_PFR=0.28, weight=4.0): a 10%-raiser (3/30) -> 0.12 (fires);
# a 28%-raiser (8/30) -> 0.27 (does not fire). Confidence >=0.25 needs ~14 total opp actions.
PREFLOP_TIGHT_OPP_PFR = 0.18       # smoothed opp pfr at/below this = tight range
PREFLOP_TIGHT_OPP_CONF = 0.25      # confidence floor (avoids noisy small-sample misclass)
SB_TIGHT_3BET_CONTINUE = 0.62      # vs tight 3bettor: continue threshold (up from 0.55)
BB_TIGHT_CALL_DELTA = 0.08         # vs tight opener: raise BB call threshold by this
...
def _preflop_jam_call_decision(win_rate, pot_odds, opponent_allin_rate, confidence):
    ...
    Caller must verify the spot is a jam (opponent_allin OR raise_to >= 50% eff stack).
    Routes jam decisions through pot-odds-equity; fixed strength thresholds are too
    tight vs opponents that jam wide ranges. Wide jammers (high allin_rate, confident
    read) get a more permissive slack so we call wider vs their trash-heavy range.
    """
...
def choose_anti_lock_pressure_action(
    ...
    # Deep-stack gate: in deep-stack play (>=30 BB), facing a moderate bet
    # should NOT trigger an all-in push even under anti-lock pressure.
    # Fall through to raise-sizing logic instead. Only push/fold when shallow.
    _deep_stack = my_chips >= 30 * BIG_BLIND  # 30 BB = 3000 chips
    _bet_ratio = to_call / max(1, pot)
    _deep_stack_bypassed = (
        to_call > 0 and _bet_ratio >= 0.35 and _deep_stack
    )
    emergency_jam = (
        hands_left <= 3
        or (to_call > 0 and _bet_ratio >= 0.35 and not _deep_stack)
    ...
    sys.stderr.write(
        f'ANTI_LOCK_EMERGENCY_JAM jam={1 if emergency_jam else 0} '
        f'deep_stack={1 if _deep_stack else 0} my_chips={my_chips} '
        f'hands_left={hands_left} bet_ratio={_bet_ratio:.2f} '
        f'weak_showdown={1 if weak_showdown else 0} tier={tier} '
        f'reason={"deep_stack_bypass" if _deep_stack_bypassed else ("emergency" if emergency_jam else "normal")}\n'
    )
    if emergency_jam:
        return -2
...
def _tight_opp_raise_suppress(opponent_model, made_strength, draw_strength, round_idx, value_profile):
    """Suppress marginal postflop raises vs confirmed tight opponents.
    ...
    # M5/M6 telemetry: print UNCONDITIONALLY (no !=0 gate) so fire-rate is honest
    sys.stderr.write(
...
def _flop_cbet_texture_ratio(wetness, dynamic, tier, has_position):
    """Continuous texture-dependent flop c-bet sizing ratio.

    Dry boards -> smaller sizing (value extraction from elastic callers).
    Wet/dynamic boards -> larger sizing (charge draws, protect equity).
    Returns None if the caller should fall through to default logic.
    Only called when round_idx==1 AND to_call==0 (we are the c-bettor).
    """
    # Continuous base: 0.42 at dry, up to ~0.77 at fully wet.
    # No deadzone, no thresholded elif chain (M5 compliant).
    base = 0.42 + 0.35 * clamp(wetness, 0.0, 1.0)
    if dynamic:
        base += 0.05  # dynamic boards (combo-draw-heavy) charge more
    # Position: IP (SB postflop) can size slightly smaller (more value); OOP sizes up
    if has_position:
        base -= 0.03
    else:
        base += 0.03
    # Tier nudges: nut/strong bet slightly bigger; thin slightly smaller
    if tier in ('nut', 'strong'):
        base += 0.02
    elif tier == 'thin':
        base -= 0.03
    ratio = clamp(base, 0.38, 0.82)
    sys.stderr.write(
        f'FLOP_CBET_TEXTURE ratio_milli={int(ratio*1000):+d} '
        f'wetness={wetness:.2f} dynamic={int(dynamic)} tier={tier} '
        f'pos={int(has_position)} reason=continuous_base\n'
    )
    return ratio
...
def choose_raise(
    ...
    # Texture-dependent flop c-bet sizing (only when we initiate the flop bet, to_call==0)
    if round_idx == 1 and to_call == 0 and board_texture is not None:
        _tier = value_profile.get('tier', 'none') if value_profile else 'none'
        _tex_ratio = _flop_cbet_texture_ratio(
            board_texture.get('wetness', 0.0),
            board_texture.get('dynamic', False),
            _tier,
            has_position,
        )
        if _tex_ratio is not None:
            ratio = _tex_ratio
    ...
    if round_idx == 0 and preflop_strength is not None:
        if spot_name == "sb_open":
            desired_total = int((SB_OPEN_BASE_BB + max(0.0, preflop_strength - SB_OPEN_STRENGTH_OFFSET) * SB_OPEN_STRENGTH_SCALE) * BIG_BLIND)
...
# ── Crossover from v40: dominated offsuit broadway penalty ──────────────────
_DOMINATED_OFFSUIT_BROADWAY = {
    # hand_key: (bb_vs_raise_penalty, sb_vs_reraise_penalty)
    # Calibrated so eff_strength = 1.0 - penalty drops below BB_VALUE_3BET(0.58)
    # and SB_VS_RERAISE_CALL(0.55). AKo/AQs/AJs/KQs are EXCLUDED (not dominated).
    'AQo': (0.12, 0.25),   # eff=0.88/0.75 — still value-3bets (AQo is strong)
    'AJo': (0.48, 0.60),   # eff=0.52/0.40 — call vs raise, fold vs 3bet
    'ATo': (0.55, 0.65),   # eff=0.45/0.35 — marginal call/fold vs raise, fold vs 3bet
    'KQo': (0.45, 0.58),   # eff=0.55/0.42 — call vs raise, fold vs 3bet
    'KJo': (0.55, 0.65),   # eff=0.45/0.35 — marginal call/fold vs raise, fold vs 3bet
    'KTo': (0.65, 0.70),   # eff=0.35/0.30 — fold vs raise, fold vs 3bet
    'QJo': (0.60, 0.68),   # eff=0.40/0.32 — fold vs raise, fold vs 3bet
}


def _dominated_broadway_penalty(my_cards, spot_name, opponent_model):
    """Preflop range construction: penalize dominated offsuit broadway hands
    that score artificially as 1.0 in estimate_preflop_strength, making them
    indistinguishable from AA/KK/AKs. Without this, KJo/QJo/ATo are 3bet for
    value in bb_vs_raise and call 3bets in sb_vs_reraise — root cause of
    big-pot stack-off losses vs value-heavy opponents."""
    r1 = card_number(my_cards[0])
    r2 = card_number(my_cards[1])
    if r1 == r2:  # pairs are never dominated-broadway penalized
        return 0.0
    s1 = card_suit(my_cards[0])
    s2 = card_suit(my_cards[1])
    if s1 == s2:  # suited hands have ~3% more equity, not penalized
        return 0.0
    high = max(r1, r2)
    low = min(r1, r2)
    if low < 10:  # only broadway combos (both cards >= Ten)
        return 0.0
    _RANK_CHAR = {14: 'A', 13: 'K', 12: 'Q', 11: 'J', 10: 'T'}
    key = _RANK_CHAR.get(high, '') + _RANK_CHAR.get(low, '') + 'o'
    if key not in _DOMINATED_OFFSUIT_BROADWAY:
        return 0.0  # AKo excluded — it dominates, not dominated
    if spot_name == 'bb_vs_raise':
        base = _DOMINATED_OFFSUIT_BROADWAY[key][0]
    elif spot_name == 'sb_vs_reraise':
        base = _DOMINATED_OFFSUIT_BROADWAY[key][1]
    else:
        return 0.0  # sb_open, bb_vs_limp: wide opponent range, no penalty
    penalty = base  # global — no per-opponent branching (direction audit)
    sys.stderr.write(
        f"DOM_BROADWAY_PENALTY hand={key} spot={spot_name} base={base:.3f} "
        f"penalty={penalty:.3f}\n"
    )
    return penalty


def _offsuit_broadway_key(my_cards):
    """Return offsuit broadway hand key (e.g. 'AJo') or None if not applicable."""
    r1 = card_number(my_cards[0])
    r2 = card_number(my_cards[1])
    if r1 == r2:
        return None
    if card_suit(my_cards[0]) == card_suit(my_cards[1]):
        return None
...
# Global preflop defense fold sets — opponent-independent (direction audit mandate)
_FOLD_VS_RAISE_OFFSUIT = frozenset({'AJo', 'ATo', 'KJo', 'KTo', 'QJo', 'JTo'})
_FOLD_VS_RERAISE_OFFSUIT = frozenset({'AQo', 'AJo', 'ATo', 'KQo', 'KJo', 'KTo', 'QJo', 'JTo'})
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    # ── Crossover from v40: dominated offsuit broadway penalty ──────────────
    _dom_penalty = _dominated_broadway_penalty(
        req["my_cards"], spot_info["preflop_spot"], opponent_model
    )
    eff_preflop_strength = preflop_strength - _dom_penalty
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # ── PREFLOP_JAM_DEFENSE: early return before BB_DEFENSE bucket logic ──
        ...
        if state.get('opponent_allin', False) or (eff_stack_jam > 0 and raise_to_total_jam >= 0.50 * eff_stack_jam):
            ...
            if jam_decision is not None:
                return jam_decision
        # === GLOBAL PREFLOP DEFENSE: hard-fold dominated offsuit broadway ===
        _fold_key = _offsuit_broadway_key(req['my_cards'])
        if _fold_key in _FOLD_VS_RAISE_OFFSUIT:
            sys.stderr.write(
                f'PREFLOP_RANGE_FOLD hand={_fold_key} spot=bb_vs_raise '
                f'reason=dominated_offsuit_global\n'
            )
            return -1
        ...
        # ── Historical sizing-tendency adjustment (NEW v287 axis) ─────────
        # Distinct from _def_adj (immediate raise size). Tighten vs confirmed
        # large openers (polarized range, doesn't fold to 3bets); widen vs
        # confirmed small openers (wide range we outdraw).
        _opp_hist_bucket = classify_opp_open_sizing(opponent_model)
        _hist_call_delta = 0.0
        _hist_bluff_freq_delta = 0.0
        if _opp_hist_bucket in ('large', 'xl'):
            _hist_call_delta = +0.015
            _hist_bluff_freq_delta = -0.10
        elif _opp_hist_bucket == 'small':
            _hist_call_delta = -0.015
        sys.stderr.write(
            f"BB_VS_RAISE_HIST_SIZING hist_bucket={_opp_hist_bucket} "
            f"immediate_bucket={_classify_open_raise_size(raise_to_total)} "
            f"call_d={_hist_call_delta:+.3f} bluff_freq_d={_hist_bluff_freq_delta:+.3f}\n"
        )
        ...
        eff_bluff_freq = BB_BLUFF_3BET_FREQ + _def_adj['bluff_freq_delta'] + _hist_bluff_freq_delta
        eff_call_threshold = BB_CALL_THRESHOLD + _def_adj['call_delta'] + _hist_call_delta
        # === v54 TIGHT-OPP BB DEFENSE: fold dominated hands vs premium-only openers ===
        _opp_pfr_bb = opponent_model.get('pfr', PRIOR_PFR)
        _opp_conf_bb = opponent_model.get('confidence', 0.0)
        _tight_opp_pf = _opp_conf_bb >= PREFLOP_TIGHT_OPP_CONF and _opp_pfr_bb <= PREFLOP_TIGHT_OPP_PFR
        if _tight_opp_pf:
            eff_call_threshold += BB_TIGHT_CALL_DELTA
            eff_bluff_freq = 0.0  # tight openers don't fold to 3bets — bluff 3bet is -EV
        sys.stderr.write(
            f'BB_TIGHT_OPP_DEFENSE fired={1 if _tight_opp_pf else 0} '
            f'pfr={_opp_pfr_bb:.3f} conf={_opp_conf_bb:.2f} '
            f'call_thr={eff_call_threshold:.3f} bluff_freq={eff_bluff_freq:.2f}\n'
        )
        # Value 3bet: TT+, AK, AQs (strength >= BB_VALUE_3BET_THRESHOLD)
        if eff_preflop_strength >= eff_value_3bet and not trash_hand:
            raise_amount = choose_raise(
        ...
        if can_bluff_3bet and eff_bluff_low <= eff_preflop_strength <= eff_bluff_high and not trash_hand:
        ...
        if eff_preflop_strength >= eff_call_threshold or win_rate >= pot_odds_pf - 0.02:
            return 0
        return -1

    elif spot_info['preflop_spot'] == 'sb_vs_reraise':
        pot_odds_sbr = to_call / (to_call + state['pot']) if to_call > 0 else 0.0
        # ── PREFLOP_JAM_DEFENSE: early return before SB_PREMIUM_4BET path ──
        ...
        if state.get('opponent_allin', False) or (eff_stack_sbr > 0 and raise_to_total_sbr >= 0.50 * eff_stack_sbr):
            ...
            if jam_decision is not None:
                return jam_decision
        # === GLOBAL PREFLOP DEFENSE: hard-fold dominated offsuit broadway vs 3bet ===
        _fold_key = _offsuit_broadway_key(req['my_cards'])
        if _fold_key in _FOLD_VS_RERAISE_OFFSUIT:
            sys.stderr.write(
                f'PREFLOP_RANGE_FOLD hand={_fold_key} spot=sb_vs_reraise '
                f'reason=dominated_offsuit_vs_3bet\n'
            )
            return -1
        # === v54 TIGHT-3BETTOR RESPECT: detect premium-only 3bet range ===
        _opp_pfr_sbr = opponent_model.get('pfr', PRIOR_PFR)
        _opp_conf_sbr = opponent_model.get('confidence', 0.0)
        _tight_3bettor = _opp_conf_sbr >= PREFLOP_TIGHT_OPP_CONF and _opp_pfr_sbr <= PREFLOP_TIGHT_OPP_PFR
        sys.stderr.write(
            f'SB_TIGHT_3BETTOR_RESPECT fired={1 if _tight_3bettor else 0} '
            f'pfr={_opp_pfr_sbr:.3f} conf={_opp_conf_sbr:.2f}\n'
        )
        # 4bet/jam with premiums: AA, KK, QQ, AKs (strength >= SB_PREMIUM_4BET_THRESHOLD)
        if eff_preflop_strength >= SB_PREMIUM_4BET_THRESHOLD:
            raise_amount = choose_raise(
                state['min_raise_action'], my_chips, state['my_round_bet'],
                to_call, state['pot'], max(win_rate, preflop_strength),
        ...
        _sb_allin_thr = SB_TIGHT_3BET_CONTINUE if _tight_3bettor else SB_FACING_ALLIN_CALL
        if state.get("opponent_allin", False) and eff_preflop_strength >= _sb_allin_thr:
            return 0
        # Non-all-in: call with strong hands if pot odds are reasonable
        _sb_call_thr = SB_TIGHT_3BET_CONTINUE if _tight_3bettor else SB_VS_RERAISE_CALL
        if eff_preflop_strength >= _sb_call_thr and win_rate >= pot_odds_sbr - SB_VS_RERAISE_POT_SLACK:
            return 0
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opponent_model=None, spr=999.0, pot_odds=0.0, win_rate=0.0, has_position=True):
    ...
    # ── Crossover from v40: calibrated per-street EQR (removes triple-discount) ──
    # Old code applied win_rate*0.6 then EQR*0.55-1.0 = triple discount.
    # Fix: use win_rate directly, apply EQR calibrated per street.
    ...
    # ── Tight-barreler fold tightening (opponent-model axis, v47) ──────────
    # vs confirmed tight opponents (low vpip + high barrel_freq), their
    # turn/river betting range is polarized to monsters; marginal thin/none
    # tier hands are dominated and should fold at a higher threshold.
    # Uses live opp fields only; no-op vs unknown/loose opponents.
    tight_barrel_active = 0
    if opponent_model is not None and round_idx >= 2:
        _tb_conf = opponent_model.get("confidence", 0.0)
        _tb_vpip = opponent_model.get("vpip", PRIOR_VPIP)
        _tb_barrel = opponent_model.get("barrel_freq", PRIOR_BARREL_FREQ)
        if _tb_conf >= 0.20 and _tb_vpip <= 0.42 and _tb_barrel >= 0.50:
            safety += 0.06 if round_idx == 2 else 0.10
            tight_barrel_active = 1
    ...
    sys.stderr.write(
        ...
        f"tight_barrel={tight_barrel_active} vpip={opponent_model.get('vpip', PRIOR_VPIP):.2f} "
        f"barrel={opponent_model.get('barrel_freq', PRIOR_BARREL_FREQ):.2f}\n"
    )
...
def select_postflop_facing_bet(
    win_rate, pot, to_call, draw_strength,
    value_profile, draw_info, opponent_model, round_idx, has_position, board_texture
):
    """EV-based parallel action selector for postflop facing bet.

    Evaluates fold/call/raise simultaneously instead of defaulting to call.
    Returns -1 (fold), 0 (call), or 'raise'.
    """
    tier = value_profile.get('tier', 'none') if value_profile else 'none'
    has_draw = draw_info is not None and draw_info.get('quality', 0) >= 0.14
    draw_quality = draw_info.get('quality', 0) if draw_info else 0.0
    ...
    # Call EV = (equity - pot_odds) * pot_after_call
    call_ev = (win_rate - pot_odds) * (pot + to_call)

    # Implied odds bonus for draws (not river)
    if has_draw and round_idx < 3:
        streets_left = 3 - round_idx
        call_ev += draw_quality * pot * 0.3 * streets_left

    # Raise EV = fold_equity * pot + (1-fe) * call_ev
    effective_fe = fold_to_raise * min(1.0, confidence * 1.5 + 0.3)
    raise_ev = effective_fe * (pot + to_call) + (1 - effective_fe) * call_ev

    # Value bonus for strong/nut hands
    if tier == 'nut':
        raise_ev += win_rate * pot * 0.6
    elif tier == 'strong':
        raise_ev += win_rate * pot * 0.3

    # Select best EV
    best_ev = max(fold_ev, call_ev, raise_ev)

    # Safety: never fold strong/nut hands
    if best_ev == fold_ev and tier in ('strong', 'nut'):
        return 0
    # Safety: don't fold decent draws with reasonable equity
    if best_ev == fold_ev and has_draw and win_rate >= pot_odds - 0.10:
        return 0

    if best_ev == fold_ev:
        return -1
    elif best_ev == raise_ev and raise_ev > call_ev + pot * 0.05:
        return 'raise'
    return 0
...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        # === SPR COMMITMENT GATE (mandatory fix #4; turn/river axis) ===
        # Committed made hands never fold regardless of pot-odds/realized-equity;
        # OR into strong_made_continue so all five downstream fold gates no-op.
        if round_idx >= 2 and len(public_cards) >= 4:
            _made_hand_class = evaluate_best(my_cards + public_cards)[0]
            _bt_flush = board_texture.get('flush_pressure', 0.0) if board_texture else 0.0
            _bt_straight = board_texture.get('straight_pressure', 0.0) if board_texture else 0.0
            spr_committed, _spr_val, _spr_thresh, _spr_label = spr_commitment_gate(
                my_chips, pot, value_profile, pair_profile, _made_hand_class,
                board_texture=board_texture,
            )
            if spr_committed:
                strong_made_continue = True
            sys.stderr.write(
                f'SPR_COMMITMENT_PROBE committed={1 if spr_committed else 0} '
                f'spr={_spr_val:.2f} threshold={_spr_thresh:.1f} label={_spr_label} '
                f'round={round_idx} made_class={_made_hand_class} '
                f'flush_pressure={_bt_flush:.2f} straight_pressure={_bt_straight:.2f} '
                f'tightened={1 if _spr_label in ("tptk_polarized_jam", "tptk_dynamic") else 0}\n'
            )
        ...
        if anti_lock_attack is not None:
            return anti_lock_attack
        # === POLARIZED-JAM TPTK CALL OVERRIDE (v293) ===
        # When spr_commitment_gate tightened TPTK commitment on polarized-jam
        # texture (flush_pressure>=1.0 OR straight_pressure>=1.0) but our raw
        # monte_carlo win_rate still clears pot_odds + safety, CALL instead of
        # over-folding via downstream gates (rescue vs v36/v45/v46/v284/v287/v291).
        # experience_pool.md RECENT_LESSONS line 42 escalation.
        if (
            round_idx >= 2
            and to_call > 0
            and not spr_committed
            and _made_hand_class == 1
            ...
            _polarized_call = polarized_jam_call_gate(
                value_profile, pair_profile, _made_hand_class, board_texture,
                win_rate, pot_odds, round_idx, to_call, opponent_model,
            )
            sys.stderr.write(
                f'POLARIZED_JAM_CALL_OVERRIDE fired={1 if _polarized_call else 0} '
                f'pair_type={pair_profile.get("pair_type", "?") if pair_profile else "?"} '
                f'tier={value_profile.get("tier", "?") if value_profile else "?"} '
                f'wr={win_rate:.2f} po={pot_odds:.2f} '
                f'spr={_spr_val:.2f} round={round_idx}\n'
            )
            if _polarized_call:
                return 0  # call — override downstream fold gates
        ...
        # [v45 mutation] pot_odds + 0.12 -> 0.10 (17% lower raise slack: more aggressive
        # value raises when facing bets, complementing the calibrated-EQR fold gate)
        _suppress_raise = _tight_opp_raise_suppress(
            opponent_model, made_strength, draw_strength, round_idx, value_profile
        )
        if not preflop_defensive_only and not _suppress_raise and (win_rate >= max(strong, pot_odds + 0.10) or semi_bluff or flop_checkraise_exploit):
            raise_amount = choose_raise(
                state["min_raise_action"],
                my_chips,
                state["my_round_bet"],
            ...
            if raise_amount is not None and raise_amount > to_call:
                return raise_amount
        # ── EV-based action selection (replaces default call) ────────────────────
        if round_idx > 0:
            ev_action = select_postflop_facing_bet(
                win_rate, pot, to_call, draw_strength,
                value_profile, draw_info, opponent_model, round_idx,
                spot_info["has_position"], board_texture,
            )
            if ev_action == -1:
                # [v49 repair] When tight-opp suppression fired, the hand (ms
                # 0.48-0.66, tier not strong/nut) was raise-worthy on the
                # heuristic offense path. The candidate's own comment below
                # already warns that blanket suppression "leaks value over 70
                # hands" — but only gated the EV-raise, NOT the EV-fold.
                # Converting raise→fold is far worse than raise→call: it
                # surrenders the pot on a callable made hand. The suppression's
                # stated goal is pot control (raise→call), so force a
                # pot-control call when equity clears pot_odds. Only allow the
                # fold when win_rate < pot_odds (genuinely bad equity vs a big
                # bet from the tight range).
                if _suppress_raise and win_rate >= pot_odds:
                    return 0
                return -1
            # [v49 repair] Do NOT gate the EV-based raise on _suppress_raise.
            # select_postflop_facing_bet already receives opponent_model and folds
            # the tightness read into its raise/call/fold math. Blanket-suppressing
            # its 'raise' output double-counts the tight-opponent read and converts
            # legitimate +EV thin-value raises to calls, leaking value over 70 hands.
            # Suppression stays active on the heuristic offense path above (line ~1652).
            if ev_action == 'raise':
                raise_amount = choose_raise(
                    state["min_raise_action"], my_chips, state["my_round_bet"],
                    to_call, pot, win_rate, round_idx, spot_info["preflop_spot"],
                    preflop_strength, spot_info["has_position"], opponent_model,
                    semi_bluff=semi_bluff,
                    value_profile=value_profile, value_plan=value_plan,
                    board_texture=board_texture, draw_info=draw_info,
                    nutted_risk_score=nutted_risk["risk"],
                    match_sizing_delta=match_profile["sizing_delta"],
                )
                if raise_amount is not None and raise_amount > to_call:
    ...
    if win_rate >= medium or semi_bluff or blocker_bluff or small_probe or check_probe or made_strength >= 0.62 or (value_profile and value_profile["tier"] in ("strong", "nut")):
        ...
        # ── Nash-ward river sizing override (de-exploitability pivot) ──────
        # On river bet-initiation (to_call==0), use a sizing tier INDEPENDENT
        # of hand strength so value and bluffs are indistinguishable by size.
        # This eliminates the sizing tell exploited by diverse opponent clusters.
        #
        # GATE (precommit repair): skip the unified-random sizing on strong/nut
        # value hands and on near-monster made hands. Those must keep the
        # hand-strength-driven choose_raise + river-cap path so the nuts is not
        # under-bet ~25% of the time (a value leak that flips close 70-hand
        # national matchups). The de-exploitability experiment is reserved for
        # bluffs / thin / marginal spots where the EV cost of size
        # randomization is low.
        if (round_idx == 3 and to_call == 0
                and not is_pure_value_raise
                and made_strength < 0.75):
            from postflop import nash_river_sizing_tier, nash_river_bet_amount
            _nash_tier = nash_river_sizing_tier(req, public_cards)
            _nash_amount = nash_river_bet_amount(
                _nash_tier, to_call, pot,
                state['min_raise_action'], my_chips, req, public_cards
            )
            if _nash_amount is not None:
                sys.stderr.write(
                    f'NASH_RIVER_SIZING tier={_nash_tier:.2f} amount={_nash_amount} '
                    f'pot={pot} noise=1 reason=unified_value_bluff '
                    f'value_tier={value_profile.get("tier","none") if value_profile else "none"} '
                    f'is_bluff={int(semi_bluff or blocker_bluff)}\n'
                )
                return _nash_amount
        raise_amount = choose_raise(
            state["min_raise_action"],
...
if __name__ == '__main__':
    # Flop c-bet texture self-test (M5/M6)
    _r_dry = _flop_cbet_texture_ratio(0.10, False, 'strong', True)
    assert 0.38 <= _r_dry <= 0.50, f'dry strong IP should be ~0.42-0.50, got {_r_dry:.3f}'
    _r_wet = _flop_cbet_texture_ratio(0.65, True, 'strong', True)
    assert _r_wet >= 0.60, f'wet dynamic strong should be >=0.60, got {_r_wet:.3f}'
    _r_none = _flop_cbet_texture_ratio(0.50, False, 'none', False)
    assert 0.50 <= _r_none <= 0.70, f'medium none OOP should be ~0.50-0.70, got {_r_none:.3f}'
    print('flop-cbet-texture self-test OK')
    # M5/M6-style live-defaults self-test (helper must return non-None for clear spots)
    assert _preflop_jam_call_decision(0.60, 0.50, 0.05, 0.0) == 0, 'clear +EV jam must call'
    assert _preflop_jam_call_decision(0.30, 0.50, 0.05, 0.0) == -1, 'clear -EV jam must fold'
    assert _preflop_jam_call_decision(0.48, 0.50, 0.05, 0.0) is None, 'borderline defers'
    assert _preflop_jam_call_decision(0.45, 0.50, 0.40, 0.30) == 0, 'wide jammer tight slack calls 0.45 vs 0.50'
    print('jam-defense self-test OK')
    # v47 tight-barreler fold-tightening self-test
    _fake_spot = {
        'facing_postflop_aggression': True,
        'has_position': False,
        'last_raise_pot_ratio': 0.6,
        'opp_current_round_bet_count': 1,
    }
    _tight_opp = {'confidence': 0.40, 'vpip': 0.34, 'barrel_freq': 0.58}
    _std_opp   = {'confidence': 0.40, 'vpip': 0.58, 'barrel_freq': 0.45}
    # River, thin pair, vs pot_odds 0.30: realized_equity ~0.42*1.0 = 0.42
    # vs std_opp threshold = 0.30 + 0.075 + 0.02 = 0.395 -> CALL (0.42 >= 0.395)
    # vs tight_opp threshold = 0.395 + 0.10 = 0.495 -> FOLD (0.42 < 0.495)
    _vp_thin = {'tier': 'thin', 'size_bonus': 0.0}
    assert should_fold_postflop(3, 0.40, 0.05, _vp_thin, _fake_spot, 'wet',
                                opponent_model=_std_opp, spr=8.0, pot_odds=0.30,
                                win_rate=0.42, has_position=False) is False, 'std opp calls thin river'
    assert should_fold_postflop(3, 0.40, 0.05, _vp_thin, _fake_spot, 'wet',
                                opponent_model=_tight_opp, spr=8.0, pot_odds=0.30,
                                win_rate=0.42, has_position=False) is True, 'tight barreler folds thin river'
    # Strong tier never folds regardless of opponent
    _vp_strong = {'tier': 'strong', 'size_bonus': 0.0}
    assert should_fold_postflop(3, 0.60, 0.05, _vp_strong, _fake_spot, 'wet',
                                opponent_model=_tight_opp, spr=8.0, pot_odds=0.30,
                                win_rate=0.62, has_position=False) is False, 'strong tier never folds'
    print('should_fold_postflop tight-barreler self-test PASSED (3/3)')

    # Anti-lock deep-stack gate self-test
    _deep_state = {
        'opponent_allin': False, 'round_raise': 200, 'min_raise_action': 200,
        'my_round_bet': 100, 'committed': [100, 100],
    }
    _deep_opp = {'confidence': 0.3, 'fold_to_raise': 0.44}
    # Deep stack (15000 chips), facing 35% pot bet: should return a raise, NOT -2
    _deep_result = choose_anti_lock_pressure_action(
        _deep_state, my_chips=15000, to_call=350, pot=1000, round_idx=2,
        win_rate=0.35, opponent_model=_deep_opp, remaining_hands=10,
    )
    assert _deep_result != -2, f'Deep-stack anti-lock must not emergency-jam, got {_deep_result}'
    # Shallow stack (2000 chips), same bet: SHOULD return -2
    _shallow_result = choose_anti_lock_pressure_action(
        _deep_state, my_chips=2000, to_call=350, pot=1000, round_idx=2,
        win_rate=0.35, opponent_model=_deep_opp, remaining_hands=10,
    )
    assert _shallow_result == -2, f'Shallow-stack anti-lock should emergency-jam, got {_shallow_result}'
    print('anti-lock deep-stack gate self-test PASSED (2/2)')
