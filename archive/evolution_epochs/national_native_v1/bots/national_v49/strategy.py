...
def _dominated_broadway_penalty(my_cards, spot_name, opponent_model):
    ...
    if confidence >= 0.15:
        if pfr <= 0.20:
            opp_factor = 1.15  # tight opener: more dominated
        elif pfr >= 0.32:
            opp_factor = 0.70  # wide opener: less dominated
    penalty = base * opp_factor
    sys.stderr.write(
        f"DOM_BROADWAY_PENALTY hand={key} spot={spot_name} base={base:.3f} "
        f"opp_factor={opp_factor:.2f} pfr={pfr:.3f} conf={confidence:.2f} "
