...
def disciplined_opp_river_margin(opponent_model, value_profile, round_idx):
    ...
    # [v296 mutation] Widen the tightness firing band per v295 RECENT_LESSONS:
    # the prior gate (vpip_dev ceiling 0.58, pfr_dev ceiling 0.28) fired
    # stderr <1% during daemon eval — too narrow to produce any behavioral
    # signal. Raise each prior ceiling by ~0.04 (≈10-14% threshold widening)
    # so slightly-loose elite opponents (vpip≈0.55-0.58, pfr≈0.26-0.28) — the
    # v285/v291/v294 cluster where v295 underperforms — now trigger the gate.
    # Output magnitude (0.035 * tightness) is unchanged; only the firing band
    # widens, so long-tail standard-bucket H2H (vpip>=0.62, pfr>=0.32) is
    # still exactly 0.0 by construction.
    vpip_dev = max(0.0, 0.62 - vpip)   # widened from 0.58
    pfr_dev = max(0.0, 0.32 - pfr)     # widened from 0.28
