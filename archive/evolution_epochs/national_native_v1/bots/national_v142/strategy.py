...
# [v117 mutation] widen upper suppression band 0.50 -> 0.56 (+12%) to keep
# marginal top-pair-weak-kicker hands as calls vs tight ranges without
# suppressing stronger thin-value raises.
...
def _turn_cr_defense(round_idx, spot_info, opponent_model, board_texture,
                    value_profile, draw_info, draw_strength, made_strength,
                    blocker_profile):
    """Turn OOP check-raise defense: raise with premium draws and strong value.

    Extends the flop check-raise exploit pattern to the turn street so the bot
    is not limited to call/fold when facing a turn barrel out of position.

    Runtime Contract: bounded single-pass dict lookups only; no file/network I/O
    and no history scan. Falls through to existing call/fold logic when False.
    """
    fired = False
    reason = 'none'
    confidence = 0.0
    if round_idx == 2 and spot_info.get('facing_postflop_aggression') and not spot_info.get('has_position'):
        confidence = opponent_model.get('confidence', 0.0)
        wetness = board_texture.get('wetness', 0.0) if board_texture else 0.0
        fold_to_raise = opponent_model.get('fold_to_raise', PRIOR_FOLD_TO_RAISE)
        is_combo = bool(draw_info.get('combo_draw')) or draw_info.get('type') == 'combo_draw'
        is_nut_flush = bool(draw_info.get('nut_flush_draw')) or draw_info.get('type') == 'nut_flush_draw'
        # Value raise with strong/nut hands
        if value_profile and value_profile.get('tier') in ('strong', 'nut'):
            fired = True
            reason = 'turn_value'
        # Premium draw raise: nut flush (~19.6% turn equity) or combo draw
        # (~32.6% turn equity) — both below 33.3% pot-odds break-even on the
        # turn, so fold equity is REQUIRED. Gate on fold_to_raise > 0.42 to
        # match the flop counterpart (BB_FLOP_CR_FOLD_TO_RAISE_MIN) and the
        # blocker branch below. Without this gate, these draws raise into
        # calling-station barrelers and worsen the exact matchup this targets.
        elif (is_combo or is_nut_flush) and draw_strength >= 0.18 and fold_to_raise > 0.42:
            fired = True
            reason = 'turn_premium_draw'
        # Blocker raise on dry boards with some fold equity
        elif (confidence >= 0.20 and fold_to_raise > 0.42
              and blocker_profile and blocker_profile.get('eligible')
              and wetness <= 0.30 and draw_strength < 0.12):
            fired = True
            reason = 'turn_blocker'
    sys.stderr.write(
        f'TURN_CR_DEFENSE fired={int(fired)} reason={reason} '
        f'conf={confidence:.2f} ms={made_strength:.2f} ds={draw_strength:.2f} '
        f'wet={board_texture.get("wetness", 0) if board_texture else 0:.2f}\n'
    )
    return fired
...
# [Crossover v141] Reverted to v112's conservative fold gate. v135's EQR
# redesign (no win_rate*0.6 discount, EQR floor 0.70, safety 0.025*round)
# caused over-calling vs value-heavy opponents. H2H evidence: v112 beats
# every opponent v135 loses to (v39: 51% vs 30%, v71: 54% vs 35%,
# v66: 56% vs 40%, v74: 57% vs 40%). Keeping v112's tighter fold gate
# preserves the conservative equity estimation that makes the bot robust
# against aggressive value-heavy ranges.
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opponent_model=None, spr=999.0, pot_odds=0.0, win_rate=0.0, has_position=True):
    ...
    # The 0.6 discount on win_rate is deliberate: simulation win_rate
    # includes future-street scenarios that overestimate realized equity
    # when facing a bet. Discounting prevents over-calling.
...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if not preflop_defensive_only and not _suppress_raise and (win_rate >= max(strong, pot_odds + 0.12) or semi_bluff or flop_checkraise_exploit or turn_cr):
            raise_amount = choose_raise(
                state["min_raise_action"],
                my_chips,
                state["my_round_bet"],
                ...
                spot_info["has_position"],
                opponent_model,
                semi_bluff=semi_bluff or (flop_checkraise_exploit and draw_info["semi_bluff"] and draw_strength >= 0.15) or (turn_cr and draw_info.get("semi_bluff", False) and draw_strength >= 0.15),
                value_profile=value_profile,
                value_plan=value_plan,
                board_texture=board_texture,
                draw_info=draw_info,
                blocker_bluff=blocker_raise,
                pressure_line=flop_checkraise_exploit or turn_cr,
