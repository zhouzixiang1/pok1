    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        if can_bluff_3bet and eff_bluff_low <= preflop_strength <= eff_bluff_high and not trash_hand:
            ...
            if bluff_roll < eff_bluff_freq:
                ...
                if raise_amount is not None:
                    return raise_amount
        # [CROSSOVER from national_v37] Preflop range pruning: fold dominated
        # offsuit broadway (KQo/KJo/QJo/ATo) that inflates big-pot showdown
        # frequency. UPSTREAM fix — distinct from postflop fold-margin axis.
        # H2H evidence (head_to_head.json): v37 beats v57 vs v59 (56.7% vs
        # 48.2%), v71 (53.3% vs 45.0%), v31 (47.1% vs 40.0%), v8 (50.0% vs
        # 43.0%) — tight-disciplined opponents where dominated broadway bleeds.
        ...
        # [CROSSOVER from national_v37] apply domination penalty so
        # KQo/QJo/ATo fold vs 3bet (upstream range pruning)
...
def _river_pot_odds_fold_gate(round_idx, made_strength, value_profile, win_rate, pot_odds):
    """[Mandatory fix #4] River pot-odds commitment fold gate.

    Closes the big-pot stack-off leak: thin/none tier river hands with
    made_strength < 0.55 fold when simulation equity (win_rate) < pot_odds.
    STRICTER than should_fold_postflop: uses raw win_rate (not max(made,win))
    and NO safety buffer. Evidence: ev_8bb8d12b5a182d2d, ev_c7c95c3716d52bc7.
    """
    if round_idx != 3:
        return False
    ...
    sys.stderr.write(
        f"SPR_FOLD_GATE round=3 tier={tier} made={made_strength:.3f} "
        f"win={win_rate:.3f} pot_odds={pot_odds:.3f} fold={int(fold)}\n"
    )
    return fold
...
#
# [v57 crossover from national_v40] Calibrated equity realization model.
# v55 applied win_rate*0.6 then EQR*0.55-1.0 = triple discount, which over-folds
# marginal made hands vs value-heavy opponents. H2H evidence (head_to_head.json):
# v40 beats national_v33 (0.550/80g) and national_v43 (0.538/65g) — both v55
# nemeses (v33 0.360/25g, v43 0.400/15g). v40's model uses win_rate directly,
# applies EQR calibrated per street (river~1.0, turn small, flop moderate), and a
# lower safety margin, so thin value continues instead of folding. This function
# is imported verbatim from v40; the structural baseline remains v55.
...
def should_fold_postflop(round_idx, made_strength, draw_strength, value_profile, spot_info, texture_class="none", opponent_model=None, spr=999.0, pot_odds=0.0, win_rate=0.0, has_position=True):
    ...
    # Old v55 code applied win_rate*0.6 then EQR*0.55-1.0 = triple discount.
    # Fix (v40 import): use win_rate directly, apply EQR calibrated per street.
...
def pot_odds_call_threshold(pot_odds, has_position, round_idx, draw_info, spr):
    ...
    # Position adjustment
    # [v57 mutation] 0.02 -> 0.024 (+20%): widen IP calling range slightly.
    # Independent axis from the v40 fold-gate equity import (crossover). Position
    # is the cleanest realization edge in HU; a small widening lets IP continue
    # with marginally +EV hands the calibrated equity model now values correctly.
    if has_position:
        threshold -= 0.024
...
def get_action(req, requests):
    ...
    if to_call > 0:
        ...
        if fragile_pair_raise_fold:
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
        # [Fix #4] River pot-odds commitment fold gate — closes big-pot stack-off leak.
        # Runs BEFORE should_fold_postflop as a stricter filter for thin/none river hands.
        if _river_pot_odds_fold_gate(round_idx, made_strength, value_profile, win_rate, pot_odds):
            if not anti_lock_call_continue and not strong_made_continue:
                return -1
