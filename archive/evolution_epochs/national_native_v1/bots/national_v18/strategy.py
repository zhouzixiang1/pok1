...
def _preflop_jam_call_decision(win_rate, pot_odds, opponent_allin_rate, confidence):
    ...
    if confidence >= 0.15 and opponent_allin_rate >= 0.20:
        # v18 revert: experience_pool RECENT_LESSONS v17 falsification gate fired
        # (v17 lost v14 6W/9L per head_to_head.json), so the 0.05->0.04 tightening
        # was wrong direction. Keep wide-jammer call band intact.
        slack = 0.05
        reason = 'wide_jammer'
    else:
        slack = 0.01
...
def choose_preflop_spot_action(req, state, spot_info, opponent_model, preflop_strength, win_rate, match_profile, opp_archetype='unknown'):
    ...
    if spot_info["preflop_spot"] == "bb_vs_limp":
        ...
        # v18 NEMESIS_TIGHT_OPEN_PFR carve-out — distinct axis from raise SIZE
        # (_def_adj, _hist_call_delta): this targets opponent RANGE TIGHTNESS.
        # H2H: v17 1-9 vs v10, 5-10 vs v6, 6-9 vs v14 (all pfr<=~0.22). Static
        # BB_CALL_THRESHOLD=0.37 calls dominated hands vs their strong ranges.
        # Inverted delta for wide openers (pfr>=0.32) defends wider.
...
if __name__ == '__main__':
    ...
    # v18: slack reverted 0.04 -> 0.05 (v17 falsification gate fired, head_to_head.json
    # v17 lost v14 6W/9L). wide-jammer call band is win_rate >= pot_odds - 0.05.
    # 0.47 calls; defer band is now [pot_odds - 0.05, pot_odds - 0.10) = [0.45, 0.40),
    # so 0.43 defers (and 0.45 itself calls, boundary-inclusive).
