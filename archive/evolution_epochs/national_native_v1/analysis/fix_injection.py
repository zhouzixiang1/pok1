...
def automatic_fix_mutation_enabled() -> bool:
    """Return whether legacy fix injection may rewrite candidate source.

    National-native evolution treats the registry as an audit catalogue only.
    Protocol invariants belong in validators or the system-owned wire runtime,
    while strategy changes must remain attributable to a Worker diff.  Legacy
    profiles retain the historical repair behaviour for archived regressions.
    """

    from workflow_profiles import get_workflow_profile

    profile = get_workflow_profile()
    return getattr(profile, "national_execution_mode", None) != "native_tcp"
...
def apply_known_fixes(bot_dir: Path) -> tuple[list[str], list[str]]:
    """Apply or report all active fixes for *bot_dir*.
    ...
    from both return lists. Under the national-native profile, matching patches
    are reported in ``skipped_fix_ids`` and source bytes are never changed.
    """
    bot_dir = Path(bot_dir)
    mutation_enabled = automatic_fix_mutation_enabled()
    applied: list[str] = []
    skipped: list[str] = []
    ...
    for fix in MANDATORY_FIXES:
        ...
        for patch in fix.patches:
            ...
            if not mutation_enabled:
                ...
                skipped_seen = True
                log.info(
                    "Reported applicable fix %s in %s; national-native source "
...
def log_fix_application(
    ...
    if not mutation_enabled:
        msg_parts.append("national-native report-only; source mutation disabled")

    log_system_event(
        (
            "pipeline.fixes_applied"
            if mutation_enabled
            else "pipeline.fix_injection_report_only"
        ),
        severity,
        f"Fix injection for {bot_dir.name} from v{source_v}: " + "; ".join(msg_parts),
        {
            ...
            "report_only": not mutation_enabled,
        },
    )
