"""Archived national_native_v1 stagnation analysis (non-authoritative).

Uses LLM to analyze current-epoch rating trends and strict lineage to determine
if the evolution strategy needs adjustment.
"""
...
async def _analyze_stagnation(source_v, active_bots, ratings, ui):
    ...
    # ── Generation-level trend from the strict active identity set ──
    gen_trend_lines = []
    try:
        strict_names = sorted(
            (
                name for name in active_bots
                if (parse_bot_version(str(name)) or 0) >= FIRST_STRICT_POLICY_VERSION
            ),
            key=lambda name: parse_bot_version(str(name)) or 0,
        )
        for v_name in strict_names[-8:]:
            try:
                v = parse_bot_version(str(v_name))
    ...
    if history_file.exists():
        ...
        if latest_run_id is not None:
            ...
            # Pre-identity history is not current-epoch prompt evidence.
            recent = []
    ...
    # downstream planning knows the "no stagnation" verdict is an infra guess,
    # not a business judgement.
