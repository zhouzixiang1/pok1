...
- BOT-001a  (wheel straight A-2-3-4-5): sandbox-import the bot's card_utils,
...
- BOT-004   (TOTAL_HANDS == 70): sandbox-import the bot's constants and
  assert TOTAL_HANDS == 70.

Each runtime verifier runs inside the mandatory candidate sandbox.  Candidate
bytes are read-only, have no network, cannot see the host filesystem, and only
receive a bounded tmpfs.  Isolation failure is trusted infrastructure failure;
...
# Sandboxed probe scripts. Each imports only the target module from the bot's
# read-only ``/work`` mount and prints a single JSON line.
...
class _IncompleteTrustedProbe:
    """An exit-zero probe which never reached the system-owned postlude."""


_INCOMPLETE_TRUSTED_PROBE = _IncompleteTrustedProbe()
...
def _run_probe(
    ...
    bot_dir: Path,
    timeout: float = 15.0,
) -> dict | _IncompleteTrustedProbe | None:
    """Run a one-off probe with ``bot_dir`` read-only at ``/work``.

    Returns the parsed JSON dict the script prints, or None on any failure
    caused by candidate behavior. Missing isolation raises so the quality gate
    records infrastructure failure and cannot certify unchecked code.
    """
    try:
        proc = run_candidate_probe(bot_dir, script, timeout=timeout)
        if proc.returncode == 0 and not proc.trusted_completion:
            # Candidate output shares stdout with the probe.  A matching JSON
            # line followed by os._exit(0) is not evidence that the trusted
            # probe postlude ran, and must not fall through to a permissive AST
            # fallback.
            return _INCOMPLETE_TRUSTED_PROBE
...
def _verify_wheel(bot_dir: Path) -> dict:
    ...
    if data is _INCOMPLETE_TRUSTED_PROBE:
        return {
            "ok": False,
            "reason": (
                "wheel probe exited 0 without parent-owned sandbox completion; "
                "candidate output is not authoritative"
            ),
        }
    if data is not None:
...
def _verify_total_hands(bot_dir: Path) -> dict:
    ...
    if data is _INCOMPLETE_TRUSTED_PROBE:
        return {
            "ok": False,
            "reason": (
                "TOTAL_HANDS probe exited 0 without parent-owned sandbox "
                "completion; candidate output is not authoritative"
            ),
        }
    if data is not None:
        assert isinstance(data, dict)
