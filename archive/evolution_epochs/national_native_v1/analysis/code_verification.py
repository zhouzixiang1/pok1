...
from evolution_infra import (
    CORE_DIR, PROJECT_ROOT, RESULTS_DIR,
    MAX_LINES_PER_FILE, MAX_LINES_HELPER, MAX_LINES_HARD_CAP,
    LINE_GROWTH_BUDGET, CORE_STRATEGY_FILES,
)
...
def run_bot_embedded_self_tests_execution(bot_dir, timeout: float = 20.0):
    ...
    for module_name in EMBEDDED_SELFTEST_MODULES:
        ...
        try:
            ...
            continue
        if proc.returncode == 0 and proc.trusted_completion:
            lines = [
                line.strip()
            ...
            if not (
                ...
                and receipt.get("schema") == "candidate-selftest-contract-v1"
                and receipt.get("ok") is True
                and receipt.get("module") == module_name
            ):
                candidate_errors.append(
                    f"{module_name}: exit 0 without trusted sandbox completion receipt"
                )
        elif proc.returncode == 0:
            candidate_errors.append(
                f"{module_name}: exit 0 without parent-owned sandbox completion receipt"
            )
        else:
...
def verify_code(directory, target_files=None):
    ...
    def compile_error(path):
        """Compile bytes without executing code or writing candidate pycache."""
        try:
...
def run_import_contract_test(directory, modules=None, timeout=20):
    """Import production bot modules in the mandatory candidate sandbox.

    Returns a list of structured error dictionaries. An empty list means the
    runtime import contract is intact.  Isolation failure raises instead of
    falling back to a host subprocess, so authoritative callers fail closed.
    """
    ...
    if proc.returncode == 0 and proc.trusted_completion:
        ...
        try:
            receipt = json.loads(stdout_lines[-1]) if stdout_lines else None
        except (TypeError, json.JSONDecodeError):
            receipt = None
        if (
            isinstance(receipt, dict)
            and receipt.get("schema") == "candidate-import-contract-v1"
            and receipt.get("ok") is True
            and receipt.get("modules") == modules
        ):
            return []
        return [{
            "module": ",".join(modules),
            "exception": "MissingSandboxReceipt",
            "message": "sandboxed import exited 0 without the trusted completion receipt",
            "traceback": (proc.stdout or "")[-800:],
        }]
    ...
    if proc.returncode == 0:
        return [{
            ...
            "exception": "MissingSandboxReceipt",
            "message": (
                "sandboxed import exited 0 without the parent-owned "
                "completion receipt"
            ),
            "traceback": (proc.stdout or "")[-800:],
        }]
...
def run_smoke_test(directory):
    """Retired API tombstone; active smoke is ``run_native_tcp_smoke``."""
...
def run_decision_test_details(directory, extra_scenarios=None):
    """Retired API tombstone; active fixtures require a raw TCP candidate."""
    ...
    return {
        ...
        "critical_passed": 0,
        "critical_total": 0,
        "critical_failures": [{"id": "retired_api", "details": "use raw TCP fixtures"}],
        "failures": [{"id": "retired_api", "severity": "critical", "details": "use raw TCP fixtures"}],
        "scenarios": [],
    }


def run_national_protocol_tests(*, native_tcp_mode: bool = False):
    """Run the sole active national TCP platform alignment shard."""

    if not native_tcp_mode:
        return ["non_native_protocol_test_mode_retired"]
    test_name = "test_national_platform_alignment.py"
