...
def _preflop_jam_call_decision(win_rate, pot_odds, opponent_allin_rate, confidence):
    ...
    if confidence >= 0.15 and opponent_allin_rate >= 0.20:
        # Mutation (national_v17): tightened from 0.05 -> 0.04 (-20%) to reduce
        # over-calling vs polarized value-heavy jamming ranges (V14-class leaks).
        # experience_pool.md BLUFF_CALIBRATION / PARAMETER_TUNING: polarized-jam
        # gates need finer calibration; 0.04 still rewards confident wide-jammer
        # reads while closing ~1pp of marginal -EV call-offs.
        slack = 0.04
        reason = 'wide_jammer'
    else:
        slack = 0.01
...
if __name__ == '__main__':
    ...
    # national_v17: slack tightened 0.05 -> 0.04; wide-jammer call band now needs
    # win_rate >= pot_odds - 0.04 (0.46 here). 0.47 calls, 0.45 now defers.
    assert _preflop_jam_call_decision(0.47, 0.50, 0.40, 0.30) == 0, 'wide jammer slack=0.04 calls 0.47 vs 0.50'
    assert _preflop_jam_call_decision(0.45, 0.50, 0.40, 0.30) is None, 'wide jammer slack=0.04 defers 0.45 vs 0.50'
