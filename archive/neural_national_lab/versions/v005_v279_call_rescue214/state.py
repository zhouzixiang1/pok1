...
def reconstruct_state(req):
    ...
    # Authoritative heads-up contract: dealer_id IS the small blind, and
    # the big blind is 1 - dealer_id. Do NOT derive via next_player(..., 1/2):
    # next_player(dealer_id, 1) returns 1 - dealer_id (the opponent), and
    # next_player(dealer_id, 2) wraps back to dealer_id, which would swap
    # SB/BB and corrupt the SB-preflop-first / BB-postflop-first ordering.
    sb = dealer_id
    bb = 1 - dealer_id
...
def forced_fold_loss_bound(req, state, my_id, remaining_hands):
    ...
    for offset in range(1, remaining_hands):
        ...
        # Heads-up: the dealer is the SB; BB is the other seat. Deriving via
        # next_player(future_dealer, 1/2) would swap SB/BB (see reconstruct_state).
