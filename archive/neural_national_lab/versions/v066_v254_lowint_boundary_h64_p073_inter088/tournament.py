...
def anti_lock_continue_floor(pot_odds, round_idx, value_profile, draw_info, made_strength):
    ...
    # v167: Increase river discount so anti_lock_can_continue returns False
    # for marginal hands (made_strength 0.35-0.55). Without this, the floor
    # = max(0.08, 0.33 - 0.145) = 0.08, so ALL hands pass on river.
    if round_idx == 3 and made_strength < 0.55 and value_profile is not None and value_profile.get('tier') not in ('strong', 'nut'):
        discount += 0.105
