...
def sanitize_action(action, state, my_chips):
    ...
    # Crossover (national_v6 import): when facing a bet that requires all-in to
    # continue, only commit when strategy EXPLICITLY chose allin (-2). Any other
    # intent (call/raise with a marginal made hand) becomes a disciplined fold.
    # V6 beats V14/V15 (~80% WR @5g each) where V12 loses (~20-40% WR); V12's old
    # 'fold only on explicit fold, else call off' behaviour leaked chips to V14's
    # polarized value-shove range. V6's tight guard is the proven edge vs elite.
