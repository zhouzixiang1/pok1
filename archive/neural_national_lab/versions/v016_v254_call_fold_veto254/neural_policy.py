...
DEFAULT_CONFIG = {
    ...
    "fold_conf": 0.82,
    "fold_rule_actions": ["call"],
    "min_fold_to_call": 0,
    "max_fold_to_call": 20000,
    "call_conf": 0.88,
    "raise_conf": 0.92,
    "allin_conf": 0.985,
    "max_call_ratio": 0.30,
    ...
    "free_check_conf": 0.80,
    "allow_free_check_rescue": True,
    "allow_fold": True,
    "allow_call": True,
    "allow_raise": True,
    "allow_allin": True,
...
def _rule_action_name(action: int) -> str:
    if action == -1:
        return "fold"
    if action == -2:
        return "allin"
...
def apply_neural_advice(req: dict[str, Any], state: dict[str, Any], rule_action: int) -> int:
    ...
    if label == 0:
        if (
            ...
            and _rule_action_name(rule_action) in set(cfg.get("fold_rule_actions", ["call", "raise", "allin"]))
            and to_call >= float(cfg.get("min_fold_to_call", 0))
            and to_call <= float(cfg.get("max_fold_to_call", 20000))
    ...
    if label == 1:
        ...
        if (
            ...
            and cfg.get("allow_free_check_rescue", True)
            and rule_action == -1
            and to_call <= 0
            and conf >= float(cfg.get("free_check_conf", cfg["call_conf"]))
        ):
            return 0
        if (
            cfg.get("allow_call", True)
