...
def _large_commit_value_veto(
    ...
    if best_margin < min_best_margin:
        return None
    if stage == "flop" and cfg.get("large_commit_veto_flop_call_redirect_enabled", False):
        call_label = LABELS.index("call")
        fold_label = LABELS.index("fold")
        call_value = float(values[call_label])
        fold_value = float(values[fold_label])
        min_call_value = float(cfg.get("large_commit_veto_flop_call_redirect_min_value", 0.05))
        margin_vs_fold = float(cfg.get("large_commit_veto_flop_call_redirect_margin_vs_fold", 0.25))
        margin_vs_rule = float(cfg.get("large_commit_veto_flop_call_redirect_margin_vs_rule", 0.45))
        if (
            rule_label == LABELS.index("allin")
            and to_call > 0
            and call_value >= min_call_value
            and call_value >= fold_value + margin_vs_fold
            and call_value >= rule_value + margin_vs_rule
        ):
            return 0
    return -1
