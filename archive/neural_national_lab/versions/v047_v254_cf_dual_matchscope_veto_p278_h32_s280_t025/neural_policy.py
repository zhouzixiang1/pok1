...
_INTERACTION_AUX_MODEL: dict[str, Any] | None = None
...
def _interaction_aux_model() -> dict[str, Any] | None:
    global _INTERACTION_AUX_MODEL
    if _INTERACTION_AUX_MODEL is not None:
        return _INTERACTION_AUX_MODEL
    cfg = _config()
    if not cfg.get("interaction_aux_enabled", False):
        return None
    path = str(cfg.get("interaction_aux_weights", "interaction_aux_weights.json"))
    if not os.path.isabs(path):
        path = os.path.join(BOT_DIR, path)
    if not os.path.exists(path):
        return None
    try:
        with open(path, "r", encoding="utf-8") as fh:
            model = json.load(fh)
        if model.get("labels") != ["bad", "good"]:
            raise ValueError("interaction aux label mismatch")
        _INTERACTION_AUX_MODEL = model
        return model
    except Exception as exc:
        print(f"INTERACTION_AUX_MODEL_ERROR {exc}", file=sys.stderr)
        return None
...
def _passes_interaction_gate(
    ...
    if conf < float(cfg.get("interaction_apply_min_conf", 0.0)):
        return True
    if score < float(cfg.get("interaction_min", 0.70)):
        return False
    aux_model = _interaction_aux_model()
    if aux_model is None:
        return not cfg.get("interaction_aux_enabled", False)
    if len(features) != int(aux_model.get("input_dim", 0)):
        return False
    aux_score = _predict_advantage(aux_model, features)
    return aux_score >= float(cfg.get("interaction_aux_min", 0.50))
