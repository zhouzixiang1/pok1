...
def _passes_multi_action_value_gate(
    ...
    if candidate <= 0 or candidate > float(cfg.get("multi_action_value_max_candidate", 125)):
        return True
    min_actions = int(cfg.get("multi_action_value_min_opponent_actions", 0) or 0)
    if min_actions > 0 and _opponent_profile_action_count(req) < min_actions:
        return False
...
def _multi_action_value_proposal(
    ...
    if rule_labels and _label_name(rule_label) not in rule_labels:
        return None
    min_actions = int(cfg.get(
        "multi_action_value_propose_min_opponent_actions",
        cfg.get("multi_action_value_min_opponent_actions", 0),
    ) or 0)
    if min_actions > 0 and _opponent_profile_action_count(req) < min_actions:
        return None
    model = _multi_action_value_model()
