...
def estimate_preflop_strength(my_cards):
    ...
    CROSSOVER (imported from v212/v187): the old formula scored K2o (0.65)
    above 55 (0.64) and clamped 88-AA all to 1.0. This formula matches real
    HU equities:
...
def is_preflop_trash_hand(my_cards, preflop_strength=None):
    ...
    # MUTATION (v213 crossover): threshold raised 0.30->0.40 to match the new
    # estimate_preflop_strength scale (non-pair min is now 0.33, not ~0.10).
    # Tighter preflop folding for marginal junk reduces -EV calls.
    if strength <= 0.40:
        return True
...
def _preflop_spr_commitment_gate(raise_to, my_chips, already_invested, pot,
    ...
    """Preflop SPR commitment gate (CROSSOVER from v212/v201).
