...
def apply_neural_advice(req: dict[str, Any], state: dict[str, Any], rule_action: int) -> int:
    ...
    if label == 0:
        if to_call > 0 and conf >= float(cfg["fold_conf"]) and 1.0 - probs[0] <= float(cfg["max_fold_proxy"]):
            return -1
        return rule_action
    if label == 1:
        if conf >= float(cfg["call_conf"]) and to_call / max(1.0, pot + to_call) <= float(cfg["max_call_ratio"]):
            return 0
        return rule_action
    if label in (2, 3, 4):
        if not state.get("opponent_allin") and conf >= float(cfg["raise_conf"]):
            return _raise_action(label, state, my_chips)
        return rule_action
    if label == 5 and conf >= float(cfg["allin_conf"]):
        return -2
    return rule_action
