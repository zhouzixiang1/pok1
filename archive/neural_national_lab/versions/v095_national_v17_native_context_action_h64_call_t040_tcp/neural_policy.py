...
def _multi_action_value_proposal(
    ...
    if len(values) != len(LABELS):
        return None
    labels = _label_set(cfg.get("multi_action_value_propose_labels", ["call"]))
    legal = _legal_mask(req, state)
    min_value = float(cfg.get("multi_action_value_propose_min", 0.10))
    margin = float(cfg.get("multi_action_value_propose_margin_vs_rule", 0.05))
    rule_value = float(values[rule_label]) if 0 <= rule_label < len(values) else 0.0
