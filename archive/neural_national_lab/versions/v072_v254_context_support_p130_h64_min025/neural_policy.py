...
_MULTI_ACTION_VALUE_SUPPORT_MODEL: dict[str, Any] | None = None
...
def _multi_action_value_support_model() -> dict[str, Any] | None:
    global _MULTI_ACTION_VALUE_SUPPORT_MODEL
    if _MULTI_ACTION_VALUE_SUPPORT_MODEL is not None:
        return _MULTI_ACTION_VALUE_SUPPORT_MODEL
    cfg = _config()
    raw_path = cfg.get("multi_action_value_support_weights")
    if not raw_path:
        return None
    path = str(raw_path)
    if not os.path.isabs(path):
    ...
    try:
        with open(path, "r", encoding="utf-8") as fh:
            model = json.load(fh)
        if model.get("format") != "tiny_mlp_multi_action_value_v1":
            raise ValueError("multi-action value support format mismatch")
        if model.get("labels") != list(LABELS):
            raise ValueError("multi-action value support label mismatch")
        _MULTI_ACTION_VALUE_SUPPORT_MODEL = model
        return model
    except Exception as exc:
        print(f"MULTI_ACTION_VALUE_SUPPORT_MODEL_ERROR {exc}", file=sys.stderr)
        return None
...
def _opponent_context_features(requests: list[dict[str, Any]] | None, req: dict[str, Any]) -> list[float]:
    model: dict[str, Any] = dict(OPPONENT_CONTEXT_DEFAULTS)
    if build_opponent_model is not None and requests:
        try:
            my_id = int(req.get("my_id", requests[-1].get("my_id", 0)) or 0)
            built = build_opponent_model(requests, my_id)
...
def _passes_multi_action_value_gate(
    ...
    if (
        ...
        return True
    support_model = _multi_action_value_support_model()
    if support_model is None:
        return False
    support_features = features
    if cfg.get("multi_action_value_support_extra_features") == "opponent_context_v1":
        support_features = support_features + _opponent_context_features(requests, req)
    if len(support_features) != int(support_model.get("input_dim", 0)):
        return False
    support_values = _predict_multi_action_values(support_model, support_features)
    if len(support_values) != len(LABELS):
        return False
    support_label_value = float(support_values[label])
    support_rule_value = float(support_values[rule_label]) if 0 <= rule_label < len(support_values) else 0.0
    return (
        support_label_value >= float(cfg.get("multi_action_value_support_min", 0.50))
        and support_label_value
        >= support_rule_value + float(cfg.get("multi_action_value_support_margin_vs_rule", 0.02))
    )
...
def apply_neural_advice(
    ...
    if (
        ...
        and _passes_multi_action_value_gate(req, state, rule_action, label, conf, probs, candidate, requests)
    ):
        return candidate
    return rule_action
