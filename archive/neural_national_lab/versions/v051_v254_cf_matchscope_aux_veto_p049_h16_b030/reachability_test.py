...
def main_paired_board_weak_one_pair_fold():
    """v247: prove _paired_board_weak_one_pair_fold fires for a weak one-pair
    holding on a paired board facing value-heavy aggression, and does NOT fire
    for two-pair (the critical over-fold guard), top pair, unpaired boards,
    standard opponents, small bets, draws, or strong tiers.

    Tests postflop._paired_board_weak_one_pair_fold directly (NOT via get_action).
    """
    import postflop

    _thin = {'tier': 'thin'}
    _strong = {'tier': 'strong'}
    _value_heavy = {'confidence': 0.50, 'barrel_freq': 0.55, 'postflop_aggr': 0.38}
    _std = {'confidence': 0.50, 'barrel_freq': 0.42, 'postflop_aggr': 0.32}
    _weak_pair = {'made_class': 1, 'pair_type': 'bottom_pair'}
    _board_pair = {'made_class': 1, 'pair_type': 'board_pair'}
    _top_pair = {'made_class': 1, 'pair_type': 'top_pair'}
    _paired_oneway = {'board_paired': True, 'hand_class': 1}
    _paired_twopair = {'board_paired': True, 'hand_class': 2}
    _unpaired = {'board_paired': False, 'hand_class': -1}
    _med_spot = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.60}
    _sm_spot = {'facing_postflop_aggression': True, 'last_raise_pot_ratio': 0.25}

    # 1. FIRES: bottom pair on paired board, turn, value-heavy opp
    fires = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _thin, _weak_pair, _paired_oneway,
        _med_spot, _value_heavy, 400, 600)
    assert fires is True, (
        "REACHABILITY FAIL: weak bottom pair on paired board should fire, got %r" % fires)

    # 2. FIRES: board pair type, river
    fires_river = postflop._paired_board_weak_one_pair_fold(
        3, 0.22, 0.03, _thin, _board_pair, _paired_oneway,
        _med_spot, _value_heavy, 400, 600)
    assert fires_river is True, (
        "REACHABILITY FAIL: board pair river should fire, got %r" % fires_river)

    # 3. CRITICAL: does NOT fire for two-pair (hand_class=2)
    no_fire_twopair = postflop._paired_board_weak_one_pair_fold(
        2, 0.40, 0.05, _thin, _weak_pair, _paired_twopair,
        _med_spot, _value_heavy, 400, 600)
    assert no_fire_twopair is False, (
        "REACHABILITY FAIL: two-pair (hand_class=2) must NOT fire — over-fold guard")

    # 4. Does NOT fire: unpaired board
    no_fire_unpaired = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _thin, _weak_pair, _unpaired,
        _med_spot, _value_heavy, 400, 600)
    assert no_fire_unpaired is False, (
        "REACHABILITY FAIL: unpaired board must not fire")

    # 5. Does NOT fire: top pair
    no_fire_top = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _thin, _top_pair, _paired_oneway,
        _med_spot, _value_heavy, 400, 600)
    assert no_fire_top is False, (
        "REACHABILITY FAIL: top pair must not fire")

    # 6. Does NOT fire: standard opp
    no_fire_std = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _thin, _weak_pair, _paired_oneway,
        _med_spot, _std, 400, 600)
    assert no_fire_std is False, (
        "REACHABILITY FAIL: standard opp must not fire")

    # 7. Does NOT fire: small bet
    no_fire_small = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _thin, _weak_pair, _paired_oneway,
        _sm_spot, _value_heavy, 400, 600)
    assert no_fire_small is False, (
        "REACHABILITY FAIL: small bet must not fire")

    # 8. Does NOT fire: has draw
    no_fire_draw = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.15, _thin, _weak_pair, _paired_oneway,
        _med_spot, _value_heavy, 400, 600)
    assert no_fire_draw is False, (
        "REACHABILITY FAIL: draw (0.15) must not fire")

    # 9. Does NOT fire: strong tier
    no_fire_strong = postflop._paired_board_weak_one_pair_fold(
        2, 0.22, 0.05, _strong, _weak_pair, _paired_oneway,
        _med_spot, _value_heavy, 400, 600)
    assert no_fire_strong is False, (
        "REACHABILITY FAIL: strong tier must not fire")

    print(
        "REACHABILITY PASS: v247 paired-board weak-one-pair fold FIRES for "
        "bottom_pair(%.1f)/board_pair_river(%.1f); does NOT fire for "
        "two_pair(%.1f), unpaired(%.1f), top_pair(%.1f), std_opp(%.1f), "
        "small_bet(%.1f), draw(%.1f), strong(%.1f)"
        % (fires, fires_river, no_fire_twopair, no_fire_unpaired,
           no_fire_top, no_fire_std, no_fire_small, no_fire_draw, no_fire_strong))
...
if __name__ == "__main__":
    ...
    main_paired_board_weak_one_pair_fold()
