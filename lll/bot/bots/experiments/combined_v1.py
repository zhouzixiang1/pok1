...
def value_hand_tier(hole_cards, public_cards, pair_profile=None, board_texture=None, paired_board_profile=None):
    ...
    if hand_class >= 6:
        ...
        tier = "strong"
        size_bonus = 0.10 + 0.03 * wetness if set_made else 0.08 + 0.03 * wetness
    elif hand_class == 2:
    ...
    info["hand_class"] = hand_class
    info["set_made"] = hand_class == 3 and hole_ranks.count(score[1]) == 2
    return info
...
def apply_anti_lock_pressure(match_profile):
    ...
    match_profile["chase"] = max(match_profile["chase"], 0.72)
    match_profile["threshold_delta"] = min(match_profile["threshold_delta"], -0.045)
    match_profile["sizing_delta"] = max(match_profile["sizing_delta"], 0.04)
    match_profile["open_delta"] = min(match_profile["open_delta"], -0.025)
    match_profile["bluff_delta"] = max(match_profile["bluff_delta"], 0.03)
...
def choose_anti_lock_pressure_action(
    ...
    reliable_fold_pressure = confidence >= 0.35 and fold_to_raise >= 0.42
    low_equity = win_rate < 0.40 and tier not in ("strong", "nut") and not has_draw

    # Anti-lock means folding is forbidden; it does not imply that every
    # continuation should become a bluff raise. Against sticky opponents,
    # preserve the match by taking the cheapest legal continuation.
    if to_call > 0 and (low_equity or (weak_showdown and not reliable_fold_pressure)):
        return 0
    ...
    if amount >= my_chips * 0.72:
        if to_call > 0:
            return 0
        if tier not in ("strong", "nut") and not has_draw:
            return None
        return -2
...
def choose_raise(
    ...
    if round_idx == 3 and value_profile.get("hand_class") == 3:
        set_cap_ratio = 0.85
        if wetness >= 0.28 or board_texture.get("straight_pressure", 0.0) >= 0.65 or board_texture.get("flush_pressure", 0.0) >= 0.75:
            set_cap_ratio = 0.70
        set_cap = max(min_raise, int(to_call + pot_after_call * set_cap_ratio))
        amount = min(amount, set_cap)
    amount = min(amount, my_chips - 1)
...
def get_action(req, requests, trace=None):
    my_id = req["my_id"]
    my_chips = req["my_chips"]
    ...
    if trace is not None:
        trace.update({
            ...
            "extra_simulations": extra if critical_spot else 0,
