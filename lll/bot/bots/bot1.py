...
def draw_profile(hole_cards, public_cards, board_texture=None):
    ...
    for suit, count in suit_counts.items():
        ...
        nut_draw = 14 in hole_flush_ranks or (14 in board_flush_ranks and max(hole_flush_ranks) >= 13)
        info["flush_draw"] = True
        info["nut_flush_draw"] = info["nut_flush_draw"] or nut_draw

        candidate = 0.21 if nut_draw else 0.16
        if max(hole_flush_ranks) <= 9:
            candidate -= 0.025
    ...
    if info["combo_draw"]:
        ...
        info["fold_threshold_delta"] = 0.02 if max(hole_ranks) >= 11 else 0.0
...
def draw_call_margin(draw_info, board_texture, round_idx, spot_info):
    ...
    if round_idx == 2 and draw_type in ("gutshot", "flush_draw"):
        margin += 0.020
    elif round_idx == 3:
        margin += 0.050

    if size_bucket == "large" and draw_type in ("gutshot", "flush_draw"):
        margin += 0.018
...
def get_action(req, requests):
    ...
    if state["opponent_allin"]:
        ...
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            return -1
        jam_buffer = clamp(jam_buffer, 0.0, 0.14)
    ...
    if to_call >= my_chips:
        ...
        if hard_repressure_fold or paired_board_stackoff["severe"]:
            return -1
        shove_buffer = clamp(shove_buffer, 0.0, 0.14)
