...
precommit runs only the 1+1 official compliance suite above. The full 5+3 suite
is opt-in and should not be used as the normal generation tracker. More granular
...
When enabled, precommit appends an `official_platform_compliance` blocker on
