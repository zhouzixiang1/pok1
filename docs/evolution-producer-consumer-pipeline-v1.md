...
The source-merge boundary is consequently exact:

| Capability | This slice | May be enabled after this merge? |
|---|---|---|
| `JobEnvelope` / `JobReceipt` closed value schemas | implemented | no; no dispatcher owns them |
| `WorkflowStore` persistence, fenced lease, heartbeat, cancel and restart reclaim | implemented through the same kernel | no; adapter remains unimported |
| Artifact three-state reducer and promotion/repair resolution contracts | pure and tested only | no; no durable projection owner/resolver |
| Retry-at scheduler, fork/join dispatcher and resource broker | absent | no |
| Canonical checkpoint/promotion/publication adapter | absent | no |
| Workboard API, SSE and frontend | absent | no |

Landing this slice adds reusable fail-closed primitives only. It must not be
described as the Producer lane, Consumer lane, background strength queue or
workboard being live. Slice 2b activation needs its own reviewed production
commit, migration receipt, stopped-runtime boundary and canary.
...
The envelope requires an explicit evaluation-contract input reference, and
pre-admission additionally compares the durable envelope with the caller's
current candidate ID, artifact digest, charter digest and evaluation-contract
digest. Internal envelope/receipt agreement alone cannot make an old frozen
job current.
...
Long model calls, native matches, official rounds, resource waits and process
joins must run outside the ASGI event loop, `WorkflowStore.command_lock` and
every SQLite transaction. A dispatcher may open a short transaction only to
claim, heartbeat, accept a receipt or append a transition. Control
health/status/checkpoint reads need a separately bounded deadline and a
last-known content-bound projection so CPU saturation or a busy writer cannot
turn a healthy active generation into a multi-second false restart alarm.
Activation is blocked until tests run a long Worker/native stub concurrently
with repeated health reads and prove the agreed latency budget without stale
green authority.
...
2. **One ahead buffer** — allow one Producer to seal an opaque draft while the
   current Consumer runs; add the persistent dispatcher, retry-at clock and
   resource broker while the existing canonical gate chain remains unchanged.
