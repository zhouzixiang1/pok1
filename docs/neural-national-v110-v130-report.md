...
`bots/neural_national_lab/versions/v123_national_v17_qjs_flop_call_tcp`

Change:

- Derived from v122.
- Keeps v122's T8o/K4s/J8s draw breakers and all prior native TCP neural value
  gates.
- Adds a narrow flop QJs top-pair call gate:
  - stage is `flop`,
  - original rule label is `raise_pot`,
...
After v122, the largest remaining old-block v3 loss was seed `2026074008`.
Trace mining showed suited QJ on Q-9-6 facing a medium flop bet. The rule bot
pot-raised to `3943`, later fired a free turn all-in, and lost `-20335` on the
hand.

Force probes:

| Probe | Seed | Result | Note |
|---|---:|---:|---|
| hand 28 decision 2 call | 2026074008 | `-17405` to `+17730` | target hand `-20335` to `+1634` |
| hand 34 decision 2 call | 2026074002 | `-1463` to `-3106` | excluded by tighter thresholds |
| hand 28 decision 0 fold | 2026074008 | `-17405` to `+19228` | rejected as too broad preflop overfit |

The implemented thresholds deliberately require the larger seed4008 sizing
window and exclude the nearby negative seed4002 spot.

All completed national bots on seed block `2026074100`:

The completed/tagged pool remained `national_v1` through `national_v18`,
`national_v20`, and `national_v27` through `national_v32`. v123 was evaluated
against all 25 native TCP opponents with six paired matches each, using the
same ordered pool as v122.

| Version | Matches | Hands | Total | Mean/hand | W-L-D |
|---|---:|---:|---:|---:|---:|
| v122 | 150 | 21000 | `+1844072` | `+87.813` | `150-0-0` |
| v123 | 150 | 21000 | `+1844072` | `+87.813` | `150-0-0` |

Regression on the older current-top8+v7 seed blocks:

| Block | Matches | Hands | v122 Total | v123 Total | Delta | v123 W-L-D |
|---|---:|---:|---:|---:|---:|---:|
| seed2026073900 | 90 | 12600 | `+918189` | `+918189` | `0` | `71-3-16` |
| seed2026074000 | 90 | 12600 | `+455667` | `+508812` | `+53145` | `39-9-42` |
| combined | 180 | 25200 | `+1373856` | `+1427001` | `+53145` | `110-12-58` |

Changed rows in the seed `2026074000` block:

| Opponent/seed | v122 | v123 | Delta |
|---|---:|---:|---:|
| national_v2 / 2026074007 | `-1575` | `-2593` | `-1018` |
| national_v3 / 2026074006 | `+18903` | `+17883` | `-1020` |
| national_v3 / 2026074008 | `-17405` | `+17730` | `+35135` |
| national_v3 / 2026074009 | `+18934` | `+38982` | `+20048` |

Combined older-block v123 opponent totals remained positive for every
opponent:
...
| national_v15 | `+144375` | `+51.563` | `12-0-8` |
| national_v16 | `+144375` | `+51.563` | `12-0-8` |
| national_v2 | `+183411` | `+65.504` | `13-7-0` |
| national_v3 | `+270176` | `+96.491` | `16-4-0` |

Relative to v122:

- Current all-completed-pool result stayed `+1844072`, mean/hand `+87.813`,
  W-L-D `150-0-0`.
- Older current-top8+v7 combined result improved by `+53145` chips, from
  `+1373856` to `+1427001`.
- Older two-block v3 total improved from `+216013` to `+270176`; v2 regressed
  slightly from `+184429` to `+183411`, while remaining strongly positive.
- All v123 recorded evaluations passed protocol compliance with 0 candidate
  illegal actions, 0 candidate timeouts, and 0 candidate adapter actions.
...
`bots/neural_national_lab/versions/v124_national_v17_t9s_reraise_fold_tcp`

Tooling addition:

`bots/neural_national_lab/tools/native_tcp_loss_counterfactual_scan.py`

The new scanner reads native TCP evaluation JSON reports, selects losing rows,
reruns the exact paired seed with decision tracing, probes legal single-action
alternatives on the worst hands, and records full-match plus target-hand deltas.
It calls `run_native_tcp_pair` directly with `require_native_a=True` and
`require_native_b=True`; no national adapter path is used.

Change:

- Derived from v123.
- Keeps v123's T8o/K4s/J8s/QJs draw breakers and all prior native TCP neural
  value gates.
- Adds a narrow preflop T9s reraise-fold gate:
  - stage is `preflop`,
  - original rule label is `raise_pot`,
  - opponent is not all-in,
...
After v123, the older seed block `2026073900` still had repeatable v2/v3
losses on seed `2026073909`. The loss counterfactual scanner found the same
preflop T9s reraise spot in both losses.

Force probes:

| Probe | Opponent/seed | Result | Note |
|---|---:|---:|---|
| hand 19 decision 1 fold | national_v2 / 2026073909 | `-4122` to `+16163` | target hand `-7726` to `-279` |
| hand 19 decision 1 fold | national_v3 / 2026073909 | `-3781` to `+16139` | target hand `-7561` to `-279` |

A forced call was stronger than fold against v3 in isolation but much weaker
against v2. The implemented gate chooses fold. Trace comparison also found the
same betting line on seed `2026073908`, where the original raise was profitable;
v124 separates the good and bad spots by opponent profile preflop raise rate
(`0.1304` on 3908 versus `0.0909` on 3909).

All completed national bots on seed block `2026074100`:

The completed/tagged pool remained `national_v1` through `national_v18`,
`national_v20`, and `national_v27` through `national_v32`. v124 was evaluated
against all 25 native TCP opponents with six paired matches each, using the
same ordered pool as v123.
...
| Block | Matches | Hands | v123 Total | v124 Total | Delta | v124 W-L-D |
|---|---:|---:|---:|---:|---:|---:|
| seed2026073900 | 90 | 12600 | `+918189` | `+958394` | `+40205` | `73-1-16` |
| seed2026074000 | 90 | 12600 | `+508812` | `+508812` | `0` | `39-9-42` |
| combined | 180 | 25200 | `+1427001` | `+1467206` | `+40205` | `112-10-58` |

Changed rows in the seed `2026073900` block:

| Opponent/seed | v123 | v124 | Delta |
|---|---:|---:|---:|
| national_v2 / 2026073909 | `-4122` | `+16163` | `+20285` |
| national_v3 / 2026073909 | `-3781` | `+16139` | `+19920` |

Combined older-block v124 opponent totals remained positive for every
opponent. The v2/v3 totals improved from v123's v2 `+183411`, v3 `+270176` to
v124's v2 `+203696`, v3 `+290096`.

Relative to v123:

- Current all-completed-pool result stayed `+1844072`, mean/hand `+87.813`,
  W-L-D `150-0-0`.
- Older current-top8+v7 combined result improved by `+40205` chips, from
  `+1427001` to `+1467206`.
- Older two-block v2/v3 match records improved from v2 `13-7-0`, v3 `16-4-0`
  to v2 `14-6-0`, v3 `17-3-0`.
...
`bots/neural_national_lab/versions/v126_national_v17_jts_profile_open_tcp`

Change:

- Derived from v125.
- Keeps v125's T8o/K4s/J8s/T9s/QJs draw breakers and all prior native TCP
  neural value gates.
- Adds a narrow suited JTs small-blind profile-open override:
  - stage is `preflop`,
...
After v125, the older seed block `2026074000` still had v2/v3 losses on
seed `2026074007`. Trace and force probes showed that the suited JTs
small-blind open was unstable: national_v2's profile preferred not opening,
while national_v3's slightly more aggressive postflop profile preferred folding
immediately. The gate is profile-bounded to exclude the nearby national_v2
seed `2026074002`, where similar JTs/QJs geometry was harmful to change.

Target checks:

| Opponent/seed | v125 | v126 | Delta |
|---|---:|---:|---:|
| national_v2 / 2026074007 | `-2593` | `+18357` | `+20950` |
| national_v3 / 2026074007 | `-2048` | `+18929` | `+20977` |

Guarded checks:

- national_v2 seed `2026074002` stayed unchanged at `-3606`.
- national_v2 seed `2026074001` stayed unchanged at `+20213`, preserving the
  v125 QJs profile-fold gain.

Completed national bots on seed block `2026074100`:

The completed/tagged pool expanded to `national_v1` through `national_v18`,
`national_v20`, and `national_v27` through `national_v34`. v126 was evaluated
against all 27 native TCP opponents with six paired matches each. The
untracked `.evolution_pok/bots/national_v36/` directory was not treated as a
completed bot because it has no committed bot directory and matching tag in the
operator checkout.
...
| Block | Matches | Hands | v125 Total | v126 Total | Delta | v126 W-L-D |
|---|---:|---:|---:|---:|---:|---:|
| seed2026073900 | 90 | 12600 | `+958394` | `+958394` | `0` | `73-1-16` |
| seed2026074000 | 90 | 12600 | `+531634` | `+573561` | `+41927` | `42-6-42` |
| combined | 180 | 25200 | `+1490028` | `+1531955` | `+41927` | `115-7-58` |

Combined older-block v126 opponent totals remained positive for every
opponent. The v2 total improved from v125's `+226518` to `+247468`; v3 improved
from `+290096` to `+311073`.

Remaining old-block losses:

| Opponent/seed | Net |
|---|---:|
| national_v2 / 2026074002 | `-3606` |
| national_v2 / 2026074009 | `-1731` |
| national_v3 / 2026074002 | `-1463` |
...
- Expanded completed-pool result is `+1988816`, mean/hand `+87.690`,
  W-L-D `162-0-0`, including the newly completed national_v33 and national_v34.
- Older current-top8+v7 combined result improved by `+41927` chips, from
  `+1490028` to `+1531955`.
- Older two-block v2/v3/v5 match records are now v2 `16-4-0`, v3 `18-2-0`,
  and v5 `11-1-8`.
...
`bots/neural_national_lab/versions/v128_national_v17_a2s_mid_reraise_fold_tcp`

Change:

- Derived from v127 as a separate native TCP bot directory.
- Keeps v127's A2s profile fold, JTs profile-open split, QJs/T9s/J8s/K4s/T8o
  draw breakers, and native national TCP entrypoint.
- Adds a narrow mid-early A2s limp-reraise fold:
  - stage is `preflop`,
  - player is big blind,
  - action history is opponent limp, our raise, opponent reraise,
  - hole cards are exactly suited A2,
  ...
  - return action `-1`, a national-protocol `fold`.
- Adds two narrow follow-up profile gates for the same v2 seed4006 row:
...
After v127, the smallest remaining v2 old-block loss was seed `2026074006`.
Trace and force probes showed the same A2s limp-reraise trap as v127's prior
fix, but with a much earlier profile: only four observed actions, preflop raise
rate `0.25`, and postflop raise rate `0`. Folding that decision reduced the
row from `-130` to `-12`. A second force probe on the residual v128 row found
hand 6 decision 0: paired action `0` represented K9o checking back versus a
limp in the forward leg and QJo limping first-in in the swapped leg. Adding
both narrow profile gates moved the row to a clear win.

Rejected probes:

- An early-profile A2s limp-reraise `call` regressed national_v2 seed
  `2026074009` from `-1731` to `-9906`.
- First-hand KQo/A2s sizing gates regressed national_v2 seed `2026074009` from
  `-1731` to `-1932`.

Target and guard checks:

| Opponent/seed | v127 | v128 | Delta |
|---|---:|---:|---:|
| national_v2 / 2026074006 | `-130` | `+15678` | `+15808` |
| national_v2 / 2026074002 | `+18473` | `+18473` | `0` |
| national_v3 / 2026074002 | `+18585` | `+18585` | `0` |
...
| Block | Matches | Hands | v127 Total | v128 Total | Delta | v128 W-L-D |
|---|---:|---:|---:|---:|---:|---:|
| seed2026073900 | 90 | 12600 | `+958394` | `+958394` | `0` | `73-1-16` |
| seed2026074000 | 90 | 12600 | `+614894` | `+652075` | `+37181` | `45-3-42` |
| combined | 180 | 25200 | `+1573288` | `+1610469` | `+37181` | `118-4-58` |

Combined older-block v128 opponent totals remained positive for every
opponent. The v2 total improved from v127's `+268787` to `+284595`; v3 improved
from `+331087` to `+352460`.
...
- Completed-pool result through national_v37 stayed `+2133560`, mean/hand
  `+87.585`, W-L-D `174-0-0`.
- Older current-top8+v7 combined result improved by `+37181` chips, from
  `+1573288` to `+1610469`.
...
Additional runtime-checkout observation:

| Opponent | Matches | Hands | Total | Mean/hand | W-L-D |
|---|---:|---:|---:|---:|---:|
| untracked `.evolution_pok/bots/national_v39` | 6 | 840 | `+72372` | `+86.157` | `6-0-0` |
...
- v130 beats every completed/tagged native national bot present through
...
- v122: older two-block v2/v3 result improves to v2 `+184429`, v3 `+216013`;
  all completed bots on seed block `2026074100` remain `150-0-0`.
- v123: older two-block v2/v3 result changes to v2 `+183411`, v3 `+270176`;
  all completed bots on seed block `2026074100` remain `150-0-0`.
...
- v130: older two-block v2/v3/v5 result improves to v2 `+341458`, v3
  ...
  `174-0-0` through national_v37, and the older two-block match record is
  lossless at `122-0-58`.
...
standard: it is lossless on both the completed/tagged native national pool and
the older current-top8+v7 regression blocks, while staying native TCP compliant.
Future work should broaden the seed/opponent surface, especially around new
runtime candidates such as untracked national_v39, without weakening the
T8o/K4s/J8s/QJs/T9s and JTs/A2s draw-breakers or protocol compliance.
