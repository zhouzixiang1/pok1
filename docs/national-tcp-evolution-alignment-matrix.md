...
| every selected Master proposal is semantically compilable into one typed state-learning primary and bounded Worker prompt before Worker dispatch | `output_schema.py` closed `MASTER_PROPOSAL_FALSIFIER_PRIMARY`, `STATE_LEARNING_PRIMARY_INTERVENTION_TARGETS`, `STATE_LEARNING_PRIMARY_CHECKS` and prompt limits; workflow-v31 invocation packet/log hashes plus schema-2 abandon transaction | proposal Scout renderer and admission validator; proposal-packet parser; strict-authority parse contract; final-Master renderer/binding validator; `plan_compiler.py` bounded system-owned contract block | Scout admission requires exact typed primary/target equality, the target literal in all three executable fields, and no foreign closed target/alias; packet v5 re-proves the contract and rejects a minimum prompt that cannot fit; strict-authority v3 rejects old v2/v4 replay; final binding requires the exact derived primary/all checks and a real nonempty provider string; provider text, selected block and the 2,048-character system block share the unchanged 12,000-character cap | Scout prompt publishes the exact falsifier→primary→intervention-target→checks mapping, requires target-only executable prose and says architecture floor checks are additional; final prompt publishes per-proposal Unicode character reserves and `max_provider_chars`; retry errors include observed/expected primary, required checks, prompt type/minimum and exact overflow arithmetic | frozen proposal claims → v3 typed mechanism/falsifier target → v5 packet → v3 strict authority → final task RuntimeContract/checks → selected immutable Worker block + bounded system block → schema/quality gate | positive: `terminal_response_adaptation` + `terminal_response` + `opponent.terminal_response`, exact prompt limit, all closed system terms including dynamic proposal digest/falsifier within 2,048, deterministic retry recovery; negative: action-profile labels with terminal-response executable prose, foreign alias, wrong primary, missing check, old v2 replay, `None`/list/dict/empty/whitespace/19-char prompt, reserved-marker injection, malformed task scope/packet collections, one-character overflow and impossible minimum prompt; focused `332 passed`, complete Web `2934 passed, 20 skipped` | an inconsistent proposal is rejected before critics/final Master; malformed or inconsistent final task is retried with canonical machine data; exhausted retry canonically abandons; no string coercion, automatic primary rewrite, cap increase, truncation or stale workflow replay is allowed | source-verified under evaluation contract 33; workflow-v31 abandoned, fresh runtime re-prepare pending |
