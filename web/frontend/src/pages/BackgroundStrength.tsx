...
export default function BackgroundStrength() {
  ...
  return (
    <div className="space-y-4">
      ...
      <EvolutionPageHeader
        title="后台 70 手评测"
        subtitle="完整 native TCP 样本才有强度权威"
        status={status}
        health={health}
        loading={loading}
        error={error}
      ...
      {!status?.epoch_initialized ? (
        ...
        <>
          <EvolutionSurface padding="sm">
            <EvolutionSection title="当前能力边界" subtitle="不要把未来设计当成已经启用的功能" />
          ...
          <EvolutionSurface padding="sm">
            <EvolutionSection title="后台评分进程是否真的在工作" subtitle="“配置为启用”不等于“进程存活且心跳新鲜”" />
            <div className="mt-3 space-y-1 text-xs">
              <div className="flex items-center gap-2">
                <EvolutionStatusBadge tone={daemonTone(view.daemon.state)}>
                  {daemonStateLabel(view.daemon.state)}
                </EvolutionStatusBadge>
          ...
          {!view.available ? (
            <EvolutionSurface padding="sm">
              <EvolutionSection title="为什么还没有强度结果" subtitle={view.reason} />
            ...
            <>
              {!view.observerComplete && (
                <EvolutionSurface padding="sm">
                  <EvolutionSection title="后台证据观察未完整" subtitle="读取超过有界预算或验证失败；部分扫描结果不会冒充完整队列" />
                  <div className="mt-3 text-xs text-error-600 dark:text-error-400">
              ...
              <EvolutionSurface padding="sm">
                <EvolutionSection title="当前不可变评分周期" subtitle="所有样本必须绑定同一评测身份" />
              ...
              <EvolutionSurface padding="sm">
                <EvolutionSection title="已纳入强度计算" subtitle="每个样本都是一场完整 70 手 native TCP；Official 与 Arena 不在这里" />
                <div className="mt-3 space-y-1 text-xs">
                  {jobs && jobs.available && jobs.admitted_samples.length === 0 ? (
                    ...
                    jobs && jobs.available && jobs.admitted_samples.slice(0, 20).map((s) => (
                      <div key={s.id ?? s.timestamp} className="flex items-center gap-2 border-b border-gray-50 py-0.5 dark:border-gray-900">
                        <span className="truncate text-gray-600 dark:text-gray-300">{s.bot0} vs {s.bot1}</span>
                        <span className="ml-auto shrink-0 font-mono text-gray-500">
                          样本胜/负/和 {s.bot0_wins ?? 0}-{s.bot1_wins ?? 0}-{s.draws ?? 0}
                        </span>
                        <span className="shrink-0 text-gray-400">{s.strength_sample_count ?? 0} 个完整 70 手样本</span>
                      </div>
                    ))
                  )}
                  {jobs && jobs.available && jobs.pagination.admitted_total > jobs.admitted_samples.slice(0, 20).length && (
                    <p className="mt-1 text-gray-400">（当前页仅显示前 20 条，共 {jobs.pagination.admitted_total} 条）</p>
                  )}
                </div>
              </EvolutionSurface>
              ...
              {view.stagedPending.length > 0 && (
                <EvolutionSurface padding="sm">
                  <EvolutionSection title="已落盘，等待发布进评分周期" subtitle="不是“正在对局”；只有周期原子发布后才有强度权威" />
                  <div className="mt-3 space-y-1 text-xs">
                    {view.stagedPending.map((s) => (
                      <div key={s.filename} className="flex items-center gap-2 border-b border-gray-50 py-0.5 dark:border-gray-900">
                        <EvolutionStatusBadge tone="warn">等待周期发布</EvolutionStatusBadge>
                        <span className="truncate text-gray-600 dark:text-gray-300">{s.bot0} vs {s.bot1}</span>
                        <span className="ml-auto shrink-0 truncate font-mono text-gray-500">{s.filename}</span>
                      </div>
                    ))}
                  </div>
                </EvolutionSurface>
              ...
              {view.inadmissibleDiagnostics.length > 0 && (
                <EvolutionSurface padding="sm">
                  <EvolutionSection title="被拒的样本（零强度权重）" subtitle="只解释拒绝原因，不修补、不复用、不计分" />
                  <div className="mt-3 space-y-2">
                    {view.inadmissibleReasonCounts.length > 0 && (
                      <div className="mb-2 flex flex-wrap gap-1.5">
                        {view.inadmissibleReasonCounts.map(({ reason, count }) => (
                          <span key={reason} className="rounded-md border border-error-300 px-1.5 py-0.5 text-xs text-error-600 dark:border-error-800 dark:text-error-400">
                    ...
                    <div className="space-y-1 text-xs">
                      {view.inadmissibleDiagnostics.slice(0, 20).map((d, i) => (
                        <div key={d.id ?? i} className="border-l-2 border-error-300 pl-2 dark:border-error-800">
...
function daemonTone(state: string): EvolutionStatusTone {
  if (state === "alive_fresh") return "ok";
  if (state === "unconfigured") return "neutral";
  return "error";
}

function daemonStateLabel(state: string): string {
  switch (state) {
    case "alive_fresh": return "进程健康";
    case "configuration_conflict": return "配置禁用但进程仍存活";
    case "configured_dead": return "已配置但进程不在";
    case "alive_stale_heartbeat": return "进程在但心跳过期";
    case "configured_unverifiable": return "进程在但无法验证心跳";
