"""Data module for ``national_alignment_matrix``.

This companion holds the long-form ``CURRENT_ALIGNMENT_ROWS`` literal together
with the dataclasses, status constants, rule-id constants, required-test/term
sets, and the prompt registry that the literal depends on.  The validation and
rendering logic stays in :mod:`national_alignment_matrix`; this module is
stdlib-only and has no runtime side effects.

The main module re-imports the public names it needs (dataclasses, constants,
helpers, and ``CURRENT_ALIGNMENT_ROWS``) so validation code continues to read
