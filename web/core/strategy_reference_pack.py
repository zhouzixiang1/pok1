...
def current_strict_runtime_prompt_overlay() -> str:
    ...
    return (
        ...
        "Only exact cross-connection proof with complementary cross-wire actions, "
        "exact all-in net settlement, and "
        "strict THP exact-prefix-or-five-card board/action/blind/hole/earnings binding may close "
        "that terminal proof. A live raw action awaiting its bounded causal flush may be "
        "provisional, but finalized replay remains strict; no role may fabricate unseen "
        "public cards.\n"
        ...
        "has been published.\n"
        "- Review exact code semantics rather than keywords. A deterministic "
        "machine-approved bounded tracker or deadline-capped refinement is not "
        "a protocol violation. A truly unreachable definition is rejected; the "
        "six heads-up line states must keep four actionable and two explicitly "
        "neutral active consumers.\n"
        ...
        "- Generation ordinal is presentation-only metadata derived from immutable "
        "paired publication history. Canonical version, parent, evidence, certificate, "
        "and annotated tag remain the execution and recovery identity; abandoned "
        "labels consume no ordinal and no role may derive one by version arithmetic.\n"
