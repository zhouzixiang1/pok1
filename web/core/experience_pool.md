...
## RECENT_LESSONS
- **v95**: estimate_preflop_strength saturation fix is now COMPLETE across ALL 5 preflop spots (sb_open, bb_vs_limp, bb_vs_raise, sb_vs_iso_raise, sb_vs_reraise) — future gens should NOT touch preflop threshold gates; next preflop axis is bucket-level calibration (KQo offsuit broadway, suited_connector low bound) or dynamic opponent-conditional bucket widening
- **v95**: Critic two-cycle rejection (3.0→5.0→7.0) successfully forced constant-tuning attempt into structural bucket rewrite — this is the correct pattern when critic flags local_optima_warning: re-architect rather than retry constants
- **v95 归档建议**: Next generation should add offsuit broadway (KQo/KJo/QJo) to sb_open raise_buckets and restore a lightweight opponent-adaptive signal (if opponent fold_to_raise > 0.50 with conf > 0.20, widen raise_buckets) — validate ≥100g vs CS lineage (v47/v48/v50) where KQo limping and passive SB ranges bleed chips to sticky callers.
