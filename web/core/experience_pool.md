...
## RECENT_LESSONS
- **v188**: Pot-odds-vs-equity framework is now LIVE at _postflop_response_margin — the v186 substitution point is OCCUPIED. Future fold-side work must TUNE target_threshold=0.48 (CONSTANT A) or the made-band 0.20-0.50, NOT add new margins; the delta-add family is exhausted and this framework is the replacement.
- **v188**: Leak band 0.20-0.50 STACKS additively with v177 _weak_one_pair_river_margin (0.20-0.55) — combined margin can reach ~0.30. If over-folding appears at >=100g, the critic's prescribed fix is to compute true_equity = bluff_freq + (1-bluff_freq)*made_strength rather than treating made_strength (a HAND-CLASS score: pair=0.22, two-pair=0.40) as calibrated equity vs a polarized range.
- **v188 归档建议**: At >=100g, verify H2H vs v165/v167 (calling-station archetype, currently in losing matchups) — if WR drops, the over-fold root cause is made_strength being a hand-class proxy not true equity; replace it with true_equity = bluff_freq + (1-bluff_freq)*made_strength where bluff_frequency is estimated from opponent large_bet_ratio/river_aggr.
