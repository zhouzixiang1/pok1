...
## RECENT_LESSONS
- **v80**: barrel_plan VALUE branches (strong/nut tier, ~postflop.py:1050) lack archetype/fold_to_raise gating while BLUFF branches are CS-gated (fold_to_raise>0.52) — asymmetric defense. Add `postflop_aggr<0.30 and tier!='nut'` exclusion if H2H vs v51/v62 CS-lineage regresses at ≥100g.
- **v80**: strategy.py at 1451/1500 lines (49 headroom after this gen). Future structural gens targeting strategy.py MUST plan helper extraction (e.g., barrel dispatch to postflop.py) as part of the worker task or the file-size gate will block commit.
- **v80 归档建议**: Gate barrel_plan's VALUE branch (postflop.py strong/nut tier, ~line 1050) with an archetype exclusion (`postflop_aggr<0.30 and tier!='nut'`) to prevent 3-barreling thin-strong hands into calling stations like v51/v62 lineage, which v79 currently exploits at 65% (20g) but risks regressing under the uncapped value-barrel escalation ladder.
