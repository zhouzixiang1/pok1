...
## RECENT_LESSONS
- **v170**: RECURRING 'Master skips complementary mandate' pattern (2nd gen): v169-critic ordered _opponent_sizing_raise_boost() (size DOWN vs bluff-catchers via opp_reraise_ratio LOW); v170 instead built _turn_probe_sizing() (size UP vs capped ranges) which critic explicitly calls COMPLEMENTARY not a substitute — next Master MUST build the DOWN-sizing arm, not a third UP-sizing primitive.
- **v170**: strategy_helpers.py at 2409/2500 (96% hard cap) leaves ~90 lines of growth budget before hard cap; next offense primitive must reuse existing scaffolding or extract/refactor, NOT add net-new ~80-line modules.
- **v170 归档建议 (mixed)**: Build the unbuilt _opponent_sizing_raise_boost() using opp_reraise_ratio LOW (≤0.5 = calling-station signal, already live in opponent.py from v169) to size DOWN to 0.45-0.55x pot on turn/river to_call==0 vs sticky opponents like v156/v160 — this is the missing DOWN-sizing complement to v170's UP-sizing probe, and reuse _turn_probe_sizing()'s OR-gate + confidence-scaling scaffolding rather than adding a parallel module.
