...
## RECENT_LESSONS
- **v189**: EV-gates that compare pot_odds against made_strength are INERT by construction: made_strength is a hand-class ordinal (pair≈0.22 / two-pair≈0.40 / trips≈0.58) that almost always exceeds pot_odds (~0.27). Any pot-odds-vs-equity fold gate MUST use a polarized-range-adjusted equity (made_strength × discount) or a derived true_equity — never the raw ordinal. Verify firing rate empirically before treating such a gate as load-bearing.
- **v189 归档建议 (improvement)**: At >=100g, monitor H2H WR vs v165/v167/v169 (calling-station / standard-aggressive, NOT overbet-exempt): if it falls below 0.45 the global POLARIZATION_DISCOUNT=0.65 is over-folding — replace the scalar with opponent-adaptive `true_equity = bluff_freq_estimate + (1-bluff_freq_estimate)*made_strength` derived from opponent_model (large_bet_ratio, river_aggr, fold_to_bet), and note it STACKS with v177 _weak_one_pair_river_margin (made 0.20-0.55, +0.06 cap) at the adjacent call site.
