...
## RECENT_LESSONS
- **v247**: Master must REJECT worker pivots off prescribed offense priorities: when an archivist names specific offense sites (L1661 gt0_after_margin, 678 confirmed reaches vs 0 at allin_cover; RESTRICTIVE inversion of _river_value_raise_construct), a worker delivering another fold-side gate on a codebase already carrying 7 fold gates (_marginal_made_river_fold_gate/_river_stackoff_guard/_multibarrel_line_fold/_allin_polarized_equity_fold/_river_potodds_equity_margin/fragile_pair_raise_fold/fragile_river_raise_fold) is the project's #1 strategic failure mode, not a recovery.
- **v247**: Placement-shadow is now structural, not hypothetical: dispatching a new gate at strategy.py:1716 — AFTER should_fold_postflop, _marginal_made_river_fold_gate, and value_heavy line_label fold blocks — guarantees upstream shadowing; reachability self-tests that exercise the function in isolation do NOT prove get_action dispatch reachability. MANDATORY telemetry (>=30g, fire-rate >=5%) must precede any claim of success on a new fold gate.
- **v247 归档建议 (regression)**: v248 must land _completed_board_nut_disadvantage_gate at strategy.py L1661 (gt0_after_margin, 678 confirmed reaches) targeting the G5H5/G2H13 river all-in defense leaks vs nemeses v197/v182/v234 — and reverse _river_value_raise_construct to RESTRICTIVE (confidence>=0.10 AND large_bet_ratio>=0.50 AND fold_to_raise<0.50) — NOT another fold-side gate.
