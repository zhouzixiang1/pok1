## OPPONENT_MODELING
- Archetype *axis* CLOSED (saturates to 'standard'; v184 rock-fold inert). Do NOT reopen — fold-gate *ports* (v215/v219/v221/v224) keep sneaking it back with NO WR-lift. [POSSIBLY EXHAUSTED]
...
- Deal-local functions can be TRIGGER-inert even when reachable. NOTE: 4bet-fold (`_preflop_shove_defense_fold`) and `revealed_shove_density` are ABSENT from v239 (lost across crossovers) — must be (re)wired AND reachability-tested before assuming active. [STALE — no WR-lift]
...
- CAP CONSTRAINTS (re-measure before ANY edit): strategy_helpers.py ~2500 = exact cap (reclamation HARD PREREQ); strategy.py v239=2445/2500 (55-line headroom); opponent.py ~694-line headroom.

## BLUFF_CALIBRATION
- Bluff only with explicit fold-equity evidence + confidence; low aggression/passivity alone may signal a calling-station.
- `_semibluff_raise_construct` (v233-origin, restored v236): ABSENT from v239 — lost AGAIN across crossovers. Cannot reachability-test a function not compiled in. Must first RE-PORT from v236, THEN grep 'SEMIBLUFF_RAISE.*reason=fired' targeting ≥5% vs v182/v213/v203; tuning knobs (fold_to_raise→0.45, SPR 3→2.5, vs v206 opp_bet_ratio 0.20-0.70) are post-activation only.
...
- Firing verification: reachability_test + ≥30g paired net-chips is the ROUTINELY-ACHIEVABLE authoritative gate (≥100g to declare success); skipped 4-6+ consecutive gens despite being "binding" — HARD prerequisite. grep counts ≠ H2H proof.
...
- **v238**: AXIS-EXHAUSTION CONFIRMED — fold-gate floor/constant pot_odds nudges are pivot-saturated (v238 reverses v192's 0.30→0.33 with no evidence). Use a DIFFERENT mechanism (deal-local opp-signal gating, structural dispatch-wiring port, or board-texture-conditional fold). [STALE — no WR-lift]
