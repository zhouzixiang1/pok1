...
## RECENT_LESSONS
- **v82**: v81 was already at parity vs v30/v62/v78, so v82's passive_exploit must be fire-rate verified via logging or ≥100g H2H before concluding it fixes a real deficit; actual recent deficits are vs v80/v48/v34.
- **v82**: strategy.py is at 1498/1500 lines — the next generation cannot add code without first extracting helpers or it will fail the size gate.
- **v82 归档建议**: Before expanding strategy.py, extract passive_exploit_trigger logic into a helper and validate with ≥100g mirror battles vs v80, v48, and v34 that the passivity_score>=0.60 gate does not blunder value against aggressive opponents.
