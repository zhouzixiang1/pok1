...
## RECENT_LESSONS
- **v74**: Action-dispatch bypasses are a high-value discovery vector: v74 found that turn-barrel was calling raw int(pot*ratio) instead of choose_raise, skipping all value floors/guards — future Masters should audit every action-selection path (flop donk, river bet, check-raise sizing) for similar raw-ratio bypasses
- **v74**: Constant-tuning is confirmed exhausted (v55-v73 all attempted threshold margins); only structural dispatch/gate changes move performance — v73 commitment gate and v74 barrel routing are the only two structural changes in 19 generations
- **v74 归档建议**: Parent v73 rated ~363 (bottom-tier) with 0% postflop fold persisting — the next generation must directly harden postflop fold-to-raise for made_strength 0.40-0.60 hands facing turn/river bets ≥35% pot, as offensive value-extraction improvements (v74) cannot offset a fold-exploit leak that costs chips every hand.
