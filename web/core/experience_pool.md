...
## RECENT_LESSONS
- **v190**: When making EV-gate calibrations opponent-adaptive, anchor the discount at standard-case bluff_freq=0.25 so the majority of matchups are byte-identical to the predecessor — this eliminates regression risk while tuning extreme-opp behavior, a pattern applicable to future river fold-gates.
- **v190 归档建议**: Capture real classify_archetype() snapshots at >=30 hands vs v171 and v160 (v189's worst matchups) — if they classify 'standard', the discount=0.65 baseline leaves the weakest-matchup gap untouched and a non-river fix axis (preflop/postflop value-extraction) is needed.
