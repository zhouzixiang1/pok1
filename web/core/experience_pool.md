...
## RECENT_LESSONS
- **v250**: Pot-odds equity fold paths that bypass opp-model gates require a principled equity estimate; made_strength*(0.40+bluff_freq) over-discounts polarized equity by ~2x (made=0.25 yields 0.16 vs true ~0.30) and produces systematic over-folds. Use eq ≈ bf*0.92 + (1-bf)*0.10 for made<0.30 polarized spots, or gate firing on opp_model signals (large_bet_ratio≥0.45 AND confidence≥0.20) rather than firing unconditionally.
- **v250**: The v242 inert-gate diagnosis (opp-signal GATING open, NOT another floor/constant) remains the correct direction; v250's bypass approach trades one inertness for an over-fold risk. Future Master should NOT re-attempt bypass — instead recalibrate the vmi≤0.40/ftb≥0.40 thresholds that caused the gate to never fire.
- **v250 归档建议 (mixed)**: Before v251, run ≥30g telemetry vs v237 and v207 (bluff-heavy opponents v249 beats 0.60/0.65) on the river — if fragile_two_pair hands in [0.38,0.45) are folding where they were calling in v249, the unconditional cal_eq bypass is leaking; either restrict firing to large_bet_ratio≥0.45 OR replace the formula with bf*0.92+(1-bf)*0.10 rather than reverting Change 2 wholesale.
