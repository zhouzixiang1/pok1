...
## RECENT_LESSONS
- **v195**: Allin betsize recording must use the actual high_bet/pot ratio (as v194 did), NOT a hard-coded ratio=2.0 — otherwise every short-stack shove inflates the 'overbet' bucket and corrupts the betsize_polarity signal that v193/v195 depend on.
- **v195**: Reachability verification on LIVE nemeses (not synthetic self-tests) is mandatory before shipping fold-side calibrations keyed to a NEW detector — v195's underbettor floor is cosmetic if v172/v173/v174 classify as 'standard' or 'overbettor' rather than 'underbettor' in actual play.
- **v195 归档建议**: Before any further tuning of the betsize_polarity axis, run reachability_test capturing _opp_betsize_polarity output on nemeses v172/v173/v174 at ≥30 hands — if they classify 'standard'/'overbettor' rather than 'underbettor', widen the detector (n≥3 or small_rate≥0.50) or pivot to fold-side calibration against the actually-detected polarity, and restore v194's actual high_bet/pot ratio in the allin branch to preserve sizing fidelity.
