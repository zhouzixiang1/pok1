...
## RECENT_LESSONS
- **v179**: RECURRING BIRTH-DEFECT: v179 is the Nth generation (v149/v158/v160/v161/v163/v170/v179) shipped with only 1-2 dispatch sites against the >=3-sites birth mandate. The single choose_raise L1902 dispatch fires BEFORE donk/probe paths, risking shadow. Future Master must enforce >=3 dispatch sites at worker-review time, not defer to daemon grep — the pattern keeps recurring because workers add the primary dispatch then stop.
- **v179**: SEMI-DEAD READ ANTI-PATTERN: reviewer flagged `fold_to_bet_turn` read from opponent_model but only used in stderr (not gating) and unused `my_round_bet` parameter. These pass review but signal the worker copied scaffolding without full integration. Workers should grep their own function body for every param/signal they read to confirm it actually gates an outcome.
- **v179 归档建议**: v179's _turn_thin_value_extraction at strategy.py L1902 fires BEFORE donk (L1962) and probe (L1982) paths for turn to_call==0 spots — add delta-returning integration at those two sites (or relocate dispatch AFTER them) to satisfy the >=3-sites mandate and avoid shadowing donk/probe thin-value lines; meanwhile the conf_gate<0.01 floor is effectively a no-op so the real filter is station_gate>=0.15, which at default turn_call_size prior 0.50 yields call_size_factor≈0.20 that barely clears — raise conf floor to >=0.10 (conf_gate>=0.67) if reachability shows firing on noise at <30g.
