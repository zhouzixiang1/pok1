...
## RECENT_LESSONS
- **v86**: exploit_dispatch AND-gate (call_down_flop_turn≥0.55 AND fold_turn≤0.30) requires 7/10 call-downs AND ≤2/10 folds simultaneously — if firing count is zero post-commit ≥100g, relax to single-street threshold like v75's original (fold_to_bet<0.30 → value_boost).
- **v86**: v86 dropped bluff_suppress from v75's exploit_dispatch — without it, bot wastes bluffs vs sticky callers who never fold. Re-add `bluff_suppress=True when fold_to_bet<0.30` to avoid chip bleed.
- **v86 归档建议**: Measure v86's exploit_dispatch firing rate vs v30/v62/v78 over ≥100 games; if zero AND-gate firings, relax to v75's single-street threshold and restore the missing bluff_suppress signal to stop bleeding chips on futile river bluffs against calling stations.
