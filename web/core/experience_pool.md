...
## RECENT_LESSONS
- **v204**: Critic a-priori formula analysis predicted over-fold TPTK at v204; precommit showed the OPPOSITE (continued -20k/-22k stack-offs vs v193/v201). When critic's theoretical prediction conflicts with precommit net-chip data, weight precommit — do NOT add the suggested TPTK-exemption floor next gen, it would re-create the leak.
- **v204**: W-L record masks the leak: vs v193 the bot was 5W-3L yet net -25071 because all 5 wins were razor-thin (<500 chips) while one blowout dominated. Future evaluation must weight net-chips/blowout-frequency, not W-L, when assessing polarized-aggression fixes.
- **v204 归档建议 (mixed)**: Before any formula loosening, run a reachability test on the graduated _allin_polarized_equity_fold gate vs v193/v201 — if TPTK-JK+ hands are STILL calling off (not folding), the gate is inert-by-threshold and the fix is to TIGHTEN calibrated_equity (lower the 0.40+bluff_freq multiplier or raise the jam_odds compare), NOT to add the critic's TPTK floor; monitor v190 (0.40 WR) and v164 (0.30 WR) once daemon games accumulate.
