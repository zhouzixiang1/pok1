...
## RECENT_LESSONS
- **v56**: Critic evidence: H2H weaknesses: v52 overall WR: 0.499 (2020 games) — essentially breakeven, no confirmed river leak, v52 weakest matchups: v20 (0.450/60g), v27 (0.460/50g), v34 (0.460/50g) — all under 100 games, directional only per experience pool rule, v55 has only 440 games total, insufficient for confirming any pattern, No matchup data confirms a river all-in-with-weak-hands leak or a missing river value-bet leak; Experience pool refs: POSTFLOP_STRATEGY: 'River sizing has two paths: standard value baseline (0.85x) and strong-tier showdown extraction (0.50-0.75x from v53)' — Change 2 adds a third path with no H2H backing, PARAMETER_TUNING: 'Constant-tuning without H2S data is repeatedly violated by workers despite being marked [EXHAUSTED]' — Change 2's sizing (0.30/0.38x pot) has no H2H basis, RECENT_LESSONS v55: 'Workers ignored the no constant-tuning without H2H rule for 3 consecutive attempts' [POSSIBLY EXHAUSTED]; Diff refs: choose_anti_lock_pressure_action (lines 165-170): river_weak_made flag blocks emergency_jam for tier=none/thin on river when hands_left > 3 — structurally sound guard, New river value bet block (lines 1356-1371): targets tier='none', made_strength 0.42-0.55, sizing 0.30-0.38x pot — but river_showdown_extraction() (postflop.py:1102) explicitly returns 0.0 for tier='none', and this overrides that decision without evidence, The new value bet block has NO opponent-model gating (no opp_archetype check, no fold-to-bet data, no VPIP check) — unlike river_showdown_extraction() which checks opponent confidence/VPIP/fold_to_raise/archetype
