...
- Deal-local opp fields (`_preflop_shove_defense_fold`, `_facing_v4bet`, `revealed_shove_density`, `large_bet_ratio`, `opp_bet_ratio`, `fold_to_raise`, `confidence`) can be wired yet trigger-inert — re-grep the current bot and prove target-scenario telemetry fires before relying on them.
...
- Placement-shadow is recurring and now chronic (v214 river-guard, v242 fold-gate, v244/v245 offense gates): a guard can exist, pass unit tests, and still be dead. Require downstream control-flow reachability + target-spot telemetry (function returns None when gate unmet AND fires when met), not function presence.
...
- LOC caps are version-sensitive — re-measure before edits: strategy_helpers.py is exactly at the 2500 cap (no headroom), strategy.py ≈2493 (~7-line headroom); reclaim LOC before adding logic.
- Fold-side floor/constant nudges (`_estimate_bluff_frequency` underbettor floors, choose_raise/value-tier ceilings) are dead as an active direction; only opp-signal-gated restructuring or a full sibling-aligned stack port with ≥30g net-chip proof is actionable. [STALE — no WR-lift] [POSSIBLY EXHAUSTED]

## BLUFF_CALIBRATION
- Bluff only with explicit fold-equity evidence + confidence; passivity often means calling-station, not foldability.
- `_semibluff_raise_construct` ancestry/crossover can drop mechanisms (confirmed v242; re-grep the current bot before relying on it). Telemetry-first diagnosis (fire-rate ≥5% @≥30g) is allowed, but no deployment without ≥100g paired net-chip proof.
- Board-texture bluff-raise direction has repeated for many generations without proven lift; pause unless ≥100g WR/net-chip evidence revives it. [STALE — no WR-lift] [POSSIBLY EXHAUSTED]
...
## RECENT_LESSONS
- **v246**: Gate DIRECTION is load-bearing: a DEFAULT-PERMIT opp-gate (blocks only on confident polarized-shover evidence) preserves the status quo for ~90% of matchups; for offense-scoping gates the master must specify RESTRICTIVE semantics (fire ONLY when opp-model confirms calling-station: confidence>=0.10 AND large_bet_ratio>=0.50 AND fold_to_raise<0.50) and workers must be checked against the master's intended fire-condition, not just presence.
- **v246**: large_bet_ratio measures opponent BET sizing, not calling tendency — for value-overbet gates, fold_to_raise is the correct EV signal (opp folds to overbet = wasted value extraction); a gate omitting fold_to_raise cannot distinguish +EV calling stations from -EV folders.
- **v246 归档建议**: v247 priority: reverse _river_value_raise_construct to RESTRICTIVE (fire only when confidence>=0.10 AND large_bet_ratio>=0.50 AND fold_to_raise<0.50) AND land Worker 2's _completed_board_nut_disadvantage_gate at the gt0_after_margin site (L1661, 678 confirmed reaches vs 0 at allin_cover) to address the G5H5/G2H13 -20k river all-in defense leaks vs nemeses v197/v182/v234.
- **v245**: `_river_value_raise_construct` shipped WITHOUT its declared opp-gate (opp_bet_ratio/fold_to_raise/confidence all dead params) — third placement-shadow-adjacent defect (v214, v242). Workers MUST verify every declared gate condition is an executable branch in the body, and that the function returns None when the gate is unmet, not just that it fires when met.
- **v245**: stacked offense (river value raise) on v244's `_thin_value_extraction_sizing` WITHOUT the reachability proof (≥5% @≥30g) mandated by pool. Do NOT add further offense mechanisms until v244 thin-value AND v245 river raise both prove fire-rate ≥5% vs nemeses (v195/v184/v214) at ≥30g.
- **v245 归档建议 (mixed)**: v246 MUST wire the missing opp-gate into `_river_value_raise_construct` (after tier/made check: confidence≥0.10, opp_bet_ratio≥0.50, fold_to_raise<0.50) to scope it to value-heavy calling stations — else the unconditional strong-tier overbet regresses vs v197/v234/v182 where v244 wins 0.63–0.67.
