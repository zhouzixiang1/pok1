...
## RECENT_LESSONS
- **v182**: strategy.py now at 2403/2500 (~0 adaptive headroom vs source_v=181 at 2388; only ~97 lines to hard cap) — FUTURE PRIMITIVES MUST TARGET opponent.py OR refactor _value_lead_upsizing_delta/_spr_calibrated_value_ship to share scaffolding; new strategy.py additions will trip hard cap.
- **v182**: Critic-validated insight: SPR<=2.0 tier-differentiation (strong=75%) is likely SUBOPTIMAL per commitment theory — at low SPR, pot-odds force a ship regardless. Move tier-differentiation to 2.0<SPR<=3.0 where pot-commitment is softer, and restore nut==strong==100% ship at SPR<=2.0 for max EV.
- **v182 归档建议 (improvement)**: Before extending the SPR-ship axis to a 4th generation (v180 fold-margin → v181 SPR-ship birth → v182 SPR-ship coverage), validate firing rate via reachability_test at SPR boundaries 2.0/3.0/4.0 (daemon-grep remains structurally impossible — v163 blindspot STILL ACTIVE, reachability_test is the only valid proxy) AND pivot the next primitive to opponent.py (now the only file with LOC headroom) to address the persistent -20k/0%-fold leak that has gone unaddressed for 15+ generations.
