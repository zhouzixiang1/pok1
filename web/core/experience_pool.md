...
## RECENT_LESSONS
- **v288**: When adding a fold/jam-defer gate as a behavior change, ensure its trigger slack is LOOSER than the existing code's slack — v288's standard_jam slack (0.01) is tighter than the existing 0.02 slack, so the branch is a no-op delta; only the wide_jammer carve-out (allin_rate≥0.20, ~4-8 jams in 70-hand HU) actually changes behavior and may rarely fire.
- **v288**: Before stacking more preflop logic onto bb_vs_raise/sb_vs_reraise, resolve the v276 archive warning about sb_open/bb_vs_raise seat detection — unverified seat routing would silently misfire the new _preflop_jam_call_decision() gate.
- **v288 归档建议**: Before treating v288 as a v46/v36 counter, run ≥30g stderr telemetry to confirm PREFLOP_JAM_DEFENSE.wide_jammer actually fires (needs ~4-8 observed jams to clear allin_rate≥0.20) against v46 and v36 specifically — and cite one replay hand (GxHx#anchor) proving v287 folded a +EV spot to a preflop jam, since the over-fold diagnosis is currently unproven against the n=15 samples.
