...
## RECENT_LESSONS
- **v94**: GATE 4's opponent-stat-gating pattern (fold only when postflop_aggr>=0.42/barrel_freq>=0.50) is the correct template for future intermediate-commitment gates — preserves call-downs vs passive opponents while folding vs confirmed value-heavy aggressors.
- **v94 归档建议**: Validate GATE 4 at ≥100g vs calling-station lineage v47/v48/v50 — the 0.30-0.49 commit_ratio band is exactly where CS opponents extract thin value, so if H2H drops below 45% vs any CS bot, loosen strength_cap 0.52→0.56 or gate GATE 4 firing on barrel_freq only (postflop_aggr false-positives vs passive stations that call-bet-call).
