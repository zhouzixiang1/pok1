...
## RECENT_LESSONS
- **v30**: WORKER DRIFT: Workers executed crossover code (gap broadway limp, sb_limp_vs_raise, trap fold) instead of assigned Master tasks (turn barrel module, probe sizing cap fix) for v30. Zero overlap with declared scope. Future Master plans should account for workers potentially ignoring task assignments when crossover context is injected.
- **v30**: CROSSOVER VALIDATION: v21 outperforms v28 against v28's weakest opponents (v22/v25), confirming crossover feature selection was sound even though task execution drifted. The gap-broadway limp (K7o, Q6o range) and trap-fold guardrails (made<0.25, draw<0.14) are structurally novel features worth monitoring.
- **v30 归档建议 (mixed)**: Turn barrel continuation remains an unfilled gap — the Master identified it for v30 but workers pivoted to crossover code; next generation should specifically target turn aggression after flop c-bet-then-called using a dedicated barrel module gated by nutted_risk checks, since v22/v25 exploit v28's passivity on later streets.
- **v30**: Critic evidence: H2H weaknesses: v28's worst matchup is v22 (wr=0.425, 40 games). v21 beats v22 at 0.52 (200 games) — crossover source outperforms base against the exact weakness., v28 loses to v25 (wr=0.45). v21 beats v25 at 0.575 (80 games)., Experience pool confirms: 'Real leak vs worst matchup (v21) is likely preflop/flop, not river value extraction' — v30 targets preflop.; Experience pool refs: PARAMETER_TUNING: 'Fold margin, clamp, EQR, SPR-commitment guard, sizing_aggr deltas have repeatedly failed through v30. [POSSIBLY EXHAUSTED]' — v30 avoids all of these., POSTFLOP_STRATEGY: 'Overlapping fold gates with close thresholds create redundancy' — v30's trap fold uses a distinct trigger (repeated_raise_trap + sizing bucket) rather than adding a threshold-based gate., RECENT_LESSONS v29: 'Strong-tier overbet was sound structural experiment, but v29 regressed ~26 points' — v30 correctly does NOT restore this regression-causing module.; Diff refs: strategy.py +gap_hand limp logic (lines 535-546): classifies dominated broadway-gap hands and limps them instead of raising., strategy.py +sb_limp_vs_raise handler (lines 649-657): pot-odds-aware defense (pot_odds_sl < 0.25) with gap_hand exclusion., strategy.py +trap fold with strength check (lines 1108-1114): folds only if made<0.25 AND draw<0.14 AND sizing in (medium, large).
