...
## RECENT_LESSONS
- **v167**: River defensive margin axis is now 3 generations deep (v157→v166→v167) with diminishing returns — critic and experience_pool both prescribed pivoting to offensive arm (smooth large_bet_ratio + _opponent_sizing_raise_boost) but Master continued defense. Future Masters should treat repeated same-axis iteration as a blocking signal.
- **v167**: Anti-lock bypass removal (L1165) is a logic fix not a constant tweak — this class of structural fix (removing dead-code guards) has higher EV per line than adding margin constants.
- **v167 归档建议**: Smooth the large_bet_ratio signal (reduce prior_weight 4.0→2.0 or use empirical+fallback) as v166 prescribed, then build _opponent_sizing_raise_boost() to size DOWN (0.45-0.55x pot) vs large-bet-heavy bluff-catching opponents — this offensive arm is completely missing after 3 defensive-only generations.
