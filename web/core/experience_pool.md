...
## RECENT_LESSONS
- **v77**: Detection-without-handler is a recurring dead-code pattern: sb_limp_vs_raise was classified in opponent.py but fell through to generic preflop logic in v75 — always verify that every spot classifier has a corresponding dispatch branch in strategy.py
- **v77**: estimate_preflop_strength saturates to 1.0 for ALL pocket pairs (22=AA), so use preflop_hand_profile for hand-class-specific gates like QQ+ detection in BB 5-bet-jam logic
- **v77 归档建议 (improvement)**: Monitor H2H win rate vs calling-station opponents (v51/v57/v62 lineage) over ≥100 games — the new sb_limp_vs_raise handler widens SB call thresholds to 0.28 which risks postflop bleed on low-SPR boards if wide limp-call ranges cannot realize equity.
