...
Extracted from national_native.py for maintainability. Contains the three
immutable source-template string literals that become the generated
``national_bot.py``, ``precompute.py``, and the stream-decoder probe
script, plus the runtime-version constant. The two post-processing
``.replace()`` steps that bake the runtime version into the bot template
and collapse triple newlines are reproduced here so the exported value is
byte-identical to the previous in-module definition (the hash-pinning tests
in test_national_runtime_probe.py assert over the post-processed bytes).

All symbols are re-exported by national_native.py for backward compatibility.
"""
