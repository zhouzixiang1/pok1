...
# PreToolUse/PostToolUse guard hooks (shell parsing, mutation detection, write
# target enumeration, operator-only / llm-availability / strict-authority
# command targeting) now live in orchestrator_context_guards.  The thin
# delegate shells below preserve the historical public surface for callers
# (e.g. orchestrator._make_bot_dir_guard_hook, tests) that still import these
# names from orchestrator_context.
import orchestrator_context_guards as _g


def _strip_heredoc_bodies(command: str) -> str:
    return _g._strip_heredoc_bodies(command)


def _shell_heredoc_delimiters(line: str) -> list[str]:
    return _g._shell_heredoc_delimiters(line)


def _read_shell_token(text: str, start: int) -> tuple[str, int]:
    return _g._read_shell_token(text, start)


def _iter_shell_write_redirect_targets(command: str):
    return _g._iter_shell_write_redirect_targets(command)


def _bash_has_file_write_redirect(command: str) -> bool:
    return _g._bash_has_file_write_redirect(command)


def _python_snippet_is_mutating(command: str) -> bool:
    return _g._python_snippet_is_mutating(command)


def _bash_has_mutation_command(command: str) -> bool:
    return _g._bash_has_mutation_command(command)


def _git_tag_invocation_is_mutating(args: list[str]) -> bool:
    return _g._git_tag_invocation_is_mutating(args)


def _git_tag_is_mutating(command: str) -> bool:
    return _g._git_tag_is_mutating(command)


def _orchestrator_bash_is_mutation(command: str) -> bool:
    return _g._orchestrator_bash_is_mutation(command)


def _orchestrator_bash_targets_operator_only_bootstrap(command: str) -> bool:
    return _g._orchestrator_bash_targets_operator_only_bootstrap(command)


def _orchestrator_bash_targets_llm_availability_control(command: str) -> bool:
    return _g._orchestrator_bash_targets_llm_availability_control(command)


def _orchestrator_bash_targets_strict_authority_control(command: str) -> bool:
    return _g._orchestrator_bash_targets_strict_authority_control(command)
