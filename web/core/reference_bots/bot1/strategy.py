from constants import BIG_BLIND, SIMULATIONS_BY_PUBLIC_COUNT, EXTRA_SIMULATIONS_BY_PUBLIC_COUNT
...
def get_action(req, requests):
    ...
    if round_idx == 0 and preflop_strength is not None:
        ...
        if spot_action is not None:
            if anti_lock_pressure and spot_action == -1 and to_call < my_chips:
                return 0
            return spot_action
    ...
    paired_board_prof = paired_board_outcome_profile(my_cards, public_cards) if len(public_cards) >= 3 else None
    value_profile = value_hand_tier(my_cards, public_cards, pair_profile, board_texture, paired_board_prof) if len(public_cards) >= 3 else None
    ...
    nutted_risk = (
        nutted_risk_profile(my_cards, public_cards, pair_profile, board_texture, value_profile, paired_board_prof)
        if len(public_cards) >= 3
        else {"risk": 0.0, "label": "none", "vulnerable": False}
    )
    value_plan = (
        value_bet_plan(value_profile, board_texture, paired_board_prof, pair_profile, nutted_risk, round_idx, pot)
        if len(public_cards) >= 3
    ...
    paired_board_stackoff = (
        paired_board_stackoff_profile(pair_profile, paired_board_prof, board_texture, spot_info, round_idx)
        if len(public_cards) >= 3
    ...
    if state["opponent_allin"]:
        ...
        if remaining_hands == 1:
            total_win_chips = req.get("total_win_chips", [0] * 2)
    ...
    if to_call > 0:
        if round_idx == 0:
            ...
            if round_idx == 3 and made_strength < 0.40 and not (blocker_profile and blocker_profile["eligible"]):
                call_margin += 0.04
            if round_idx == 3 and paired_board_prof is not None and paired_board_prof["fold_to_raise"]:
                call_margin += 0.05
        ...
        fragile_river_raise_fold = (
            ...
            and paired_board_prof is not None
            and paired_board_prof["fold_to_raise"]
            and paired_board_prof["hand_class"] == 2
            and (value_profile is None or value_profile["tier"] != "nut")
        )
    ...
    bad_river_value_bet = (
        ...
        and to_call == 0
        and paired_board_prof is not None
        and paired_board_prof["board_paired"]
        and paired_board_prof["prefer_check"]
        and paired_board_prof["hand_class"] == 2
