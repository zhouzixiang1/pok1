...
def _existing_local_bot_tag_matches_certificate(version, certificate):
    """Delegate to tool_commit_publication."""
    return _pub._existing_local_bot_tag_matches_certificate(version, certificate)


def _push_existing_bot_refs(version):
    """Delegate to tool_commit_publication."""
    return _pub._push_existing_bot_refs(version)


def _official_certificate_projection(status):
    """Delegate to tool_commit_publication."""
    return _pub._official_certificate_projection(status)


def _write_completed_sentinel_durable(bot_dir, publication_id):
    """Delegate to tool_commit_publication."""
    return _pub._write_completed_sentinel_durable(bot_dir, publication_id)


def _publication_pending_result(v, source_v, *, error, **extra):
    """Delegate to tool_commit_publication."""
    return _pub._publication_pending_result(v, source_v, error=error, **extra)
...
def _revalidate_publication_authority_before_push(
    ...
    *,
    ...
    """Delegate to tool_commit_publication."""
    return _pub._revalidate_publication_authority_before_push(
        v,
        source_v,
        intent=intent,
        bot_dir=bot_dir,
...
def _resume_publication_transaction(v, source_v, ckpt):
    """Delegate to tool_commit_publication."""
    return _pub._resume_publication_transaction(v, source_v, ckpt)
...
def _safe_log_tree_manifest(*args, **kwargs):
    """Delegate to tool_commit_archivist_orchestrator."""
    return _ao._safe_log_tree_manifest(*args, **kwargs)


def _converge_and_verify_reaped_target(*args, **kwargs):
    """Delegate to tool_commit_archivist_orchestrator."""
    return _ao._converge_and_verify_reaped_target(*args, **kwargs)


async def _run_durable_post_publication_archivist(*args, **kwargs):
    """Delegate to tool_commit_archivist_orchestrator (async)."""
    return await _ao._run_durable_post_publication_archivist(*args, **kwargs)
...
async def _record_crossover_infrastructure(*args, **kwargs):
    """Delegate to tool_commit_crossover (async)."""
    return await _cx._record_crossover_infrastructure(*args, **kwargs)
...
async def run_crossover(args):
    ...
    if isinstance(success, dict) and success.get("outcome") == "synthesis_effect_unrecoverable":
        return await _cx._abandon_crossover_effect_conflict(
            parent_a, parent_b, target_v, success, ui
        )
    ...
    if success:
        ...
        return await _cx._finish_crossover_llm_exhausted(parent_a, parent_b, target_v, ui)
