...
PIPELINE_RUNTIME_HEARTBEAT_SCHEMA = 4
...
_NATIVE_MATCH_PROGRESS_SCHEMA = 4
_NATIVE_MATCH_TERMINAL_HANDOFF_SCHEMA = 1
NATIVE_MATCH_TERMINAL_HANDOFF_MAX_AGE_SEC = 30.0
_NATIVE_MATCH_PROGRESS_OWNERS = {
    "run_quality_gates": "workers_done",
    "run_precommit_eval": "critic_checked",
}
_NATIVE_MATCH_PROGRESS_EVENTS = frozenset({
    "launching",
    "engine_started",
    "hand_start",
    "action_requested",
...
_NATIVE_MATCH_DISPATCH_NONCE = contextvars.ContextVar(
    "native_match_provider_dispatch_nonce",
    default="",
)
_NATIVE_MATCH_DISPATCH_NONCES: set[str] = set()
_NATIVE_MATCH_TERMINAL_HANDOFFS: dict[str, dict] = {}
_NATIVE_MATCH_DISPATCH_LOCK = threading.RLock()
_PIPELINE_RUNTIME_HEARTBEAT_LOCK = threading.RLock()
...
def _valid_dispatch_nonce(value):
    """Return whether ``value`` is an opaque SDK-attempt nonce.

    The orchestrator uses the owned provider attempt UUID (32 lower-case hex
    characters).  It is not a credential; it is a one-dispatch capability
    fence for non-semantic native-match liveness only.
    """
...
def activate_native_match_dispatch_nonce(nonce):
    ...
    if not _valid_dispatch_nonce(nonce):
        raise ValueError("native match dispatch nonce is invalid")
    with _NATIVE_MATCH_DISPATCH_LOCK:
        ...
        _NATIVE_MATCH_TERMINAL_HANDOFFS.pop(nonce, None)
        _NATIVE_MATCH_DISPATCH_NONCES.add(nonce)
    return _NATIVE_MATCH_DISPATCH_NONCE.set(nonce)


def reset_native_match_dispatch_nonce(token):
    """Reset the current-task binding without revoking the shared dispatch."""

    _NATIVE_MATCH_DISPATCH_NONCE.reset(token)
...
def native_match_dispatch_nonce_is_active(nonce):
    ...
    if not _valid_dispatch_nonce(nonce):
        return False
    with _NATIVE_MATCH_DISPATCH_LOCK:
        return nonce in _NATIVE_MATCH_DISPATCH_NONCES


def current_native_match_dispatch_nonce():
    """Return this task's active provider-dispatch fence, else ``None``."""

    nonce = _NATIVE_MATCH_DISPATCH_NONCE.get()
    return nonce if native_match_dispatch_nonce_is_active(nonce) else None
...
def _native_match_phase_budget_us(plan, event_type, liveness_phase):
    ...
    if (
        event_type in _NATIVE_MATCH_PROGRESS_EVENTS - {"launching", "finalizing"}
...
def _normalize_native_match_progress(
    ...
    *,
    ...
    expected_plan = _expected_native_match_timing_plan(checkpoint, owner)
    ...
    if event_type == "launching" and liveness_phase == "launching":
        ...
        hand_valid = _plain_int(hand) and int(hand) == 70
    else:
        hand_valid = _plain_int(hand) and 1 <= int(hand) <= 70
    if (
        value.get("schema_version") != _NATIVE_MATCH_PROGRESS_SCHEMA
        or _NATIVE_MATCH_PROGRESS_OWNERS.get(owner)
        != str(checkpoint.get("stage") or "")
        or expected_plan is None
        or not _valid_dispatch_nonce(value.get("provider_dispatch_nonce"))
        or not _valid_digest(value.get("match_identity_digest"))
        or value.get("timing_plan_digest") != expected_plan.digest()
        or value.get("hands") != 70
        or not _plain_int(value.get("event_seq"))
        or int(value.get("event_seq") or 0) < 1
        or event_type not in _NATIVE_MATCH_PROGRESS_EVENTS
        or not hand_valid
        or value.get("terminal") is not False
        or not _plain_int(value.get("operation_budget_us"))
        or not _plain_int(value.get("phase_budget_us"))
        or not _plain_int(value.get("effective_timeout_us"))
    ):
        return None
    if provider_dispatch_nonce is not None and (
        value.get("provider_dispatch_nonce") != provider_dispatch_nonce
        or not native_match_dispatch_nonce_is_active(provider_dispatch_nonce)
    ):
        return None
    ...
    if timeout_us != expected_plan.effective_timeout_us:
        return None
    phase_budget_us = _native_match_phase_budget_us(
        expected_plan,
        event_type,
        liveness_phase,
    )
    ...
    if (
        ...
        return None
    return {
        "schema_version": _NATIVE_MATCH_PROGRESS_SCHEMA,
        "owner_tool": owner,
...
def validate_native_match_progress(
    ...
    *,
    ...
    return _normalize_native_match_progress(
        checkpoint,
        value,
...
def write_pipeline_runtime_heartbeat(
    ...
    *,
    ...
    normalized_native_progress = (
        _normalize_native_match_progress(
            checkpoint,
    ...
    def write_locked():
        ...
        try:
            ...
            if dispatch_nonce:
                ...
                _NATIVE_MATCH_TERMINAL_HANDOFFS.pop(dispatch_nonce, None)
    ...
    if dispatch_nonce:
        ...
        with _NATIVE_MATCH_DISPATCH_LOCK:
            if dispatch_nonce not in _NATIVE_MATCH_DISPATCH_NONCES:
                temporary.unlink(missing_ok=True)
                return False
            with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
                return write_locked()
    with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
        return write_locked()
...
def read_pipeline_runtime_heartbeat(checkpoint, *, now=None, max_age=None):
    ...
    try:
        with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
            encoded = PIPELINE_RUNTIME_HEARTBEAT_FILE.read_text(encoding="utf-8")
        ...
        if native_progress is not None:
            normalized_native_progress = validate_native_match_progress(
                checkpoint,
                native_progress,
                now=current,
            )
            if (
                ...
                or not native_match_dispatch_nonce_is_active(
...
def read_pipeline_native_match_progress(
    checkpoint,
    *,
    now=None,
    max_age=300.0,
    ...
    """Return only fresh, checkpoint-bound engine progress for one match."""
    ...
    if not isinstance(progress, dict):
        return None
    normalized = validate_native_match_progress(
        checkpoint,
        progress,
        now=time.time() if now is None else float(now),
        provider_dispatch_nonce=provider_dispatch_nonce,
...
def publish_native_match_terminal_handoff(
    ...
    *,
    ...
    if (
        ...
        or _NATIVE_MATCH_PROGRESS_OWNERS.get(owner)
        != str(checkpoint.get("stage") or "")
        or not _valid_dispatch_nonce(nonce)
        or not _valid_digest(match_digest)
        or not _valid_digest(timing_digest)
        or outcome not in {
            "runner_returned",
            "runner_raised",
            "runner_cancelled",
    ...
    path = PIPELINE_RUNTIME_HEARTBEAT_FILE
    with _NATIVE_MATCH_DISPATCH_LOCK:
        if nonce not in _NATIVE_MATCH_DISPATCH_NONCES:
            _NATIVE_MATCH_TERMINAL_HANDOFFS.pop(nonce, None)
            return False
        with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
            heartbeat = read_pipeline_runtime_heartbeat(
            ...
            try:
                ...
                return False
            expires_at = min(
                created_at + NATIVE_MATCH_TERMINAL_HANDOFF_MAX_AGE_SEC,
                operation_deadline,
                phase_deadline,
            )
            if last_event_seq < 1 or expires_at <= created_at:
                ...
                return False
            receipt = {
                "schema_version": _NATIVE_MATCH_TERMINAL_HANDOFF_SCHEMA,
                "owner_tool": owner,
            ...
            _NATIVE_MATCH_TERMINAL_HANDOFFS[nonce] = receipt
            try:
                path.unlink()
            except Exception:
...
def consume_native_match_terminal_handoff(
    ...
    *,
    ...
    current = time.time() if now is None else float(now)
    if not _valid_dispatch_nonce(nonce) or not math.isfinite(current):
        return None
    with _NATIVE_MATCH_DISPATCH_LOCK:
        with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
            receipt = _NATIVE_MATCH_TERMINAL_HANDOFFS.pop(nonce, None)
        active = nonce in _NATIVE_MATCH_DISPATCH_NONCES
    ...
    if (
        ...
        != _NATIVE_MATCH_TERMINAL_HANDOFF_SCHEMA
...
def make_native_match_heartbeat_reporter(
    ...
    *,
    ...
    if _NATIVE_MATCH_PROGRESS_OWNERS.get(owner) != str(
    ...
    dispatch_nonce = (
        ...
        else current_native_match_dispatch_nonce()
    )
    ...
    if not native_match_dispatch_nonce_is_active(dispatch_nonce):
        return None
    event_seq = 0
    last_written = 0.0
    ...
    async def report(progress):
        ...
        if progress.get("terminal") is True:
            handed_off = publish_native_match_terminal_handoff(
                checkpoint,
                owner_tool=owner,
            ...
            revoke_native_match_dispatch_nonce(dispatch_nonce)
            return False
        if not native_match_dispatch_nonce_is_active(dispatch_nonce):
            return False
        if (
        ...
        written = write_pipeline_runtime_heartbeat(
            checkpoint,
...
def clear_pipeline_native_match_heartbeat(
    ...
    *,
    ...
    if not _valid_dispatch_nonce(provider_dispatch_nonce):
        return False
    if match_identity_digest is not None and not _valid_digest(match_identity_digest):
        return False
    if timing_plan_digest is not None and not _valid_digest(timing_plan_digest):
        return False
    path = PIPELINE_RUNTIME_HEARTBEAT_FILE
    with _PIPELINE_RUNTIME_HEARTBEAT_LOCK:
        try:
...
def revoke_native_match_dispatch_nonce(nonce):
    ...
    if not _valid_dispatch_nonce(nonce):
        return False
    with _NATIVE_MATCH_DISPATCH_LOCK:
        _NATIVE_MATCH_DISPATCH_NONCES.discard(nonce)
        _NATIVE_MATCH_TERMINAL_HANDOFFS.pop(nonce, None)
    ...
    return clear_pipeline_native_match_heartbeat(
        provider_dispatch_nonce=nonce,
    )
