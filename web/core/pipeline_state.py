...
import pipeline_state_native_match_heartbeat as _nmh  # noqa: E402  (circular by design)
...
def _expected_native_match_timing_plan(checkpoint, owner):
    ...
    """
    return _nmh._expected_native_match_timing_plan(checkpoint, owner)
...
def _native_match_phase_budget_us(plan, event_type, liveness_phase):
    ...
    return _nmh._native_match_phase_budget_us(plan, event_type, liveness_phase)


def _normalize_native_match_progress(checkpoint, value, *, now, provider_dispatch_nonce=None):
    """Validate the non-semantic, engine-originated native-match projection."""
    return _nmh._normalize_native_match_progress(checkpoint, value, now=now, provider_dispatch_nonce=provider_dispatch_nonce)


def validate_native_match_progress(checkpoint, value, *, now=None, provider_dispatch_nonce=None):
    """Public fail-closed validator for a checkpoint-bound live match."""
    return _nmh.validate_native_match_progress(checkpoint, value, now=now, provider_dispatch_nonce=provider_dispatch_nonce)
...
def write_pipeline_runtime_heartbeat(checkpoint, *, phase, audit_attempt=None, audit_context=None, native_match_progress=None):
    """Atomically publish non-semantic in-process liveness for one checkpoint."""
    return _nmh.write_pipeline_runtime_heartbeat(checkpoint, phase=phase, audit_attempt=audit_attempt, audit_context=audit_context, native_match_progress=native_match_progress)
...
def read_pipeline_runtime_heartbeat(checkpoint, *, now=None, max_age=None):
    ...
    return _nmh.read_pipeline_runtime_heartbeat(checkpoint, now=now, max_age=max_age)
...
def publish_native_match_terminal_handoff(checkpoint, *, owner_tool, provider_dispatch_nonce, match_identity_digest, timing_plan_digest, terminal_outcome, now=None):
    ...
    """
    return _nmh.publish_native_match_terminal_handoff(checkpoint, owner_tool=owner_tool, provider_dispatch_nonce=provider_dispatch_nonce, match_identity_digest=match_identity_digest, timing_plan_digest=timing_plan_digest, terminal_outcome=terminal_outcome, now=now)


def consume_native_match_terminal_handoff(checkpoint, expected_progress, *, now=None):
    """Consume exactly once a terminal receipt matching one granted extension."""
    return _nmh.consume_native_match_terminal_handoff(checkpoint, expected_progress, now=now)


def make_native_match_heartbeat_reporter(checkpoint, *, owner_tool, provider_dispatch_nonce=None):
    ...
    """
    return _nmh.make_native_match_heartbeat_reporter(checkpoint, owner_tool=owner_tool, provider_dispatch_nonce=provider_dispatch_nonce)


def clear_pipeline_native_match_heartbeat(checkpoint=None, *, owner_tool=None, provider_dispatch_nonce, match_identity_digest=None, timing_plan_digest=None):
    ...
    """
    return _nmh.clear_pipeline_native_match_heartbeat(checkpoint, owner_tool=owner_tool, provider_dispatch_nonce=provider_dispatch_nonce, match_identity_digest=match_identity_digest, timing_plan_digest=timing_plan_digest)


def revoke_native_match_dispatch_nonce(nonce):
    """Revoke one provider dispatch and remove its live/terminal control state."""
    return _nmh.revoke_native_match_dispatch_nonce(nonce)
