...
2. **Protocol**:
   ...
   - The official Windows EXE is timing-sensitive: formal native entries must keep an action-send throttle in the TCP wire layer. Preserve `POK_OFFICIAL_ACTION_DELAY` with a default near `0.30` seconds and send actions through `_send_wire_action`; local strength evaluation may override it to `0`, but generated bots must not delete, bypass, or move this throttle into strategy code.
   - Do not implement unsolicited timeout-rescue actions. A bot must send only when the platform is waiting for its current decision; fallback loops that periodically send `call` or `check` without a pending action are not compliant for generated bots.
   - Raise/all-in legality: first preflop raise-to >= 200, first postflop raise-to >= 100, and every re-raise must be at least 2x the previous raise-to. Exact `prev * 2` is legal per controlled official-EXE evidence; `prev * 2 + 1` is optional conservative headroom. Positive raises must not consume all remaining chips, and all-in must be `-2`.
