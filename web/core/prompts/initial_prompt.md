...
2. **Protocol**:
   - The formal national entry is `national_bot.py`: it must connect to the TCP server directly and send line-delimited actions. It must not rely on `sever/bot_adapter.py`, and it must not output `{"response": ...}` as the formal national entry.
