...
def _write_jsonl_atomic(path: Path, records: list[dict]) -> None:
    ...
    Reads the existing file, truncates, writes all records, and fsyncs.
    """
    with locked_file(path, "a+") as f:
        f.seek(0)
        f.truncate()
...
def submit_jobs(jobs: list[BattleJob]) -> list[str]:
    ...
    MAX_PENDING_JOBS.
    """
    existing = _read_jsonl(BATTLE_JOBS_FILE)
    if len(existing) + len(jobs) > MAX_PENDING_JOBS:
        raise RuntimeError(
            f"Pending job limit exceeded: {len(existing)} + {len(jobs)} > {MAX_PENDING_JOBS}"
        )
    ...
    _append_jsonl(BATTLE_JOBS_FILE, records)
...
def drain_pending_jobs() -> list[dict]:
    ...
    if valid:
        ...
        _write_jsonl_atomic(BATTLE_JOBS_FILE, [])

    log.info("Drained %d pending jobs, claimed %d valid", len(pending), len(valid))
    return valid
