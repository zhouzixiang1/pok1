...
# ---------------------------------------------------------------------------
# Fenced effect-lease cluster (claim / renew / cancel / reclaim / fail / defer
# / interrupt / resume).  These eight methods were extracted to the
# ``workflow_kernel_effects`` companion as ``_WorkflowEffectsMixin`` for size
# and re-bound onto ``WorkflowStore`` here.  The companion's top-level imports
# (``EffectLease``, ``WorkflowConflict``, ``canonical_json``,
# ``KERNEL_SCHEMA_VERSION``, ...) resolve against this module, so the import
# must run *after* those names are defined -- i.e. at module bottom, not head.
#
# The methods are installed as plain class attributes rather than via
# ``__bases__`` mutation (CPython rejects that with "deallocator differs" once
# the mixin has its own ``__dict__``).  Each function's ``__globals__`` still
# points at the companion module, where those kernel utility symbols are
# imported at top level, so LOAD_GLOBAL inside the bodies resolves correctly.
# This is fully monkeypatch-compatible: an instance-level
# ``monkeypatch.setattr(store, "claim_effect", fake)`` shadows the class
# attribute exactly as it did before extraction, and no test patches these
# methods at the class level.  Signatures and behaviour are byte-for-byte
# unchanged.
# ---------------------------------------------------------------------------
import workflow_kernel_effects as _effects  # noqa: E402

WorkflowStore.claim_effect = _effects._WorkflowEffectsMixin.claim_effect
WorkflowStore.renew_effect_lease = _effects._WorkflowEffectsMixin.renew_effect_lease
WorkflowStore.cancel_effect = _effects._WorkflowEffectsMixin.cancel_effect
WorkflowStore.reclaim_effect_lease = _effects._WorkflowEffectsMixin.reclaim_effect_lease
WorkflowStore.fail_effect = _effects._WorkflowEffectsMixin.fail_effect
WorkflowStore.defer_effect = _effects._WorkflowEffectsMixin.defer_effect
WorkflowStore.interrupt_effect = _effects._WorkflowEffectsMixin.interrupt_effect
WorkflowStore.resume_effect = _effects._WorkflowEffectsMixin.resume_effect
